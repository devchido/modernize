...
QUnit.test("phonePL", function (assert) {
  ...
  assert.ok(
    !method("100 000 000"),
    "Invalid phone PL: cannot start with 10x xxx xxx"
  );
  assert.ok(
    !method("111 111 111"),
    "Invalid phone PL: cannot start with 11x xxx xxx"
  );
  assert.ok(!method("123 456 78"), "Invalid phone PL: too short");
  assert.ok(!method("123 4567890"), "Invalid phone PL: too long");
  assert.ok(
    !method("700 123 456"),
    "Invalid phone PL: intelligent network, premium rate"
  );
  assert.ok(
    !method("800 123 456"),
    "Invalid phone PL: intelligent network, freephone"
  );
  assert.ok(
    !method("980 000 000"),
    "Invalid phone PL: cannot start with 98x xxx xxx"
  );
  assert.ok(
    !method("990 000 000"),
    "Invalid phone PL: cannot start with 99x xxx xxx"
  );
});
...
QUnit.test("nisBR", function (assert) {
  ...
  assert.ok(
    !method("10757995752"),
    "Invalid NIS/PIS Number: check number failed"
  );
});
...
QUnit.test("file size - over max", function (assert) {
  ...
  assert.equal(
    proxy(),
    false,
    "the selected file for upload is greater than max"
  );
});
...
QUnit.test("file maxsize - valid size for each file", function (assert) {
  var selectedFiles = [
      { name: "test1.jpg", size: 500000 },
      { name: "test2.jpg", size: 500000 },
    ],
    input = fileDummyInput(selectedFiles),
    $form = $("<form />"),
    proxy = $.proxy(
...
QUnit.test("file maxsize - second file too big", function (assert) {
  ...
  assert.equal(
    proxy(),
    false,
    "the size of the second file exceeds the maximum"
  );
});

QUnit.test("file maxsizetotal - valid size", function (assert) {
  var selectedFiles = [
      { name: "test1.jpg", size: 250000 },
      { name: "test2.jpg", size: 250000 },
    ],
    input = fileDummyInput(selectedFiles),
    $form = $("<form />"),
    proxy = $.proxy(
      $.validator.methods.maxsizetotal,
      new $.validator({}, $form[0]),
      null,
      input,
      500000
  ...
  assert.ok(
    proxy(),
    "the size of the files together does not exceed the maximum"
  );
});
...
QUnit.test("file maxsizetotal - too big", function (assert) {
  ...
  assert.equal(
    proxy(),
    false,
    "the size of the files together exceeds the maximum"
  );
});
