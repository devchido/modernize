...
(function ($, undefined) {
  jPlayerPlaylist = function (cssSelector, playlist, options) {
    ...
    this.cssSelector.details =
      this.cssSelector.cssSelectorAncestor + " .jp-details"; // Note that jPlayer controls the text in the title element.
    ...
    $(this.cssSelector.shuffle).click(function (e) {
      ...
      if (
        self.shuffled &&
        $(self.cssSelector.jPlayer).jPlayer("option", "useStateClassSkin")
      ) {
        self.shuffle(false);
      } else {
        self.shuffle(true);
  ...
  jPlayerPlaylist.prototype = {
    ...
    _updateControls: function () {
      ...
      if (
        $(this.cssSelector.shuffle).length &&
    ...
    _highlight: function (index) {
      if (this.playlist.length && index !== undefined) {
        $(this.cssSelector.playlist + " .jp-playlist-current").removeClass(
          "jp-playlist-current"
        );
            ...
            $(
              ...
              if (self.original.length) {
                if (index === self.current) {
                  self.current =
                    index < self.original.length
                      ? self.current
                      : self.original.length - 1; // To cope when last element being selected when it was removed
                  self.select(self.current);
    ...
    next: function () {
      var index =
        this.current + 1 < this.playlist.length ? this.current + 1 : 0;
      ...
      if (shuffled || shuffled !== this.shuffled) {
        $(this.cssSelector.playlist + " ul").slideUp(
          ...
          function () {
            ...
            if (
              playNow ||
              !$(self.cssSelector.jPlayer).data("jPlayer").status.paused
            ) {
              self.play(0);
            } else {
              self.select(0);
