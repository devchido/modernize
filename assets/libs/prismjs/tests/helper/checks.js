...
module.exports = (Prism) => {
  ...
  function extendTest(id, redef) {
    ...
    if (typeof redef !== "object") {
      throw new TypeError(
        `The redef argument has to be an 'object'.` + details
      );
    }
    ...
    if (!redef.__cloned) {
      ...
      const appendKeys = redefKeys.filter(
        (k) => languageKeys.indexOf(k) === -1
      );
  ...
  function insertBeforeTest(inside, before, insert, root) {
    ...
    if (typeof inside !== "string") {
      throw new TypeError(
        `The inside argument has to be a 'string'.` + details
      );
    }
    if (typeof before !== "string") {
      throw new TypeError(
        `The before argument has to be a 'string'.` + details
      );
    }
    ...
    if (!(before in grammar)) {
      throw new Error(
        `"${before}" has to be a key of the grammar "${inside}".`
      );
    }
  }
