 ...
 * @returns {Prism}
 */
const defaultCreateInstance = (languages) =>
  PrismLoader.createInstance(languages);
...
class TestCaseFile {
  ...
  static parse(content) {
    ...
    file.expectedLineStart =
      codeLineCount + expectedStartSpaces.split(/\r\n/).length;
    file.descriptionLineStart =
      codeLineCount +
      expectedLineCount +
      descriptionStartSpaces.split(/\r\n/).length;
...
class TokenizeJSONRunner {
  ...
  isEqual(actual, expected) {
    ...
    return (
      JSON.stringify(simplifiedActual) === JSON.stringify(simplifiedExpected)
    );
  }
  /**
   * @param {TokenStream} actual
  ...
  runTestCaseWithRunner(
    ...
    if (!testCase.expected) {
      ...
      runner.assertEqual(actualValue, testCase.expected, (diffIndex) => {
        const expectedLines = testCase.expected
          .substr(0, diffIndex)
          .split(/\r\n?|\n/g);
