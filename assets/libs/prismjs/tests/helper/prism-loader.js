...
module.exports = {
  ...
  createPrismDOM() {
    ...
    const load = (languagesOrPlugins) => {
      getLoader(components, toArray(languagesOrPlugins), [...loaded]).load(
        (id) => {
          let source;
  ...
  loadLanguages(languages, context) {
    getLoader(components, toArray(languages), [...context.loaded]).load(
      (id) => {
        if (!languagesCatalog[id]) {
  ...
  loadPluginSource(name) {
    return this.loadFileSource(
      `${__dirname}/../../plugins/${name}/prism-${name}.js`
    );
  },
