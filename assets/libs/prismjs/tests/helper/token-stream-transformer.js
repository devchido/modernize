...
module.exports = {
  ...
  prettyprint(tokenStream, indentation) {
    return printPrettyTokenStream(
      toPrettyTokenStream(tokenStream),
      undefined,
      indentation
    );
  },
                ...
                .map((item) => {
                  if (isToken(item)) {
                    if (typeof item === "string") {
                      ...
                      return (
                        ", ".length + JSON.stringify(item).length + " ".length
                      );
                    }
                  } else {
                    return 0;
 ...
 * @returns {string}
 */
function printPrettyTokenStream(
  prettyStream,
  indentationLevel = 1,
  indentationChar = "    "
) {
