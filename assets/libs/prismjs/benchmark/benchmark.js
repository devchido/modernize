...
async function runBenchmark(config) {
  ...
  const maxCandidateNameLength = candidates.reduce(
    (a, c) => Math.max(a, c.name.length),
    0
  );

  const totalNumberOfCaseFiles = cases.reduce((a, c) => a + c.files.length, 0);
  console.log(
    `Found ${cases.length} cases with ${totalNumberOfCaseFiles} files in total.`
  );
  console.log(
    `Test ${candidates.length} candidates with Prism.${config.options.testFunction}`
  );
  const estimate =
    candidates.length * totalNumberOfCaseFiles * config.options.maxTime;
  console.log(
    `Estimated duration: ${Math.floor(estimate / 60)}m ${Math.floor(estimate % 60)}s`
  );
  ...
  for (const $case of cases) {
    ...
    for (const caseFile of $case.files) {
      ...
      results.forEach((r, index) => {
        ...
        const r_moe =
          ((100 * r.stats.moe) / r.stats.mean).toFixed(0).padStart(3) + "%";
        ...
        const color =
          r === best ? "\x1b[32m" : r === worst ? "\x1b[31m" : "\x1b[0m";
  ...
  console.log("summary");
  console.log(
    `${" ".repeat(maxCandidateNameLength + 2)}  \x1b[90mbest  worst  relative\x1b[0m`
  );
  ...
  totalSummary.forEach((s, i) => {
    ...
    const relative = (
      (s.avgRelative / minAvgRelative).toFixed(2) + "x"
    ).padStart("relative".length);
...
async function getCases(config) {
  ...
  function isEnabled(languages) {
    if (config.options.language) {
      ...
      const required = new Set(
        config.options.language.split(/,/g).filter(Boolean)
      );
  ...
  for (const id of Object.keys(config.cases)) {
    ...
    cases.push({
      ...
      files: [...(await getCaseFiles(id))].sort((a, b) =>
...
async function getCandidates(config) {
  ...
  for (const remote of config.remotes) {
    ...
    const remotePrismLoader = require(
