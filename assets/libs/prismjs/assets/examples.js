...
(function () {
  ...
  function fileExists(filepath) {
    return treePromise.then(function (tree) {
      ...
      if (
        ...
        return getFileContents(filepath).then(
          function () {
            return true;
          },
          function () {
  ...
  function buildContentsHeader(id) {
    ...
    if (language.alias) {
      ...
      header +=
        '<ul><li><code class="language-none">"language-' + id + '"</code></li>';
    ...
    if (language.require) {
      deps.push(
        "requires " + toArray(language.require).map(wrapCode).join(", ")
      );
    }
    if (language.optional) {
      deps.push(
        "optionally uses " + toArray(language.optional).map(wrapCode).join(", ")
      );
    }
    if (language.modify) {
      deps.push(
        "modifies " + toArray(language.modify).map(wrapCode).join(", ")
      );
    }
    if (deps.length) {
      header += "<p>";
      header +=
        '<a href="https://prismjs.com/extending.html#dependencies"><strong>Dependencies:</strong></a>';
  ...
  function update(id) {
    ...
    if (language.enabled) {
      ...
      language.examplesPromise.then(function (contents) {
        ...
        $$("pre", container).forEach(
          /** @param {HTMLElement} pre */ function (pre) {
            var dependencies = (
              pre.getAttribute("data-dependencies") || ""
            ).trim();
