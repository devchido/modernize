...
(function () {
        ...
        contents: [
          {
            ...
            properties: {
              ...
              onclick: (function (category, all) {
                return function () {
                  var checkAll = this;
                  $$('input[name="download-' + category + '"]').forEach(
                    function (input) {
  ...
  form.elements.compression[0].onclick = form.elements.compression[1].onclick =
    function () {
      minified = !!+this.value;
  ...
  function update(updatedCategory, updatedId) {
    ...
    if (updatedId) {
      ...
      $u.element.prop($('label[data-id="' + updatedId + '"] .filesize'), {
        ...
        title:
          (updated.js
            ? Math.round((100 * updated.js) / updated.all) + "% JavaScript"
            : "") +
          (updated.js && updated.css ? " + " : "") +
          (updated.css
            ? Math.round((100 * updated.css) / updated.all) + "% CSS"
            : ""),
      });
    }
        ...
        p.contentsPromise["catch"](function () {
          errors.push(
            $u.element.create({
              ...
              prop: {
                textContent:
                  'An error occurred while fetching the file "' + p.path + '".',
              },
            })
          );
          i++;
