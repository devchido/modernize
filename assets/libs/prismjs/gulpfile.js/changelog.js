...
const revisionRanges = {
  nextRelease() {
    return git
      .raw(["describe", "--abbrev=0", "--tags"])
      .then((res) => `${res.trim()}..HEAD`);
  },
};
...
async function changes() {
  ...
  function notGenerated(change) {
    return (
      ...
      ["prism.js", "components.js", "package-lock.json"].indexOf(
        change.file
      ) === -1
    );
  }
  ...
  const commitSorters = [
    function rebuild(info) {
      if (
        ...
        info.changes.filter(notGenerated).length === 0
      ) {
    ...
    function addedComponent(info) {
      ...
      if (relevantChanges.some((c) => c.file === "components.json")) {
        ...
        if (relevantChanges.length === 1) {
          ...
          if (
            change.mode === "A" &&
            change.file.startsWith("components/prism-")
          ) {
    ...
    function changedPlugin(info) {
      ...
      if (
        ...
        relevantChanges.every(
          (c) => c.mode === "M" && /^plugins\/.*\.js$/.test(c.file)
        )
      ) {
        if (relevantChanges.length === 1) {
          const change = relevantChanges[0];
    ...
    function changedTheme(info) {
      if (
        ...
        if (info.changes.length === 1) {
          ...
          let name = (change.file.match(/prism-(\w+)\.css$/) || [
            ,
            "Default",
          ])[1];
    ...
    function changedInfrastructure(info) {
      ...
      if (
        excludeTests.length === 1 &&
        excludeTests[0].file === "dependencies.js"
      ) {
        addEntry("Other >> Infrastructure", info);
        return true;
      }
    ...
    function changedWebsite(info) {
      if (
        ...
        info.changes.every((c) => {
          return (
            /[\w-]+\.html$/.test(c.file) || /^(?:assets|docs)\//.test(c.file)
          );
        })
      ) {
    ...
    function otherChanges(info) {
      ...
      if (
        info.changes.length > 0 &&
        info.changes.every((c) => /\.md$/i.test(c.file))
      ) {
        addEntry("Other", info);
        return true;
      }
