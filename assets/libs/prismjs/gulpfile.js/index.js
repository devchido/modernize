...
function minifyThemes(cb) {
  pump(
    [src(paths.themes), cleanCSS(), rename({ suffix: ".min" }), dest("themes")],
    cb
  );
}
function build(cb) {
...
async function languagePlugins() {
          ...
          const stream = src(task.plugin)
            .pipe(
              replace(
                ...
                (m, mapName) =>
                  `/*${mapName}_placeholder[*/${task.maps[mapName]}/*]*/`
              )
            )
  ...
  const rejectedTasks = taskResults.filter(
    /** @returns {r is {status: 'rejected', reason: any}} */ (r) =>
      r.status === "rejected"
  );
...
module.exports = {
  ...
  default: series(
    parallel(components, plugins, minifyThemes, componentsJsonToJs, build),
    docs
  ),
  linkify,
  changes,
