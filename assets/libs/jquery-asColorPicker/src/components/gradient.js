...
var Gradient = function (api, options) {
  ...
  $.extend(that, {
    ...
    markers: {
      ...
      add(stop) {
        $(
          `<li data-id="${stop.id}" style="left:${conventToPercentage(stop.position)}" class="${that.classes.marker}"><span style="background-color: ${stop.color.toHEX()}"></span><i style="background-color: ${stop.color.toHEX()}"></i></li>`
