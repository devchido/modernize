...
(function () {
    ...
    panels: [
      {
        type: "box",
        content:
          '<ul class="ckconsole_list ckconsole_value" data-value="instances"></ul>',
  ...
  function obj2Array(obj) {
    var arr = [];
    for (var id in obj) arr.push(obj[id]);

    return arr;
  }

  var itemTpl = new CKEDITOR.template(
    "<li>id: <code>{id}</code>, name: <code>{name}</code>, data: <code>{data}</code></li>"
  );
})();
