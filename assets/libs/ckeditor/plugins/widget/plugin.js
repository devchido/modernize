...
(function () {
  ...
  Repository.prototype = {
    ...
    checkSelection: function () {
      ...
      if (!range || range.collapsed) return updater.commit();
    ...
    del: function (widget) {
      if (this.focused === widget) {
        ...
        if (
          !(found = range.moveToClosestEditablePosition(widget.wrapper, true))
        )
    ...
    getByElement: (function () {
      ...
      function getWidgetId(element) {
        return (
          element.is(validWrapperElements) && element.data("cke-widget-id")
        );
      }
    ...
    initOn: function (element, widgetDef, startupData) {
      if (!widgetDef) widgetDef = this.registered[element.data("widget")];
    ...
    initOnAll: function (container) {
      var newWidgets = (container || this.editor.editable()).find(
          ".cke_widget_new"
        ),
        newInstances = [],
        instance;
      ...
      for (var i = newWidgets.count(); i--; ) {
        instance = this.initOn(
          newWidgets.getItem(i).getFirst(Widget.isDomWidgetElement)
        );
    ...
    parseElementClasses: function (classes) {
      ...
      while ((cl = classes.pop())) {
        if (cl.indexOf("cke_") == -1) obj[cl] = hasClasses = 1;
      }
    ...
    wrapElement: function (element, widgetName) {
      ...
      if (element instanceof CKEDITOR.dom.element) {
        ...
        wrapper.data(
          "cke-display-name",
          widgetDef.pathName ? widgetDef.pathName : element.getName()
        );

        // Replace element unless it is a detached one.
        if (element.getParent(true)) wrapper.replace(element);
        ...
        if (
          wrapper &&
          wrapper.type == CKEDITOR.NODE_ELEMENT &&
          wrapper.attributes["data-cke-widget-wrapper"]
        )
          return wrapper;

        // If attribute isn't already set (e.g. for pasted widget), set it.
        if (!("data-cke-widget-keep-attr" in element.attributes))
          element.attributes["data-cke-widget-keep-attr"] = element.attributes[
            "data-widget"
          ]
            ? 1
            : 0;
        if (widgetName) element.attributes["data-widget"] = widgetName;
        ...
        wrapper = new CKEDITOR.htmlParser.element(
          isInline ? "span" : "div",
          getWrapperAttributes(isInline, widgetName)
        );
    ...
    CKEDITOR.tools.extend(
      ...
      widgetDef,
      {
        ...
        data: CKEDITOR.tools.extend(
          {},
          typeof widgetDef.defaults == "function"
            ? widgetDef.defaults()
            : widgetDef.defaults
        ),
        ...
        _: {
          downcastFn:
            widgetDef.downcast && typeof widgetDef.downcast == "string"
  ...
  Widget.prototype = {
    ...
    checkStyleActive: function (style) {
      ...
      while ((cl = classes.pop())) {
        if (!this.hasClass(cl)) return false;
      }
      return true;
    },
    ...
    destroy: function (offline) {
      ...
      if (this.editables) {
        for (var name in this.editables) this.destroyEditable(name, offline);
      }
    ...
    edit: function () {
      ...
      if (this.fire("edit", evtData) === false || !evtData.dialog) return false;

      this.editor.openDialog(evtData.dialog, function (dialog) {
        var showListener, okListener;

        // Allow to add a custom dialog handler.
        if (that.fire("dialog", dialog) === false) return;
    ...
    initEditable: function (editableName, definition) {
      ...
      if (editable && editable.is(CKEDITOR.dtd.$editable)) {
        editable = new NestedEditable(this.editor, editable, {
          filter: createEditableFilter.call(
            this.repository,
            this.name,
            editableName,
            definition
        ...
        if (editable.filter) editable.data("cke-filter", editable.filter.id);
    ...
    setSelected: function (selected) {
      this.wrapper[selected ? "addClass" : "removeClass"](
        "cke_widget_selected"
      );
    ...
    updateDragHandlerPosition: function () {
      ...
      if (oldPos && newPos.x == oldPos.x && newPos.y == oldPos.y) return;
  ...
  Widget.getNestedEditable = function (guard, node) {
    if (!node || node.equals(guard)) return null;

    if (Widget.isDomNestedEditable(node)) return node;
  ...
  Widget.isDomDragHandler = function (node) {
    return (
      ...
      node.hasAttribute("data-cke-widget-drag-handler")
    );
  };
  ...
  Widget.isDomDragHandlerContainer = function (node) {
    return (
      node.type == CKEDITOR.NODE_ELEMENT &&
      node.hasClass("cke_widget_drag_handler_container")
    );
  };
  ...
  Widget.isDomNestedEditable = function (node) {
    return (
      node.type == CKEDITOR.NODE_ELEMENT &&
      node.hasAttribute("data-cke-widget-editable")
    );
  };
  ...
  Widget.isDomWidgetElement = function (node) {
    return (
      node.type == CKEDITOR.NODE_ELEMENT && node.hasAttribute("data-widget")
    );
  };
  ...
  Widget.isDomWidgetWrapper = function (node) {
    return (
      node.type == CKEDITOR.NODE_ELEMENT &&
      node.hasAttribute("data-cke-widget-wrapper")
    );
  };
  ...
  Widget.isDomWidget = function (node) {
    return node
      ? this.isDomWidgetWrapper(node) || this.isDomWidgetElement(node)
      : false;
  };
  ...
  Widget.isParserWidgetElement = function (node) {
    return (
      node.type == CKEDITOR.NODE_ELEMENT && !!node.attributes["data-widget"]
    );
  };
  ...
  Widget.isParserWidgetWrapper = function (node) {
    return (
      node.type == CKEDITOR.NODE_ELEMENT &&
      !!node.attributes["data-cke-widget-wrapper"]
    );
  };
  ...
  function NestedEditable(editor, element, config) {
    ...
    if (!CKEDITOR.dtd[this.getName()].p)
      ...
      this.enterMode = filter
        ? filter.getAllowedEnterMode(editor.enterMode)
        : editor.enterMode;
      this.shiftEnterMode = filter
        ? filter.getAllowedEnterMode(editor.shiftEnterMode, true)
        : editor.shiftEnterMode;
    }
  }

  NestedEditable.prototype = CKEDITOR.tools.extend(
    ...
    editor.addCommand(widgetDef.name, {
      exec: function (editor, commandData) {
        ...
        if (focused && focused.name == widgetDef.name) {
          ...
          temp.append(wrapper);
          instance = editor.widgets.initOn(
            element,
            widgetDef,
          ...
          var editListener = instance.once(
            "edit",
            function (evt) {
              if (evt.data.dialog) {
  ...
  function addWidgetProcessors(widgetsRepo, widgetDef) {
    ...
    function addUpcast(upcast, def, priority) {
      ...
      var index = CKEDITOR.tools.getIndex(
        widgetsRepo._.upcasts,
        function (element) {
      ...
      widgetsRepo._.upcasts.splice(index, 0, [
        CKEDITOR.tools.bind(upcast, def),
        def.name,
        priority,
      ]);
    }
  ...
  function checkWidgets(evt) {
    ...
    if (options && options.initOnlyNew) newInstances = this.initOnAll();
    else {
  ...
  function cleanUpWidgetElement(el) {
    ...
    if (
      ...
      parent.attributes["data-cke-widget-wrapper"]
    ) {
  ...
  function createEditableFilter(widgetName, editableName, editableDefinition) {
    if (
      !editableDefinition.allowedContent &&
      !editableDefinition.disallowedContent
    )
      return null;

    var editables = this._.filters[widgetName];

    if (!editables) this._.filters[widgetName] = editables = {};

    var filter = editables[editableName];

    if (!filter) {
      filter = editableDefinition.allowedContent
        ? new CKEDITOR.filter(editableDefinition.allowedContent)
  ...
  function createUpcastIterator(widgetsRepo) {
    ...
    return {
      ...
      iterator: function (element) {
        var upcast, upcasted, data, i, upcastsLength, upcastCallbacksLength;
        ...
        if ("data-cke-widget-wrapper" in element.attributes) {
          ...
          if (element) toBeWrapped.push([element]);
        ...
        // and wrapped and stop iterating in this branch.
          ...
          // Do not iterate over descendants.
          if (element.attributes["data-cke-widget-upcasted"]) return false;

          // Check element with upcast callbacks first.
          // If any of them return false abort upcasting.
          for (
            i = 0, upcastCallbacksLength = upcastCallbacks.length;
            i < upcastCallbacksLength;
            ++i
          ) {
            if (upcastCallbacks[i](element) === false) return;
          ...
          for (i = 0; i < upcastsLength; ++i) {
            ...
            if ((upcasted = upcast[0](element, data))) {
              ...
              element.attributes["data-cke-widget-data"] = encodeURIComponent(
                JSON.stringify(data)
              );
  ...
  function insertElement(parent, index, element) {
    ...
    if (parent.type == CKEDITOR.NODE_ELEMENT) {
      ...
      if (parentAllows && !parentAllows[element.name]) {
        ...
        if (!parent2.children.length) parent2.remove();
  ...
  function isWidgetInline(widgetDef, elementName) {
    return typeof widgetDef.inline == "boolean"
      ? widgetDef.inline
      : !!CKEDITOR.dtd.$inline[elementName];
  }
  ...
  function onEditableKey(widget, keyCode) {
    ...
    if (keyCode == CKEDITOR.CTRL + 65) {
      ...
      if (bogus) range.setEndAt(bogus, CKEDITOR.POSITION_BEFORE_START);
  ...
  function setFocusedEditable(widgetsRepo, widget, editableElement, offline) {
    ...
    if (editableElement) {
      ...
      editor.setActiveEnterMode(
        editableInstance.enterMode,
        editableInstance.shiftEnterMode
      );
    } else {
      if (!offline)
  ...
  function setupContextMenu(editor) {
    ...
    editor.contextMenu.addListener(function (element) {
      ...
      if (widget) return widget.fire("contextMenu", {});
    });
  }
  ...
  function setupDragAndDrop(widgetsRepo) {
    ...
    editor.on("drop", function (evt) {
      ...
      evt.data.dataTransfer.setData(
        "text/html",
        editor.editable().getHtmlFromRange(dragRange).getHtml()
      );
        ...
        widgetsRepo,
        {
          finder: new lineutils.finder(editor, {
            lookups: {
              ...
              default: function (el) {
                if (el.is(CKEDITOR.dtd.$listItem)) return;

                if (!el.is(CKEDITOR.dtd.$block)) return;

                // Allow drop line inside, but never before or after nested editable (https://dev.ckeditor.com/ticket/12006).
                if (Widget.isDomNestedEditable(el)) return;
                ...
                if (nestedEditable) {
                  ...
                  var filter =
                      CKEDITOR.filter.instances[
                        nestedEditable.data("cke-filter")
                      ],
                  ...
                  if (
                    ...
                    !filter.check(draggedRequiredContent)
                  )
                    return;
                }
  ...
  function setupMouseObserver(widgetsRepo) {
    ...
    editor.on("contentDom", function () {
      ...
      editable.attachListener(evtRoot, "mousedown", function (evt) {
        ...
        widget =
          target instanceof CKEDITOR.dom.element
            ? widgetsRepo.getByElement(target)
            : null;
        ...
        if (widget) {
          ...
          if (
            widget.inline &&
            target.type == CKEDITOR.NODE_ELEMENT &&
            target.hasAttribute("data-cke-widget-drag-handler")
          ) {
          ...
          if (!Widget.getNestedEditable(widget.wrapper, target)) {
            ...
            if (!CKEDITOR.env.ie) widget.focus();
          } else {
    ...
    editor.on(
      ...
      function (evt) {
        ...
        if (
          !widget ||
          Widget.getNestedEditable(widget.wrapper, evt.data.element)
        )
          return;
      ...
      function (evt) {
        var focused = widgetsRepo.focused,
          widgetHoldingFocusedEditable =
            widgetsRepo.widgetHoldingFocusedEditable,
          ret;

        if (focused) ret = focused.fire("key", { keyCode: evt.data.keyCode });
  ...
  function setupSelectionObserver(widgetsRepo) {
    ...
    editor.on("selectionChange", function (evt) {
      var nestedEditable = Widget.getNestedEditable(
          editor.editable(),
          evt.data.selection.getStartElement()
        ),
      ...
      if (oldWidget) {
        if (
          oldWidget !== newWidget ||
          !oldWidget.focusedEditable.equals(nestedEditable)
        ) {
    ...
    editor.on("blur", function () {
      ...
      if ((widget = widgetsRepo.focused)) blurWidget(widgetsRepo, widget);
  ...
  function setupWidgetsLifecycleEnd(widgetsRepo) {
        ...
        evt.data.dataValue.forEach(
          function (element) {
            ...
            if ("data-cke-widget-white-space" in attrs) {
              ...
              if (
                firstTextNode.parent.attributes["data-cke-white-space-first"]
              ) {
            ...
            if ("data-cke-widget-id" in attrs) {
              ...
              if (widget) {
                ...
                if (
                  widgetElement.attributes["data-cke-widget-keep-attr"] != "1"
                )
            ...
            // Nested editable.
              ...
              if (toBeDowncasted.length > 0) {
                toBeDowncasted[toBeDowncasted.length - 1].editables[
                  attrs["data-cke-widget-editable"]
                ] = element;
              }
    ...
    editor.on(
      ...
      function (evt) {
        ...
        if (!evt.data.downcastingSessionId) return;
        ...
        while ((toBe = toBeDowncasted.shift())) {
          ...
          if (!retElement) retElement = widgetElement;
  ...
  function setupWidgetsLifecycleStart(widgetsRepo) {
    ...
    editor.on(
      ...
      function (evt) {
        ...
        if (evt.data.protectedWhitespaces) {
          ...
          processedWidgetOnly =
            evt.data.dataValue.children.length == 3 &&
            Widget.isParserWidgetWrapper(evt.data.dataValue.children[1]);
        } else {
          processedWidgetOnly =
            evt.data.dataValue.children.length == 1 &&
    ...
    editor.on(
      ...
      function (evt) {
        ...
        if (/data-cke-widget/.test(evt.data)) snapshotLoaded = 1;
    ...
    editor.on("paste", function (evt) {
      ...
      if (data.range) {
        ...
        if (nestedEditable) {
          ...
          // in clipboard plugin.
          var filter =
            CKEDITOR.filter.instances[nestedEditable.data("cke-filter")];
  ...
  function stateUpdater(widgetsRepo) {
    ...
    return {
      select: function (widget) {
        ...
        if (index >= 0) toBeDeselected.splice(index, 1);
      ...
      commit: function () {
        ...
        while ((widget = toBeDeselected.pop())) {
          currentlySelected.splice(
            CKEDITOR.tools.indexOf(currentlySelected, widget),
            1
          );
          // Widget could be destroyed in the meantime - e.g. data could be set.
          if (widget.isInited()) {
  ...
  function applyRemoveStyle(widget, style, apply) {
    ...
    while ((cl = classes.pop())) {
      if (apply) {
        if (!updatedClasses[cl]) changed = updatedClasses[cl] = 1;
      } else {
    ...
    if (changed) widget.setData("classes", updatedClasses);
  }
  ...
  function copySingleWidget(widget, isCut) {
    ...
    if (doc.getById("cke_copybin")) return;
    ...
    var copybinName =
        (editor.blockless || CKEDITOR.env.ie) && !isEdge16 ? "span" : "div",
    ...
    if (needsScrollHack) docElement.scrollTop = scrollTop;

    setTimeout(function () {
      // [IE] Focus widget before removing copybin to avoid scroll jump.
      if (!isCut) widget.focus();
  ...
  function onEditableBlur() {
    ...
    if (
      (editable.isInline()
        ? editable
        : editor.document.getWindow().getFrame()
      ).equals(active)
    )
  ...
  function onEditableFocus() {
    ...
    if (CKEDITOR.env.gecko) this.editor.unlockSelection();
  ...
  function setupDataClassesListener(widget) {
    ...
    widget.on("data", function () {
      ...
      if (previousClasses == newClasses) return;

      for (cl in previousClasses) {
        // Avoid removing and adding classes again.
        if (!(newClasses && newClasses[cl])) this.removeClass(cl);
      }
      for (cl in newClasses) this.addClass(cl);
  ...
  function setupDragHandler(widget) {
    ...
    if (container) img = container.findOne("img");
    else {
  ...
  function onBlockWidgetDrag(evt) {
    ...
    listeners.push(
      editable.on("mousemove", function (evt) {
        y = evt.data.$.clientY;
        buffer.input();
      })
    );
  ...
  function setupEditables(widget) {
    ...
    if (!widget.editables) return;

    for (editableName in definedEditables) {
      editableDef = definedEditables[editableName];
      widget.initEditable(
        editableName,
        typeof editableDef == "string" ? { selector: editableDef } : editableDef
      );
    }
  }
  ...
  function setupWidget(widget, widgetDef) {
    ...
    if (CKEDITOR.env.ie && CKEDITOR.env.version < 9) {
      widget.wrapper.on("dragstart", function (evt) {
        ...
        if (
          !Widget.getNestedEditable(widget, target) &&
          !(widget.inline && Widget.isDomDragHandler(target))
        )
          evt.data.preventDefault();
      });
    }
    ...
    widget.on(
      ...
      function (evt) {
        ...
        if (keyCode == 13) {
          ...
          keyCode == CKEDITOR.CTRL + 67 ||
          keyCode == CKEDITOR.CTRL + 88
        ) {
          ...
          keyCode in keystrokesNotBlockedByWidget ||
    ...
    if (widgetDef.data) widget.on("data", widgetDef.data);

    if (widgetDef.edit) widget.on("edit", widgetDef.edit);
  }
  ...
  function setupWidgetData(widget, startupData) {
    ...
    if (startupData) widget.setData(startupData);

    // Populate classes if they are not preset.
    if (!widget.data.classes) widget.setData("classes", widget.getClasses());
  ...
  function writeDataToElement(widget) {
    widget.element.data(
      "cke-widget-data",
      encodeURIComponent(JSON.stringify(widget.data))
    );
  }
  ...
  function addCustomStyleHandler() {
    ...
    CKEDITOR.style.addCustomHandler({
      ...
      setup: function (styleDefinition) {
         ...
         */
        this.group =
          typeof styleDefinition.group == "string"
            ? [styleDefinition.group]
            : styleDefinition.group;
      ...
      apply: function (editor) {
        ...
        if (!(editor instanceof CKEDITOR.editor)) return;
      ...
      removeStylesFromSameGroup: function (editor) {
        ...
        // Before CKEditor 4.4.0 wasn't a required argument, so we need to
        // handle a case when it wasn't provided.
        if (!(editor instanceof CKEDITOR.editor)) return false;
        ...
        if (this.checkApplicable(path, editor)) {
          ...
          for (var i = 0, l = this.group.length; i < l; i++) {
            ...
            for (var j = 0; j < stylesFromSameGroup.length; j++) {
              if (
                stylesFromSameGroup[j] !== this &&
                stylesFromSameGroup[j].checkActive(path, editor)
              ) {
      ...
      checkElement: function (element) {
        if (!Widget.isDomWidgetWrapper(element)) return false;
    ...
    function checkElementMatch(element, fullMatch, editor) {
      ...
      if (!this.checkElement(element)) return false;
        ...
        function deepCompare(left, right) {
          ...
          for (var key in left) {
            var areSameObjects =
              ...
              deepCompare(left[key], right[key]);
