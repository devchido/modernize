      ...
      if (CKEDITOR.env.webkit) {
        ...
        editor.on("contentDom", function (evt) {
          ...
          editor.on("paste", function (evt) {
            evt.data.dataValue = widgetselection.cleanPasteData(
              evt.data.dataValue
            );
          });
  ...
  CKEDITOR.plugins.widgetselection = {
    ...
    removeFillers: function (editable) {
      ...
      if (this.hasFiller(editable) && !this.isWholeContentSelected(editable)) {
        var startFillerContent = editable.findOne(
            this.fillerTagName + "[" + this.fillerAttribute + "=start]"
          ),
          endFillerContent = editable.findOne(
            this.fillerTagName + "[" + this.fillerAttribute + "=end]"
          );

        if (
          this.startFiller &&
          startFillerContent &&
          this.startFiller.equals(startFillerContent)
        ) {
        ...
        if (
          this.endFiller &&
          endFillerContent &&
          this.endFiller.equals(endFillerContent)
        ) {
      ...
      if (range) {
        if (range && range.collapsed) {
          ...
          return !!(
            ...
            rangeClone.endOffset === editable.getChildCount() &&
            rangeClone.startContainer.equals(editable) &&
            rangeClone.endContainer.equals(editable)
          );
        }
      }
      return false;
    ...
    removeFiller: function (filler, editable) {
      if (filler) {
        ...
        if (
          ...
          insertedHtml != this.fillerContent
        ) {
          ...
          range.setStartAt(
            editable.getChild(editable.getChildCount() - 1),
          ...
          if (fillerOnStart) {
            range.setStartAt(
              editable.getFirst().getNext(),
              CKEDITOR.POSITION_AFTER_START
            );
          } else {
            range.setEndAt(
              editable.getLast().getPrevious(),
              CKEDITOR.POSITION_BEFORE_END
            );
          }
    ...
    createFillerRegex: function (onEnd) {
      var matcher = this.createFiller(onEnd)
        .getOuterHtml()
