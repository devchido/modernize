...
(function () {
  function defaultCallback(editor, dialog, element, tabName) {
    var lang = editor.lang.devtools,
      link =
        '<a href="https://ckeditor.com/docs/ckeditor4/latest/api/CKEDITOR_dialog_definition_' +
        (element
          ? element.type == "text"
            ? "textInput"
            : element.type
    ...
    if (element)
      str +=
        "<li><strong>" + lang.elementId + "</strong> : " + element.id + "</li>";
  ...
  function showTooltip(callback, el, editor, dialog, obj, tabName) {
    var pos = el.getDocumentPosition(),
      styles = {
        ...
        top: pos.y + el.getSize("height") + "px",
      };
    ...
    if (editor._.showDialogDefinitionTooltips) {
      ...
      dialog.on("load", function () {
        ...
        for (var i = 0, len = tabs.count(); i < len; i++) {
          ...
          tab.on("mouseover", function () {
            ...
            showTooltip(
              ...
              dialog,
              null,
              id.substring(4, id.lastIndexOf("_"))
            );
          });
        ...
        dialog.foreach(function (obj) {
          if (obj.type in { hbox: 1, vbox: 1 }) return;
