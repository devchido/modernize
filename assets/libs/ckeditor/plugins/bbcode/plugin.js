...
(function () {
  ...
  var dtd = CKEDITOR.dtd,
    blockLikeTags = CKEDITOR.tools.extend(
      { table: 1 },
      dtd.$block,
      dtd.$listItem,
      dtd.$tableContent,
  ...
  for (var i in smileyMap) {
    ...
    smileyRegExp.push(
      smileyMap[i].replace(/\(|\)|\:|\/|\*|\-|\|/g, function (match) {
        return "\\" + match;
      })
    );
  }
  ...
  var decodeHtml = (function () {
    ...
    for (var entity in entities) regex.push(entity);
    ...
    parse: function (bbcode) {
      ...
      while ((parts = this._.bbcPartsRegex.exec(bbcode))) {
        ...
        if (parts[1]) {
          ...
          if (optionPart) {
            if (part == "list") {
              if (!isNaN(optionPart)) optionPart = "decimal";
              else if (/^[a-z]+$/.test(optionPart)) optionPart = "lower-alpha";
              else if (/^[A-Z]+$/.test(optionPart)) optionPart = "upper-alpha";
            }
            ...
            if (stylesMap[part]) {
              ...
              attribs[attributesMap[part]] =
                CKEDITOR.tools.htmlDecode(optionPart);
            }
          }
  ...
  CKEDITOR.htmlParser.fragment.fromBBCode = function (source) {
    ...
    function checkPendingBrs(tagName, closing) {
      var len = currentNode.children.length,
        ...
        lineBreakParent =
          !previous &&
          writer.getRule(tagnameMap[currentNode.name], "breakAfterOpen"),
        lineBreakPrevious =
          previous &&
      ...
      if (
        pendingBrs &&
        (lineBreakParent || lineBreakPrevious || lineBreakCurrent)
      )
        pendingBrs--;
      ...
      // the bogus br effect.
      if (pendingBrs && tagName in blockLikeTags) pendingBrs++;

      while (pendingBrs && pendingBrs--)
        currentNode.children.push(
          (previous = new CKEDITOR.htmlParser.element("br"))
        );
    }
    ...
    parser.onTagOpen = function (tagName, attributes) {
      ...
      if (currentDtd && !currentDtd[tagName]) {
        ...
        if (tagName == currentName) addElement(currentNode, currentNode.parent);
    ...
    parser.onText = function (text) {
      ...
      if (!currentDtd || currentDtd["#"]) {
        ...
        text.replace(/(\r\n|[\r\n])|[^\r\n]*/g, function (piece, lineBreak) {
          if (lineBreak !== undefined && lineBreak.length) pendingBrs++;
            ...
            piece.replace(smileyRegExp, function (match, index) {
              addElement(
                new CKEDITOR.htmlParser.text(piece.substring(lastIndex, index)),
                currentNode
              );
              addElement(
            ...
            if (lastIndex != piece.length)
              addElement(
                new CKEDITOR.htmlParser.text(
                  piece.substring(lastIndex, piece.length)
                ),
                currentNode
              );
          }
  ...
  var BBCodeWriter = CKEDITOR.tools.createClass({
    ...
    proto: {
      ...
      openTag: function (tag) {
        if (tag in bbcodeMap) {
          if (this.getRule(tag, "breakBeforeOpen")) this.lineBreak(1);
      ...
      openTagClose: function (tag) {
        if (tag == "br") this._.output.push("\n");
        else if (tag in bbcodeMap) {
          this.write("]");
          if (this.getRule(tag, "breakAfterOpen")) this.lineBreak(1);
        }
      },
      ...
      closeTag: function (tag) {
        if (tag in bbcodeMap) {
          if (this.getRule(tag, "breakBeforeClose")) this.lineBreak(1);

          tag != "*" && this.write("[/", tag, "]");

          if (this.getRule(tag, "breakAfterClose")) this.lineBreak(1);
        }
      },
  ...
  CKEDITOR.plugins.add("bbcode", {
    ...
    init: function (editor) {
        ...
        elements: {
          blockquote: function (element) {
            ...
            if (citeText) {
              ...
              cite.add(
                new CKEDITOR.htmlParser.text(citeText.replace(/^"|"$/g, ""))
              );
            ...
            if (element.attributes.listType) {
              if (element.attributes.listType != "decimal")
                element.attributes.style =
                  "list-style-type:" + element.attributes.listType;
            } else {
              element.name = "ul";
            }
      ...
      editor.dataProcessor.htmlFilter.addRules(
        {
          elements: {
            $: function (element) {
              ...
              if (tagName in convertMap) tagName = convertMap[tagName];
                ...
                if ((value = attributes.href)) {
                  if (value.indexOf("mailto:") !== -1) {
                    ...
                    var singleton =
                      element.children.length == 1 && element.children[0];
                    if (
                      singleton &&
                ...
                else element.children = [new CKEDITOR.htmlParser.text(src)];
              }
            ...
            div: function (element) {
              var alignment =
                CKEDITOR.tools.parseCssText(element.attributes.style, 1)[
                  "text-align"
                ] || "";
            ...
            br: function (element) {
              ...
              if (next && next.name in blockLikeTags) return false;
            },
          },
        },
        1
      ...
      else editor.on("setData", onSetData);
    },
      ...
      if (editor._.elementsPath) {
        ...
        if ((filters = editor._.elementsPath.filters)) {
          filters.push(function (element) {
            ...
            if (
              name == "link" &&
              element.getAttribute("href").indexOf("mailto:") === 0
            )
              name = "email";
            // Styled span could be either size or color.
            else if (htmlName == "span") {
              if (element.getStyle("font-size")) name = "size";
              else if (element.getStyle("color")) name = "color";
