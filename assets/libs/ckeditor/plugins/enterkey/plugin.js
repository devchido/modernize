...
(function () {
  ...
  CKEDITOR.plugins.enterkey = {
    enterBlock: function (editor, mode, range, forceMode) {
      ...
      if (block && atBlockStart && atBlockEnd) {
        ...
        // Exit the list when we're inside an empty list item block. (https://dev.ckeditor.com/ticket/5376).
          ...
          var blockGrandParent = blockParent.getParent(),
            ...
            needsBlock =
              enterMode != CKEDITOR.ENTER_BR || dirLoose || style || className,
            child;
          ...
          if (blockGrandParent.is("li")) {
            ...
            if (firstChild || lastChild) {
              ...
              block[lastChild ? "insertAfter" : "insertBefore"](
                blockGrandParent
              );
            ...
            if (path.block.is("li")) {
              ...
              newBlock = doc.createElement(
                mode == CKEDITOR.ENTER_P ? "p" : "div"
              );
            ...
            if (firstChild || lastChild)
              newBlock[firstChild ? "insertBefore" : "insertAfter"](
                blockParent
              );
            // If the empty block is neither first nor last child
      ...
      // We don't want to add new block, because it was created with range.splitBlock().
        ...
        if (previousBlock) {
          ...
          if (
            previousBlock.is("li") ||
            !(
              headerTagRegex.test(previousBlock.getName()) ||
        ...
        if (!newBlock) {
          ...
          if (node && node.is("li")) newBlock = node;
          else {
        ...
        if (elementPath) {
          for (var i = 0, len = elementPath.elements.length; i < len; i++) {
            ...
            if (
              element.equals(elementPath.block) ||
        ...
        if (
          CKEDITOR.env.ie &&
          isStartOfBlock &&
          (!isEndOfBlock || !previousBlock.getChildCount())
        ) {
      ...
      function preventExtraLineInsideTable(mode) {
        ...
        if (
          CKEDITOR.tools.indexOf(["td", "th"], path.lastElement.getName()) ===
          -1
        ) {
          return false;
        }
  ...
  function enter(editor, mode, forceMode) {
    ...
    if (!mode) mode = editor.activeEnterMode;
