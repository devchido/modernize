...
(function () {
  var List,
    ...
    invalidTags = ["o:p", "xml", "script", "meta", "link"],
    shapeTags = [
      "v:arc",
      "v:curve",
      "v:line",
  ...
  CKEDITOR.cleanWord = function (mswordHtml, editor) {
    var msoListsDetected = Boolean(
        mswordHtml.match(/mso-list:\s*l\d+\s+level\d+\s+lfo\d+/)
      ),
      shapesIds = [];
    ...
    function shapeTagging(element) {
      ...
      if (
        element.attributes["o:gfxdata"] ||
        element.parent.name === "v:group"
      ) {
    ...
    if (CKEDITOR.plugins.clipboard.isCustomDataTypesSupported) {
      mswordHtml = CKEDITOR.plugins.pastefromword.styles.inliner
        .inline(mswordHtml)
        .getBody()
        .getHtml();
    }
    ...
    var filterDefinition = {
      ...
      elements: {
        ...
        div: function (element) {
          ...
          if (
            ...
            element.attributes["data-cke-pagebreak"]
          ) {
            return element;
          }
        ...
        img: function (element) {
          ...
          if (
            ...
            element.attributes.src.match(/^file:\/\//) &&
            element.attributes.alt &&
            element.attributes.alt.match(/^https?:\/\//)
          ) {
            element.attributes.src = element.attributes.alt;
          }

          var imgShapesIds = element.attributes["v:shapes"]
            ? element.attributes["v:shapes"].split(" ")
            : [];
          ...
          var isShapeFromList = CKEDITOR.tools.array.every(
            imgShapesIds,
            function (shapeId) {
        ...
        p: function (element) {
          ...
          if (
            ...
            element.attributes.style.match(/display:\s*none/i)
          ) {
            return false;
          }
          ...
          if (List.thisIsAListItem(editor, element)) {
            ...
            tools.array.reduce(
              element.children,
              function (paragraphsReplaced, node) {
                if (node.name === "p") {
            ...
            if (
              ...
              container.attributes["cke-list-level"] =
                style["mso-list"].match(/level(\d+)/)[1];
            }
        ...
        font: function (element) {
          ...
          if (
            CKEDITOR.dtd.tr[element.parent.name] &&
            CKEDITOR.tools.arrayCompare(
              CKEDITOR.tools.object.keys(element.attributes),
              ["class", "style"]
            )
          ) {
            Style.createStyleStack(element, filter, editor);
        ...
        ul: function (element) {
          ...
          if (
            ...
            tools.indexOf(element.parent.children, element) === 0
          ) {
        ...
        table: function (element) {
          ...
          if (
            parent.name &&
            parent.name === "div" &&
            parent.attributes.align &&
            tools.object.keys(parent.attributes).length === 1 &&
        ...
        td: function (element) {
          var ascendant = element.getAscendant("table"),
            ascendantStyle = tools.parseCssText(
              ascendant.attributes.style,
              true
            );
      ...
      text: function (content, node) {
        ...
        if (
          ...
          grandparent.attributes.style &&
          grandparent.attributes.style.match(/mso-list:\s*ignore/i)
        ) {
  ...
  CKEDITOR.plugins.pastefromword.styles = {
    ...
    normalizedStyles: function (element, editor) {
      ...
      var resetStyles = [
        ...
        removeFontStyles =
          editor && editor.config.pasteFromWordRemoveFontStyles === true;
      ...
      if (element.name == "cke:li") {
        ...
        if (styles["TEXT-INDENT"] && styles.MARGIN) {
          element.attributes["cke-indentation"] =
            List.getElementIndentation(element);
          styles.MARGIN = styles.MARGIN.replace(
            /(([\w\.]+ ){3,3})[\d\.]+(\w+$)/,
            "$10$3"
          );
        } else {
      ...
      for (var i = 0; i < keys.length; i++) {
        var styleName = keys[i].toLowerCase(),
          ...
          toBeRemoved =
            removeFontStyles &&
            indexOf(textStyles, styleName.toLowerCase()) !== -1;
      ...
      var keepZeroMargins =
        editor && editor.config.pasteFromWord_keepZeroMargins;
    ...
    pushStylesLower: function (element, exceptions, wrapText) {
      if (!element.attributes.style || element.children.length === 0) {
        return false;
      }
      ...
      for (var style in styles) {
        if (
          style.toLowerCase() in retainedStyles ||
    ...
    inliner: {
      ...
      parse: function (styles) {
        ...
        function getStyles(cssText) {
          ...
          return parseCssText(
            cssText.substring(startIndex + 1, endIndex),
            true
          );
        }
      ...
      inline: function (html) {
        ...
        function applyStyle(document, selector, style) {
          ...
          for (i = 0; i < elements.count(); i++) {
            ...
            oldStyle = CKEDITOR.tools.parseCssText(
            ...
            element.setAttribute(
              "style",
              CKEDITOR.tools.writeCssText(newStyle)
            );
          }
        }
  ...
  CKEDITOR.plugins.pastefromword.lists = {
    ...
    thisIsAListItem: function (editor, element) {
      ...
      if (
        (element.attributes.style &&
          element.attributes.style.match(/mso\-list:\s?l\d/) &&
          element.parent.name !== "li") ||
    ...
    convertToFakeListItem: function (editor, element) {
      ...
      var styles =
        element.attributes && tools.parseCssText(element.attributes.style);
    ...
    removeSymbolText: function (element) {
      // ...from a list element.
      ...
      switch (list.attributes["cke-list-style-type"]) {
        ...
        case "upper-roman":
          list.attributes.start =
            List.toArabic(List.getSubsectionSymbol(symbols[offset])) - offset;
          break;
        case "lower-alpha":
        case "upper-alpha":
          list.attributes.start =
            List.getSubsectionSymbol(symbols[offset])
              .replace(/\W/g, "")
              .toLowerCase()
              .charCodeAt(0) -
            96 -
        ...
        case "decimal":
          list.attributes.start =
            parseInt(List.getSubsectionSymbol(symbols[offset]), 10) - offset ||
            1;
          break;
      }
    ...
    createList: function (element) {
      ...
      if (
        (element.attributes["cke-symbol"].match(/([\da-np-zA-NP-Z]).?/) ||
          [])[1]
      ) {
      ...
      for (i = 0; i < lists.length; i++) {
        ...
        if (containerStack[0].children.length) {
          ...
          if (!level1Symbol && containerStack[0].children.length > 1) {
            level1Symbol =
              containerStack[0].children[1].attributes["cke-symbol"];
          }
        ...
        return CKEDITOR.tools.array.reduce(
          ...
          function (total, element) {
            if (element.attributes && element.attributes.style) {
              var marginLeft = CKEDITOR.tools.parseCssText(
                element.attributes.style
              )["margin-left"];
            }
    ...
    determineListItemValue: function (element) {
      ...
      if (cleanSymbol) {
        ...
        listType =
          element.parent.attributes["cke-list-style-type"] ||
          this.numbering.getStyle(cleanSymbol);
    ...
    calculateValue: function (element) {
      ...
      for (i = elementIndex; i >= 0 && valueFound === null; i--) {
        ...
        if (
          ...
          curElement.attributes.value !== undefined
        ) {
          valueElementIndex = i;
      ...
      if (valueFound === null) {
        valueFound =
          list.attributes.start !== undefined
            ? parseInt(list.attributes.start, 10)
            : 1;
        valueElementIndex = 0;
      }
    ...
    dissolveList: function (element) {
      ...
      arrayTools.forEach(lists, function (list) {
        ...
        arrayTools.forEach(
          arrayTools.filter(list.children, nameIs("li")),
          function (child, index) {
            ...
            switch (type) {
              ...
              case "lower-alpha":
                symbol =
                  String.fromCharCode("a".charCodeAt(0) + start - 1 + index) +
                  ".";
                break;
              case "A":
              case "upper-alpha":
      ...
      children = arrayTools.reduce(
        items,
        function (acc, listElement) {
          ...
          if (
            ...
            var styleName = childStyle.display
              ? "display"
              : childStyle.DISPLAY
                ? "DISPLAY"
                : "";
    ...
    chopDiscontinuousLists: function (list, lists) {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        if (lastSymbol) {
          // An "h" before an "i".
          forceType =
            lastSymbol.type.match(/alpha/) && lastSymbol.index == 7
              ? "alpha"
              : forceType;
          // An "n" before an "o".
          forceType =
            list[i].attributes["cke-symbol"] == "o" && lastSymbol.index == 14
              ? "alpha"
              : forceType;

          currentSymbol = List.getSymbolInfo(
            list[i].attributes["cke-symbol"],
            forceType
          );
          currentListInfo = this.getListItemInfo(list[i]);
          ...
          if (
            ...
            (lastListInfo &&
              currentListInfo.id != lastListInfo.id &&
              !this.isAListContinuation(list[i]))
          ) {
            choppedLists.push([]);
          }
        } else {
        ...
        for (
          var j = parseInt(list[i].attributes["cke-list-level"], 10) + 1;
          j < 20;
          j++
        ) {
          if (levelSymbols[j]) {
      ...
      [].splice.apply(
        lists,
        [].concat([tools.indexOf(lists, list), 1], choppedLists)
      );
    },
    ...
    isAListContinuation: function (listElement) {
      ...
      do {
        ...
        if (prev && prev.type === CKEDITOR.NODE_ELEMENT) {
          ...
          if (
            prev.attributes["cke-list-level"] ===
            listElement.attributes["cke-list-level"]
          ) {
            // Same level, so we want to check if this is a continuation.
            return (
              prev.attributes["cke-list-id"] ===
              listElement.attributes["cke-list-id"]
            );
          }
        }
      } while (prev);
    ...
    getListItemInfo: function (list) {
      ...
      list.attributes["cke-list-level"] =
        list.attributes["cke-list-level"] !== undefined
          ? list.attributes["cke-list-level"]
          : ret.level;
  ...
  CKEDITOR.plugins.pastefromword.heuristics = {
    ...
    isEdgeListItem: function (editor, item) {
      if (
        !CKEDITOR.env.edge ||
        !editor.config.pasteFromWord_heuristicsEdgeList
      ) {
        return false;
      }
    ...
    isDegenerateListItem: function (editor, item) {
      return (
        !!item.attributes["cke-list-level"] ||
        (item.attributes.style &&
          !item.attributes.style.match(/mso\-list/) &&
          !!item.find(function (child) {
            ...
            if (
              ...
              item.name.match(/h\d/i) &&
              child.getHtml().match(/^[a-zA-Z0-9]+?[\.\)]$/)
            ) {
              return true;
            }

            var css = tools.parseCssText(
            ...
            return (
              (fontSize.match(/7pt/i) && !!child.previous) ||
    ...
    assignListLevels: function (editor, item) {
      ...
      while (
        item.next &&
        item.next.attributes &&
        !item.next.attributes["cke-list-level"] &&
        Heuristics.isDegenerateListItem(editor, item.next)
      ) {
        item = item.next;
      ...
      var indentationDiffs = map(indents, function (curIndent, i) {
        ...
        indentationPerLevel = this.guessIndentationStep(
          array.filter(indents, function (val) {
            return val !== 0;
          })
        );
    ...
    correctLevelShift: function (element) {
      var isShiftedList = function (list) {
        return (
          ...
          Heuristics.isShifted(list.children[0])
        );
      };

      if (this.isShifted(element)) {
        var lists = CKEDITOR.tools.array.filter(
          element.children,
          function (child) {
        ...
        var listChildren = CKEDITOR.tools.array.reduce(
          lists,
          function (acc, list) {
    ...
    isShifted: function (element) {
      ...
      return (
        CKEDITOR.tools.array.filter(element.children, function (child) {
          if (child.name) {
  ...
  List.setListSymbol.removeRedundancies = function (style, level) {
    ...
    if (
      (level === 1 && style["list-style-type"] === "disc") ||
