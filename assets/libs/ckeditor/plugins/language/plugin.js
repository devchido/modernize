...
(function () {
  ...
  CKEDITOR.plugins.add("language", {
    ...
    init: function (editor) {
      var languagesConfigStrings = editor.config.language_list || [
          "ar:Arabic:rtl",
          "fr:French",
          "es:Spanish",
        ],
      ...
      editor.addCommand("language", {
        ...
        exec: function (editor, languageId) {
          ...
          if (item)
            editor[
              item.style.checkActive(editor.elementPath(), editor)
                ? "removeStyle"
                : "applyStyle"
            ](item.style);
        },
        refresh: function (editor) {
          this.setState(
            plugin.getCurrentLangElement(editor)
      ...
      items.language_remove = {
        ...
        onClick: function () {
          ...
          if (currentLanguagedElement)
            editor.execCommand(
              "language",
              currentLanguagedElement.getAttribute("lang")
            );
        },
      };
      ...
      editor.ui.add("Language", CKEDITOR.UI_MENUBUTTON, {
        ...
        onMenu: function () {
          ...
          for (var prop in items) activeItems[prop] = CKEDITOR.TRISTATE_OFF;

          activeItems.language_remove = currentLanguagedElement
            ? CKEDITOR.TRISTATE_OFF
            : CKEDITOR.TRISTATE_DISABLED;

          if (currentLanguagedElement)
            activeItems[
              "language_" + currentLanguagedElement.getAttribute("lang")
            ] = CKEDITOR.TRISTATE_ON;
