  ...
  var pasteTextCmd = {
    ...
    exec: function (editor, data) {
      var lang = editor.lang,
        ...
        keyInfo = CKEDITOR.tools.keystrokeToString(
          lang.common.keyboard,
          editor.getCommandKeystroke(
            CKEDITOR.env.ie ? editor.commands.paste : this
          )
        ),
        notification =
          data && typeof data.notification !== "undefined"
            ...
            : !data ||
              !data.from ||
              (data.from === "keystrokeHandler" && CKEDITOR.env.ie),
        msg =
          notification && typeof notification === "string"
            ? notification
            : lang.pastetext.pasteNotification.replace(
                /%1/,
                '<kbd aria-label="' +
                  keyInfo.aria +
                  '">' +
  ...
  CKEDITOR.plugins.add("pastetext", {
    ...
    init: function (editor) {
      var commandName = "pastetext",
        pasteKeystroke = !CKEDITOR.env.safari
          ? CKEDITOR.CTRL + CKEDITOR.SHIFT + 86 // Ctrl + Shift + V
