...
(function () {
  ...
  function getStylesForEditor(editor) {
    ...
    return CKEDITOR.tools.object.merge(
      defaultStyles,
      editor.config.easyimage_styles
    );
  }
  ...
  function addCommands(editor, styles) {
    function createCommandRefresh(enableCheck) {
      return function (editor, path) {
        ...
        if (widget && widget.name === WIDGET_NAME) {
          var callbackResolution =
            enableCheck && enableCheck.call(this, widget, editor, path);

          newState = callbackResolution
    ...
    function addStylesCommands(styles) {
      ...
      editor.on("beforeCommandExec", function (evt) {
        ...
        if (
          getStyleNameFromCommand(evt.data.name, styles) &&
  ...
  function registerWidget(editor, styles) {
      ...
      widgetDefinition = {
        ...
        upcasts: {
          figure: function (element) {
            if (
              (!figureClass || element.hasClass(figureClass)) &&
        ...
        _loadDefaultStyle: function () {
          ...
          for (var styleName in styles) {
            ...
            if (
              ...
              cmd &&
              cmd.style &&
              CKEDITOR.tools.array.indexOf(cmd.style.group, "easyimage") !==
                -1 &&
    ...
    if (editor.plugins.link) {
      widgetDefinition = CKEDITOR.plugins.imagebase.addFeature(
        editor,
        "link",
        widgetDefinition
      );
    ...
    editor.on("paste", function (evt) {
      ...
      for (i = 0; i < imgs.count(); i++) {
        ...
        if (isDataInSrc && isRealObject && !img.isReadOnly(1)) {
          ...
          widgetElement = easyImageDef._insertWidget(
            editor,
            easyImageDef,
            imgSrc,
            false,
