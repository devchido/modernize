...
(function () {
  ...
  var Jsonp = {
    ...
    sendRequest: function (urlTemplate, urlParams, callback, errorCallback) {
      ...
      scriptElement = this._attachScript(
        urlTemplate.output(urlParams),
        function () {
          cleanUp();
  ...
  function createWidgetBaseDefinition(editor) {
    ...
    return {
      ...
      isUrlValid: function (url) {
        return (
          this.urlRegExp.test(url) && this.fire("validateUrl", url) !== false
        );
      },
      ...
      getErrorMessage: function (messageTypeOrMessage, url, suffix) {
        var message =
          editor.lang.embedbase[messageTypeOrMessage + (suffix || "")];
        if (!message) {
          message = messageTypeOrMessage;
        }

        return new CKEDITOR.template(message).output({
          url: url || "",
        });
      },
      ...
      _handleError: function (request, messageTypeOrMessage) {
        if (request.task) {
          ...
          if (!request.noNotifications) {
            editor.showNotification(
              this.getErrorMessage(messageTypeOrMessage, request.url),
              "warning"
            );
          }
        }
      ...
      _responseToHtml: function (url, response) {
        if (response.type == "photo") {
          return (
            '<img src="' +
            CKEDITOR.tools.htmlEncodeAttr(response.url) +
            '" ' +
            'alt="' +
            CKEDITOR.tools.htmlEncodeAttr(response.title || "") +
      ...
      _createTask: function () {
        if (!aggregator || aggregator.isFinished()) {
          aggregator = new CKEDITOR.plugins.notificationAggregator(
            editor,
            lang.fetchingMany,
            lang.fetchingOne
          );
