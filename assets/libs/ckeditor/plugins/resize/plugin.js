  ...
  init: function (editor) {
    function dragHandler(evt) {
      ...
      if (resizeHorizontal)
        width = Math.max(
          ...
          Math.min(internalWidth, config.resize_maxWidth)
        );

      if (resizeVertical)
        height = Math.max(
          config.resize_minHeight,
          Math.min(internalHeight, config.resize_maxHeight)
        );
    ...
    if (config.resize_enabled !== false) {
      var container = null,
        ...
        startSize,
        resizeHorizontal =
          (config.resize_dir == "both" || config.resize_dir == "horizontal") &&
          config.resize_minWidth != config.resize_maxWidth,
        resizeVertical =
          (config.resize_dir == "both" || config.resize_dir == "vertical") &&
          config.resize_minHeight != config.resize_maxHeight;

      var mouseDownFn = CKEDITOR.tools.addFunction(function ($event) {
        if (!container) container = editor.getResizable();
      ...
      editor.on("maximize", function (event) {
        editor.ui
          .space("resizer")
          [event.data == CKEDITOR.TRISTATE_ON ? "hide" : "show"]();
      });
    }
  },
});
