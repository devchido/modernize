  ...
  commands: {
    removeformat: {
      exec: function (editor) {
        var tagsRegex =
          editor._.removeFormatRegex ||
          (editor._.removeFormatRegex = new RegExp(
            "^(?:" + editor.config.removeFormatTags.replace(/,/g, "|") + ")$",
            "i"
          ));

        var removeAttributes =
            editor._.removeAttributes ||
            (editor._.removeAttributes =
              editor.config.removeFormatAttributes.split(",")),
        ...
        while ((range = iterator.getNextRange())) {
          ...
          bookmarkForRangeRecreation.endNode &&
            range.setEndAfter(bookmarkForRangeRecreation.endNode);

          if (!range.collapsed) range.enlarge(CKEDITOR.ENLARGE_ELEMENT);
          ...
          var breakParent = function (node) {
            ...
            for (var i = 1, pathElement; (pathElement = pathElements[i]); i++) {
              ...
              if (
                tagsRegex.test(pathElement.getName()) &&
                filter(editor, pathElement)
              )
          ...
          if (endNode) {
            ...
            while (currentNode) {
              ...
              if (currentNode.isReadOnly()) {
                ...
                if (
                  currentNode.getPosition(endNode) & CKEDITOR.POSITION_CONTAINS
                ) {
                  break;
                }
