...
CKEDITOR.dialog.add("textfield", function (editor) {
  ...
  return {
    ...
    onOk: function () {
      ...
      if (isInsertMode) editor.insertElement(data.element);

      this.commitContent(data);

      // Element might be replaced by commitment.
      if (!isInsertMode) editor.getSelection().selectElement(data.element);
    },
    onLoad: function () {
    ...
    contents: [
      {
        ...
        elements: [
              ...
              },
              {
                ...
                commit: function (data) {
                  if (CKEDITOR.env.ie && !this.getValue()) {
                    var element = data.element,
                      fresh = new CKEDITOR.dom.element(
                        "input",
                        editor.document
                      );
          ...
          },
          {
            ...
            commit: function (data) {
              var element = data.element;
              if (this.getValue()) element.setAttribute("required", "required");
