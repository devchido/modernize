...
CKEDITOR.dialog.add("button", function (editor) {
  function commitAttributes(element) {
    ...
    if (val) {
      ...
      if (this.id == "name") element.attributes["data-cke-saved-name"] = val;
    } else {
      delete element.attributes[this.id];
      if (this.id == "name") delete element.attributes["data-cke-saved-name"];
    }
  }
  ...
  return {
    ...
    onOk: function () {
      ...
      var fake = element
        ? CKEDITOR.htmlParser.fragment.fromHtml(element.getOuterHtml())
            .children[0]
      ...
      if (isInsertMode) editor.insertElement(newElement);
      else {
