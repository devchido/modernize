...
(function () {
  ...
  function LoadStylesCSS(theDoc, skipSelectors, validSelectors) {
    ...
    for (i = 0; i < theDoc.styleSheets.length; i++) {
      ...
      if (node.getAttribute("data-cke-temp")) continue;

      // Exclude stylesheets injected by extensions
      if (sheet.href && sheet.href.substr(0, 9) == "chrome://") continue;
        ...
        function (evt) {
          ...
          editor.once("contentDom", function () {
            editor.getStylesSet(function (definitions) {
              ...
              var skipSelectors =
                  editor.config.stylesheetParser_skipSelectors ||
                  /(^body\.|^\.)/i,
                // Rules that are valid
                validSelectors =
                  editor.config.stylesheetParser_validSelectors || /\w+\.\w+/;

              cachedDefinitions = definitions.concat(
                LoadStylesCSS(editor.document.$, skipSelectors, validSelectors)
              );

              editor.getStylesSet = function (callback) {
                if (cachedDefinitions) return callback(cachedDefinitions);
              };
