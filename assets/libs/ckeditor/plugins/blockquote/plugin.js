...
(function () {
  function noBlockLeft(bqBlock) {
    for (
      var i = 0, length = bqBlock.getChildCount(), child;
  ...
  var commandObject = {
    exec: function (editor) {
      ...
      if (CKEDITOR.env.ie) {
        ...
        if (
          bookmarkStart &&
          bookmarkStart.getParent().getName() == "blockquote"
        ) {
          cursor = bookmarkStart;
          while ((cursor = cursor.getNext())) {
            if (
              cursor.type == CKEDITOR.NODE_ELEMENT &&
      ...
      if (state == CKEDITOR.TRISTATE_OFF) {
        ...
        while ((block = iterator.getNextParagraph())) {
          ...
          if (
            bqParent &&
            bqChild &&
            !bqChild.getCustomData("blockquote_moveout")
          ) {
          ...
          while (movedNodes.length) {
            ...
            if (node.getName() == "div") {
              ...
              var needBeginBr =
                ...
                node.getPrevious() &&
                !(
                  node.getPrevious().type == CKEDITOR.NODE_ELEMENT &&
              ...
              var needEndBr =
                node.getNext() &&
                !(
                  node.getNext().type == CKEDITOR.NODE_ELEMENT &&
                  node.getNext().isBlockBoundary()
                );
              while (node.getFirst())
    ...
    refresh: function (editor, path) {
      ...
      this.setState(
        editor.elementPath(firstBlock).contains("blockquote", 1)
