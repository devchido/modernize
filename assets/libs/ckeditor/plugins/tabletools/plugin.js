...
(function () {
  ...
  function getSelectedCells(selection, table) {
    ...
    function isInTable(cell) {
      ...
      return (
        table.contains(cell) && cell.getAscendant("table", true).equals(table)
      );
    }
      ...
      if (range.collapsed) {
        ...
        while ((node = walker.next())) {
          ...
          if (
            ...
            if (
              parent &&
              !parent.getCustomData("selected_cell") &&
              isInTable(parent)
            ) {
  ...
  function insertRow(selectionOrCells, insertBefore) {
    var cells = isArray(selectionOrCells)
        ? selectionOrCells
        : getSelectedCells(selectionOrCells),
      firstCell = cells[0],
  ...
  function deleteColumns(selection) {
    function processSelection(selection) {
      ...
      var ranges, range, endNode, endNodeName, previous;
      ...
      while (
        ...
        previous.getName().toLowerCase() !== "br"
      ) {
              ...
              [
                {
                  ...
                  left: function (element) {
                    return (
                      ...
                      styleParse.background(element.styles.background).color
                    );
                  },
                  right: function (element) {
                    element.styles["background-color"] = styleParse.background(
                      element.styles.background
                    ).color;
                  },
                },
                {
                  ...
                  right: function (element) {
                    element.styles["vertical-align"] =
                      element.attributes.valign;
                    ...
                    CKEDITOR.tools.array.forEach(
                      ...
                      function (node) {
                        if (node.name in { td: 1, th: 1 }) {
                          node.attributes["cke-row-height"] =
              ...
              [
                {
                  ...
                  right: function (element) {
                    element.styles.height =
                      element.attributes["cke-row-height"];
              ...
              var selection = editor.getSelection(),
                ...
                items = {
                  ...
                  tablecell_split_vertical: verticalSplitCell(selection, true)
                    ? CKEDITOR.TRISTATE_OFF
                    : CKEDITOR.TRISTATE_DISABLED,
                  tablecell_split_horizontal: horizontalSplitCell(
                    selection,
                    true
                  )
                    ? CKEDITOR.TRISTATE_OFF
 ...
 */
CKEDITOR.tools.buildTableMap = function (
  table,
  startRow,
  startCell,
  endRow,
  ...
  for (var i = startRow; i <= endRow; i++) {
    ...
    for (
      var j = startCell;
      j <= (endCell === -1 ? aRows[i].cells.length - 1 : endCell);
      j++
    ) {
