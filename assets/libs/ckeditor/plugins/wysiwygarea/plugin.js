...
(function () {
    ...
    init: function (editor) {
      ...
      editor.addMode("wysiwyg", function (callback) {
        ...
        if (CKEDITOR.env.air) {
          ...
          src =
            "javascript:void(function(){" + encodeURIComponent(src) + "}())"; // jshint ignore:line
        } else {
          src = "";
        }
        ...
        if (frameLabel) {
          if (CKEDITOR.env.ie && helpLabel) frameLabel += ", " + helpLabel;
  ...
  function onDomReady(win) {
    ...
    if (
      CKEDITOR.env.ie &&
      !CKEDITOR.env.edge &&
      editor.enterMode != CKEDITOR.ENTER_P
    ) {
    ...
    function removeSuperfluousElement(tagName) {
      ...
      editable.attachListener(editable, "keyup", function () {
        ...
        if (lockRetain) {
          if (
            elements.count() == 1 &&
            !elements.getItem(0).getCustomData("retain") &&
      ...
      editor.editable().attachListener(editor, "selectionChange", function () {
        ...
        if (selectedElement) {
          if (lastListeningElement) {
            lastListeningElement.detachEvent(
              "onresizestart",
              resizeStartListener
            );
