...
(function () {
  ...
  function isWidget(element) {
    return (
      CKEDITOR.plugins.widget && CKEDITOR.plugins.widget.isDomWidget(element)
    );
  }
  ...
  function rangeContainsTableElement(range) {
    if (range) {
      ...
      return (
        enclosedNode &&
        enclosedNode.is &&
        enclosedNode.is(CKEDITOR.dtd.$tableContent)
      );
    }
  }
  ...
  function clearFakeCellSelection(editor, reset) {
    var selectedCells = editor.editable().find("." + fakeSelectedClass),
      selectedTable = editor
        .editable()
        .findOne("[data-" + fakeSelectedTableDataAttribute + "]"),
      i;
  ...
  function fakeSelectByMouse(editor, cellOrTable, evt) {
    ...
    if (
      ...
      fakeSelection.first
        .getAscendant("table")
        .equals(cell.getAscendant("table"))
    ) {
  ...
  function fakeSelectionChangeHandler(evt) {
    ...
    if (
      ranges[0]
        ...
        .hasAttribute(ignoredTableAttribute)
    ) {
      return;
    }
  ...
  function fakeSelectionMouseHandler(evt) {
    ...
    function isSameTable(selectedTable, table) {
      ...
      return (
        selectedTable.equals(table) ||
        selectedTable.contains(table) ||
        table.contains(selectedTable) ||
    ...
    function isOutsideTable(node) {
      return (
        !node.getAscendant("table", true) &&
        node.getDocument().equals(editor.document)
      );
    }
    ...
    function canClearSelection(evt, selection, selectedTable, table) {
      ...
      var isProperMouseEvent =
        evt.name === (CKEDITOR.env.gecko ? "mousedown" : "mouseup");
      // Covers a case when:
    ...
    if (evt.name === "mouseup") {
      ...
      if (
        detectLeftMouseButton(evt) &&
        (isOutsideTable(evt.data.getTarget()) ||
          isSameTable(selectedTable, table))
      ) {
  ...
  TableSelection.prototype.getTableMap = function () {
    ...
    return CKEDITOR.tools.buildTableMap(
      this._getTable(),
      getRowIndex(this.rows.first),
      startIndex,
      getRowIndex(this.rows.last),
      endIndex
    );
  };
  ...
  TableSelection.prototype.insertRow = function (
    count,
    insertBefore,
    clearSelection
  ) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      row = insertRow(
        clearSelection ? this.cells.all : selectedCells,
        insertBefore
      );

      // Append cells from added row.
      newCells = CKEDITOR.tools.array.filter(
        row.find("td, th").toArray(),
        function (cell) {
          return clearSelection
            ? true
            : cell.$.cellIndex >= cellIndexFirst &&
                cell.$.cellIndex <= cellIndexLast;
        }
      );
  ...
  TableSelection.prototype.insertColumn = function (count) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      selectedCells = selectedCells.concat(
        CKEDITOR.tools.array.filter(insertColumn(selectedCells), limitCells)
      );
    }
  ...
  var fakeSelectionPasteHandler = {
    ...
    findTableInPastedContent: function (editor, dataValue) {
      ...
      return tmpContainer.getChildCount() > 1
        ? null
        : tmpContainer.findOne("table");
    },
    ...
    pasteTable: function (tableSel, selectedTableMap, pastedTableMap) {
      ...
      for (i = 0; i < pastedTableMap.length; i++) {
        currentRow = new CKEDITOR.dom.element(
          selectedTable.$.rows[tableSel.rows.first.$.rowIndex + i]
        );
            ...
            if (cellToReplace && cellToReplace.getParent()) {
              ...
              if (prevCell && currentRow.equals(prevCell.getParent())) {
                ...
                if (currentRow.$.cells[startIndex]) {
                  cellToPaste.insertAfter(
                    new CKEDITOR.dom.element(currentRow.$.cells[startIndex])
                  );
                } else {
  ...
  function pasteListener(evt) {
    ...
    var selectedCells = getSelectedCells(selection),
      ...
      boundarySelection =
        selection.isInTable(true) && this.isBoundarySelection(selection),
      tableSel,
      selectedTable,
      selectedTableMap,
      pastedTableMap;
    ...
    editor.once("afterPaste", function () {
      var toSelect = pastedTableMap
        ? getCellsBetween(
            new CKEDITOR.dom.element(pastedTableMap[0][0]),
            new CKEDITOR.dom.element(
              getLastArrayItem(getLastArrayItem(pastedTableMap))
            )
          )
        : tableSel.cells.all;
    ...
    tableSel.insertColumn(
      getLongestRowLength(pastedTableMap) -
        getLongestRowLength(selectedTableMap)
    );
    ...
    keyboardIntegration: function (editor) {
      ...
      function getTableOnKeyDownListener(editor) {
        ...
        function deleteEmptyTablePart(node, ranges) {
          ...
          if (startNode && startNode.is && startNode.is(tags)) {
            ...
            var boundaryTable = startNode.getAscendant("table", true),
              targetNode = startNode.getPreviousSourceNode(
                false,
              ...
              matchingElement = function (elem) {
                ...
                return (
                  !startNode.contains(elem) && elem.is && elem.is("td", "th")
                );
              };

            while (targetNode && !matchingElement(targetNode)) {
              targetNode = targetNode.getPreviousSourceNode(
                false,
                CKEDITOR.NODE_ELEMENT,
                boundaryTable
              );
            ...
            if (!targetNode) {
              ...
              rng["moveToElementEdit" + (selectBeginning ? "Start" : "End")](
                targetNode
              );
            }
        ...
        return function (evt) {
          ...
          if (key > 36 && key < 41) {
            // Arrows.
            ranges[0].moveToElementEditablePosition(
              toStart ? firstCell : lastCell,
              !toStart
            );
            ...
            if (
              key === 13 &&
              !(keystroke === 13 || keystroke === CKEDITOR.SHIFT + 13)
            ) {
              return;
            }
      ...
      editable.attachListener(
        editable,
        "keydown",
        getTableOnKeyDownListener(editor),
        null,
        null,
        -1
      );
  ...
  CKEDITOR.plugins.add("tableselection", {
    ...
    init: function (editor) {
      ...
      editor.on(
        "paste",
        fakeSelectionPasteHandler.onPaste,
        fakeSelectionPasteHandler
      );

      customizeTableCommand(
        editor,
        [
          "rowInsertBefore",
          "rowInsertAfter",
      ...
      customizeTableCommand(editor, ["cellDelete"], function (editor) {
        clearFakeCellSelection(editor, true);
      });
    },
  });
