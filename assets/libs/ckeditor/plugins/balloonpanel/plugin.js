...
(function () {
  ...
  CKEDITOR.ui.balloonPanel.prototype = {
    ...
    templateDefinitions: {
      panel:
        "<div" +
        ' class="cke {id} cke_reset_all cke_chrome cke_balloon cke_editor_{name} cke_{langDir} ' +
      ...
      triangleOuter:
        '<span class="cke_balloon_triangle cke_balloon_triangle_outer"></span>',

      triangleInner:
        '<span class="cke_balloon_triangle cke_balloon_triangle_inner">&#8203;</span>',
    },
    ...
    attach: (function () {
      ...
      function getTopAndBottomRects(rectList) {
        var topAlignedRects = getAlignedRects(rectList, true),
          ...
          last = createLineRect(
            bottomAlignedRects[0],
            bottomAlignedRects.pop()
          );
      ...
      return function (elementOrSelection, options) {
        ...
        var panelWidth = this.getWidth(),
          ...
          areaDifference,
          elementRect =
            elementOrSelection.getClientRect &&
            elementOrSelection.getClientRect(true),
          editorRect = isInline
          ...
          allowedRect = {
            ...
            right: Math.min(
              editorRect.right,
              viewPaneSize.width + winGlobalScroll.x
            ),
            bottom: Math.min(
              editorRect.bottom,
              viewPaneSize.height + winGlobalScroll.y
            ),
          },
          alignmentKeys;
        ...
        if (selectionRects) {
          CKEDITOR.tools.array.forEach(
            selectionRects,
            function (item) {
              this._adjustElementRect(
                item,
                isInline ? allowedRect : editorRect
              );
            },
            this
          );

          alignments = this._getAlignments(
            selectionRects[0],
            panelWidth,
            panelHeight
          );
          ...
          if (selectionRects.length > 1) {
            alignments["bottom hcenter"] = this._getAlignments(
              selectionRects[1],
              panelWidth,
              panelHeight
            )["bottom hcenter"];
        ...
        for (var a in alignmentKeys || alignments) {
          ...
          areaDifference = alignments[a].areaDifference =
            panelArea - rectIntersectArea(alignmentRect, allowedRect);
          ...
          if (
            areaDifference < alignments[minDifferenceAlignment].areaDifference
          ) {
            minDifferenceAlignment = a;
          }
        }
        ...
        var parent = this.parts.panel.getAscendant(function (el) {
            return el instanceof CKEDITOR.dom.document
              ? false
              : el.getComputedStyle("position") !== "static";
          }),
          parentMargin = {
            left: parent
              ? parseInt(parent.getComputedStyle("margin-left"), 10)
              : 0,
            top: parent
              ? parseInt(parent.getComputedStyle("margin-top"), 10)
              : 0,
          };

        this.move(
          alignments[minDifferenceAlignment].top - parentMargin.top,
          alignments[minDifferenceAlignment].left - parentMargin.left
        );

        minDifferenceAlignment = minDifferenceAlignment.split(" ");
        this.setTriangle(
          triangleRelativePosition[minDifferenceAlignment[0]],
          minDifferenceAlignment[1]
        );
    ...
    _getAlignments: function (elementRect, panelWidth, panelHeight) {
      return {
        ...
        "top left": {
          top: elementRect.top - panelHeight - this.triangleHeight,
          left:
            elementRect.left + elementRect.width / 2 - this.triangleMinDistance,
        },
        "top right": {
          top: elementRect.top - panelHeight - this.triangleHeight,
          left:
    ...
    _adjustElementRect: function (elementRect, editorRect) {
      elementRect.left = numberInRange(
        editorRect.left,
        editorRect.right - 1,
        elementRect.left
      );
      elementRect.right = numberInRange(
        editorRect.left,
        editorRect.right,
        elementRect.right
      );
      elementRect.top = numberInRange(
        editorRect.top,
        editorRect.bottom - 1,
        elementRect.top
      );
      elementRect.bottom = numberInRange(
        editorRect.top,
        editorRect.bottom,
        elementRect.bottom
      );
