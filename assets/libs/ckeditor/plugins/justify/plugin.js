  ...
  function getAlignment(element, useComputedState) {
    ...
    if (useComputedState) align = element.getComputedStyle("text-align");
      ...
      while (
        ...
        !(element.hasAttribute("align") || element.getStyle("text-align"))
      ) {
    ...
    !align &&
      useComputedState &&
      (align =
        element.getComputedStyle("direction") == "rtl" ? "right" : "left");
  ...
  justifyCommand.prototype = {
    exec: function (editor) {
      ...
      for (var i = ranges.length - 1; i >= 0; i--) {
        ...
        while (
          ...
          isAllowedCssClass = editor.activeFilter.check(
            tag + "(" + cssClassName + ")"
          );
          ...
          var className =
            cssClassName &&
            (block.$.className = CKEDITOR.tools.ltrim(
              block.$.className.replace(this.cssClassRegex, "")
            ));
    ...
    refresh: function (editor, path) {
      ...
      if (isEditable && !CKEDITOR.dtd.$list[path.lastElement.getName()]) {
        ...
        this.setState(
          getAlignment(firstBlock, this.editor.config.useComputedState) ==
            this.value
