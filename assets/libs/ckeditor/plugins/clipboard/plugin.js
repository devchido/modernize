...
(function () {
      ...
      if (CKEDITOR.env.gecko) {
        ...
        editor.on(
          ...
          function (evt) {
            ...
            if (
              !data &&
              dataObj.method == "paste" &&
              dataTransfer &&
              dataTransfer.getFilesCount() == 1 &&
              latestId != dataTransfer.id
            ) {
              var file = dataTransfer.getFile(0);

              if (
                CKEDITOR.tools.indexOf(supportedImageTypes, file.type) != -1
              ) {
      ...
      editor.on(
        ...
        function (evt) {
          ...
          // Or force filtering even for internal and cross-editor paste, when forcePAPT is active (#620).
            ...
            (isExternalPaste && editor.pasteFilter && !dataObj.dontFilter) ||
            isActiveForcePAPT
          ) {
  ...
  function initPasteClipboard(editor) {
    ...
    function createCutCopyCmd(type) {
      return {
        ...
        startDisabled: true,
        fakeKeystroke:
          type == "cut" ? CKEDITOR.CTRL + 88 /*X*/ : CKEDITOR.CTRL + 67 /*C*/,
        exec: function () {
      ...
      return {
        ...
        exec: function (editor, data) {
          ...
          var cmd = this,
            ...
            keystroke = CKEDITOR.tools.keystrokeToString(
              editor.lang.common.keyboard,
              editor.getCommandKeystroke(this)
            ),
            msg =
              typeof notification === "string"
                ? notification
                : editor.lang.clipboard.pasteNotification.replace(
                    /%1/,
                    '<kbd aria-label="' +
                      keystroke.aria +
                      '">' +
          ...
          function callback(data, withBeforePaste) {
            withBeforePaste =
              typeof withBeforePaste !== "undefined" ? withBeforePaste : true;
            ...
            if (data) {
              ...
              editor.showNotification(
                msg,
                "info",
                editor.config.clipboard_notificationDuration
              );
            }
          ...
          if (
            ...
            editor.config.forcePasteAsPlainText !== "allow-word"
          ) {
    ...
    return {
      get: function (type) {
        if (type == "plain-text") {
          ...
          return (
            filters.plainText ||
            (filters.plainText = new CKEDITOR.filter(editor, "br"))
          );
  ...
  function initDragDrop(editor) {
    ...
    editor.on("contentDom", function () {
      ...
      function crossEditorDrop(dragRange, dropRange, dataTransfer) {
        ...
        firePasteEvents(
          editor,
          { dataTransfer: dataTransfer, method: "drop" },
          1
        );
  ...
  CKEDITOR.plugins.clipboard = {
    ...
    isCustomCopyCutSupported:
      (!CKEDITOR.env.ie || CKEDITOR.env.version >= 16) && !CKEDITOR.env.iOS,
    ...
    getRangeAtDropPosition: function (dropEvt, editor) {
      ...
      if (
        ...
        editor.document.$.caretRangeFromPoint(x, y)
      ) {
  ...
  clipboardIdDataType = CKEDITOR.plugins.clipboard.isCustomDataTypesSupported
    ? "cke/id"
    : "Text";
  ...
  CKEDITOR.plugins.clipboard.dataTransfer.prototype = {
    ...
    getData: function (type, getNative) {
      ...
      var data =
        type == "text/html" && getNative
          ? this._.nativeHtmlCache
          : this._.data[type];
  ...
  CKEDITOR.plugins.clipboard.fallbackDataTransfer.prototype = {
        ...
        if (!nativeDataTransfer) {
          ...
          try {
            ...
            fallbackDataTransfer._isCustomMimeTypeSupported =
              nativeDataTransfer.getData(testType) === testValue;
    ...
    setData: function (type, value) {
      ...
      try {
        ...
        if (this._isUnsupportedMimeTypeError(e)) {
          ...
          if (
            CKEDITOR.tools.indexOf(fallbackDataTransfer._customTypes, type) ===
            -1
          ) {
    ...
    _getFallbackTypeContent: function () {
      ...
      if (!fallbackTypeContent) {
        fallbackTypeContent = this._extractDataComment(
          this._getData(this._customDataFallbackType, true)
        ).content;
      }
    ...
    _applyDataComment: function (content, data) {
      ...
      if (data && CKEDITOR.tools.object.keys(data).length) {
        customData =
          "<!--cke-data:" + encodeURIComponent(JSON.stringify(data)) + "-->";
      }
