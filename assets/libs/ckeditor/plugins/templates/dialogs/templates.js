...
(function () {
  CKEDITOR.dialog.add("templates", function (editor) {
    ...
    function renderTemplatesList(container, templatesDefinitions) {
      ...
      for (
        var i = 0, totalDefs = templatesDefinitions.length;
        i < totalDefs;
        i++
      ) {
    ...
    function keyNavigation(evt) {
      ...
      if (onList || listContainer.contains(target)) {
        ...
        if (items) {
          ...
          if (onList) focusItem = items.getItem(0);
          else {
            switch (keystroke) {
              case 40: // ARROW-DOWN
    ...
    CKEDITOR.document.appendStyleSheet(
      CKEDITOR.getUrl(plugin.path + "dialogs/templates.css")
    );

    var listContainer;

    var templateListLabelId =
        "cke_tpl_list_label_" + CKEDITOR.tools.getNextNumber(),
    ...
    return {
      ...
      onShow: function () {
        var templatesListField = this.getContentElement(
          "selectTpl",
          "templatesList"
        );
