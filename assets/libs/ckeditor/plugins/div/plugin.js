      ...
      editor.addCommand(
        ...
        new CKEDITOR.dialogCommand("creatediv", {
          ...
          refresh: function (editor, path) {
            var context = editor.config.div_wrapTable
              ? path.root
              : path.blockLimit;
            this.setState(
              "div" in context.getDtd()
      ...
      editor.addCommand("removediv", {
        ...
        exec: function (editor) {
          ...
          for (var i = 0; i < ranges.length; i++) {
            range = ranges[i];
            if (range.collapsed) findDiv(selection.getStartElement());
            else {
          ...
          for (i = 0; i < toRemove.length; i++) toRemove[i].remove(true);
