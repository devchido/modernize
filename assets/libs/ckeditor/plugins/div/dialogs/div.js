...
(function () {
  ...
  function divDialog(editor, command) {
    ...
    function getDivContainer(element) {
      ...
      if (container.isReadOnly()) container = container.getParent();
      ...
      for (i = 0; i < blockGroups.length; i++) {
        ...
        for (j = 0; j < blockGroups[i].length; j++) {
          ...
          while (
            ...
            !currentNode.getParent().equals(ancestor)
          ) {
        ...
        for (j = 0; j < blockGroups[i].length; j++) {
          ...
          if (
            !(
              currentNode.getCustomData &&
              currentNode.getCustomData("block_processed")
            )
          ) {
            currentNode.is &&
              CKEDITOR.dom.element.setMarker(
    ...
    function commitInternally(targetFields) {
      var dialog = this.getDialog(),
        element =
          (dialog._element && dialog._element.clone()) ||
    ...
    return {
              ...
              children: [
                {
                  ...
                  setup: function (element) {
                    for (var name in styles)
                      styles[name].checkElementRemovable(
                        element,
                        true,
                        editor
      ...
      onShow: function () {
        ...
        if (command == "editdiv") {
          ...
          this.setupContent(
            (this._element = CKEDITOR.plugins.div.getSurroundDiv(editor))
          );
        }
      },
      onOk: function () {
        if (command == "editdiv") containers = [this._element];
        ...
        for (var i = 0; i < size; i++) {
          ...
          !containers[i].getAttribute("style") &&
