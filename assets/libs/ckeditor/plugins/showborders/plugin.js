...
(function () {
  ...
  CKEDITOR.plugins.add("showborders", {
    ...
    init: function (editor) {
      ...
      editor.on("removeFormatCleanup", function (evt) {
        ...
        if (
          editor.getCommand("showborders").state == CKEDITOR.TRISTATE_ON &&
          element.is("table") &&
          (!element.hasAttribute("border") ||
            parseInt(element.getAttribute("border"), 10) <= 0)
        )
        ...
        dataFilter.addRules({
          elements: {
            table: function (element) {
              ...
              if (
                (!border || border <= 0) &&
                (!cssClass || cssClass.indexOf(showBorderClassName) == -1)
              )
                attributes["class"] =
  ...
  CKEDITOR.on("dialogDefinition", function (ev) {
    ...
    if (dialogName == "table" || dialogName == "tableProperties") {
      ...
      borderField.commit = CKEDITOR.tools.override(
        originalCommit,
        function (org) {
          return function (data, selectedTable) {
            org.apply(this, arguments);
            var value = parseInt(this.getValue(), 10);
            selectedTable[!value || value <= 0 ? "addClass" : "removeClass"](
              showBorderClassName
            );
          };
        }
      ...
      if (classField) {
        classField.setup = CKEDITOR.tools.override(
          classField.setup,
        ...
        classField.commit = CKEDITOR.tools.override(
          classField.commit,
