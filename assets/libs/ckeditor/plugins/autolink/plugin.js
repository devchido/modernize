  ...
  CKEDITOR.plugins.add("autolink", {
    ...
    init: function (editor) {
      var urlTemplate = new CKEDITOR.template('<a href="{link}">{text}</a>'),
        emailTemplate = new CKEDITOR.template(
      ...
      var commitKeystrokes =
        editor.config.autolink_commitKeystrokes ||
        CKEDITOR.config.autolink_commitKeystrokes;
      editor.on("key", function (evt) {
        if (
          editor.mode !== "wysiwyg" ||
          CKEDITOR.tools.indexOf(commitKeystrokes, evt.data.keyCode) == -1
        ) {
          return;
        }
      ...
      function matchLink(query) {
        return (
          query.match(CKEDITOR.config.autolink_urlRegex) ||
