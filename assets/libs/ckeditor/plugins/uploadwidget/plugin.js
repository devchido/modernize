          ...
          for (i = 0; i < filesCount; i++) {
            ...
            if (
              ...
              if (el) {
                loader[loadMethod](
                  def.uploadUrl,
                  def.additionalRequestParameters
                );

                CKEDITOR.fileTools.markElement(el, name, loader.id);

                if (
                  (loadMethod == "loadAndUpload" || loadMethod == "upload") &&
    ...
    CKEDITOR.tools.extend(def, {
      ...
      init: function () {
        ...
        loader.on("update", function (evt) {
          ...
          if (
            loader.status !== "abort" &&
            typeof widget[methodName] === "function"
          ) {
      ...
      _getLoader: function () {
        ...
        return marker
          ? this.editor.uploadRepository.loaders[marker.data("cke-upload-id")]
          : null;
      },
