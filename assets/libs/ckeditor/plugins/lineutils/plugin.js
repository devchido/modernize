...
(function () {
  ...
  Finder.prototype = {
    ...
    start: function (callback) {
      ...
      var moveBuffer = CKEDITOR.tools.eventsBuffer(50, function () {
        if (editor.readOnly || editor.mode != "wysiwyg") return;
    ...
    getRange: (function () {
      ...
      return function (location) {
        ...
        range.moveToPosition(
          this.relations[location.uid].element,
          where[location.type]
        );
    ...
    store: (function () {
      ...
      return function (el, type) {
        ...
        if (
          is(type, CKEDITOR.LINEUTILS_AFTER) &&
          isStatic((alt = el.getNext())) &&
          alt.isVisible()
        ) {
        ...
        if (
          is(type, CKEDITOR.LINEUTILS_INSIDE) &&
          isStatic((alt = el.getFirst())) &&
          alt.isVisible()
        ) {
          merge(alt, CKEDITOR.LINEUTILS_BEFORE, this.relations);
  ...
  Locator.prototype = {
    ...
    locate: (function () {
      function locateSibling(rel, type) {
        var sib =
          rel.element[
            type === CKEDITOR.LINEUTILS_BEFORE ? "getPrevious" : "getNext"
          ]();
        ...
        for (var uid in relations) {
          ...
          if (is(rel.type, CKEDITOR.LINEUTILS_BEFORE)) {
            this.store(
              ...
              locateSibling(rel, CKEDITOR.LINEUTILS_BEFORE)
            );
          }
          ...
          if (is(rel.type, CKEDITOR.LINEUTILS_AFTER)) {
            this.store(
              ...
              locateSibling(rel, CKEDITOR.LINEUTILS_AFTER)
            );
          }
          ...
          if (is(rel.type, CKEDITOR.LINEUTILS_INSIDE)) {
            this.store(
              ...
              (rel.elementRect.top + rel.elementRect.bottom) / 2
            );
          }
        }
  ...
  function Liner(editor, def) {
    ...
    editable.attachListener(
      this.inline ? editable : this.frame,
      "mouseout",
      function (evt) {
        ...
        if (
          x <= 0 ||
          x >= this.winTopPane.width ||
          y <= 0 ||
          y >= this.winTopPane.height
        ) {
          this.hideVisible();
        }
      },
  ...
  Liner.prototype = {
    ...
    getStyle: function (uid, type) {
      ...
      if (
        styles.top - this.winTopScroll.y < this.rect.top ||
        styles.top - this.winTopScroll.y > this.rect.bottom
      ) {
        return false;
      }
      ...
      if (this.inline) {
        ...
        if (
          (hdiff =
            styles.left +
            styles.width -
            (this.rect.left + this.winPane.width)) > 0
        ) {
          styles.width -= hdiff;
        }
      }
    ...
    getClientRect: function (el) {
      var rect = el.getClientRect(),
        ...
        relativeContainerComputedPosition =
          this.container.getComputedStyle("position");
  ...
  function isFloated(el) {
    return !!(
      floats[el.getComputedStyle("float")] || floats[el.getAttribute("align")]
    );
  }
