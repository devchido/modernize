...
(function () {
  var guardElements = { table: 1, ul: 1, ol: 1, blockquote: 1, div: 1 },
    ...
    allGuardElements = {};
  CKEDITOR.tools.extend(directSelectionGuardElements, guardElements, {
    tr: 1,
    p: 1,
    div: 1,
    li: 1,
  });
  CKEDITOR.tools.extend(allGuardElements, directSelectionGuardElements, {
    td: 1,
  });
  ...
  function setToolbarStates(editor, path) {
    ...
    if (!useComputedState)
      selectedElement = getElementForDirection(
        path.lastElement,
        editor.editable()
      );
    ...
    var selectionDir = useComputedState
      ...
      : selectedElement.getStyle("direction") ||
        selectedElement.getAttribute("dir");

    editor
      .getCommand("bidirtl")
      .setState(
        selectionDir == "rtl" ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF
      );
    editor
      .getCommand("bidiltr")
      .setState(
        selectionDir == "ltr" ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF
      );
  }
  ...
  function switchDir(element, dir, editor, database) {
    if (element.isReadOnly() || element.equals(editor.editable())) return;
    ...
    var useComputedState =
      "useComputedState" in editor.config ? editor.config.useComputedState : 1;

    var elementDir = useComputedState
      ? element.getComputedStyle("direction")
  ...
  function getFullySelected(range, elements, enterMode) {
    ...
    if (
      range.checkBoundaryOfElement(ancestor, CKEDITOR.START) &&
        ...
        if (ranges && ranges.length) {
          ...
          while ((range = rangeIterator.getNextRange(1))) {
            ...
            if (
              ...
              (selectedElement &&
                !(
                  selectedElement.type == CKEDITOR.NODE_ELEMENT &&
                  selectedElement.getName() in directSelectionGuardElements
                ))
            )
              selectedElement = getFullySelected(
                range,
                guardElements,
                enterMode
              );
            ...
            walker.evaluator = function (node) {
              ...
              return !!(
                ...
                isGuard(node) &&
                !(
                  node.is(enterTagName) &&
                  isNodeElement(node.getParent()) &&
                  node.getParent().is("blockquote")
                ) &&
            ...
            while (
              (block = iterator.getNextParagraph(
                enterMode == CKEDITOR.ENTER_P ? "p" : "div"
              ))
            )
              switchDir(block, dir, editor, database);
          }
  ...
  CKEDITOR.plugins.add("bidi", {
    ...
    init: function (editor) {
      ...
      function addButtonCommand(
        ...
        editor.addCommand(
          commandName,
          new CKEDITOR.command(editor, commandDef)
        );
  ...
  function isOffline(el) {
    ...
    while (el) {
      if (el.equals(html)) return false;
      el = el.getParent();
    }
    return true;
  }
  ...
  function dirChangeNotifier(org) {
    ...
    return function (name, val) {
      if (!this.isReadOnly()) {
        var orgDir;
        if (
          (name == (isAttribute || isRemoveAttribute ? "dir" : "direction") ||
            (name == "style" &&
              (isRemoveAttribute || dirStyleRegexp.test(val)))) &&
          !isOffline(this)
        ) {
  ...
  for (var i = 0; i < methods.length; i++)
    elementProto[methods[i]] = CKEDITOR.tools.override(
      elementProto[methods[i]],
      dirChangeNotifier
    );
})();
