...
(function () {
  ...
  function inheritInlineStyles(parent, el) {
    ...
    style &&
      el.setAttribute(
        "style",
        style.replace(/([^;])$/, "$1;") + (el.getAttribute("style") || "")
      );
  }
  ...
  CKEDITOR.plugins.list = {
    ...
    listToArray: function (
      ...
      if (!listNodeNames[listNode.getName()]) return [];
      ...
      for (var i = 0, count = listNode.getChildCount(); i < count; i++) {
        ...
        if (
          listItem.type == CKEDITOR.NODE_ELEMENT &&
          listItem.getName() in CKEDITOR.dtd.$list
        )
        ...
        if (!grandparentNode) {
          ...
          if (
            itemObj.grandparent &&
            itemObj.grandparent.$.nodeName.toLowerCase() == "li"
          )
        ...
        for (
          var j = 0, itemChildCount = listItem.getChildCount(), child;
          j < itemChildCount;
          j++
        ) {
    ...
    arrayToList: function (listArray, database, baseIndex, paragraphMode, dir) {
      ...
      while (1) {
        ...
        if (item.indent == indentLevel) {
          if (
            !rootNode ||
            listArray[currentIndex].parent.getName() != rootNode.getName()
          ) {
          ...
          var currDir = listArray[currentIndex - 1].element.getDirection(1),
            listData = CKEDITOR.plugins.list.arrayToList(
              listArray,
              null,
              currentIndex,
              paragraphMode,
          ...
          var needsBlock =
            currentListItem.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT &&
          ...
          for (i = 0; i < count; i++) {
            ...
            if (bookmarks(child) && count > 1) {
              ...
              if (!needsBlock) currentListItem.append(child.clone(1, 1));
              else cachedBookmark = child.clone(1, 1);
            }
  ...
  function changeListType(editor, groupObj, database, listsCreated) {
    ...
    for (i = 0; i < length && (child = newList.listNode.getChild(i)); i++) {
      if (child.getName() == this.type) listsCreated.push(child);
    }
  ...
  function createList(editor, groupObj, listsCreated) {
    ...
    while (listContents.length) {
      ...
      if (shouldPreserveBlock(contentBlock)) contentBlock.appendTo(listItem);
      else {
    ...
    function compensateBrs(isStart) {
      if (
        ...
        (siblingNode = groupObj.root[isStart ? "getPrevious" : "getNext"](
          CKEDITOR.dom.walker.invisible(true)
        )) &&
        !(siblingNode.is && siblingNode.isBlockBoundary({ br: 1 }))
      ) {
        editor.document
          .createElement("br")
          [isStart ? "insertBefore" : "insertAfter"](boundaryNode);
      }
    }
    compensateBrs(true);
    compensateBrs();
  ...
  listCommand.prototype = {
    exec: function (editor) {
      ...
      while (listGroups.length > 0) {
        ...
        if (this.state == CKEDITOR.TRISTATE_OFF) {
          ...
          listNodeNames[groupObj.root.getName()] &&
          !isEmptyList(groupObj)
        ) {
    ...
    refresh: function (editor, path) {
      ...
      if (list && limit.contains(list))
        this.setState(
          list.is(this.type) ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF
        );
  ...
  function joinNextLineToCursor(editor, cursor, nextCursor) {
    ...
    while (nextCursor.checkStartOfBlock() && nextCursor.checkEndOfBlock()) {
      ...
      if (nextBlock.is("li")) {
        ...
        if (
          nextBlock.equals(parent.getLast(nonEmpty)) &&
              ...
              if (isAtEnd && next) {
                ...
                if (isAtEnd == 2) {
                  ...
                  if (nextLine.endPath().block) {
                    nextLine.moveToPosition(
                      nextLine.endPath().block,
                      CKEDITOR.POSITION_AFTER_START
                    );
                  }
                }
