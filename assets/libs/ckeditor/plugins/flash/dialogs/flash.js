...
(function () {
  ...
  for (i = 0; i < names.length; i++)
    attributesMap[names[i]][0]["default"] = attributesMap[names[i]][1][
      "default"
    ] = true;
      ...
      switch (attrDef.type) {
        ...
        case ATTRTYPE_PARAM:
          ...
          if (attrDef.name in paramMap) {
            ...
            if (isCheckbox) this.setValue(value.toLowerCase() == "true");
    ...
    for (var i = 0; i < attributes.length; i++) {
      ...
      switch (attrDef.type) {
        case ATTRTYPE_OBJECT:
          ...
          else objectNode.setAttribute(attrDef.name, value);
          break;
        case ATTRTYPE_PARAM:
          if (!objectNode) continue;
          value = this.getValue();
          if (isRemove || (isCheckbox && value === attrDef["default"])) {
            if (attrDef.name in paramMap) paramMap[attrDef.name].remove();
          } else {
            if (attrDef.name in paramMap)
              ...
              if (objectNode.getChildCount() < 1) param.appendTo(objectNode);
              else param.insertBefore(objectNode.getFirst());
            }
          }
          break;
        case ATTRTYPE_EMBED:
  ...
  CKEDITOR.dialog.add("flash", function (editor) {
    var makeObjectTag = !editor.config.flashEmbedTagOnly,
      makeEmbedTag =
        editor.config.flashAddEmbedTag || editor.config.flashEmbedTagOnly;
      ...
      onShow: function () {
        ...
        if (
          ...
          if (realElement.getName() == "cke:object") {
            ...
            if (embedList.count() > 0) embedNode = embedList.getItem(0);
      ...
      onOk: function () {
        ...
        if (!this.fakeImage) {
          ...
          if (makeEmbedTag) {
            ...
            if (objectNode) embedNode.appendTo(objectNode);
          }
        } else {
                            ...
                            dialog.preview.setHtml(
                              '<embed height="100%" width="100%" src="' +
                                CKEDITOR.tools.htmlEncode(
                                  previewPreloader.getAttribute("src")
                                ) +
              ...
              children: [
                ...
                },
                {
                  ...
                  validate: CKEDITOR.dialog.validate.integer(
                    editor.lang.flash.validateHSpace
                  ),
                  setup: loadValue,
                  commit: commitValue,
                },
                {
                  ...
                  validate: CKEDITOR.dialog.validate.integer(
                    editor.lang.flash.validateVSpace
                  ),
                  setup: loadValue,
                  commit: commitValue,
                },
