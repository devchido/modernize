              ...
              node.on(eventName, function (evt) {
                ...
                if (callFuncArgs) {
                  ...
                  for (var i = 0; i < nums; i++) {
                    ...
                    callFuncArgs[i] = argName = CKEDITOR.tools.trim(
                      callFuncArgs[i]
                    );
                  ...
                  var retval = CKEDITOR.tools.callFunction.apply(
                    window,
                    callFuncArgs
                  );
                  if (hasReturn && retval === false) preventDefault = 1;
                }

                if (preventDefault) evt.data.preventDefault();
              });
            }
          })(eventNameList[j]);
        }
        ...
        function (original_write) {
          function appendElement(parent, tagName, fullTag, text) {
            var node = parent.append(tagName),
              attrs =
                CKEDITOR.htmlParser.fragment.fromHtml(fullTag).children[0]
                  .attributes;
            ...
            if (this.getBody()) {
              ...
              html.replace(
                ...
                function (match, startTag, innerHTML) {
                  ...
                  var attrs =
                    CKEDITOR.htmlParser.fragment.fromHtml(startTag).children[0]
                      .attributes;
