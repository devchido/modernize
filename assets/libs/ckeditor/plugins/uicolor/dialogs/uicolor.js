...
CKEDITOR.dialog.add("uicolor", function (editor) {
  ...
  function onColorChanged(color, predefinedColor) {
    ...
    if (
      color &&
      dialog.getContentElement("picker", "predefined").getValue() !== newColor
    ) {
      ...
      if (
        dialog.getContentElement("picker", "selectedColor").getValue() !==
        newColor
      ) {
  ...
  function getUiColor(editor) {
    return editor.getUiColor()
      ? CKEDITOR.tools.parseCssText("color:" + editor.getUiColor(), true).color
      : null;
  }
            ...
            children: [
              ...
              },
              {
                ...
                onChange: function () {
                  ...
                  if (
                    color &&
                    CKEDITOR.tools.style.parse._findColor(color).length
                  ) {
                    this.setValue(
                      'config.uiColor = "' + color.toLowerCase() + '"',
                      true
                    );
                  }
                },
