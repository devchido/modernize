...
(function () {
  function protectFormStyles(formElement) {
    if (
      !formElement ||
      formElement.type != CKEDITOR.NODE_ELEMENT ||
  ...
  function saveStyles(element, isInsideEditor) {
    ...
    if (!isInsideEditor)
      // Reset any external styles that might interfere. (https://dev.ckeditor.com/ticket/2474)
  ...
  function refreshCursor(editor) {
    if (editor.editable().isInline()) return;
      ...
      editor.addCommand("maximize", {
        ...
        exec: function () {
          var container = editor.container.getFirst(function (node) {
            return (
              node.type == CKEDITOR.NODE_ELEMENT && node.hasClass("cke_inner")
            );
          });
          ...
          if (editor.mode == "wysiwyg") {
            ...
            savedSelection = !CKEDITOR.env.ie && [
          ...
          if (this.state == CKEDITOR.TRISTATE_OFF) {
            ...
            while ((currentNode = currentNode.getParent())) {
              currentNode.setCustomData(
                "maximize_saved_styles",
                saveStyles(currentNode)
              );
            ...
            contents.setCustomData(
              "maximize_saved_styles",
              saveStyles(contents, true)
            );
            container.setCustomData(
              "maximize_saved_styles",
              saveStyles(container, true)
            );
          ...
          // Restore from fullscreen if the state is on.
            ...
            for (var i = 0; i < editorElements.length; i++) {
              restoreStyles(
                editorElements[i],
                editorElements[i].getCustomData("maximize_saved_styles")
              );
            ...
            while ((currentNode = currentNode.getParent())) {
              restoreStyles(
                currentNode,
                currentNode.getCustomData("maximize_saved_styles")
              );
