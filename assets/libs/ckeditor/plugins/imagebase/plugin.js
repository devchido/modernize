...
(function () {
  ...
  function hasWidgetFeature(widget, feature) {
    return (
      widget.features &&
      CKEDITOR.tools.array.indexOf(widget.features, feature) !== -1
    );
  }

  function getWidgetsWithFeature(widgets, feature) {
    return CKEDITOR.tools.array.reduce(
      CKEDITOR.tools.object.keys(widgets),
      function (featuredWidgets, id) {
      ...
      setUp: function (editor) {
        ...
        editor.on("dialogShow", function (evt) {
          ...
          okListener = dialog.once(
            "ok",
            createOkListener(evt, dialog, widget),
            null,
            null,
            9
          );
  ...
  function getUploadFeature() {
    ...
    var ret = {
          ...
          if (method === "drop" || (method === "paste" && filesCount)) {
            ...
            if (matchedFiles.length) {
              ...
              CKEDITOR.tools.array.forEach(
                matchedFiles,
                function (curFile, index) {
                  var loader = ret._spawnLoader(
                    editor,
                    curFile,
                    definition,
                    curFile.name
      ...
      _spawnLoader: function (editor, file, widgetDef, fileName) {
        var loadMethod = widgetDef.loadMethod || "loadAndUpload",
          loader = editor.uploadRepository.create(
            file,
            fileName,
            widgetDef.loaderType
          );

        loader[loadMethod](
          widgetDef.uploadUrl,
          widgetDef.additionalRequestParameters
        );
      ...
      _beginUpload: function (widget, loader) {
        ...
        if (
          widget.fire("uploadStarted", loader) !== false &&
          widget.progressReporterType
        ) {
      ...
      _insertWidget: function (editor, widgetDef, blobUrl, finalize, data) {
        ...
        var element = CKEDITOR.dom.element.createFromHtml(
            widgetDef.template.output(tplParams)
          ),
  ...
  function getCaptionFeature() {
    ...
    function isEmptyOrHasPlaceholder(widget) {
      return (
        !widget.editables.caption.getData() ||
        !!widget.parts.caption.data("cke-caption-placeholder")
      );
    }

    function addPlaceholder(widget) {
      widget.parts.caption.data(
      ...
      _refreshCaption: function (sender) {
        ...
        if (isFocused) {
          if (!editable.getData() && !sender.equals(caption)) {
            ...
            !sender ||
            (sender.equals(caption) && sender.data("cke-caption-placeholder"))
          ) {
  ...
  function createWidgetDefinition(editor, definition) {
     ...
     * @property {String}
     */
    definition.upcast = CKEDITOR.tools.object
      .keys(definition.upcasts)
      .join(",");
  ...
  function ProgressReporter(wrapperHtml) {
    ...
    this.wrapper = CKEDITOR.dom.element.createFromHtml(
      wrapperHtml || '<div class="cke_loader"></div>'
    );
  }
  ...
  ProgressReporter.prototype = {
    ...
    bindLoader: function (loader) {
      ...
      var updateListener = CKEDITOR.tools.eventsBuffer(
        UPLOAD_PROGRESS_THROTTLING,
        function () {
  ...
  function ProgressBar() {
    ProgressReporter.call(
      this,
      '<div class="cke_loader">' +
        '<div class="cke_bar" styles="transition: width ' +
        UPLOAD_PROGRESS_THROTTLING / 1000 +
        's"></div>' +
        "</div>"
    );
  ...
  CKEDITOR.plugins.imagebase = {
    ...
    addImageWidget: function (editor, name, definition) {
      var widget = editor.widgets.add(
        name,
        createWidgetDefinition(editor, definition)
      );
    ...
    addFeature: function (editor, name, definition) {
      var featureDefinition = CKEDITOR.tools.clone(
          this.featuresDefinitions[name]
        ),
        ret;
      ...
      featureDefinition.init = mergeMethods(
        definition.init,
        featureDefinition.init
      );
      featureDefinition.data = mergeMethods(
        definition.data,
