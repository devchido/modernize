...
(function () {
  ...
  function commitValue(iframeNode) {
    var isRemove = this.getValue() === "",
      ...
      value = this.getValue();
    if (isRemove) iframeNode.removeAttribute(this.att || this.id);
    else if (isCheckbox)
      iframeNode.setAttribute(this.id, checkboxValues[this.id][value]);
    else iframeNode.setAttribute(this.att || this.id, value);
  }
              ...
              children: [
                {
                  ...
                  validate: CKEDITOR.dialog.validate.htmlLength(
                    commonLang.invalidHtmlLength.replace("%1", commonLang.width)
                  ),
                  setup: loadValue,
                  commit: commitValue,
                },
