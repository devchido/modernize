...
(function () {
  ...
  function syncFieldsBetweenTabs(currentPageId) {
    ...
    for (i = 0; i < l; i++) {
      sourceField = this.getContentElement(
        pages[sourceIndex],
        fieldsMapping[i][sourceIndex]
      );
      targetField = this.getContentElement(
        pages[targetIndex],
  ...
  function findDialog(editor) {
    ...
    var highlightConfig = {
      ...
      highlightStyle = new CKEDITOR.style(
        CKEDITOR.tools.extend(
          highlightConfig,
      ...
      move: function (rtl) {
        ...
        if (currentTextNode && rtl && this.offset > 0) {
          ...
          while (!currentTextNode) {
            currentTextNode = this._.walker[rtl ? "previous" : "next"].call(
              this._.walker
            );
    ...
    characterRange.prototype = {
      ...
      highlight: function () {
        ...
        if (this._.cursors.length < 1) return;
      ...
      removeHighlight: function () {
        if (!this._.highlightRange) return;
      ...
      getNextCharacterRange: function (maxLength) {
        ...
        if ((lastCursor = cursors[cursors.length - 1]) && lastCursor.textNode)
          nextRangeWalker = new characterWalker(
    ...
    kmpMatcher.prototype = {
      feedCharacter: function (c) {
        if (this._.ignoreCase) c = c.toLowerCase();
    ...
    var finder = {
      ...
      find: function (
        ...
        if (!this.matchRange)
          this.matchRange = new characterRange(
        ...
        while (character !== null) {
          ...
          if (matchState == KMP_MATCHED) {
            if (matchWord) {
              ...
              if (
                !(
                  isWordSeparator(headWalker.back().character) &&
      ...
      replace: function (
        ...
        var result = 0,
          matchOptionsChanged = this.hasMatchOptionsChanged(
            pattern,
            matchCase,
            matchWord
          );
    ...
    return {
                ...
                },
                {
                  ...
                  onClick: function () {
                    ...
                    if (finder.replaceCounter) {
                      alert(
                        lang.replaceSuccessMsg.replace(
                          /%1/,
                          finder.replaceCounter
                        )
                      ); // jshint ignore:line
                      editor.fire("saveSnapshot");
                    } else {
        ...
        this.selectPage = CKEDITOR.tools.override(
          ...
          function (originalFunc) {
            return function (pageId) {
              ...
              patternFieldId =
                pageId === "find" ? "txtFindFind" : "txtFindReplace";
              wholeWordChkFieldId =
                pageId === "find" ? "txtFindWordChk" : "txtReplaceWordChk";

              patternField = dialog.getContentElement(pageId, patternFieldId);
              wholeWordChkField = dialog.getContentElement(
                pageId,
                wholeWordChkFieldId
              );
              ...
              if (!currPage.initialized) {
                patternFieldInput = CKEDITOR.document.getById(
                  patternField._.inputId
                );
      ...
      onShow: function () {
        ...
        var startupPage = this._.currentTabId,
          selectedText = this.getParentEditor()
            .getSelection()
            .getSelectedText(),
          patternFieldId =
            startupPage == "find" ? "txtFindFind" : "txtFindReplace",
        ...
        this[
          (startupPage == "find" && this._.editor.readOnly ? "hide" : "show") +
            "Page"
        ]("replace");
      },
      onHide: function () {
