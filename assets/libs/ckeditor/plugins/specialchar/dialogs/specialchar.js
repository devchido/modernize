...
CKEDITOR.dialog.add("specialchar", function (editor) {
  ...
  var onChoice = function (evt) {
    var target, value;
    if (evt.data) target = evt.data.getTarget();
    else target = new CKEDITOR.dom.element(evt);
  ...
  var onFocus = function (evt, target) {
    ...
    if (target.getName() == "a" && (value = target.getChild(0).getHtml())) {
      ...
      if (focusedNode) onBlur(null, focusedNode);

      var htmlPreview = dialog
  ...
  return {
    ...
    onLoad: function () {
      ...
      var charsTableLabel =
        CKEDITOR.tools.getNextId() + "_specialchar_table_label";
      var html = [
