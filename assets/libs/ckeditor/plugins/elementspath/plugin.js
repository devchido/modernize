...
(function () {
  ...
  var pathItemTpl = CKEDITOR.addTemplate(
    "pathItem",
    "<a" +
      ' id="{id}"' +
      ' href="{jsTitle}"' +
  ...
  CKEDITOR.plugins.add("elementspath", {
    ...
    init: function (editor) {
      ...
      editor.on("uiSpace", function (event) {
        if (event.data.space == "bottom") initElementsPath(editor, event.data);
      });
    },
  });
  ...
  function initElementsPath(editor, bottomSpaceData) {
    var spaceId = editor.ui.spaceId("path"),
      ...
      getSpaceElement = function () {
        if (!spaceElement) spaceElement = CKEDITOR.document.getById(spaceId);
        return spaceElement;
      },
    ...
    bottomSpaceData.html +=
      ...
      editor.lang.elementspath.eleLabel +
      "</span>" +
      '<span id="' +
      spaceId +
    ...
    var onClickHanlder = CKEDITOR.tools.addFunction(onClick),
      onKeyDownHandler = CKEDITOR.tools.addFunction(
        function (elementIndex, ev) {
          ...
          switch (ev.getKeystroke()) {
            ...
            case CKEDITOR.SHIFT + 9: // SHIFT + TAB
              element = CKEDITOR.document.getById(idBase + (elementIndex - 1));
              if (!element)
                element = CKEDITOR.document.getById(
                  idBase + (elementsPath.list.length - 1)
                );
              element.focus();
              return false;
    ...
    editor.on("selectionChange", function (evt) {
      ...
      for (
        var iterationLimit = elementsList.length, index = 0;
        index < iterationLimit;
        index++
      ) {
