...
(function () {
  ...
  function getNodeAndApplyCmd(range, query, cmd, stopOnFirst) {
    ...
    if (
      (currentNode =
        range.startContainer.getAscendant(query, true) ||
  ...
  function checkForStyle(element, styles) {
    ...
    return (
      indexOf(styles, function (style) {
        return (
          style.element === element ||
          style.element === stylesAlternatives[element]
        );
      }) !== -1
    );
  }
    ...
    init: function (editor) {
      ...
      editor.on("contentDom", function () {
        ...
        editable.attachListener(mouseupHost, "mouseup", function (evt) {
          ...
          if (
            getMouseButton(evt) === CKEDITOR.MOUSE_BUTTON_LEFT &&
        ...
        if (copyFormattingButton) {
          copyFormattingButtonEl = CKEDITOR.document.getById(
            copyFormattingButton._.id
          );
      ...
      if (editor.config.copyFormatting_keystrokeCopy) {
        editor.setKeystroke(
          editor.config.copyFormatting_keystrokeCopy,
          "copyFormatting"
        );
      }
      ...
      editor.on("key", function (evt) {
        ...
        if (domEvent.getKeystroke && domEvent.getKeystroke() === 27) {
          // ESC
      ...
      editor.copyFormatting.on("extractFormatting", function (evt) {
        ...
        if (
          element.contains(editor.editable()) ||
          element.equals(editor.editable())
        ) {
          return evt.cancel();
        }
        ...
        if (
          !editor.copyFormatting.filter.check(
            new CKEDITOR.style(style),
            true,
            true
          )
        ) {
      ...
      editor.copyFormatting.on(
        ...
        function (evt) {
          ...
          if (
            context === "list" &&
            editor.copyFormatting._isContextAllowed("list")
          ) {
            plugin._applyStylesToListContext(
              evt.editor,
              evt.data.range,
              evt.data.styles
            );
            ...
            context === "table" &&
            editor.copyFormatting._isContextAllowed("table")
          ) {
            plugin._applyStylesToTableContext(
              evt.editor,
              evt.data.range,
              evt.data.styles
            );
          } else if (editor.copyFormatting._isContextAllowed("text")) {
            plugin._applyStylesToTextContext(
              evt.editor,
              evt.data.range,
              evt.data.styles
            );
  ...
  function State(editor) {
    ...
    this.filter = new CKEDITOR.filter(
      editor,
      editor.config.copyFormatting_allowRules
    );
  ...
  CKEDITOR.plugins.copyformatting = {
    ...
    commands: {
      copyFormatting: {
        exec: function (editor, data) {
          ...
          copyFormatting.styles = plugin._extractStylesFromElement(
            editor,
      ...
      applyFormatting: {
        ...
        exec: function (editor, data) {
          ...
          plugin._putScreenReaderMessage(
            editor,
            isApplied ? "applied" : "canceled"
          );
        },
      },
    },
      ...
      function getSiblingNodeOffset(startNode, isPrev) {
        ...
        do {
          ...
          if (
            sibling &&
            sibling.getName &&
            indexOf(boundaryElements, sibling.getName()) !== -1
          ) {
            isBoundary = true;
            break;
          }
    ...
    _filterStyles: function (styles) {
      ...
      for (i = 0; i < styles.length; i++) {
        ...
        if (
          ...
          isEmpty(styleDef.attributes) &&
          isEmpty(styleDef.styles)
        ) {
          continue;
        }
    ...
    _applyStylesToTextContext: function (editor, range, styles) {
      ...
      for (i = 0; i < styles.length; i++) {
        ...
        if (
          indexOf(plugin.excludedElementsFromInlineTransform, style.element) !==
          -1
        ) {
          continue;
        }
    ...
    _applyFormat: function (editor, newStyles) {
      ...
      applyEvtData = {
        styles: newStyles,
        range: range,
        preventFormatStripping: false,
      };

      // Now apply new styles.
      if (
        !editor.copyFormatting.fire("applyFormatting", applyEvtData, editor)
      ) {
        return false;
      }
    ...
    _addScreenReaderContainer: function () {
      ...
      return CKEDITOR.document
        .getBody()
        .append(CKEDITOR.dom.element.createFromHtml(notificationTpl))
        .getChild(0);
    },
    ...
    _attachPasteKeystrokeHandler: function (editor) {
      ...
      if (keystrokePaste) {
        this._initialKeystrokePasteCommand =
          editor.keystrokeHandler.keystrokes[keystrokePaste];
    ...
    _detachPasteKeystrokeHandler: function (editor) {
      ...
      if (keystrokePaste) {
        editor.setKeystroke(
          keystrokePaste,
          this._initialKeystrokePasteCommand || false
        );
      }
    },
  };
