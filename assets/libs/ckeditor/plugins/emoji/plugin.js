...
(function () {
    ...
    EmojiDropdown = CKEDITOR.tools.createClass({
      $: function (editor, plugin) {
        ...
        editor.ui.add("EmojiPanel", CKEDITOR.UI_PANELBUTTON, {
          ...
          onBlock: function (panel, block) {
            ...
            self.inputIndex = self.getItemIndex(
              self.items,
              self.elements.input
            );
            ...
            self.elements.statusIcon = block.element.findOne(
              ".cke_emoji-status_icon"
            );
            self.elements.statusDescription = block.element.findOne(
              "p.cke_emoji-status_description"
            );
            self.elements.statusName = block.element.findOne(
      ...
      proto: {
        ...
        createGroupsNavigation: function () {
          var itemTemplate, items, svgUrl, imgUrl, useAttr;
          ...
          if (CKEDITOR.env.ie && !CKEDITOR.env.edge) {
            ...
            useAttr = CKEDITOR.env.safari
              ? 'xlink:href="' + svgUrl + '#{svgId}"'
              : 'href="' + svgUrl + '#{svgId}"';
            ...
            itemTemplate = new CKEDITOR.template(
              '<li class="cke_emoji-navigation_item" data-cke-emoji-group="{group}"><a href="#" title="{name}" draggable="false" _cke_focus="1">' +
                ...
                '<title id="{svgId}-title">{name}</title><use ' +
                useAttr +
                "></use></svg></a></li>"
            );
                ...
                if (!item.items.length) {
                  ...
                  return (
                    ...
                    itemTemplate.output({
                      ...
                      translateX:
                        item.translate && item.translate.x
                          ? htmlEncode(item.translate.x)
                          : 0,
                      translateY:
                        item.translate && item.translate.y
                          ? htmlEncode(item.translate.y)
                          : 0,
                    })
                  );
                }
        ...
        createSearchSection: function () {
          ...
          return (
            '<label class="cke_emoji-search">' +
            this.getLoupeIcon() +
            '<input placeholder="' +
            htmlEncode(this.lang.searchPlaceholder) +
            '" type="search" aria-label="' +
            htmlEncode(this.lang.searchLabel) +
            '" role="search" _cke_focus="1"></label>'
          );
        },
        ...
        createStatusBar: function () {
          return (
            '<div class="cke_emoji-status_bar">' +
        ...
        getLoupeIcon: function () {
          var loupeSvgUrl = CKEDITOR.getUrl(
              this.plugin.path + "assets/iconsall.svg"
            ),
            loupePngUrl = CKEDITOR.getUrl(
          ...
          if (CKEDITOR.env.ie && !CKEDITOR.env.edge) {
            return (
              '<span class="cke_emoji-search_loupe" aria-hidden="true" style="background-image:url(' +
              loupePngUrl +
              ');"></span>'
            );
          } else {
            useAttr = CKEDITOR.env.safari
              ? 'xlink:href="' + loupeSvgUrl + '#cke4-icon-emoji-10"'
              : 'href="' + loupeSvgUrl + '#cke4-icon-emoji-10"';
            return (
              '<svg viewBox="0 0 34 34" role="img" aria-hidden="true" class="cke_emoji-search_loupe"><use ' +
              useAttr +
              "></use></svg>"
            );
          }
        ...
        filter: function (evt) {
          ...
          arrTools.forEach(
            this.elements.emojiItems.toArray(),
            function (element) {
              if (
                isNameOrKeywords(
                  query,
                  element.data("cke-emoji-name"),
                  element.data("cke-emoji-keywords")
                ) ||
                query === ""
              ) {
        ...
        refreshNavigationStatus: function () {
          ...
          groupName = section.length
            ? section[0].data("cke-emoji-group")
            : false;
        ...
        updateStatusbar: function (element) {
          if (
            element.getName() !== "a" ||
            !element.hasAttribute("data-cke-emoji-name")
          ) {
            return;
          }
          ...
          this.elements.statusDescription.setText(
            htmlEncode(element.data("cke-emoji-name"))
          );
          this.elements.statusName.setText(
            htmlEncode(element.data("cke-emoji-full-name"))
          );
        },
  ...
  CKEDITOR.plugins.add("emoji", {
    ...
    init: function (editor) {
      ...
      var emojiListUrl =
          editor.config.emoji_emojiListUrl || "plugins/emoji/emoji.json",
        arrTools = CKEDITOR.tools.array;
      ...
      CKEDITOR.ajax.load(CKEDITOR.getUrl(emojiListUrl), function (data) {
        ...
        var emojiList = editor._.emoji.list,
          charactersToStart =
            editor.config.emoji_minChars === undefined
              ? 2
              : editor.config.emoji_minChars;
        ...
        function initPlugin() {
          editor._.emoji.autocomplete = new CKEDITOR.plugins.autocomplete(
            editor,
            {
              textTestCallback: getTextTestCallback(),
              dataCallback: dataCallback,
              itemTemplate:
                '<li data-id="{id}" class="cke_emoji-suggestion_item">{symbol} {id}</li>',
        ...
        function matchCallback(text, offset) {
          var left = text.slice(0, offset),
            ...
            match = left.match(
              new RegExp("(?:\\s|^)(:\\S{" + charactersToStart + "}\\S*)$")
            );
        ...
        function dataCallback(matchInfo, callback) {
          var emojiName = matchInfo.query.substr(1).toLowerCase(),
            data = arrTools
              .filter(emojiList, function (item) {
