...
CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {
  ...
  clone: function (includeChildren, cloneId) {
    ...
    function removeIds(node) {
      ...
      if (
        node.nodeType != CKEDITOR.NODE_ELEMENT &&
        node.nodeType != CKEDITOR.NODE_DOCUMENT_FRAGMENT
      )
        return;
  ...
  getIndex: function (normalized) {
    ...
    if (
      ...
      var adjacent =
        ...
        getAdjacentNonEmptyTextNode(current, true);
    ...
    function getAdjacentNonEmptyTextNode(node, lookForward) {
      ...
      return isEmpty(sibling)
        ? getAdjacentNonEmptyTextNode(sibling, lookForward)
        : sibling;
    }
    ...
    function isEmpty(textNode) {
      return (
        !textNode.nodeValue ||
        textNode.nodeValue == CKEDITOR.dom.selection.FILLING_CHAR_SEQUENCE
      );
    }
  },
  ...
  getAscendant: function (query, includeSelf) {
    ...
    if (typeof query == "function") {
      ...
      evaluator = function ($) {
        var name =
          typeof $.nodeName == "string" ? $.nodeName.toLowerCase() : "";
