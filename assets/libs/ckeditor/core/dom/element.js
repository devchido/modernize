...
(function () {
  var elementsClassList = document.createElement("_").classList,
    supportsClassLists =
      typeof elementsClassList !== "undefined" &&
      String(elementsClassList.add).match(/\[Native code\]/gi) !== null,
    rclass = /[\n\t\r]/g;
  ...
  CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
    ...
    getClientRect: function (isAbsolute) {
      ...
      var elementRect = CKEDITOR.tools.extend(
        {},
      ...
      return CKEDITOR.tools.getAbsoluteRectPosition(
        this.getWindow(),
        elementRect
      );
    },
    ...
    getDocumentPosition: function (refDocument) {
      ...
      if (
        ...
        if (needAdjustScrollAndBorders) {
          ...
          if (
            CKEDITOR.env.webkit ||
            (CKEDITOR.env.ie && CKEDITOR.env.version >= 12)
          ) {
    ...
    disableContextMenu: function () {
      ...
      function enablesContextMenu(node) {
        return (
          ...
          node.hasClass("cke_enable_context_menu")
        );
      }
    },
  ...
  function marginAndPaddingSize(type) {
    ...
    for (var i = 0, len = sides[type].length; i < len; i++)
      adjustment +=
        parseFloat(this.getComputedStyle(sides[type][i]) || 0, 10) || 0;
    return adjustment;
  }
