...
(function () {
  ...
  iterator.prototype = {
    ...
    getNextParagraph: function (blockTag) {
      ...
      while (currentNode) {
        ...
        currentNode = this._getNextSourceNode(
          currentNode,
          continueFromSibling,
          lastNode
        );
      ...
      if (!block) {
        ...
        if (
          ...
          !block ||
          (this.enforceRealBlocks && block.is(listItemNames))
        ) {
          ...
          this._.nextNode = block.equals(lastNode)
            ...
            : this._getNextSourceNode(
                range.getBoundaryNodes().endNode,
                1,
                lastNode
              );
        }
      ...
      if (!this._.nextNode) {
        this._.nextNode =
          isLast || block.equals(lastNode) || !lastNode
            ? null
            : this._getNextSourceNode(block, 1, lastNode);
      }
  ...
  function startIterator() {
    var range = this.range.clone(),
      ...
      startAtInnerBoundary =
        !range.collapsed && rangeAtInnerBlockBoundary(range, startPath.block),
      endAtInnerBoundary =
        !range.collapsed && rangeAtInnerBlockBoundary(range, endPath.block, 1);
  ...
  function rangeAtInnerBlockBoundary(range, block, checkEnd) {
    ...
    return testRange.checkBoundaryOfElement(
      block,
      checkEnd ? CKEDITOR.START : CKEDITOR.END
    );
  }
