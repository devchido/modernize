...
(function () {
  ...
  function execContentsAction(range, action, docFrag, mergeThen, cloneId) {
    ...
    if (
      ...
      docFrag.append(
        range.document.createText(endNode.substring(startOffset, endOffset))
      );
      return;
    }
    ...
    for (var level = minLevel; level <= maxLevelLeft; level++) {
      ...
      if (
        level == maxLevelLeft &&
        !(leftNode.equals(endParents[level]) && maxLevelLeft < maxLevelRight)
      ) {
        if (cloneStartNode) {
          consume(leftNode, levelParent, false, doNotRemoveStartNode);
        } else if (cloneStartText) {
          levelParent.append(
            range.document.createText(leftNode.substring(startOffset))
          );
        }
      } else if (doClone) {
    ...
    for (level = minLevel; level <= maxLevelRight; level++) {
      ...
      if (!rightNode.equals(startParents[level])) {
        ...
        if (
          level == maxLevelRight &&
          !(
            rightNode.equals(startParents[level]) &&
            maxLevelRight < maxLevelLeft
          )
        ) {
          if (cloneEndNode) {
            ...
            levelParent.append(
              range.document.createText(rightNode.substring(0, endOffset))
            );
          }
        } else if (doClone) {
    ...
    function mergeAndUpdate() {
      ...
      if (
        commonLevel < maxLevelLeft - 1 ||
        commonLevel < maxLevelRight - 1 ||
        boundariesInEmptyNode
      ) {
        if (boundariesInEmptyNode) {
          ...
          range.moveToPosition(
            endParents[commonLevel],
            CKEDITOR.POSITION_BEFORE_END
          );
        } else {
          range.moveToPosition(
            endParents[commonLevel + 1],
        ...
        if (mergeThen) {
          ...
          if (topLeft && topLeft.type == CKEDITOR.NODE_ELEMENT) {
            var span = CKEDITOR.dom.element.createFromHtml(
              "<span " +
  ...
  function getCheckStartEndBlockEvalFunction() {
    ...
    return function (node) {
      ...
      if (bookmarkEvaluator(node) || whitespaces(node)) return true;
      ...
      if (
        ...
        (node.hasAscendant("pre") || CKEDITOR.tools.trim(node.getText()).length)
      ) {
        return false;
      }
      ...
      if (
        ...
        !node.is(inlineChildReqElements)
      )
        return false;
  ...
  function elementBoundaryEval(checkStart) {
    ...
    return function (node) {
      // First skip empty nodes.
      if (bookmark(node) || whitespaces(node)) return true;
  ...
  function getNextEditableNode(isPrevious) {
    return function () {
      ...
      return this[isPrevious ? "getPreviousNode" : "getNextNode"](
        function (node) {
          // Cache first not ignorable node.
          if (!first && notIgnoredEval(node)) first = node;
  ...
  CKEDITOR.dom.range.prototype = {
    ...
    cloneContents: function (cloneId) {
      ...
      if (!this.collapsed) execContentsAction(this, 2, docFrag, false, cloneId);
    ...
    createBookmark2: (function () {
      ...
      function betweenTextNodes(container, offset) {
        ...
        if (
          container.type != CKEDITOR.NODE_ELEMENT ||
          offset === 0 ||
        ...
        return (
          container.getChild(offset - 1).type == CKEDITOR.NODE_TEXT &&
        ...
        if (container.type == CKEDITOR.NODE_TEXT) {
          ...
          if (container.getText()) {
            ...
            if (precedingLength) {
              ...
              offset = precedingContainer
                ? precedingContainer.getIndex(true) + 1
                : 0;
            }
          }
        }
    ...
    moveToBookmark: function (bookmark) {
      ...
      if (bookmark.is2) {
        ...
        var startContainer = this.document.getByAddress(
            bookmark.start,
            bookmark.normalized
          ),
        ...
        var endContainer =
            bookmark.end &&
            this.document.getByAddress(bookmark.end, bookmark.normalized),
    ...
    trim: function (ignoreStart, ignoreEnd) {
      ...
      if (
        !(ignoreEnd || collapsed) &&
        endContainer &&
        endContainer.type == CKEDITOR.NODE_TEXT
      ) {
        // If the offset is zero, we just insert the new node before
        // the start.
        if (!endOffset) {
    ...
    enlarge: function (unit, excludeBrs) {
      ...
      switch (unit) {
        ...
        case CKEDITOR.ENLARGE_ELEMENT:
          ...
          if (container.type == CKEDITOR.NODE_TEXT) {
            if (offset) {
              ...
              container =
                !CKEDITOR.tools.trim(container.substring(0, offset)).length &&
                container;
          ...
          while (enlargeable || sibling) {
            if (enlargeable && !sibling) {
              ...
              if (
                enlargeInlineOnly
                  ? enlargeable.isBlockBoundary()
                  : !boundary.contains(enlargeable)
              )
                break;
              ...
              if (
                !needsWhiteSpace ||
                enlargeable.getComputedStyle("display") != "inline"
              ) {
            ...
            while (sibling) {
              ...
              if (sibling.type == CKEDITOR.NODE_COMMENT) {
                ...
                if (
                  (sibling.$.offsetWidth > offsetWidth0 ||
                    (excludeBrs && sibling.is("br"))) &&
                  ...
                  if (
                    needsWhiteSpace &&
                    CKEDITOR.dtd.$removeEmpty[sibling.getName()]
                  ) {
          ...
          function onlyWhiteSpaces(startContainer, startOffset) {
            ...
            walker.guard = function (node) {
              ...
              return !(
                node.type == CKEDITOR.NODE_ELEMENT && node.isBlockBoundary()
              );
            };
            ...
            while ((node = walker.next())) {
              if (node.type != CKEDITOR.NODE_TEXT) {
                ...
                if (node != startContainer) siblingText = node.getText();
        ...
        case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
          ...
          walker.guard =
            unit == CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS
              ? tailBrGuard
              : boundaryGuard;
      ...
      function getValidEnlargeable(enlargeable) {
        return enlargeable &&
          enlargeable.type == CKEDITOR.NODE_ELEMENT &&
          enlargeable.hasAttribute("contenteditable")
          ? null
          : enlargeable;
      }
    },
    ...
    shrink: function (mode, selectContents, options) {
      var shrinkOnBlockBoundary =
          typeof options === "boolean"
            ? options
            : options && typeof options.shrinkOnBlockBoundary === "boolean"
      ...
      if (!this.collapsed) {
        ...
        if (moveStart) {
          var textStart =
            walker[mode == CKEDITOR.SHRINK_ELEMENT ? "lastForward" : "next"]();
          textStart &&
            this.setStartAt(
              textStart,
              selectContents
        ...
        if (moveEnd) {
          ...
          var textEnd =
            walker[
              mode == CKEDITOR.SHRINK_ELEMENT ? "lastBackward" : "previous"
            ]();
          textEnd &&
            this.setEndAt(
              textEnd,
    ...
    setStart: function (startNode, startOffset) {
      ...
      if (
        ...
        (startOffset = startNode.getIndex()),
    ...
    setEnd: function (endNode, endOffset) {
      ...
      if (
        endNode.type == CKEDITOR.NODE_ELEMENT &&
    ...
    splitBlock: function (blockTag, cloneId) {
      ...
      if (blockTag != "br") {
        if (!startBlock) {
          ...
          endBlock = new CKEDITOR.dom.elementPath(this.endContainer, this.root)
            .block;
        }
      ...
      if (startBlock && startBlock.equals(endBlock)) {
        if (isEndOfBlock) {
          elementPath = new CKEDITOR.dom.elementPath(
            this.startContainer,
            this.root
          );
    ...
    removeEmptyBlocksAtEnd: (function () {
      ...
      return function (atEnd) {
        ...
        while (
          block &&
          !block.equals(path.root) &&
          !block.getFirst(childEval(block))
        ) {
          parent = block.getParent();
          this[atEnd ? "setEndAt" : "setStartAt"](
            block,
            CKEDITOR.POSITION_AFTER_END
          );
          block.remove(1);
    ...
    checkStartOfBlock: function () {
      ...
      if (
        ...
        var textBefore = CKEDITOR.tools.ltrim(
          startContainer.substring(0, startOffset)
        );
    ...
    checkReadOnly: (function () {
      ...
      return function () {
        ...
        return !(
          checkNodesEditable(startNode, endNode) &&
          checkNodesEditable(endNode, startNode)
        );
      };
    })(),
    ...
    moveToClosestEditablePosition: function (element, isMoveForward) {
      ...
      // next to it.
      if (element && !element.is(CKEDITOR.dtd.$block)) found = 1;
      else {
        // Look for first node that fulfills eval function and place range next to it.
        sibling =
          range[
            isMoveForward ? "getNextEditableNode" : "getPreviousEditableNode"
          ]();
        if (sibling) {
          found = 1;
          ...
          if (
            isElement &&
            sibling.is(CKEDITOR.dtd.$block) &&
            sibling.getAttribute("contenteditable") == "false"
          ) {
          ...
          // Handle empty blocks which can be selection containers on old IEs.
            ...
            isElement &&
            sibling.is(CKEDITOR.dom.walker.validEmptyBlockContainers)
          ) {
    ...
    getEnclosedNode: function () {
      ...
      if (
        walkerRange.startContainer.type != CKEDITOR.NODE_ELEMENT ||
    ...
    _getTableElement: function (tableElements) {
      ...
      if (
        startTable &&
        endTable &&
        (startTable.equals(endTable) ||
          startTable.contains(endTable) ||
    ...
    scrollIntoView: function () {
      ...
      // to this element.
      var reference = new CKEDITOR.dom.element.createFromHtml(
          "<span>&nbsp;</span>",
          this.document
        ),
        afterCaretNode,
    ...
    getClientRects: (function () {
      if (this.document.getSelection !== undefined) {
        ...
        return function (isAbsolute) {
          return [
            convertRect(getRect(this.createBookmark()), isAbsolute, this),
          ];
        };
      }
      ...
      function fixWidgetsRects(rectList, context) {
        ...
        if (context.startContainer instanceof CKEDITOR.dom.element) {
          moveStart =
            ...
            context.startContainer.hasAttribute("data-widget");
        }
        if (context.endContainer instanceof CKEDITOR.dom.element) {
          moveEnd =
            context.endOffset ===
            (context.endContainer.getChildCount
              ? context.endContainer.getChildCount()
              : context.endContainer.length);
          moveEnd = moveEnd && context.endContainer.hasAttribute("data-widget");
        }

        if (moveStart) {
          newRange.setStart(
            context.startContainer.getParent(),
            context.startContainer.getIndex()
          );
        }
        if (moveEnd) {
          newRange.setEnd(
            context.endContainer.getParent(),
            context.endContainer.getIndex() + 1
          );
        }
        ...
        widgetElements = documentFragment
          .find("[data-cke-widget-id]")
          .toArray();
        widgetElements = CKEDITOR.tools.array.map(
          widgetElements,
          function (item) {
        ...
        widgetRects = CKEDITOR.tools.array.map(
          widgetElements,
          function (element) {
            var rects,
              container = element.getParent().hasClass("cke_widget_wrapper")
                ? element.getParent()
                : element;
        ...
        CKEDITOR.tools.array.forEach(widgetRects, function (item) {
          ...
          function cleanWidgetRects(startIndex) {
            CKEDITOR.tools.array.forEach(
              rectArray,
              function (rectArrayItem, index) {
                var compare = CKEDITOR.tools.objectCompare(
                  item[startIndex],
                  rectArrayItem
                );

                if (!compare) {
                  compare = CKEDITOR.tools.objectCompare(
                    item.widgetRect,
                    rectArrayItem
                  );
                }
      ...
      function fixEmptyRectList(rectList, range, context) {
        var first, textNode, itemToInsertAfter;
        ...
        if (!range.collapsed) {
          ...
          if (first) {
            ...
            if (context.startOffset === 0) {
              ...
              itemToInsertAfter = context.startContainer
                .getChildren()
                .getItem(context.startOffset - 1);
      ...
      function convertRect(rect, isAbsolute, context) {
        ...
        if (isAbsolute) {
          newRect = CKEDITOR.tools.getAbsoluteRectPosition(
            context.document.getWindow(),
            newRect
          );
        }
      ...
      function getRect(bookmark) {
        ...
        return {
          ...
          height:
            Math.max(rects[0].bottom, rects[1].bottom) -
            Math.min(rects[0].top, rects[1].top),
        };
      }
    })(),
    ...
    _setStartContainer: function (startContainer) {
      // %REMOVE_START%
      var isRootAscendantOrSelf =
        this.root.equals(startContainer) || this.root.contains(startContainer);

      if (!isRootAscendantOrSelf) {
        CKEDITOR.warn("range-startcontainer", {
    ...
    _setEndContainer: function (endContainer) {
      // %REMOVE_START%
      var isRootAscendantOrSelf =
        this.root.equals(endContainer) || this.root.contains(endContainer);

      if (!isRootAscendantOrSelf) {
        CKEDITOR.warn("range-endcontainer", {
    ...
    _find: function (query, includeNonEditables) {
      ...
      if (ancestor && ancestor.find) {
        ...
        for (i = 0; i < initialMatches.count(); i++) {
          ...
          isStartGood =
            curItem.getPosition(boundaries.startNode) &
              CKEDITOR.POSITION_FOLLOWING ||
            boundaries.startNode.equals(curItem);
          isEndGood =
            curItem.getPosition(boundaries.endNode) &
    ...
    return CKEDITOR.tools.array.reduce(
      ...
      function (ret, rng) {
        ...
        if (lastRange) {
          ...
          while (
            isWhitespace(nodeInBetween) ||
            rng.endContainer.equals(nodeInBetween)
          ) {
