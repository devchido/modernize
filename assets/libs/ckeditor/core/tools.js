...
(function () {
  ...
  CKEDITOR.tools = {
    ...
    extend: function (target) {
      ...
      for (var i = 1; i < argsLength; i++) {
        ...
        CKEDITOR.tools.array.forEach(
          CKEDITOR.tools.object.keys(source),
    ...
    normalizeHex: function (styleText) {
      return styleText.replace(
        /#(([0-9a-f]{3}){1,2})($|;|\s+)/gi,
        function (match, hexColor, hexColorPart, separator) {
    ...
    parseCssText: function (styleText, normalize, nativeNormalize) {
      ...
      if (nativeNormalize) {
        ...
        styleText =
          temp.setAttribute("style", styleText).getAttribute("style") || "";
      }

      // Normalize colors.
      if (styleText) {
        styleText = CKEDITOR.tools.normalizeHex(
    ...
    getMouseButton: function (evt) {
      ...
      if (
        CKEDITOR.env.ie &&
        (CKEDITOR.env.version < 9 || CKEDITOR.env.ie6Compat)
      ) {
    ...
    style: {
      ...
      parse: {
        ...
        margin: function (value) {
          return CKEDITOR.tools.style.parse.sideShorthand(
            value,
            function (width) {
              return (
                width.match(
        ...
        _findColor: function (value) {
          ...
          ret = ret.concat(
            arrayTools.filter(value.split(/\s+/), function (colorEntry) {
              ...
              return (
                colorEntry.toLowerCase() in CKEDITOR.tools.style.parse._colors
              );
            })
          );
    ...
    object: {
      ...
      merge: function (obj1, obj2) {
        ...
        tools.array.forEach(tools.object.keys(copy2), function (key) {
          if (
            typeof copy2[key] === "object" &&
            typeof copy1[key] === "object"
          ) {
  ...
  function EventsBuffer(minInterval, output, context) {
    ...
    this.input = function () {
      ...
      if (diff < that._minInterval) {
        that._scheduledTimer = setTimeout(
          triggerOutput,
          that._minInterval - diff
        );
      } else {
        triggerOutput();
      }
  ...
  ThrottleBuffer.prototype = CKEDITOR.tools.prototypedCopy(
    EventsBuffer.prototype
  );
  ...
  CKEDITOR.tools.style.border = CKEDITOR.tools.createClass({
    ...
    _: {
      ...
      normalize: function () {
        for (var propName in this._.normalizeMap) {
          ...
          if (val) {
            this[propName] = CKEDITOR.tools.array.reduce(
              this._.normalizeMap[propName],
              function (cur, rule) {
    ...
    proto: {
      toString: function () {
        return CKEDITOR.tools.array
          .filter([this.width, this.style, this.color], function (item) {
            return !!item;
          })
          .join(" ");
      },
    ...
    statics: {
      ...
      splitCssValues: function (styles, fallback) {
        ...
        var stylesMap = CKEDITOR.tools.array.reduce(
          types,
          function (cur, type) {
            var style = styles["border-" + type] || fallback[type];

            cur[type] = style
              ? CKEDITOR.tools.style.parse.sideShorthand(style)
              : null;
