...
(function () {
  ...
  CKEDITOR.filter = function (editorOrRules, rules) {
    ...
    var editor = (this.editor =
      editorOrRules instanceof CKEDITOR.editor ? editorOrRules : null);
  ...
  CKEDITOR.filter.prototype = {
    ...
    allow: function (newRules, featureName, overrideCustom) {
      ...
      if (typeof newRules == "string") newRules = parseRulesString(newRules);
        ...
        if (newRules.toAllowedContentRules)
          return this.allow(
            newRules.toAllowedContentRules(this.editor),
            featureName,
            overrideCustom
          );
      ...
      fragment.forEach(
        function (el) {
          if (el.type == CKEDITOR.NODE_ELEMENT) {
            ...
            if (processRetVal & FILTER_ELEMENT_MODIFIED) isModified = true;
            else if (processRetVal & FILTER_SKIP_TREE) return false;
          } else if (
    ...
    check: function (test, applyTransformations, strictCheck) {
      ...
      if (toBeRemoved.length > 0) {
        ...
        result = CKEDITOR.tools.objectCompare(
          element.attributes,
          clone.attributes,
          true
        );
  ...
  function applyAllowedRule(rule, element, status, skipRequired) {
    ...
    if (!status.allAttributes)
      status.allAttributes = applyAllowedRuleToHash(
        rule.attributes,
        element.attributes,
    ...
    if (!status.allStyles)
      status.allStyles = applyAllowedRuleToHash(
        rule.styles,
        element.styles,
        status.validStyles
      );
    ...
    if (!status.allClasses)
      status.allClasses = applyAllowedRuleToArray(
        rule.classes,
        element.classes,
        status.validClasses
      );
  ...
  function applyDisallowedRule(rule, element, status) {
    ...
    status.hadInvalidAttribute =
      applyDisallowedRuleToHash(rule.attributes, element.attributes) ||
      status.hadInvalidAttribute;
    status.hadInvalidStyle =
      applyDisallowedRuleToHash(rule.styles, element.styles) ||
      status.hadInvalidStyle;
    status.hadInvalidClass =
      applyDisallowedRuleToArray(rule.classes, element.classes) ||
      status.hadInvalidClass;
  }
  ...
  function convertStyleToRules(style) {
    ...
    rules[styleDef.element] = rule = {
      ...
      requiredStyles:
        styleDef.styles && CKEDITOR.tools.object.keys(styleDef.styles),
    };
  ...
  function processElement(that, element, toBeRemoved, opts) {
    ...
    if (opts.doCallbacks && that.elementCallbacks) {
      ...
      if (
        (callbacksRetVal = executeElementCallbacks(
          element,
          that.elementCallbacks
        ))
      )
  ...
  function regexifyPropertiesWithWildcards(validators) {
    ...
    if (patterns.length) return new RegExp("^(?:" + patterns.join("|") + ")$");
    else return null;
  }
  ...
  transformationsTools = CKEDITOR.filter.transformationsTools = {
    ...
    splitBorderShorthand: function (element) {
      ...
      var borderSplittedStyles = CKEDITOR.tools.style.parse.border(
