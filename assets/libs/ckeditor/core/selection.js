...
(function () {
  ...
  function isTableSelection(ranges, allowPartially) {
    ...
    function isPartiallySelected(range) {
      ...
      if (
        ...
        startCell.findOne("td, th, tr, tbody, table")
      ) {
        return false;
      }
  ...
  function isRealTableSelection(selection, fakeSelection) {
    ...
    function isValidTableSelection(table, fakeTable, ranges, fakeRanges) {
      var isMenuOpen = ranges.length === 1 && ranges[0].collapsed,
        ...
        isInTable =
          isTableSelection(ranges, !!CKEDITOR.env.webkit) &&
  ...
  function checkSelectionChange() {
    ...
    if (sel) {
      ...
      if (
        !realSel ||
        (!realSel.isHidden() && !isRealTableSelection(realSel, sel))
      ) {
    ...
    if (!currentPath.compare(this._.selectionPreviousPath)) {
      ...
      var sameBlockParent =
        ...
        this._.selectionPreviousPath.blockLimit.equals(currentPath.blockLimit);
  ...
  function removeFillingCharSequenceNode(editable, keepSelection) {
    ...
    if (fillingChar) {
      ...
      if (keepSelection !== false) {
        ...
        if (
          fillingChar.getLength() > fillingCharSeqLength &&
          range &&
          ...
          if (
            sel.anchorNode == fillingChar.$ &&
            sel.anchorOffset > fillingCharSeqLength
          ) {
          ...
          if (
            sel.focusNode == fillingChar.$ &&
            sel.focusOffset > fillingCharSeqLength
          ) {
      ...
      fillingChar.setText(
        removeFillingCharSequenceString(fillingChar.getText(), 1)
      );
  ...
  function hideSelection(editor, ariaLabel) {
    var content =
        (ariaLabel && CKEDITOR.tools.htmlEncode(ariaLabel)) || "&nbsp;",
      style =
        CKEDITOR.env.ie && CKEDITOR.env.version < 14
          ? "display:none"
  ...
  CKEDITOR.on("instanceCreated", function (ev) {
    ...
    editor.on("contentDom", function () {
      ...
      function getNonEditableAscendant(sel) {
        ...
        if (
          ascendant &&
          ascendant.getAttribute("contenteditable") === "false"
        ) {
          return ascendant;
        }
        ...
        if (last && last.hasAttribute("data-cke-hidden-sel")) {
          ...
          if (CKEDITOR.env.gecko) {
            ...
            if (
              first &&
              first.is("br") &&
              first.getAttribute("_moz_editor_bogus_node")
            ) {
              first.remove();
            }
          }
  ...
  if (CKEDITOR.env.webkit) {
    CKEDITOR.on("instanceReady", function (evt) {
      ...
      editor.on(
        "toDataFormat",
        function (evt) {
          evt.data.dataValue = removeFillingCharSequenceString(
            evt.data.dataValue
          );
        },
        null,
  ...
  CKEDITOR.dom.selection.prototype = {
    ...
    getRanges: (function () {
      ...
      return function (onlyEditables) {
        ...
        return extractEditableRanges(
          new CKEDITOR.dom.rangeList(ranges.slice())
        );
      };
    })(),
    ...
    unlock: function (restore) {
      ...
      if (restore) {
        ...
        } else if (faked) this.fake(selectedElement);
    ...
    selectBookmarks: function (bookmarks) {
      ...
      if (bookmarks.isFake) {
        node = isTableSelection(ranges)
          ? ranges[0]._getTableElement()
          : ranges[0].getEnclosedNode();
