...
(function () {
    ...
    editor.on(
      ...
      function (evt) {
        ...
        if (evtData.fixForBody === false) {
          ...
          fixBodyTag = getFixBodyTag(
            evtData.enterMode,
            editor.config.autoParagraph
          );
        }

        // Now use our parser to make further fixes to the structure, as
        // well as apply the filter.
        data = CKEDITOR.htmlParser.fragment.fromHtml(
          data,
          evtData.context,
          fixBodyTag
        );
    ...
    editor.on(
      ...
      function (evt) {
        ...
        evt.data.dataValue = CKEDITOR.htmlParser.fragment.fromHtml(
          data,
          evt.data.context,
          getFixBodyTag(evt.data.enterMode, editor.config.autoParagraph)
        );
      },
      null,
      null,
  ...
  function createBogusAndFillerRules(editor, type) {
    ...
    function blockFilter(isOutput, fillEmptyBlock) {
      return function (block) {
        ...
        var shouldFillBlock =
          !isOutput ||
          (typeof fillEmptyBlock == "function"
    ...
    function maybeBogus(node, atBlockEnd) {
      ...
      if (
        !(isOutput && !CKEDITOR.env.needsBrFiller) &&
      ...
      if (
        ...
        if (match.index) {
          new CKEDITOR.htmlParser.text(
            node.value.substring(0, match.index)
          ).insertBefore(node);
          node.value = match[0];
        }
    ...
    function isEmptyBlockNeedFiller(block) {
      ...
      if (
        !isOutput &&
        !CKEDITOR.env.needsBrFiller &&
        block.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT
      )
        return false;
  ...
  function getFixBodyTag(enterMode, autoParagraph) {
    return enterMode != CKEDITOR.ENTER_BR && autoParagraph !== false
  ...
  function isEmpty(node) {
    return (
      (node.type == CKEDITOR.NODE_TEXT && !CKEDITOR.tools.trim(node.value)) ||
      (node.type == CKEDITOR.NODE_ELEMENT &&
  ...
  var defaultHtmlFilterRulesForAll = {
    ...
    elements: {
      ...
      title: function (element) {
        ...
        !titleText &&
          append(element, (titleText = new CKEDITOR.htmlParser.text()));
    ...
    attributes: {
      class: function (value) {
        ...
        return (
          CKEDITOR.tools.ltrim(value.replace(/(?:^|\s+)cke_[^\s]*/g, "")) ||
          false
        );
      },
    },
  ...
  var protectElementNamesRegex =
      /(<\/?)((?:object|embed|param|html|body|head|title)([\s][^>]*)?>)/gi,
    unprotectElementNamesRegex = /(<\/?)cke:((?:html|body|head|title)[^>]*>)/gi;

  var protectSelfClosingRegex =
        ...
        return (
          ...
          attributes.replace(
            ...
            function (fullAttr, attrName) {
              ...
              if (
                protectAttributeNameRegex.test(attrName) &&
                attributes.indexOf("data-cke-saved-" + attrName) == -1
              )
                return (
                  " data-cke-saved-" +
                  fullAttr +
  ...
  function unprotectSource(html, editor) {
    ...
    return html
      .replace(/<!--\{cke_protected\}([\s\S]+?)-->/g, function (match, data) {
        return decodeURIComponent(data);
      })
  ...
  function protectSource(data, editor) {
    ...
    for (var i = 0; i < regexes.length; i++) {
      data = data.replace(regexes[i], function (match) {
        match = match.replace(
          tempRegex, // There could be protected source inside another one. (https://dev.ckeditor.com/ticket/3869).
        ...
        return /cke_temp(comment)?/.test(match)
          ? match
    ...
    data = data.replace(
      ...
      function (match, tagName, tagAttributes, innerText) {
        return (
          "<" +
          tagName +
          tagAttributes +
          ">" +
          unprotectSource(unprotectRealComments(innerText), editor) +
          "</" +
          tagName +
          ">"
        );
