...
(function () {
    ...
    proto: {
      focus: function () {
        ...
        try {
          if (
            CKEDITOR.env.ie &&
            !(CKEDITOR.env.edge && CKEDITOR.env.version > 14) &&
            this.getDocument().equals(CKEDITOR.document)
          ) {
            this.$.setActive();
          } else {
      ...
      attachListener: function (
        obj /*, event, fn, scope, listenerData, priority*/
      ) {
      ...
      fixInitialSelection: function () {
        ...
        if (
          CKEDITOR.env.ie &&
          (CKEDITOR.env.version < 9 || CKEDITOR.env.quirks)
        ) {
          if (this.hasFocus) {
            this.focus();
            fixMSSelection();
      ...
      extractHtmlFromRange: function (range, removeEmptyBlock) {
        ...
        targetRange.moveToPosition(
          that.bookmark.startNode,
        ...
        if (removeEmptyBlock) {
          ...
          if (isEmpty(range.startContainer)) range.startContainer.appendBogus();
        }
      ...
      setup: function () {
        ...
        if (CKEDITOR.env.edge && CKEDITOR.env.version > 14) {
          var fixScrollOnFocus = function () {
            ...
            if (
              ...
              editable.getDocument().equals(CKEDITOR.document)
            ) {
        ...
        if (styles) {
          ...
          if (!stylesElement) {
            ...
            CKEDITOR.env.ie && CKEDITOR.env.version < 9
              ? (stylesElement.$.styleSheet.cssText = styles)
              : stylesElement.setText(styles);
          }
        }
        ...
        if (!CKEDITOR.env.ie || CKEDITOR.env.edge) {
          this.attachListener(this, "mousedown", function (ev) {
            ...
            if (
              control.is("img", "hr", "input", "textarea", "select") &&
              !control.isReadOnly()
            ) {
        ...
        if (CKEDITOR.env.gecko) {
          this.attachListener(this, "mouseup", function (ev) {
            if (ev.data.$.button == 2) {
              ...
              if (
                !target.getAscendant("table") &&
                !target.getOuterHtml().replace(emptyParagraphRegexp, "")
              ) {
                var range = editor.createRange();
        ...
        if (CKEDITOR.env.webkit) {
          this.attachListener(
            ...
            function (evt) {
              ...
              if (!(key in backspaceOrDelete)) return;
            ...
            null,
            100
          ); // Later is better – do not override existing listeners.
        }
      },
    },
  ...
  function isNotBubbling(fn, src) {
    return function (evt) {
      ...
      other =
        other && other.nodeType == CKEDITOR.NODE_ELEMENT
          ? new CKEDITOR.dom.element(other)
          : null;
  ...
  insert = (function () {
    ...
    function isSingleNonEditableElement(nodesData) {
      if (nodesData.length != 1) return false;

      var nodeData = nodesData[0];

      return (
        nodeData.isElement &&
        nodeData.node.getAttribute("contenteditable") == "false"
      );
    }
  ...
  fixListAfterContentsDelete = (function () {
    ...
    function getFixListSelectionWalker(testRange) {
      ...
      walker.evaluator = function (node) {
        return (
          node.type == CKEDITOR.NODE_ELEMENT && node.is(CKEDITOR.dtd.$listItem)
        );
      };
  ...
  function mergeBlocksCollapsedSelection(editor, range, backspace, startPath) {
    ...
    if (
      !range.moveToClosestEditablePosition(startBlock, !backspace) ||
      !range.collapsed
    )
      return false;
    ...
    if (range.startContainer.type == CKEDITOR.NODE_ELEMENT) {
      var touched = range.startContainer.getChild(
        range.startOffset - (backspace ? 1 : 0)
      );
      if (
        touched &&
        touched.type == CKEDITOR.NODE_ELEMENT &&
        touched.is("hr")
      ) {
        editor.fire("saveSnapshot");
        touched.remove();
    ...
    // Merge blocks.
    (backspace ? startBlock : siblingBlock).moveChildren(
      backspace ? siblingBlock : startBlock,
      false
    );
  ...
  function mergeBlocksNonCollapsedSelection(editor, range, startPath) {
    ...
    if (!startBlock || !endBlock || startBlock.equals(endBlock)) return false;

    editor.fire("saveSnapshot");

    // Remove bogus to avoid duplicated boguses.
    var bogus;
    if ((bogus = startBlock.getBogus())) bogus.remove();
    ...
    eol: {
      detect: function (that, editable) {
        var range = that.range,
          ...
          startPath = new CKEDITOR.dom.elementPath(
        ...
        if (
          startPath.block &&
          rangeStart.checkBoundaryOfElement(startPath.block, CKEDITOR.END)
        ) {
        ...
        if (
          endPath.block &&
          rangeEnd.checkBoundaryOfElement(endPath.block, CKEDITOR.START)
        ) {
    ...
    bogus: {
      exclude: function (that) {
        ...
        if (
          endNode &&
          isBogus(endNode) &&
          (!startNode || !startNode.equals(endNode))
        )
    ...
    tree: {
      rebuild: function (that, editable) {
        ...
        if (!limit) {
          limit = function (node) {
            return (
              !node.equals(commonPath.block) &&
              !node.equals(commonPath.blockLimit)
            );
          };
        }
  ...
  extractHtmlFromRangeHelpers = (function () {
    ...
    function mergeElements(merged, startBookmark, endBookmark) {
      ...
      if (isEmpty(merged)) merged.remove();
    }
    ...
    var list = {
      detectMerge: function (that, editable) {
        ...
        that.mergeListItems =
          ...
          startPath.block.is(CKEDITOR.dtd.$listItem) &&
          endPath.block.is(CKEDITOR.dtd.$listItem);
      ...
      merge: function (that, editable) {
        if (!that.mergeListBookmark) return;
    ...
    var block = {
      ...
      detectMerge: function (that, editable) {
        ...
        if (that.tableContentsRanges || that.mergeListBookmark) return;
      ...
      merge: function (that, editable) {
        if (!that.mergeBlockBookmark || that.purgeTableBookmark) return;
    ...
    var table = (function () {
      ...
      function findTableContentsRanges(range) {
        ...
        walker.guard = function (node, leaving) {
          ...
          if (
            (!leaving && checkRemoveCellContents(node)) ||
        ...
        function checkNested(node) {
          ...
          return (
            (startTable && startTable.contains(nodeTable)) ||
      ...
      function getNormalizedAncestor(range) {
        ...
        if (
          common.is(CKEDITOR.dtd.$tableContent) &&
          !common.is(tableEditable)
        ) {
      ...
      return {
        ...
        detectPurge: function (that) {
          ...
          if (editablesCount > 1) {
            ...
            if (
              startTable &&
              endTable &&
              range.checkBoundaryOfElement(startTable, CKEDITOR.START) &&
        ...
        detectRanges: function (that, editable) {
          var range = createRangeFromBookmark(editable, that.bookmark),
            ...
            endPath = new CKEDITOR.dom.elementPath(
              range.endContainer,
              commonAncestor
            ),
          ...
          if (
            startTable &&
            endTable &&
            checkDisjointNodes(startTable, endTable)
          ) {
            that.tableSurroundingRange = surroundingRange;
            surroundingRange.setStartAt(
              startTable,
            ...
            tableContentsRanges = findTableContentsRanges(leftRange).concat(
              findTableContentsRanges(rightRange)
            );
          }
          ...
          that.tableContentsRanges = tableContentsRanges
            ? tableContentsRanges
            : findTableContentsRanges(range);
        ...
        purge: function (that) {
          if (!that.purgeTableBookmark) return;
    ...
    return {
      ...
      autoParagraph: function (editor, range) {
        ...
        if (
          shouldAutoParagraph(editor, path.block, path.blockLimit) &&
          (fixBlock = autoParagraphTag(editor))
        ) {
