...
(function () {
  ...
  function getConfigPath() {
    return CKEDITOR.getUrl(
      CKEDITOR.skinName.split(",")[1] || "skins/" + getName() + "/"
    );
  }
    ...
    loadPart: function (part, fn) {
      if (CKEDITOR.skin.name != getName()) {
        CKEDITOR.scriptLoader.load(
          CKEDITOR.getUrl(getConfigPath() + "skin.js"),
          function () {
            loadCss(part, fn);
          }
        );
      ...
      showCallback = function (event) {
        ...
        if (!iframe.getById("cke_ui_color")) {
          ...
          editor.on("destroy", function () {
            uiColorMenus = CKEDITOR.tools.array.filter(
              uiColorMenus,
              function (storedNode) {
          ...
          if (color) {
            updateStylesheets(
              [node],
              CKEDITOR.skin.chameleon(editor, "panel"),
              [[uiColorRegexp, color]]
            );
          }
        }
