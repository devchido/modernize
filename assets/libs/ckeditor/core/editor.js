...
(function () {
      ...
      if (editor.elementMode == CKEDITOR.ELEMENT_MODE_INLINE) {
        if (editor.element.is("textarea")) {
          return (
            ...
            editor.element.hasAttribute("readonly")
          );
        } else {
    ...
    function isSelectedCell(range) {
      ...
      if (
        ...
        (start.is("tr") ||
          (start.is("td") &&
            start.equals(end) &&
            range.endOffset === start.getChildCount()))
      ) {
        return true;
      }
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (isSelectedCell(range)) {
        if (!tableClone) {
          ...
          tableClone.append(
            container.getAscendant({ thead: 1, tbody: 1, tfoot: 1 }).clone()
          );
  ...
  CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
    ...
    addCommand: function (commandName, commandDefinition) {
      ...
      var cmd =
        commandDefinition instanceof CKEDITOR.command
          ? commandDefinition
          : new CKEDITOR.command(this, commandDefinition);
    ...
    destroy: function (noUpdate) {
      ...
      CKEDITOR.tools.array.forEach(
        CKEDITOR.tools.object.keys(filters),
        function (id) {
    ...
    execCommand: function (commandName, data) {
      ...
      if (command && command.state != CKEDITOR.TRISTATE_DISABLED) {
        if (this.fire("beforeCommandExec", eventData) !== false) {
          ...
          if (
            !command.async &&
            this.fire("afterCommandExec", eventData) !== false
          )
    ...
    extractSelectedHtml: function (toString, removeEmptyBlock) {
      ...
      for (i = 0; i < ranges.length; i++) {
        docFragment.append(
          editable.extractHtmlFromRange(ranges[i], removeEmptyBlock)
        );
      }
    ...
    getCommandKeystroke: function (command, all) {
      var commandInstance =
          typeof command === "string" ? this.getCommand(command) : command,
        ret = [];

      if (commandInstance) {
        var commandName = CKEDITOR.tools.object.findKey(
            this.commands,
            commandInstance
          ),
