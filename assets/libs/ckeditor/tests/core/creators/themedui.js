...
(function () {
  ...
  bender.test({
    "test resize event": function () {
      ...
      assert.areSame(
        400,
        lastResizeData.outerHeight,
        "Outer height should be same as passed one in 2nd argument."
      );
      assert.areSame(
        100,
        lastResizeData.outerWidth,
        "Outer width should be same as passed one in 1st argument."
      );
      assert.areSame(
        getEditorContentHeight(editor),
        lastResizeData.contentsHeight,
        "Content height should be same as calculated one."
      );
      assert.areSame(
        400,
        getEditorOuterHeight(editor),
        "Outer height should be properly set."
      );
      ...
      assert.areSame(
        getEditorOuterHeight(editor),
        lastResizeData.outerHeight,
        "Outer height should be same as calculated one."
      );
      assert.areSame(
        100,
        lastResizeData.outerWidth,
      ...
      assert.areSame(
        400,
        lastResizeData.contentsHeight,
        "Content height should be same as passed one in 2nd argument."
      );
      assert.areSame(
        400,
        getEditorContentHeight(editor),
        "Content height should be properly set."
      );
    },
