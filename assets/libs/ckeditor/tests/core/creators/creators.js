...
(function () {
  ...
  bender.test({
    ...
    checkEditorEvents: function (editor, events) {
      ...
      for (
        var i = 0, evt, length = events.length;
        (evt = events[i]), i < length;
        i++
      )
    ...
    checkEditorTheme: function (editor) {
      ...
      assert.areSame(
        ...
        editor.mode,
        "editor.mode should matches startup mode config."
      );
      assert.areSame(
        CKEDITOR.instances.foo,
        editor,
        "instance reference is globally available."
      );
      assert.areSame(
        CKEDITOR.document.getById("foo").$,
        editor.element.$,
        "editor.element"
      );
      assert.areSame(
    ...
    checkEditorProperties: function (editor) {
      ...
      if (
        ...
        assert.areSame(
          CKEDITOR.document.getById(editor.name),
          editor.element,
          "editor.name matches id of editor.element."
        );
    },
    ...
    checkEditorStatuses: function (editor) {
      ...
      editor.on(
        ...
        null,
        0
      ); // Check with the highest priority.
      editor.on(
        "instanceReady",
        function () {
          statuses.push(editor.status);
      ...
      return function () {
        arrayAssert.itemsAreEqual(
          ["unloaded", "loaded", "ready", "destroyed"],
          statuses,
          "Statuses values doesn't match"
        );
      };
    },
    ...
    "test creator replace": function () {
      var tc = this,
        target = CKEDITOR.document
          .getBody()
          .append(
            CKEDITOR.dom.element.createFromHtml(
              '<textarea id="foo">&lt;p&gt;foo&lt;/p&gt;</textarea>'
            )
          );
        ...
        on: {
          instanceReady: function (evt) {
            resume(function () {
              ...
              assert.areSame(
                editor.element.getNext().$,
                editor.container.$,
                "editor.container should be the adjacent element of editor.element"
              );
              assert.isFalse(
                editor.element.isVisible(),
                "editor.element should be hidden from view."
              );
              checkEvents();
      ...
      assert.areSame(
        "<p>foo</p>",
        editor.getData(),
        "editor initial data doesn't match"
      );

      var checkEvents = this.checkEditorEvents(editor, [
          "configLoaded",
          "langLoaded",
          "pluginsLoaded",
          "loaded",
    ...
    // (#2748)
    "test creator replace on bogus DOM element":
      createBogusElementTest("replace"),
          ...
          instanceReady: function () {
            resume(function () {
              ...
              assert.areSame(
                container.getLast(),
                editor.container,
                "editor.container should be the last child element of editor.element"
              );
    ...
    "test creator inline": function () {
      var tc = this,
        target = CKEDITOR.document
          ...
          .append(
            CKEDITOR.dom.element.createFromHtml(
              '<div id="foo" contenteditable="true"><p>foo</p></div>'
            )
          );
          ...
          instanceReady: function () {
            resume(function () {
              ...
              assert.areSame(
                editor.editable().$,
                editor.element.$,
                "editor.editable should be the same with editor.element"
              );
              checkEvents(editor);
    ...
    // (#2748)
    "test creator inline on bogus DOM element":
      createBogusElementTest("inline"),
