...
(function () {
  ...
  bender.test({
    "test creation": function () {
      ...
      assert.isTrue(
        this.editor.editable().isInline(),
        "Container is div - editor 1"
      );
      assert.areSame(
        CKEDITOR.document.getById("editor1"),
        this.editor.element,
        "Element is a textarea - editor1"
      );
        ...
        on: {
          instanceReady: function (evt) {
            resume(function () {
              ...
              assert.areSame(
                "true",
                editor.container.getAttribute("contenteditable"),
                "Container is contentediable"
              );
              assert.isTrue(
                editor.container.equals(editor.editable()),
                "Editable is container"
              );

              assert.areSame(
                "none",
                textarea.getStyle("display"),
                "Textarea is hidden"
              );
              ...
              assert.areSame(
                ...
                "Editor is available in CKEDITOR.instances"
              );
    ...
    "test setData": function () {
      ...
      this.editorBot.setData("<p>test setData</p>", function () {
        assert.areSame(
          "<p>test setData</p>",
          editor.getData(),
          "New data set correctly"
        );
        assert.areSame(
          initialValue,
          editor.element.getValue(),
          "Textarea value remains untouched"
        );
      });
    },
    ...
    "test create concurrent editor: framed on bound textarea":
      createConcurrentEditorTest("replace", "editor1"),

    "test create concurrent editor: inline on bound textarea":
      createConcurrentEditorTest("inline", "editor1"),
      ...
      editor.setData("<p>test auto update</p>", function () {
        ...
        resume(function () {
          assert.areSame(
            ...
            editor.element.getValue(),
            "Textarea content should be updated on submit."
          );
        });
      });
