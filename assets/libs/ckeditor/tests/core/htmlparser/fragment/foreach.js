...
function record(callback) {
  ...
  return {
    tokens: tokens,
    fn: function (node) {
      if (node.type == CKEDITOR.NODE_ELEMENT) tokens.push(node.name);
      else if (node.type == CKEDITOR.NODE_TEXT) tokens.push("#" + node.value);
      else if (node.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT)
        tokens.push("docfrag");
      else tokens.push("not implemented type");
...
bender.test({
  "test forEach": function () {
    ...
    assertItems(
      ...
      rec.tokens,
      "Second forEach() gives the same result."
    );
  },
  ...
  "test forEach - stop on false": function () {
    var fragment = fromHtml(
        "<p>text0<b>text1</b><i><b>text2<u>text3</u></b>text4</i></p>"
      ),
