...
bender.test({
  ...
  "test getOuterHtml": function () {
    ...
    assert.areSame(
      '<p foo="1"><b foo="1">A<u foo="2">C</u></b><i>B</i></p>',
      el.getOuterHtml()
    );
  },
  ...
  "test clone": function () {
    ...
    assert.isTrue(
      CKEDITOR.tools.objectCompare(el.attributes, clone.attributes)
    );
  },
  ...
  "test split": function () {
    ...
    assert.areSame(
      elPChildren,
      elP.children,
      "elP.children was modified, not overwritten"
    );

    assert.areSame(
      '<p foo="1"><b>A</b></p><p foo="1"><i>B</i></p>',
      write(frag)
    );
  },
  ...
  "test split at the beginning": function () {
    ...
    assert.areSame(
      '<p foo="1"></p><p foo="1"><b>A</b><i>B</i></p>',
      write(frag)
    );
  },
  ...
  "test split at the end": function () {
    ...
    assert.areSame(
      '<p foo="1"><b>A</b><i>B</i></p><p foo="1"></p>',
      write(frag)
    );
  },
  ...
  "test getFilterContext - no changes": function () {
    var el = new CKEDITOR.htmlParser.fragment.fromHtml("<p>foo</p>")
      .children[0];
  ...
  "test getFilterContext - non-editable": function () {
    var el = new CKEDITOR.htmlParser.fragment.fromHtml(
  ...
  "test getFilterContext - editable body (fullPage)": function () {
    var el = new CKEDITOR.htmlParser.fragment.fromHtml(
      '<body contenteditable="true">x</body>'
    ).children[0];
