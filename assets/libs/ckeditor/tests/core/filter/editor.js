...
(function () {
  ...
  bender.test({
    ...
    "test extraAllowedContent": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.areSame(
            "<p>foo <b>bar</b> bom</p>",
            editor.dataProcessor.toHtml("<p>foo <b>bar</b> <i>bom</i></p>")
          );
    ...
    "test dynamic configuration": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.areSame(
            "<p>foo bar</p>",
            editor.dataProcessor.toHtml("<p>foo <b>bar</b></p>"),
            "Test before."
          );
    ...
    "test disabling filter": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.areSame(
            '<p>foo <b class="a" foo="bar">bar</b></p>',
    ...
    "test whether rules are marked with feature name": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.isTrue(
            CKEDITOR.tools.objectCompare({ a: true }, allowed[2].elements)
          );

          assert.areSame("testcommand", allowed[3].featureName);
          assert.isTrue(
            CKEDITOR.tools.objectCompare({ b: true }, allowed[3].elements)
          );
        }
      );
    },
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          bot.setData("<p></p>", function () {
            ...
            editor.insertHtml(
              '<u class="active">A1</u>-<u class="main">A2</u>'
            );

            filter.applyTo = function () {
              assert.fail(
                "active filter should not be used while getting data from editor"
              );
            };
            assert.areSame(
              '<p><u class="active">A1</u>-A2</p>',
              editor.getData(),
              "data was filtered using active filter"
            );

            bot.setData(
              '<p><u class="active">A1</u>-<u class="main">A2</u></p>',
              function () {
                assert.areSame(
                  '<p>A1-<u class="main">A2</u></p>',
                  editor.getData(),
                  "data was filtered using main filter"
                );
              }
            );
          });
    ...
    // Rule of thumb - contextual processing - active filter,
    // global processing - main filter.
    "test active/main filter usage depending on context - inline editor":
      function () {
        bender.editorBot.create(
          {
    ...
    "test filter is derefernced on editor.destroy()": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.isFalse(
            "activeFilter" in editor,
            "editor.activeFilter was deleted"
          );
        }
      );
    },
