  ...
  bender.test({
    ...
    "test callback is executed on all elements when filtering": function () {
      ...
      assert.areSame(
        "h1,p,u",
        names.join(","),
        "not allowed elements are visited"
      );

      names = [];
      filter(
        '<p>x<img src="foo" alt="" />y<textarea cols="10" rows="10">x</textarea>z</p>',
        "<p>xyxz</p>"
      );
      assert.areSame(
        "p,img,textarea",
        names.join(","),
        "empty elements are visited"
      );
    },

    // In this test we need to use real editor, because script tag is protected by the htmlDP.
    "test callback is executed on protected elements when filtering":
      function () {
        ...
        try {
          ...
          assert.areSame(
            "p,script,p",
            names.join(","),
            "protected elements are visited"
          );
        } catch (e) {
          throw e;
        } finally {
    ...
    "test callback is not executed when checking": function () {
      ...
      filter.addElementCallback(function (el) {
        assert.fail(
          "callback should not be executed when checking rule: " + el.name
        );
      });
    ...
    "test callbacks are executed until one of them returns a value":
      function () {
