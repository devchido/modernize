...
(function () {
  ...
  function createTest(rules, strictCheck) {
    ...
    return function (allowed, data, msg) {
      assert[allowed ? "isTrue" : "isFalse"](
        filter.check(data, true, strictCheck),
        msg || "Assertion (" + number + ") failed."
      );
      number += 1;
    };
  }
  ...
  bender.test({
    ...
    "test required attributes with strict check": function () {
      var test = createTest(
        "c[!attr1,attr2,!attr3]; b[!attr1]; b[!attr2]; d[!data-*,attr1]",
        true
      );
    ...
    "test required styles with strict check": function () {
      var test = createTest(
        "c{!st1,st2,!st3}; b{!st1}; b{!st2}; d{!data-*,st1}",
        true
      );
    ...
    "test required classes with strict check": function () {
      var test = createTest(
        "c(!cl1,cl2,!cl3); b(!cl1); b(!cl2); d(!cl-*,cl1)",
        true
      );
    ...
    "test two required properties with wildcards with strict check":
      function () {
    ...
    "test checks cache": function () {
      ...
      assert.isFalse(
        filter.check("img", false, true),
        "Cache should be aware of 3rd arg."
      );
      assert.isTrue(
        filter.check("img", true, true),
        "Cache should be aware of 2nd arg."
      );
    },
