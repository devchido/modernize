...
(function () {
  ...
  function assertToHtml(editor, input, html, msg) {
    assert[typeof html == "string" ? "areSame" : "isMatching"](
      html,
      bender.tools.compatHtml(editor.dataProcessor.toHtml(input), 0, 1),
      msg + " - toHtml"
    );
  }

  function assertToDF(editor, html, output, msg) {
    assert[typeof output == "string" ? "areSame" : "isMatching"](
      output,
      bender.tools.compatHtml(editor.dataProcessor.toDataFormat(html), 0, 1),
      msg + " - toDF"
    );
  }

  function assertToBeautifiedHtml(editor, input, html, msg) {
    var result = bender.tools.compatHtml(
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          filter.addFeature({
            ...
            allowedContent:
              "img[width,height,alt,src,foo]; table{color,width,height}[width,height]; tr td tbody",
    ...
    "test form transformations": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          var emStyle = new CKEDITOR.style({
            element: "span",
            attributes: { class: "i" },
          });
          editor.addFeature({
            allowedContent: "em",
            contentForms: ["em", emStyle],
          });
          ...
          assertToHtml(editor, "<p><em>A</em></p>", "<p><em>A</em></p>", "em");
          assertToHtml(
            editor,
    ...
    "test alignment transformations": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assertToHtml(
            editor,
            '<h1 style="float:right">A</h1>',
            '<h1 style="float:right">A</h1>',
            "h1 2"
          );
          assertToHtml(
            editor,
            '<h1 style="float:none">A</h1>',
            '<h1 style="float:none">A</h1>',
            "h1 3"
          );
          assertToHtml(
          ...
          assertToHtml(
            editor,
            '<h1 align="bottom">A</h1>',
            "<h1>A</h1>",
            "h1 7"
          );
          assertToHtml(
            editor,
            '<h1 align="left" style="float:right">A</h1>',
            '<h1 style="float:right">A</h1>',
            "h1 8"
          );
          ...
          assertToHtml(
            editor,
            '<h2 style="float:left">A</h2>',
            '<h2 align="left">A</h2>',
            "h2 1"
          );
          assertToHtml(
            editor,
            '<h2 style="float:right">A</h2>',
            '<h2 align="right">A</h2>',
            "h2 2"
          );
          assertToHtml(
            editor,
            '<h2 style="float:none">A</h2>',
            "<h2>A</h2>",
            "h2 3"
          );
          assertToHtml(
          ...
          assertToHtml(
            editor,
            '<h2 align="middle">A</h2>',
            /<h2 align="(middle|center)">A<\/h2>/,
            "h2 6"
          );
          assertToHtml(
            editor,
            '<h2 align="bottom">A</h2>',
            '<h2 align="bottom">A</h2>',
            "h2 7"
          );
          assertToHtml(
            editor,
            '<h2 align="left" style="float:right">A</h2>',
            '<h2 align="left">A</h2>',
            "h2 8"
          );
          ...
          assertToHtml(
            editor,
            '<h3 align="left" style="float:right">A</h3>',
            '<h3 style="float:right">A</h3>',
            "h3 3"
          );

          assertToHtml(
            editor,
            '<h4 style="float:left">A</h4>',
            '<h4 style="float:left">A</h4>',
            "h4 1"
          );
          assertToHtml(
          ...
          assertToHtml(
            editor,
            '<h4 align="left" style="float:right">A</h4>',
            '<h4 style="float:right">A</h4>',
            "h4 3"
          );
        }
      );
    ...
    "test splitBorderShorthand transformation": function () {
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          editor.filter.addTransformations([["p: splitBorderShorthand"]]);

          assertToBeautifiedHtml(
            editor,
            '<p style="border: 1px solid red">A</p>',
            '<p style="border-color:red; border-style:solid; border-width:1px">A</p>',
            "border split shorthand 1"
          );
          assertToBeautifiedHtml(
            editor,
            '<p style="border: 5px dashed rgb(23, 45, 89)">A</p>',
            '<p style="border-color:#172d59; border-style:dashed; border-width:5px">A</p>',
            "border split shorthand 2"
          );
          assertToBeautifiedHtml(
            editor,
            '<p style="border: 2em dotted #345678">A</p>',
            '<p style="border-color:#345678; border-style:dotted; border-width:2em">A</p>',
            "border split shorthand 3"
          );
          assertToBeautifiedHtml(
            editor,
            '<p style="border: 3px double">A</p>',
            '<p style="border-style:double; border-width:3px">A</p>',
            "border split shorthand 4"
          );
          assertToBeautifiedHtml(
            editor,
            '<p style="border:3px">A</p>',
            '<p style="border-width:3px">A</p>',
            "border split shorthand 5"
          );
          assertToBeautifiedHtml(
            editor,
            '<p style="border:thick ridge hsla(50, 100%, 50%, 0.7)">A</p>',
            '<p style="border-color:hsla(50, 100%, 50%, 0.7); border-style:ridge; border-width:thick">A</p>',
            "border split shorthand 6"
          );
        }
      );
    },
