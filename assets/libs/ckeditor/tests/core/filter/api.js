...
(function () {
  ...
  function getByFeature(filter, featureName) {
    for (var i = 0, acrs = filter.allowedContent; i < acrs.length; ++i) {
      if (acrs[i].featureName == featureName) return acrs[i];
    }
    return null;
  }
  ...
  function assertRule(filter, featureName, expected) {
    ...
    for (var name in expected) {
      ...
      if (CKEDITOR.tools.isArray(expectedValue))
        ...
        assert.isTrue(
          CKEDITOR.tools.objectCompare(expectedValue, actualValue),
          msg
        );
      else assert.areSame(expectedValue, actualValue, msg);
    }
  }
  ...
  bender.test({
    "test filters instances are registered": function () {
      assert.areSame(
        this.editor.filter,
        CKEDITOR.filter.instances[this.editor.filter.id],
        "editor.filter"
      );
      ...
      assert.areSame(
        filter,
        CKEDITOR.filter.instances[filter.id],
        "standalone filter"
      );
    },
    ...
    "test addFeature - default configuration": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.isTrue(
            filter.addFeature({ requiredContent: "b", allowedContent: "b" })
          );
    ...
    "test addFeature - custom configuration": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.isTrue(
            filter.addFeature({ requiredContent: "b", allowedContent: "i" })
          );
    ...
    "test allow - styles - editor instance is passed to toAllowedContentRules":
      function () {
        var editor = this.editor,
          style = st({ element: "p", styles: { "font-style": "italic" } });
    ...
    "test getAllowedEnterMode": function () {
      ...
      assert.areSame(
        CKEDITOR.ENTER_P,
        filter1.getAllowedEnterMode(CKEDITOR.ENTER_P),
        "f1a"
      );
      assert.areSame(
        CKEDITOR.ENTER_DIV,
        filter1.getAllowedEnterMode(CKEDITOR.ENTER_DIV),
        "f1b"
      );
      assert.areSame(
        CKEDITOR.ENTER_P,
        filter1.getAllowedEnterMode(CKEDITOR.ENTER_BR),
        "f1c"
      );

      assert.areSame(
        CKEDITOR.ENTER_P,
        filter1.getAllowedEnterMode(CKEDITOR.ENTER_P, true),
        "f1a - reverse"
      );
      assert.areSame(
        CKEDITOR.ENTER_DIV,
        filter1.getAllowedEnterMode(CKEDITOR.ENTER_DIV, true),
        "f1b - reverse"
      );
      assert.areSame(
        CKEDITOR.ENTER_DIV,
        filter1.getAllowedEnterMode(CKEDITOR.ENTER_BR, true),
        "f1c - reverse"
      );

      assert.areSame(
        CKEDITOR.ENTER_BR,
        filter2.getAllowedEnterMode(CKEDITOR.ENTER_P),
        "f2"
      );
      assert.areSame(
        CKEDITOR.ENTER_BR,
        filter2.getAllowedEnterMode(CKEDITOR.ENTER_P, true),
        "f2 - reverse"
      );
      assert.areSame(
        CKEDITOR.ENTER_BR,
        filter3.getAllowedEnterMode(CKEDITOR.ENTER_P),
        "f3"
      );
      assert.areSame(
        CKEDITOR.ENTER_BR,
        filter3.getAllowedEnterMode(CKEDITOR.ENTER_P, true),
        "f3 - reverse"
      );
      assert.areSame(
        CKEDITOR.ENTER_P,
        filter4.getAllowedEnterMode(CKEDITOR.ENTER_P),
        "f4"
      );
      assert.areSame(
        CKEDITOR.ENTER_P,
        filter4.getAllowedEnterMode(CKEDITOR.ENTER_P, true),
        "f4a - reverse"
      );
      assert.areSame(
        CKEDITOR.ENTER_BR,
        filter4.getAllowedEnterMode(CKEDITOR.ENTER_BR, true),
        "f4b - reverse"
      );
    },
    ...
    "test destroy": function () {
      ...
      assert.areSame(
        filter,
        CKEDITOR.filter.instances[id],
        "filter is registered before destroy"
      );

      filter.destroy();
      assert.isFalse(
        id in CKEDITOR.filter.instances,
        "filter is not registered after destroy"
      );
      assert.isFalse("_" in filter, "filter's private parts are deleted");
      assert.isFalse(
        "allowedContent" in filter,
        "filter.allowedContent is deleted"
      );
      assert.isFalse(
        "disallowedContent" in filter,
        "filter.disallowedContent is deleted"
      );
    },
  });
})();
