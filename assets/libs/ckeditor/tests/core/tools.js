...
(function () {
  ...
  var vendorPrefix = CKEDITOR.env.gecko
      ? "-moz-"
      : CKEDITOR.env.webkit
        ? "-webkit-"
        : CKEDITOR.env.ie
  ...
  bender.test({
    ...
    "test extend dont enum attribute": function () {
      var dontEnumObj = CKEDITOR.tools.convertArrayToObject(
    ...
    "test htmlEncodeAttr - all covered entities": function () {
      assert.areSame(
        "&lt;a b=&quot;c&amp;d&quot;/&gt;",
        CKEDITOR.tools.htmlEncodeAttr('<a b="c&d"/>')
      );
    },
    ...
    test_cssStyleToDomStyle1: function () {
      assert.areSame(
        "backgroundColor",
        CKEDITOR.tools.cssStyleToDomStyle("background-color")
      );
    },
    ...
    test_cssStyleToDomStyle2: function () {
      assert.areSame(
        CKEDITOR.env.ie && document.documentMode <= 8
          ? "styleFloat"
          : "cssFloat",
        CKEDITOR.tools.cssStyleToDomStyle("float")
      );
    },
    ...
    test_createClass: function () {
      ...
      assert.isTrue(
        ...
        "check instanceof both A & B & C"
      );
    },

    testNormalizeCssText: assertNormalizeCssText(
      "color:red;font-size:10px;width:10.5em;",
      " width: 10.5em ; COLOR : red; font-size:10px  ; ",
      "order, lowercase and white spaces"
    ),
    ...
    testNormalizeCssText2: function () {
      ...
      assert.areSame(
        "border-radius:3px;cursor:default;float:left;height:18px;opacity:0.4;outline:none;padding:2px 4px;",
        n(
          "height: 18px; padding: 2px 4px; border-radius: 3px; outline: none; cursor: default;	float: left; opacity: 0.4"
        ),
        "various 1"
      );

      assert.areSame(
        "color:red;filter:alpha(opacity = 70);width:10px;",
        n("color: red; filter: alpha(opacity = 70); width: 10px"),
        "filter"
      );

      assert.areSame(
        "color:red;width:bold;",
        n("COLOR: red; WIDTH: bold"),
        "upper case"
      );

      assert.areSame(
        "color:rgba(255, 35, 35, 0.4);top:0;",
        n("color: rgba(255, 35, 35, 0.4); top: 0"),
        "rgba"
      );

      assert.areSame(
        "background-image:url('http://cke.com/g.gif?hello=1&bye=0');display:none;",
        n(
          "background-image: url('http://cke.com/g.gif?hello=1&bye=0'); display: none;"
        ),
    ...
    testNormalizeCssText3: function () {
      ...
      assert.areSame(
        "color:red;float:left;margin:0.5em;width:10px;",
        n("color: red; width: 10px; margin: 0.5em; float: left", true),
        "various"
      );
    },

    testQuoteEntity: assertNormalizeCssText(
      'font-family:"foo";',
      "font-family: &quot;foo&quot;;",
      ""
    ),
    ...
    "test Normalize double quote": assertNormalizeCssText(
      'font-family:"crazy font";',
      'font-family: "crazy font";',
      "quoted font name"
    ),
    "test Normalize single quote": assertNormalizeCssText(
      "font-family:'crazy font';",
      "font-family: 'crazy font';",
      "single-quoted font name"
    ),

    "test Normalize generic family name serif": assertNormalizeCssText(
      "font-family:serif;",
      "font-family: serif;",
      "generic-family name is not escaped"
    ),
    "test Normalize generic family name sans-serif": assertNormalizeCssText(
      "font-family:sans-serif;",
      "font-family: sans-serif;",
      "generic-family name is not escaped"
    ),
    "test Normalize generic family name cursive": assertNormalizeCssText(
      "font-family:cursive;",
      "font-family: cursive;",
      "generic-family name is not escaped"
    ),
    "test Normalize generic family name fantasy": assertNormalizeCssText(
      "font-family:fantasy;",
      "font-family: fantasy;",
      "generic-family name is not escaped"
    ),
    "test Normalize generic family name monospace": assertNormalizeCssText(
      "font-family:monospace;",
      "font-family: monospace;",
      "generic-family name is not escaped"
    ),

    "test Normalize generic and non-generic mix": assertNormalizeCssText(
      'font-family:"foo",serif;',
      'font-family: "foo", serif;',
      "family-name and generic-family mix"
    ),
    "test Normalize letter casing sensitivity": assertNormalizeCssText(
      'font-family:"FFo baR";',
      'font-family: "FFo baR";',
    ...
    "test Normalize generic-family token as family-name":
      assertNormalizeCssText(
        'font-family:"serif";',
        'font-family:"serif";',
        "accept generic-family token as family-name"
      ),
    "test Normalize unquoted family name with hyphen": assertNormalizeCssText(
      "font-family:my-cool-font;",
      "font-family:my-cool-font;",
      "unquoted family name with hyphen"
    ),
    "test Normalize font name with multiple spaces": assertNormalizeCssText(
      'font-family:"Space    font";',
      'font-family:"Space    font";',
      "font name with multiple spaces"
    ),

    "test Normalize family name with quotes": assertNormalizeCssText(
      "font-family:\"'Sarcasm'\";",
      "font-family:\"'Sarcasm'\";",
      "family name with quotes"
    ),
    "test Normalize family name with special characters":
      assertNormalizeCssText(
        "font-family:\"'This is   -!$   custom Font'\";",
        "font-family:\"'This is   -!$   custom Font'\";",
        "family name with special characters"
      ),

    // If there's a syntax error in the style - just leave it like that.
    "test Normalize syntax error": assertNormalizeCssText(
      'font-family:"crazy font",;',
      'font-family:"crazy font",;',
      "style syntax error"
    ),
    ...
    testConvertRgbToHex: function () {
      ...
      assert.areSame(
        "color:#010203; border-color:#ffff00;",
        c("color:rgb(1,2,3); border-color:rgb(255,255,0);"),
        "multiple"
      );
    },
    ...
    "test convertArrayToObject": function () {
      ...
      arrayAssert.itemsAreEqual(
        ["foo", "bar"],
        CKEDITOR.tools.object.keys(obj)
      );
      ...
      arrayAssert.itemsAreEqual(
        [],
        CKEDITOR.tools.object.keys(CKEDITOR.tools.convertArrayToObject({}))
      );
    },

    "test eventsBuffer": function () {
      assert.isTrue(
        CKEDITOR.tools.eventsBuffer(200, function () {}) instanceof
    ...
    "test buffers.event context": function () {
      ...
      assert.areSame(
        ...
        "callback was executed with the right context"
      );
    },
    ...
    "test throttle": function () {
      assert.isTrue(
        CKEDITOR.tools.throttle(200, function () {}) instanceof
    ...
    "test buffers.throttle": function () {
      ...
      assert.isTrue(
        inputSpy.calledWithExactly(foo),
        "Call argument after the first call"
      );
      ...
      assert.isTrue(
        inputSpy.calledWithExactly(foo),
        "Call argument the after second call"
      );
      ...
      wait(function () {
        assert.areSame(
          1,
          inputSpy.callCount,
          "Call count after the second call timeout (1st)"
        );
        assert.isTrue(
          inputSpy.calledWithExactly(foo),
          "Call argument after the second call timeout (1st)"
        );
        ...
        wait(function () {
          assert.areSame(
            2,
            inputSpy.callCount,
            "Call count after the second call timeout (2nd)"
          );
          assert.isTrue(
            inputSpy.getCall(1).calledWithExactly(baz),
            "Call argument after the second call timeout (2nd)"
          );
          ...
          wait(function () {
            assert.areSame(
              3,
              inputSpy.callCount,
              "Call count after the third call"
            );
            assert.isTrue(
              inputSpy.getCall(2).calledWithExactly(foo),
              "Call argument after the third call"
            );
            ...
            wait(function () {
              ...
              assert.areSame(
                3,
                inputSpy.callCount,
                "Call count after the fourth call"
              );
              ...
              wait(function () {
                assert.areSame(
                  4,
                  inputSpy.callCount,
                  "Call count after the fourth call timeout"
                );
                assert.isTrue(
                  inputSpy.getCall(3).calledWithExactly(baz),
                  "Call argument after the fourth call timeout"
                );
              }, 140);
            }, 70);
          }, 210);
    ...
    "test getUniqueId": function () {
      ...
      assert.isMatching(
        /[a-z]/,
        uuid[0],
        "First character of UUID should be z letter."
      );
    ...
    "test getCookie": function () {
      ...
      assert.areSame(
        CKEDITOR.tools.getCookie(name),
        value,
        "getCookie returns proper cookie"
      );
    },
    ...
    "test getCsrfToken": function () {
      ...
      assert.isMatching(
        "ckCsrfToken=" + token,
        document.cookie,
        "getCsrfToken sets proper cookie"
      );
      ...
      assert.areEqual(
        ...
        "getCsrfToken returns token from cookie"
      );
    },
    ...
    "test hexstring to bytes converter": function () {
      ...
      CKEDITOR.tools.array.forEach(testCases, function (test) {
        arrayAssert.itemsAreEqual(
          test.bytes,
          CKEDITOR.tools.convertHexStringToBytes(test.hex)
        );
      });
    },
    ...
    "test bytes to base64 converter": function () {
      var testCases = [
        ...
        },
        {
          // jscs:disable
          bytes: [
            0, 16, 131, 16, 81, 135, 32, 146, 139, 48, 211, 143, 65, 20, 147,
            81, 85, 151, 97, 150, 155, 113, 215, 159, 130, 24, 163, 146, 89,
            167, 162, 154, 171, 178, 219, 175, 195, 28, 179, 211, 93, 183, 227,
            158, 187, 243, 223, 191,
          ],
          // jscs:enable
          base64:
      ...
      CKEDITOR.tools.array.forEach(testCases, function (test) {
        assert.areSame(
          test.base64,
          CKEDITOR.tools.convertBytesToBase64(test.bytes)
        );
      });
    },
    ...
    "test convertToPx": function () {
      ...
      CKEDITOR.tools.array.forEach(conversionArray, function (item) {
        assert.areSame(
          item.output,
          CKEDITOR.tools.convertToPx(item.input),
          "Value " + item.input + " should be converted to " + item.output
        );
      });
    },
  });
