...
(function () {
  ...
  bender.test({
    ...
    "test addCustomHandler returns class inheriting from CKEDITOR.style":
      function () {
        ...
        assert.isInstanceOf(
          CKEDITOR.style,
          style,
          "returned class inherits from CKEDITOR.style"
        );
        assert.isInstanceOf(
          styleClass,
          style,
          "returned constructor creates proper instances"
        );
        assert.areSame("testAdd2", style.type, "style instance has a type");
        assert.areSame(
          1,
          styleClass.prototype.foo,
          "prototype of style class contains properties defined in custom handler"
        );
        assert.areSame(
          2,
          style.apply,
          "existing CKEDITOR.style's method has been overriden"
        );
      },
    ...
    "test CKEDITOR.style constructor returns custom styles if styleDefinition.type is set":
      function () {
    ...
    "test setup": function () {
        ...
        styleClass = CKEDITOR.style.addCustomHandler({
          ...
          setup: function (definition) {
            ...
            assert.isInstanceOf(
              styleClass,
              this,
              "setup executed in style class context"
            );
          },
        });
      ...
      assert.areSame(
        "a",
        setupCalled.join(","),
        "setup called when instantiating style directly"
      );
      ...
      assert.areSame(
        "a,b",
        setupCalled.join(","),
        "setup called when instantiating style with CKEDITOR.style"
      );
    },
    ...
    "test assignedTo": function () {
      ...
      assert.areSame(
        ...
        style1a.assignedTo,
        "default - custom class constructor"
      );
      assert.areSame(
        CKEDITOR.STYLE_OBJECT,
        style1b.assignedTo,
        "default - CKEDITOR.style constructor"
      );
      assert.areSame(
        CKEDITOR.STYLE_INLINE,
        style2a.assignedTo,
        "specified - custom class constructor"
      );
      assert.areSame(
        CKEDITOR.STYLE_INLINE,
        style2b.assignedTo,
        "specified - CKEDITOR.style constructor"
      );
    },

    "test editor#applyStyle and editor#removeStyle execute custom style methods":
      function () {
        var applied = 0,
          removed = 0;
