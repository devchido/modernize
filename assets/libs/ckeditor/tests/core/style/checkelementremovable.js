...
bender.test({
  ...
  test_checkElementRemovable5: function () {
    var element = CKEDITOR.dom.element.createFromHtml(
      '<span lang="pt" style="color : #fff">Test</span>',
      doc
    );
    playground.append(element);
  ...
  test_checkElementRemovable8: function () {
    var element = CKEDITOR.dom.element.createFromHtml(
      '<span lang="pt" style="font-size: 10px">Test</span>',
      doc
    );
    playground.append(element);
    ...
    var style = new CKEDITOR.style({
      element: "span",
      attributes: { lang: "pt", style: "font-size:10px;" },
    });
  ...
  test_checkElementRemovable_fontFamily: function () {
    var element = CKEDITOR.dom.element.createFromHtml(
      '<span style="font-family: Georgia,  serif">Test Font</span>',
      doc
    );
    playground.append(element);
