...
(function () {
  ...
  function assertCheckApplicable(
    containerName,
    definitionOrStyle,
    expected,
    htmlWithRange,
    ...
    if (start.type != CKEDITOR.NODE_ELEMENT) start = start.getParent();

    var applicable = getStyle(definitionOrStyle).checkApplicable(
      new CKEDITOR.dom.elementPath(start, container),
      filter
    );
  ...
  function createAssertionFunction(tcs, tcsGroupName, definitionOrStyle) {
    ...
    return function (expected, containerName, htmlWithRange, filterRules, msg) {
      ...
      if (filterRules) filter = new CKEDITOR.filter(filterRules);

      if (tcs[name])
        throw new Error('Test named "' + name + '" already exists');

      tcs[name] = function () {
        assertCheckApplicable(
          containerName,
          style,
          expected,
          htmlWithRange,
  ...
  a = createAssertionFunction(tcs, "test filter", {
    element: "p",
  ...
  CKEDITOR.tools.extend(tcs, {
    ...
    "test checkApplicable arguments (filter vs editor) discovery": function () {
      ...
      assert.isFalse(
        style.checkApplicable(editor.elementPath(), editor, filter),
        "style is not applicable when filter does not allow it"
      );
      assert.isFalse(
        style.checkApplicable(editor.elementPath(), filter),
