...
bender.test({
  ...
  "test getCommandKeystroke": function () {
    assert.isNull(
      this.editor.getCommandKeystroke("command_without_keystrokes"),
      "Command without keystroke"
    );
    assert.areEqual(
      75,
      this.editor.getCommandKeystroke("command_with_keystrokes"),
      "Command with keystroke."
    );
  },
  ...
  "test getCommandKeystroke multiple keystrokes": function () {
    arrayAssert.isEmpty(
      this.editor.getCommandKeystroke("command_without_keystrokes", true),
      "Command without keystrokes."
    );
    arrayAssert.itemsAreEqual(
      [75, 76],
      this.editor.getCommandKeystroke("command_with_keystrokes", true),
      "Command with keystrokes."
    );
  },
  ...
  "test getCommandKeystroke in commands with fake keystrokes": function () {
    assert.areSame(
      77,
      this.editor.getCommandKeystroke("command_with_fake_keystrokes"),
      "Single keystroke result."
    );

    // (#2493)
    var ret = this.editor.getCommandKeystroke(
      "command_with_fake_keystrokes",
      true
    );
  ...
  "test keystroke assignment": function () {
    ...
    assert.areEqual(
      keyCombo1,
      keystroke,
      "Keystrokes should be equal (command)."
    );
    ...
    assert.areEqual(
      keyCombo1,
      keystroke,
      "Keystrokes should be equal (command name)."
    );
  },
  ...
  "test editor#key event": function () {
    ...
    assert.areSame(
      CKEDITOR.CTRL + CKEDITOR.SHIFT + 66,
      evtData.keyCode,
      "keyCode"
    );
