...
bender.test({
      ...
      on: {
        configLoaded: function (event) {
          tc.resume(function () {
            ...
            // All other settings must match CKEDITOR.config.
            for (var prop in CKEDITOR.config) {
              if (!Object.hasOwnProperty(CKEDITOR.config, prop)) continue;
  ...
  "test getData/setData() - events and arguments": function () {
      ...
      return function (evt) {
        ...
        if (value)
          assert.areSame(
            value,
            evt.data.dataValue,
            "check data on event: " + evt.name
          );
        // Alter the data value.
        if (evt.name == "setData") evt.data.dataValue = "bar";
      };
    }
    ...
    function callSetData(data, params, legacyInterface) {
      ...
      else editor.setData(data, params);
    }
    ...
    for (i = 0; i <= 1; i++) {
      var useOldAPI = Boolean(i),
        ...
        interfaceName = useOldAPI
          ? "old API params inline"
          : "new API params as object";
      ...
      assert.areSame(
        "foo",
        editor.getData(true),
        "setData internally - " + interfaceName
      );
      ...
      assert.areSame(
        "saveSnapshot,setData,afterSetData",
        events.join(","),
      ...
      assert.areSame(
        "bar",
        editor.getData(),
        "setData listener change data value - " + interfaceName
      );
    ...
    assert.areSame(
      "setData,afterSetData",
      events.join(","),
      "Invalid events with params.noSnapshot = true"
    );
  },
  ...
  assertUpdatingElement: function (bot) {
    ...
    function getHtml(element) {
      return element.is("textarea")
        ? element.getValue()
        : bender.tools.compatHtml(element.getHtml());
    }
  },
  ...
  "test editor.activeEnterMode": function () {
    ...
    assert.areSame(
      ...
      "initial activeEnterMode"
    );
    assert.areSame(
      CKEDITOR.ENTER_BR,
      editor.activeShiftEnterMode,
      "initial activeShiftEnterMode"
    );
    ...
    assert.areSame(
      CKEDITOR.ENTER_BR,
      editor.activeShiftEnterMode,
      "1st activeShiftEnterMode"
    );
    assert.areSame(
      1,
      activeEnterModeChanged,
      "activeEnterModeChange was fired once"
    );
    ...
    assert.areSame(
      CKEDITOR.ENTER_DIV,
      editor.activeEnterMode,
      "2nd activeEnterMode - no change"
    );
    assert.areSame(
      CKEDITOR.ENTER_P,
      editor.activeShiftEnterMode,
      "2nd activeShiftEnterMode - no change"
    );
    assert.areSame(
      2,
      activeEnterModeChanged,
      "activeEnterModeChange was not fired"
    );
    ...
    assert.areSame(
      CKEDITOR.ENTER_P,
      editor.activeEnterMode,
      "3rd activeEnterMode - reseted"
    );
    assert.areSame(
      CKEDITOR.ENTER_DIV,
      editor.activeShiftEnterMode,
      "3rd activeShiftEnterMode"
    );
    assert.areSame(
      3,
      activeEnterModeChanged,
  ...
  "test insertHtml": function () {
    ...
    assert.areSame(
      "foo",
      insertHtmlEventListener.firstCall.args[0].data.dataValue,
      "event dataValue"
    );
    assert.areSame(
      "html",
      insertHtmlEventListener.firstCall.args[0].data.mode,
      "event mode"
    );
    assert.areSame(
      range,
      insertHtmlEventListener.firstCall.args[0].data.range,
      "event range"
    );
