...
(function () {
  ...
  bender.test({
    ...
    "test getSnapshot returns empty string after editor destroyed":
      function () {
        bender.editorBot.create({}, function (bot) {
          this.wait(function () {
    ...
    "test destroy editor before it is fully initialized": function () {
      ...
      wait(function () {
        ...
        assert.isTrue(
          warnStub.calledOnce,
          "CKEDITOR.warn should be called once."
        );
        assert.areEqual(
  ...
  function countFilters(editor) {
    ...
    return editor
      ? CKEDITOR.tools.array.filter(filters, function (filter) {
