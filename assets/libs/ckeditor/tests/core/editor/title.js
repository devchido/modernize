...
(function () {
  ...
  function assertTitle(expected, editor, msg) {
    assert.areSame(
      expected,
      bender.editors[editor].title,
      msg ||
        "editor.title of " +
          bender.editors[editor].name +
          " based on editor.config"
    );
  }

  function assertTitleInherited(editor) {
    assert.isTrue(
      !!~bender.editors[editor].title.indexOf(bender.editors[editor].name),
      "editor.title of " + editor.name + " based on editor.name"
    );
  }
  ...
  function assertTitleSetOnEditable(editor) {
    ...
    if (editor.title === false || editor.title === "") {
      if (element.data("startup-title"))
        assert.areSame(
          element.data("startup-title"),
          element.getAttribute("title"),
          "Startup title of " + editor.name + " preserved"
        );
      else
        assert.isFalse(
          element.hasAttribute("title"),
          "Title attribute set on editable of " + editor.name
        );
    } else {
      assert.isTrue(
        !!~element.getAttribute("title").indexOf(editor.title),
        "editor.title used as an attribute of editable of " + editor.name
      );
    }
  }
  ...
  bender.test({
    ...
    "test config.title implies editor.title": function () {
      ...
      assertTitle(
        false,
        "existing3",
        "The original title of the element remains untouched."
      );
    },
    ...
    "test editor.title transferred to editable element": function () {
      for (var i in this.editors) assertTitleSetOnEditable(this.editors[i]);
    },
        ...
        bender.editors[name].once("destroy", function () {
          resume(function () {
            assert.areSame(
              tcs[name],
              element.getAttribute("title"),
              "Original title correctly restored once instance " +
                name +
                " destroyed."
            );
