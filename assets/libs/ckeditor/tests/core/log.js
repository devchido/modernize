...
(function () {
  ...
  bender.test({
    ...
    _should: {
      ignore: {
        "no log event and output after CKEDITOR.warn() when verbosity = 0":
          ignore,
        "log event and output from CKEDITOR.warn() when verbosity = VERBOSITY_WARN":
          ignore,
        "log event and output from CKEDITOR.warn() when verbosity = VERBOSITY_WARN | VERBOSITY_ERROR":
          ignore,
        "no log event and output after CKEDITOR.error() when verbosity = 0":
          ignore,
        "log event and output from CKEDITOR.error() when verbosity = VERBOSITY_ERROR":
          ignore,
        "log event and output from CKEDITOR.error() when verbosity = VERBOSITY_WARN | VERBOSITY_ERROR":
          ignore,
    ...
    "is defined": function () {
      assert.isFunction(
        CKEDITOR.error,
        "CKEDTIOR.error function should be defined."
      );
      assert.isFunction(
        CKEDITOR.warn,
        "CKEDTIOR.warn function should be defined."
      );
    },
    ...
    "no log event and output after CKEDITOR.warn() when verbosity = 0":
      function () {
        ...
        assert.isFalse(
          warnStub.called,
          "Console.warn function should not be called."
        );
    ...
    "log event and output from CKEDITOR.warn() when verbosity = VERBOSITY_WARN":
      function () {
        ...
        assert.isTrue(
          logEventSpy.calledOnce,
          "Log event should be fired once."
        );
        data = logEventSpy.firstCall.args[0].data;
        assert.areEqual(
          "warn",
          data.type,
          'Event data.type should be equal to "warn".'
        );
        assert.areEqual(
          errorCode,
          data.errorCode,
          "Event data.errorCode should match provided errorCode."
        );
        assert.areEqual(
          additionalData,
          data.additionalData,
          "Event data.additionalData should match provided additionalData."
        );
        ...
        assert.isTrue(
          warnStub.calledTwice,
          "Console.warn function should be called twice."
        );
        assert.isTrue(
          warnStub.firstCall.calledWith(
            errorPrefix + errorCodeLabel + errorCode + ".",
            additionalData
          ),
          "Console.warn should be called with errorCode and additionalData."
        );
    ...
    "no log event and output after CKEDITOR.error() when verbosity = 0":
      function () {
        ...
        assert.isFalse(
          errorStub.called,
          "Console.error function should not be called."
        );
    ...
    "log event and output from CKEDITOR.error() when verbosity = VERBOSITY_ERROR":
      function () {
        ...
        assert.areEqual(
          "error",
          data.type,
          'Event data.type should be equal to "error".'
        );
        assert.areEqual(
          errorCode,
          data.errorCode,
        ...
        // - second call is to show link to the documentation providing more information about errorCode.
        assert.isTrue(
          errorStub.calledTwice,
          "Console.error function should be called twice."
        );
        assert.isTrue(
          errorStub.firstCall.calledWith(
            errorPrefix + errorCodeLabel + errorCode + ".",
            additionalData
          ),
          "Console.error should be called with errorCode and additionalData."
        );
    ...
    "block default log event handler": function () {
      ...
      assert.isFalse(
        errorStub.called,
        'Console.error should not be called when default "log" event handler is blocked.'
      );
      assert.isFalse(
        warnStub.called,
        'Console.warn should not be called when default "log" event handler is blocked.'
      );
