...
bender.test({
  assertFocus: function (truly) {
    ...
    assert[truly === false ? "isFalse" : "isTrue"](
      fm.hasFocus,
      "check focusManager.hasFocus"
    );
    assert[truly === false ? "isFalse" : "isTrue"](
      ed.container.hasClass("cke_focus"),
      " editor container receives focused class name."
    );
  },
  ...
  "test inheriting the initial focus": function () {
    ...
    editor.on("instanceReady", function () {
      resume(function () {
        ...
        assert.areSame(
          editor.editable(),
          editor.focusManager.currentActive,
          "currentActive after init"
        );
