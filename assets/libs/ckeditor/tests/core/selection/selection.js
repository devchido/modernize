...
bender.test({
  ...
  "test contructor": function () {
    ...
    assert.isTrue(
      sel2.rev > sel1.rev,
      "sel2's revision is greater than sel1's"
    );
  ...
  "test ranges manipulation": function () {
    ...
    testSelection(
      "<ul><li>bullet line 1</li><li>bullet line 2</li></ul>^second line"
    );
    ...
    // Table selection.
    testSelection(
      "<div><table><tr><td>[foo</td><td>bar]</td></tr></table></div>"
    );
    testSelection(
      "<div><table><tr><td>[foo</td></tr><tr><td>bar]</td></tr></table></div>"
    );
  ...
  "test selectRanges - range containing table cell": function () {
    var editor = this.editor,
      range = bender.tools.range.setWithHtml(
        editor.editable(),
        "<table><tr><td>x</td>[<td>foo</td>]<td>x</td></tr></table>"
      );
    ...
    assert.areSame(
      "foo",
      CKEDITOR.tools.trim(editor.getSelection().getSelectedText())
    );
  },
  ...
  "test selectRanges - range containing empty table cell": function () {
    var editor = this.editor,
      data =
        '<table><tbody><tr><td>x</td><td id="cell"></td><td>x</td></tr></tbody></table>';
    ...
    this.editorBot.setData(data, function () {
      ...
      if (
        ...
        assert.isTrue(
          cell.equals(selected) || cell.contains(selected),
          "selection is inside the cell"
        );
      } else {
  ...
  "test selectRanges - after empty inline element": function () {
    ...
    assert.areSame(
      strong,
      editor.getSelection().getRanges()[0].getPreviousNode(isElement),
      "the selection was located after the strong element"
    );
  },

  "test selectRanges - after empty inline element with the filler char":
    function () {
      // IE8 can't handle this selection.
      if (CKEDITOR.env.ie && CKEDITOR.env.version == 8) {
        assert.ignore();
      ...
      assert.areSame(
        "strong",
        editor.getSelection().getStartElement().getName(),
  ...
  "test selectRanges - after empty inline element with an empty text node":
    function () {
      // IE8 can't handle this selection.
      if (CKEDITOR.env.ie && CKEDITOR.env.version == 8) {
        assert.ignore();
  ...
  "test selectRanges - inside empty inline element": function () {
    ...
    editor
      .editable()
      .setHtml(
        '<p>x<span style="font-size:48px"><strong id="target"></strong></span>x</p>'
      );
  ...
  "test getSelectedElement does not modify ranges": function () {
    ...
    this.editorBot.setData("<div>foo</div><p>bar</p>", function () {
      ...
      assert.isTrue(
        elDiv.contains(range.startContainer) ||
          elDiv.equals(range.startContainer),
      ...
      assert.isTrue(
        range.startContainer.equals(range2.startContainer) &&
          range.startOffset == range2.startOffset,
  ...
  "test getRanges(true) does not modify cached ranges": function () {
    ...
    bender.tools.setHtmlWithSelection(
      editor,
      '<div>te[xt<span contenteditable="false">foo</span>te]xt</div>'
    );
    ...
    assert.areNotSame(
      editableRanges,
      allRanges,
      "both method calls returned different arrays"
    );
    assert.areSame(
      2,
      editableRanges.length,
      "2 editable ranges returned by getRanges( true )"
    );
    assert.areSame(1, allRanges.length, "only 1 range returned by getRanges()");
  },

  // https://dev.ckeditor.com/ticket/11493
  "test getRanges(true) called after getRanges() does not modify cached ranges":
    function () {
      var editor = this.editor;
      ...
      assert.areSame(
        1,
        allRanges.length,
        "only 1 range returned by getRanges()"
      );
    },
  ...
  "test lock and unlock": function () {
    ...
    var sourceRange = makeSelection(
      '<strong id="start">foo</strong>bar[<img />]'
    )[0];
    var sel = doc.getSelection(),
      initialRev = sel.rev;
    ...
    assert.isTrue(
      checkRangeEqual(resultRange, sourceRange),
      "get ranges result from locked selection doesn't match the original."
    );
    assert.isTrue(
      sel.getStartElement().is("img"),
      "start element result from locked selection doesn't match the original."
    );
    assert.isTrue(
      sel.getSelectedElement().is("img"),
      "selected element result from locked selection doesn't match the original."
    );
    assert.areSame(
      initialRev,
      sel.rev,
      "selection's rev has not been modified"
    );
    ...
    assert.isTrue(
      checkRangeEqual(resultRange, newRange),
      "get ranges result from locked selection doesn't match the original."
    );
    assert.isTrue(
      sel.getStartElement().is("strong"),
      "start element result from locked selection doesn't match the original."
    );
  ...
  "test selectRanges after locked": function () {
    ...
    assert.areSame(
      ...
      "focus should remains in the text input"
    );
    ...
    assert.areSame(
      el,
      sel.getStartElement(),
      "start element of locked should match"
    );
    assert.areSame(
      CKEDITOR.SELECTION_TEXT,
      sel.getType(),
      "selection type of locked should match"
    );
    assert.areSame(
      "foo",
      CKEDITOR.tools.trim(sel.getSelectedText()),
      "selected text of locked should match"
    );
    assert.isTrue(
      sel.rev > initialRev,
      "New revision after selectRanges on locked selection"
    );
    ...
    checkRangeEqual(
      domRange,
      newRange,
      "actual selection range should be the same"
    );
  },
  ...
  "test removeAllRanges is limited to its root": function () {
    ...
    assert.areSame(
      "foo",
      new CKEDITOR.dom.selection(editable1).getSelectedText(),
  ...
  "selection scrolls into view": function () {
    ...
    editable.setHtml(
      "<p>" + linebreaks + 'foo<span id="scroll_marker">bar</span></p>'
    );
  ...
  "test cloning - standalone selection": function () {
    ...
    assert.areNotSame(
      sel.getSelectedText(),
      clone.getSelectedText(),
      "getSelectedText()"
    );
    assert.areSame(
      initialRev,
      clone.rev,
      "Clone's revision has not been modified"
    );
  },
  ...
  "test remove filling char sequence on keydown blur": function () {
    ...
    this.editor.document.fire(
      "keydown",
      new CKEDITOR.dom.event({ keyCode: 46 })
    );
  ...
  "test keys with readonly editor": function () {
    ...
    editable.fire(
      "keydown",
      new CKEDITOR.dom.event({ keyCode: CKEDITOR.CTRL })
    );
