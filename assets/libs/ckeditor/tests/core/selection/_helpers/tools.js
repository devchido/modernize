...
function testSelection(markup) {
  ...
  // Make the selection.
  for (
    i = 0, range, length = sourceRanges.length;
    (range = sourceRanges[i]), i < length;
    i++
  ) {
  ...
  var sel = doc.getSelection(),
    readRanges = sel.getRanges();
  for (i = 0, length = readRanges.length; i < length; i++)
    assert.isTrue(
      checkRangeEqual(readRanges[i], sourceRanges[i]),
      "get ranges result doesn't match original on selection: " + markup
    );
  ...
  for (i = 0; i < domSel.rangeCount; i++)
    assert.isTrue(
      checkRangeEqual(sourceRanges[i], madeRanges[i]),
      "select ranges result doesn't match original on selection: " + markup
    );
}
...
function testSelectedElement(markup, tag) {
  ...
  assert.isTrue(
    selected && selected.is(tag),
    "selected element doesn't match on selection: " + markup
  );
}
...
function testSelectedText(markup, text) {
  ...
  assert.areSame(
    text,
    selectedText,
    "selected text doesn't match on selection: " + markup
  );
}
...
function testStartElement(markup, tag) {
  ...
  assert.isTrue(
    startElement.is(tag),
    "start element doesn't match on selection: " + markup
  );
}
...
function convertRange(range) {
  ...
  if (range instanceof CKEDITOR.dom.range) {
    ...
    rng = new CKEDITOR.dom.range(
      new CKEDITOR.dom.document(range.getDocument())
    );
    rng.setStart(
      new CKEDITOR.dom.node(range.startContainer),
      range.startOffset
    );
...
function checkRangeEqual(one, theOther) {
  ...
  var equals = true,
    ...
    walkerRange = new CKEDITOR.dom.range(
      one instanceof CKEDITOR.dom.range
        ? one.document
  ...
  if (start) {
    walkerRange.setStart(
      $(start < 0 ? one.startContainer : theOther.startContainer),
  ...
  if (equals && !(one.collapsed && theOther.collapsed)) {
    ...
    if (end) {
      walkerRange.setStart(
        $(end < 0 ? one.endContainer : theOther.endContainer),
...
function checkSelection(
  ...
  type !== false &&
    assert.areSame(type, this.getType(), "check selection type failed");
  startElement !== false &&
    assert.areSame(
      startElement,
      this.getStartElement(),
      "check selection start element failed"
    );
  selectedElement !== false &&
    assert.areSame(
      selectedElement,
      this.getSelectedElement(),
      "check selection selected element failed"
    );
  selectedText !== false &&
    assert.areSame(
      selectedText,
      this.getSelectedText(),
      "check selection selected text failed"
    );
  ...
  if (ranges.length) {
    for (var i = 0; i < selRanges.length; i++)
      assert.isTrue(
        checkRangeEqual(ranges[i], selRanges[i]),
        "check selection range failed at position:" + i
      );
...
function assertSelectionsAreEqual(sel1, sel2) {
  checkSelection.call(
    sel1,
    sel2.getType(),
    sel2.getStartElement(),
    sel2.getSelectedElement(),
    sel2.getSelectedText(),
    sel2.getRanges()
  );
}
