...
function createTestKeyNavFn(editor, target) {
  ...
  return function (key, rangeOrEl, pos, msg) {
    ...
    editor
      .editable()
      .fire("keydown", getKeyEvent(keyCodes[key], preventDefaultCallback));
...
bender.test({
  ...
  "Make fake-selection": function () {
    ...
    bender.tools.setHtmlWithSelection(
      editor,
      '<p>[foo <span id="bar">bar</span>]</p>'
    );
    ...
    assert.areSame(
      span,
      sel.getStartElement(),
      "getStartElement() must return the fake-selected element"
    );
    assert.areSame(
      span,
      sel.getSelectedElement(),
      "getSelectedElement() must return the fake-selected element"
    );
    assert.areSame(
      CKEDITOR.SELECTION_ELEMENT,
      sel.getType(),
      "getType() must be SELECTION_ELEMENT"
    );
    ...
    assert.isTrue(
      editor.editable() === range.root,
      "range.root is set to editable (the same class, not only DOM element)"
    );
    assert.areSame(
      span,
      range.getEnclosedNode(),
      "range.getEnclosedNode() returns the fake-selected element"
    );

    assert.areSame(
      span.getParent(),
      sel.getCommonAncestor(),
      "getCommonAncestor() must return the parent <p>"
    );
  ...
  "Reset fake-selection": function () {
    ...
    assert.isFalse(
      !!span.equals(sel.getStartElement()),
      "getStartElement() must not be the fake-selected"
    );
    assert.isFalse(
      !!span.equals(sel.getSelectedElement()),
      "getSelectedElement() must not be the fake-selected"
    );
  ...
  "Fire selectionchange event": function () {
    ...
    wait(function () {
      ...
      assert.areSame(
        span,
        selectedElement,
        "getSelectedElement() must be the fake-selected"
      );
    }, 50);
  },
  ...
  "Change selection": function () {
    ...
    bender.tools.setHtmlWithSelection(
      editor,
      '<p>[foo <span id="bar">bar</span></p><p id="bom">bom]</p>'
    );
    ...
    wait(function () {
      ...
      wait(function () {
        assert.areSame(
          1,
          selectionChange,
          "selectionChange was fired only once"
        );
  ...
  "Hiding selection": function () {
    ...
    bender.tools.setHtmlWithSelection(
      editor,
      '<p>[foo <span id="bar">bar</span> <span id="bom">bom</span>]</p>'
    );

    editor.getSelection().fake(editor.document.getById("bar"));
    assert.isTrue(
      editor.getSelection(1).isHidden(),
      "Real selection is placed in hidden element"
    );
    ...
    assert.areSame(
      1,
      countHiddenContainers(editor.document),
      "There is only one hidden selection container"
    );
    ...
    assert.isFalse(
      editor.getSelection(1).isHidden(),
      "Real selection is not placed in hidden element"
    );
    assert.isFalse(
      !!countHiddenContainers(editor.document),
      "There sould be no hidden selection container"
    );
  },
  ...
  "Hiding selection encodes label": function () {
    ...
    bender.tools.setHtmlWithSelection(
      editor,
      '<p>[<span id="bar">bar</span>]</p>'
    );

    editor.getSelection().fake(editor.document.getById("bar"), "<i>foo</i>");
    assert.areEqual(
      "&lt;i&gt;foo&lt;/i&gt;",
      editor.editable().findOne("div[data-cke-hidden-sel]").getHtml()
    );
  },
  ...
  "Clean up on setData in framed editor": function () {
    ...
    this.editorBot.setData("<p>foo!</p>", function () {
      ...
      assert.isFalse(
        !!editor._.hiddenSelectionContainer,
      ...
      assert.isTrue(
        !!editor
          .editable()
          .fire("keydown", new CKEDITOR.dom.event({ keyCode: 37 })),
        "Key 37 should not be blocked"
      );
    });
  },
  ...
  "Fake-selection bookmark mark as not faked when no enclosed node found. (https://dev.ckeditor.com/ticket/13280)":
    function () {
  ...
  "Fake-selection does not create undo snapshots": function () {
    ...
    assert.areSame(
      ...
      editor.getCommand("undo").state,
      "Not undoable after making fake selection"
    );
    assert.areSame(
      1,
      countHiddenContainers(editor.document),
    ...
    assert.areSame(
      CKEDITOR.TRISTATE_DISABLED,
      editor.getCommand("undo").state,
      "Not undoable after removing fake selection"
    );
    assert.areSame(
      0,
      countHiddenContainers(editor.document),
  ...
  "Fake-selection undo": function () {
    ...
    bender.tools.setHtmlWithSelection(
      editor,
      '<p>[foo <span id="bar">bar</span> bom]</p>'
    );
  ...
  "Fake-selection moved between elements": function () {
    ...
    assert.isTrue(
      ...
      "check whether editor returns faked selection too"
    );
  },
      ...
      editor.setMode("source", function () {
        resume(function () {
          ...
          assert.areSame(
            0,
            selectionChange,
            "no selectionChange when switching to source mode"
          );
    ...
    bender.editorBot.create(
      {
        ...
        startupData:
          '<p>foo<span contenteditable="false" id="el1">bar</span></p><div contenteditable="false" id="el2">bom</div>',
        config: {
  ...
  "Test leaving fake selection by arrow keys": function () {
    ...
    this.editorBot.setData(
      '<p>foo</p><p id="start">X</p><p>bar</p>',
      function () {
  ...
  "Test staying in selected non-editable element by arrow keys if there is not editing place by its side":
    function () {
      var editor = this.editor,
        prevented = 0;

      this.editorBot.setData(
        '<p id="start" contenteditable="false">foo</p>',
        function () {
          var ps = editor.document.getById("start"),
            editable = editor.editable();
  ...
  "Test moving to sibling non-editable blocks by arrow keys": function () {
    ...
    this.editorBot.setData(
      '<p>X</p><p id="p1" contenteditable="false">X</p><p id="p2" contenteditable="false">X</p><p id="p3" contenteditable="false">X</p>',
      function () {
  ...
  "Test deleting selected element (del)": function () {
    ...
    this.editorBot.setData(
      '<p>X</p><p id="start" contenteditable="false">foo</p><p>bar</p>',
      function () {
        var ps = editor.document.getById("start"),
          editable = editor.editable(),
          prevented;
  ...
  "Test deleting selected element (backspace)": function () {
    ...
    this.editorBot.setData(
      '<p>bar</p><p id="start" contenteditable="false">foo</p><p>X</p>',
      function () {
        var ps = editor.document.getById("start"),
          editable = editor.editable(),
          prevented;
  ...
  "Test deleting selected inline element (del)": function () {
    ...
    this.editorBot.setData(
      '<p>bar<span id="start">foo</span>bom</p>',
      function () {
        var ps = editor.document.getById("start"),
          editable = editor.editable(),
          prevented;
  ...
  "Test deleting selected element - no editable space after element (del)":
    function () {
      var editor = this.editor;

      this.editorBot.setData(
        '<p>bar</p><p id="start" contenteditable="false">foo</p>',
        function () {
          var ps = editor.document.getById("start"),
            editable = editor.editable(),
            prevented;
  ...
  "Test deleting selected element - no editable space before element (backspace)":
    function () {
      var editor = this.editor;

      this.editorBot.setData(
        '<p id="start" contenteditable="false">foo</p><p>bar</p>',
        function () {
          var ps = editor.document.getById("start"),
            editable = editor.editable(),
            prevented;
  ...
  "Test selecting non-editable element by keys - inline": function () {
    ...
    this.editorBot.setData(
      '<p id="el1">foo<span id="target" contenteditable="false">bar</span><em id="el2">bom</em></p>',
      function () {
  ...
  "Test selecting non-editable element by keys - block": function () {
    ...
    this.editorBot.setData(
      '<p id="el1">foo</p><p id="target" contenteditable="false">bar</p><ul><li id="el2">bom</li></ul>',
      function () {
        var el1 = editor.document.getById("el1"),
          el2 = editor.document.getById("el2"),
          target = editor.document.getById("target"),
  ...
  "Test selecting non-editable element by keys - opt out": function () {
    ...
    this.editorBot.setData(
      '<p id="el1">foo</p><p id="target" contenteditable="false">bar</p><p></p><p id="el2">bom</p>',
      function () {
        var el1 = editor.document.getById("el1"),
          el2 = editor.document.getById("el2"),
          range = editor.createRange(),
        ...
        editor
          .editable()
          .fire("keydown", new CKEDITOR.dom.event({ keyCode: 39 })); // RIGHT
        assert.isNull(
          editor.getSelection().getSelectedElement(),
          "<p>[foo]</p>"
        );
  ...
  "Test auto fake selection for inline element containing nested non-editable":
    function () {
      ...
      bot.setData(bender.tools.getValueAsHtml("editor_content"), function () {
        ...
        assert.areSame(
          editor.document.findOne("#fake"),
          sel.getSelectedElement()
        );
      });
    },

  "Test auto fake selection for inline element containing only non-editable":
    function () {
      var bot = this.editorBot;
  ...
  "Test no auto fake selection if non-editable element is not the only child of enclosed element":
    function () {
      var bot = this.editorBot;
  ...
  "Test select editable contents when fake selection was on and DOM has been overwritten":
    function () {
      var editor = this.editor;
      ...
      this.editorBot.setData('<p id="a">a<em id="b">b</em>c</p>', function () {
        ...
        // Remove nbsp which breaks comparison on FF.
        var html = bender.tools
          .getHtmlWithSelection(editor)
          .replace(/\xa0/g, "");
  ...
  // #1516
  "Test delete/backspace keys are not removing readonly selection":
    function () {
  ...
  "test hidden selection container styles": function () {
    ...
    bot.setData('<p>[<span id="bar">bar</span>]</p>', function () {
      ...
      hiddenSelectionContainer = editor
        .editable()
        .findOne("[data-cke-hidden-sel]");
      ...
      if (CKEDITOR.env.ie && CKEDITOR.env.version < 14) {
        ...
        for (var key in expected) {
          assert.areEqual(
            expected[key],
            hiddenSelectionContainer.getComputedStyle(key)
          );
        }
      }
    });
