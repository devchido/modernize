...
bender.test({
  ...
  assertFillingChar: function (editable, parent, contents, msg) {
    ...
    assert.areSame(
      parent,
      fillingChar.getParent(),
      "Filling char parent - " + msg
    );
    assert.areSame(
      fillingCharSequence + contents,
      fillingChar.getText(),
      "Filling char contents - " + msg
    );
  ...
  "test selection on initial focus": function () {
    ...
    assert.areEqual(
      "<p>^foo</p>",
      bender.tools.getHtmlWithSelection(ed),
      "Selection goes into editable on focus (https://dev.ckeditor.com/ticket/9507)."
    );
  },
      ...
      ed.setData("<p>foo</p>", function () {
        resume(function () {
          ...
          assert.areEqual(
            "<p>^foo</p>",
            bender.tools.getHtmlWithSelection(ed),
            "Selection goes into editable on focus (https://dev.ckeditor.com/ticket/10115)."
          );
        });
      });
    });
  ...
  "test editor selection lock on blur": function () {
    ...
    if (!noSelectionOnBlur(ed)) assert.ignore();
    ...
    this.wait(function () {
      ...
      // Check saved range.
      assert.isTrue(
        checkRangeEqual(range, savedRange),
        "saved range does not match original"
      );
      ...
      assert.isTrue(
        checkRangeEqual(range, restoredRange),
        "restored range does not match original"
      );
  ...
  'test "selectionChange" fires properly': function () {
    ...
    this.wait(function () {
      ...
      assert.areSame(
        2,
        firedTimes,
        "times of selectionChange fired doesn't match."
      );
    }, 200);
  },
  ...
  'test "selectionChange" not fired when editor selection is locked':
    function () {
      var ed = this.editors.editor,
        editable = ed.editable();
  ...
  'test "selectionChange" fired after the same selection set after data loaded':
    function () {
      var bot = this.editorBots.editor,
        editor = this.editors.editor,
        selectionChange = 0;
      ...
      bot.setData("<p>foo<strong>bar</strong></p>", function () {
        ...
        bot.setData("<p>foo<strong>bar</strong></p>", function () {
          ...
          assert.isTrue(
            selectionChange > 0,
            "At least one selectionChange was fired"
          );
        });
      });
    },
  ...
  "test collapsed text selection": function () {
    ...
    this.assertGetSelection(
      "<table><tbody><tr><td>^</td></tr></tbody></table><p>foo</p>"
    );
  ...
  "test initial selection after set data in autoparagraphing editor":
    function () {
      doc.getById("input1").focus();
        ...
        editor.setData("", function () {
          resume(function () {
            ...
            assert.isInnerHtmlMatching(
              ...
              htmlMatchingOpts,
              "Selection is in the right place."
            );
            ...
            assert.isInnerHtmlMatching(
              ...
              bender.tools.selection.getWithHtml(editor),
              htmlMatchingOpts,
              "Text was inserted in the right place."
            );
          });
        });
      });
  ...
  "test initial selection after set data in autoparagraphing inline editor":
    function () {
  ...
  "test remove filling character from snapshots and data": function () {
    ...
    bot.setData("<p>foo</p>", function () {
      ...
      assert.areSame(
        "<p>" + fillingCharSequence + "foo</p>",
        editor.editable().getHtml(),
        "FC in DOM."
      );
      assert.areSame(
        "<p>foo</p>",
        new CKEDITOR.plugins.undo.Image(editor).contents,
        "No FC in snapshots."
      );
  ...
  "test selection is invalidating filling char after editable is replaced by new one":
    function () {
      if (!CKEDITOR.env.webkit) assert.ignore();
          ...
          editor.setMode("source", function () {
            ...
            editor.setMode("wysiwyg", function () {
              resume(function () {
                ...
                assert.isMatching(
                  '^<p id="p">foo' + fillingCharSequence + "<em>bar</em></p>$",
                  editor.editable().getHtml(),
                  "Filling char was inserted"
                );

                // Fire event imitating left arrow, what will trigger
                // removeFillingChar() on Webkit.
                editor.document.fire(
                  "keydown",
                ...
                assert.isMatching(
                  ...
                  editor.editable().getHtml(),
                  "Filling char was removed on keydown"
                );
                ...
                assert.isMatching(
                  '^<p id="p">foo' + fillingCharSequence + "<em>bar</em></p>$",
                  editor.editable().getHtml(),
                  "Filling char was inserted 2"
                );
                assert.isMatching(
                  /^<p id="p">foo<em>bar<\/em><\/p>$/,
                  editor.getData(),
                  "Filling char was removed on getData"
                );
                ...
                assert.isMatching(
                  /^<p id="p">foo<em>bar<\/em><\/p>$/,
                  new CKEDITOR.plugins.undo.Image(editor).contents,
                  "Filling char was removed on beforeUndoImage"
                );
              });
            });
          });
  ...
  "test filling char remains untouched when taking snapshot": function () {
    ...
    var uEl = editable.findOne("u"),
      fillingChar = this.assertFillingChar(
        editable,
        uEl,
        "",
        "after set selection"
    ...
    assert.areSame(
      ...
      "Selection remains - container"
    );
    assert.areSame(1, range.startOffset, "Selection remains - offset after FC");
  },

  // https://dev.ckeditor.com/ticket/12489
  "test filling char remains when taking snapshot if selection is not right after the filling char":
    function () {
      if (!CKEDITOR.env.webkit) assert.ignore();
      ...
      editor.document.$.getSelection().setPosition(
        fillingChar.$,
        fillingChar.$.nodeValue.length - 2
      ); // FCab^cd
      ...
      assert.areSame(
        fillingCharSequenceLength + 2,
        range.startOffset,
        "Selection remains - offset in FCab^cd"
      );
    },

  // https://dev.ckeditor.com/ticket/8617
  "test selection is preserved when removing filling char on left-arrow":
    function () {
      if (!CKEDITOR.env.webkit) assert.ignore();
      ...
      assert.areSame(
        "abc",
        uEl.getHtml(),
        "Filling char is removed on left-arrow press"
      );
      ...
      assert.areSame(
        uEl.getFirst(),
        range.startContainer,
        "Selection was restored - container"
      );
      assert.areSame(
        3,
        range.startOffset,
        "Selection was restored - offset in abc^"
      );
    },

  // https://dev.ckeditor.com/ticket/12419
  "test selection is preserved when removing filling char on select all":
    function () {
      if (!CKEDITOR.env.webkit) assert.ignore();
      ...
      assert.areSame(
        "abc",
        uEl.getHtml(),
        "Filling char is removed on selection change"
      );
      assert.isInnerHtmlMatching(
        "<p>[x<u>abc</u>x]</p>",
        bender.tools.selection.getWithHtml(editor),
  ...
  "test direction of selection is preserved when removing filling char":
    function () {
      if (!CKEDITOR.env.webkit) assert.ignore();
  ...
  "test direction of selection is preserved when taking snapshot": function () {
    ...
    nativeSel = editor.document.$.getSelection();
    assert.areSame(
      fillingCharSequenceLength + 3,
      nativeSel.anchorOffset,
      "sel.anchorOffset"
    );
    assert.areSame(
      fillingCharSequenceLength + 1,
      nativeSel.focusOffset,
      "sel.focusOffset"
    );
  },
          ...
          editor.setMode("source", function () {
            ...
            resume(function () {
              assert.areSame(
                0,
                selectionChange,
                "No selectionChange fired when switching to source mode"
              );
              assert.isNull(
                editor.getSelection(),
                "getSelection() returns null for locked selection"
              );
              assert.isNull(
                editor.getSelection(1),
                "getSelection() returns null for real selection"
              );
            });
          });
        });
  ...
  // (#1113)
  'test "selectionChange" fires properly with nested contentEditable':
    function () {
      var editor = this.editors.editorInline,
        editable = editor.editable(),
      ...
      bender.tools.setHtmlWithSelection(
        editor,
        '<div contenteditable=true><div contenteditable=false><div contenteditable=true id="nested">xxx</div></div></div>'
      );

      var event =
        (CKEDITOR.env.webkit && "DOMFocusIn") ||
        (CKEDITOR.env.gecko && "focusin") ||
        "focus";
      editable.fire(
        event,
        new CKEDITOR.dom.event({ target: editable.findOne("#nested") })
      );
  ...
  "test focused nested editable changes elementPath": function () {
    ...
    bender.tools.setHtmlWithSelection(
      editor,
      '<div contenteditable=true>^xxx<div contenteditable=false><em contenteditable=true id="nested">xxx</em></div></div>'
    );
...
function testSelectionCheck(editor, event) {
  ...
  wait(function () {
    ...
    assert.isTrue(
      onSelectionChange.calledOnce,
      "selectionCheck not fired on " + event
    );
  }, 200);
}
