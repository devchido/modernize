...
bender.test({
  ...
  "test command states with readonly editor": function () {
    ...
    this.checkAllCmds(function (cmd, name) {
      if (
        cmd.state != CKEDITOR.TRISTATE_DISABLED &&
        CKEDITOR.tools.indexOf(READ_ONLY_CMDS, name) == -1
      )
  ...
  "test command states in details>summary": function () {
    ...
    this.checkAllCmds(function (cmd, name) {
      if (
        cmd.state != CKEDITOR.TRISTATE_DISABLED &&
        cmd.context in CKEDITOR.dtd.$block
      )
        assert.fail(
          "command: " + name + " should be disabled inside of summary"
        );
    });
    ...
    bender.editorBot.create(
                ...
                function () {
                  ...
                  resume(function () {
                    ...
                    assert.areSame(
                      CKEDITOR.TRISTATE_OFF,
                      st1,
                      'first "mode" cmd1.state'
                    );
                    assert.areSame(
                      CKEDITOR.TRISTATE_DISABLED,
                      st2,
                      'first "mode" cmd2.state'
                    );
                    assert.areSame(
                      CKEDITOR.TRISTATE_OFF,
                      st3,
                      'first "mode" cmd3.state'
                    );
      ...
      function (bot) {
        ...
        assert.areSame(
          "enable,setState",
          changedState1.join(","),
          "cmd1.state was changed once during initialization"
        );
        ...
        assert.areSame(
          CKEDITOR.TRISTATE_OFF,
          cmd1.state,
          "refreshed cmd1.state"
        );
        assert.areSame(
          CKEDITOR.TRISTATE_DISABLED,
          cmd2.state,
          "refreshed cmd2.state"
        );
        ...
        assert.areSame(
          CKEDITOR.TRISTATE_DISABLED,
          cmd2.state,
          "after disable()&enable() cmd2.state"
        );
        ...
        assert.areSame(
          CKEDITOR.TRISTATE_DISABLED,
          cmd1.state,
          "after disable() cmd1.state"
        );
        cmd1.enable();
        assert.areSame(
          CKEDITOR.TRISTATE_OFF,
          cmd1.state,
          "after enable() cmd1.state"
        );
        ...
        assert.areSame(
          CKEDITOR.TRISTATE_OFF,
          cmd5.state,
          "command added after instanceReady is refreshed automatically"
        );
      }
    );
  },
  ...
  "test copy command not disabled after clicking on elements path":
    function () {
      if (!CKEDITOR.env.ie) {
        assert.ignore();
        return;
