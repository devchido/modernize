...
(function () {
  ...
  bender.test({
    "test beforeCommandExec canceling": function () {
      ...
      assert.isFalse(
        cmdCalled,
        "Command should not be called if beforeCommandExec evt was canceled"
      );
    },
    ...
    "test default event data value": function () {
      ...
      this.editor.once("beforeCommandExec", function (evt) {
        ...
        assert.isObject(
          beforeExecData,
          "Event data is initialized as an empty object"
        );
      });
      ...
      this.editor.once("afterCommandExec", function (evt) {
        assert.areSame(
          beforeExecData,
          evt.data.commandData,
          "The same object is passed to afterCommandExec"
        );
      });
