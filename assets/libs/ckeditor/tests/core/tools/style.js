  ...
  bender.test({
    ...
    "test style.parse.background single background": function () {
      ...
      objectAssert.areEqual(
        { color: "red", unprocessed: "url(foo.bar) no-repeat" },
        ret
      );
    },
    ...
    "test style.parse.background docs sample": function () {
      var background =
        CKEDITOR.tools.style.parse.background("#0C0 url(foo.png)");
      objectAssert.areEqual(
        { color: "#0C0", unprocessed: "url(foo.png)" },
        background
      );
    },
    ...
    "test style.parse._findColor hex": function () {
      var ret = this.parse._findColor(
        "foo #foo #aa1 #000 aa #1234 #123456 #aag #AAB #AAB"
      );

      arrayAssert.itemsAreEqual(
        ["#aa1", "#000", "#123456", "#AAB", "#AAB"],
        ret
      );
    },
    ...
    "test style.parse._findColor rgba": function () {
      var ret = this.parse._findColor(
        "rgb(255,0,128), rgb(),rgb,rgba(0000), rgb(10%, 10% ,10%) rgba(0,0,0,0.3) rgba(0,0,0,0)"
      );
    ...
    "test style.parse._findColor hsla": function () {
      var ret = this.parse._findColor(
        "hsl(10,30%,30%)   hsla(10,30%,30%,1)  hsla( 90.5, 10%, 10%, 1)"
      );
    ...
    "test style.parse.margin docs sample": function () {
      assertObject(
        { top: "3px", right: "0", bottom: "2", left: "0" },
    ...
    "test style.parse.sideShorthand docs sample": function () {
      assertObject(
        { top: "solid", right: "dotted", bottom: "solid", left: "dotted" },
        CKEDITOR.tools.style.parse.sideShorthand("solid dotted")
      );
    },

    // (#1490)
    "test style.parse.sideShorthand docs sample with split function":
      function () {
        assertObject(
          { top: "foo", right: "baz", bottom: "foo", left: "baz" },
          CKEDITOR.tools.style.parse.sideShorthand("foo baz", split),
          "foo baz"
        );

        assertObject(
          { top: "bar", right: "quix", bottom: "bar", left: "quix" },
          CKEDITOR.tools.style.parse.sideShorthand("something else", split),
          "something else"
        );
    ...
    "test style.border color normalization": function () {
      assertObject(
        { color: "black", width: null, style: null },
    ...
    "test style.border.toString is overwriten": function () {
      assert.areEqual(
        "1px solid red",
        new CKEDITOR.tools.style.border({
          color: "red",
          width: "1px",
          style: "solid",
        })
      ...
      assert.areEqual(
        "10px dotted",
        new CKEDITOR.tools.style.border({ width: "10px", style: "dotted" })
      );
      assert.areEqual(
        "solid blue",
        new CKEDITOR.tools.style.border({ style: "solid", color: "blue" })
      );
    },
    ...
    // (#1490)
    "test style.border.splitCssValues side border modifiers with fallback":
      function () {
        var styles = {
            "border-style": "solid",
            "border-width": "2px",
    ...
    "test style.border.fromCssRule docs sample": function () {
      assertObject(
        { width: "3px", style: "solid", color: "#ffeedd" },
    ...
    "test style.border.fromCssRule only with width": function () {
      assertObject(
        { width: "0%", style: null, color: null },
        CKEDITOR.tools.style.border.fromCssRule("0%")
      );
    },
    ...
    "test style.border.fromCssRule with width and style": function () {
      assertObject(
        { width: "0%", style: "groove", color: null },
    ...
    "test style.border.fromCssRule with mixed color and style": function () {
      assertObject(
        { color: "#ff0000", style: "dotted", width: null },
        CKEDITOR.tools.style.border.fromCssRule("#ff0000 dotted")
      );
    },

    // (#1490)
    "test style.border.fromCssRule with mixed color, style and width":
      function () {
        assertObject(
          { width: "7.5em", color: "#ff0000", style: "dotted" },
    ...
    "test style.border.fromCssRule with style only": function () {
      assertObject(
        { width: null, style: "dotted", color: null },
    ...
    "test style.border.fromCssRule with style and rgba color": function () {
      assertObject(
        { width: null, style: "dotted", color: "rgba(0,0,0,0)" },
    ...
    "test style.border.fromCssRule with style and hsla color": function () {
      assertObject(
        { style: "dotted", color: "hsla(10,30%,30%,1)" },
    ...
    "test style.border.fromCssRule with color white spaces": function () {
      assertObject(
        { style: "solid", color: "rgba(10,  20, 30,  .75)", width: "1px" },
        CKEDITOR.tools.style.border.fromCssRule(
          "1px solid rgba(10,  20, 30,  .75)"
        )
      );
    },
  });
  ...
  function assertObject(expected, actual) {
    for (var key in expected) {
      assert.areEqual(
        expected[key],
        actual[key],
        'Property "' + key + '" should have correct value.'
      );
    }
  }
})();
