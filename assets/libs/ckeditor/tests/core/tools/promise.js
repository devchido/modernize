    ...
    "test promise initialization": function () {
      if (window.Promise) {
        assert.areSame(
          ...
          "Native Promise should be enabled"
        );
        assert.isUndefined(
          window.ES6Promise,
          "Polyfill Promise should not be loaded"
        );
      } else {
        assert.isNotUndefined(
          window.ES6Promise,
          "Polyfill Promise should be loaded"
        );
        assert.areSame(
          window.ES6Promise,
          CKEDITOR.tools.promise,
          "Polyfill Promise should be enabled"
        );
      }
    },
    ...
    "test missing promise polyfill": function () {
      ...
      setTimeout(function () {
        resume(function () {
          ...
          assert.isTrue(
            errorStub.calledWith("no-vendor-lib", {
              path: CKEDITOR.getUrl("vendor/promise.js"),
            })
          );
        });
      }, 100);
