...
(function () {
  ...
  bender.test({
    "test skin with no chameleon functionality and custom uiColor":
      function () {
        bender.error = originalErrorFunc;

        assert.isNull(
          caughtError,
          "An error is not thrown during editor initialisation"
        );
        assert.areSame(
          "ready",
          this.editors.editor.status,
          "Editor is really ready"
        );
