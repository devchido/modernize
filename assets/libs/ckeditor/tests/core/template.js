...
(function () {
  ...
  bender.test({
    "template output string": function () {
      var tpl = new CKEDITOR.template(
        '<a href="#" id="{id}" title="{text}">{text}</a>'
      );
      ...
      assert.areSame(
        '<a href="#" id="btn" title="{text}">{text}</a>',
        tpl.output({ id: "btn" })
      );

      assert.areSame(
        '<a href="#" id="btn" title="foo">foo</a>',
        tpl.output({ id: "btn", text: "foo" })
      );
    },
    ...
    "add/retrieve global template": function () {
      ...
      CKEDITOR.on("template", function (evt) {
        var data = evt.data;
        if (data.name == "tpl2") data.source = "{foo},{foo}";
      });
    ...
    "code replacement": function () {
      this.assertTemplateOutput("alert('foo!')", "{code}", {
        code: "alert('foo!')",
      });
    },
    ...
    "supports source as a function": function () {
      ...
      this.assertTemplateOutput("bar", getTemplate, {
        image: null,
        label: "bar",
      });
      this.assertTemplateOutput(
