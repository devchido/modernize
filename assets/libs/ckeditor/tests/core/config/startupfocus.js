      ...
      function (bot) {
        resume(function () {
          ...
          if (setup.startupFocus) {
            assert.isTrue(
              bot.editor.focusManager.hasFocus,
              "editor is focused"
            );
          } else {
            assert.isFalse(
  ...
  bender.test({
    ...
    'test startup focus "end" table': function () {
      assertStartupFocus({
        ...
        startupFocus: "end",
        expected:
          "<p>foo@</p><table><tbody><tr><td>cell^@</td></tr></tbody></table>",
      });
    },
    ...
    'test startup focus "end" img': function () {
      assertStartupFocus({
        ...
        startupFocus: "end",
        expected:
          '<p>foo@</p><p><img alt="Saturn V" data-cke-saved-src="%BASE_PATH%_assets/logo.png" src="%BASE_PATH%_assets/logo.png" style="width:200px" />^@</p>',
        extraConfig: {
          extraPlugins: "image",
        },
      });
