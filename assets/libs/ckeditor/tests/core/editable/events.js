...
function addTests(tcs, creator) {
        ...
        instanceReady: function () {
          resume(function () {
            eventsRecorder.assert(
              creator == "replace"
                ? ["setData", "afterSetData", "contentDom", "dataReady"]
  ...
  tcs["test events order on setMode to source - " + creator] = function () {
    if (creator == "inline") assert.ignore();
  ...
  tcs["test events on editable detach - " + creator] = function () {
    ...
    assert.areSame(
      1,
      eventFiredTimes,
      "after calling editable.detach() contentDomUnload should be fired exactly once"
    );
  };
}
