  ...
  return {
    ...
    run: function (config, additionalTCs) {
      ...
      while ((editableName = editablesNamesArray.shift())) {
        ...
        if (inline) el.setAttribute("contenteditable", "true");
        ...
        CKEDITOR[inline ? "inline" : "replace"](el, config).on(
          "instanceReady",
          function (evt) {
    ...
    assertInsertion: function (
      ...
      while ((editableName = editablesNames.shift())) {
        ...
        for (mode in modes) {
          ...
          rangeList = new CKEDITOR.dom.rangeList(
            tools.setHtmlWithRange(root, source, root)
          );
          ...
          if (mode == "insertElement")
            editor.insertElement(
              CKEDITOR.dom.element.createFromHtml(insertion, editor.document)
            );
          else if (mode == "insertText") editor.insertText(insertion);
          else editor.insertHtml(insertion, mode);
          ...
          assert[expectedForMode.exec ? "isMatching" : "areSame"](
            expectedForMode,
            result,
          ...
          if (mode != "insertElement") {
            assert.areSame(
              1,
              afterInsertCount,
              "There should be 1 afterInsertHtml event after every insertion."
            );
            assert.isUndefined(
              afterInsertData.intoRange,
              "intoRange parameter should be undefined."
            );
          }
        }
      }
      ...
      function replaceMethods(obj, newFns) {
        ...
        return function () {
          for (var name in oldFns) obj[name] = oldFns[name];
        };
      }
    },

    createAssertInsertionFunction: function (
      editablesNames,
      insertion,
      mode,
      enterMode
