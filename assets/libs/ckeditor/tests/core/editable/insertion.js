...
(function () {
  ...
  var tests = {
    testInsertElement: function () {
      ...
      var ins = CKEDITOR.dom.element.createFromHtml(
        "<strong>baz</strong>",
        editor.document
      );
      ...
      assert.areSame(
        "foo<strong>baz</strong>bar",
        tools.compatHtml(editor.getData()),
        "insert element with existing selection, editor focused"
      );

      // When editor loose focus.
      tools.setHtmlWithSelection(editor, "foo^bar");
      ins = CKEDITOR.dom.element.createFromHtml(
        "<strong>baz</strong>",
        editor.document
      );
      ...
      assert.areSame(
        "foo<strong>baz</strong>bar",
        tools.compatHtml(editor.getData()),
        "insert element with existing selection, editor blurred"
      );
    },
    ...
    testInsertHtml: function () {
      ...
      assert.areSame(
        "foobazbar",
        tools.compatHtml(editor.getData()),
        "insert html with existing selection, editor focused"
      );
      ...
      assert.areSame(
        "foobazbar",
        tools.compatHtml(editor.getData()),
        "insert html with existing selection, editor blurred"
      );
    },
    ...
    testInsertText: function () {
      ...
      assert.areSame(
        "foobazbar",
        tools.compatHtml(editor.getData()),
        "insert text with existing selection, editor focused"
      );
      ...
      assert.areSame(
        "foobazbar",
        tools.compatHtml(editor.getData()),
        "insert text with existing selection, editor blurred"
      );
    },
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.areSame(
            CKEDITOR.ENTER_BR,
            mode,
            "dynamic enter mode was used - BR"
          );
    ...
    "test insertHtml sets options.protectedWhitespaces of dP.toHtml":
      function () {
        var editor = this.editor;
    ...
    "test insertHtml with range": function () {
      ...
      assert.isInnerHtmlMatching(
        '<p id="p1">foo</p><p>bam^@</p><p>bar@</p>',
        tools.selection.getWithHtml(editor),
    ...
    CKEDITOR.tools.array.forEach(methods, function (methodName) {
      tests["test " + methodName + " " + options.name] = function () {
        ...
        if (methodName === "insertText") {
          assertInsertText(
            editor,
            options.initial.replace(/{.*}/, "text"),
            "text"
          );
        } else {
          assertInsertionMethod(
            editor,
            options.expected,
            methodName,
            options.data
