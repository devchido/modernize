  ...
  bender.test({
    checkInsertHtmlIntoRange: function (insertedHtml, range, expectedHtml) {
      ...
      assert.areSame(
        1,
        afterInsertCount,
        "afterInsertHtml should be fired once."
      );
      assert.areSame(
        range,
        afterInsertData.intoRange,
        "intoRange should contain range"
      );
    },
    ...
    "test insertHtmlIntoRange - inline": function () {
      ...
      this.checkInsertHtmlIntoRange(
        "<b>b</b><i>i</i>",
        range,
        "<p>fo<b>b</b><i>i</i>ar@</p>"
      );
    },

    "test insertHtmlIntoRange - inline, the same range": function () {
      var range = bender.tools.range.setWithHtml(
    ...
    "test insertHtmlIntoRange - read-only": function () {
      bender.tools.selection.setWithHtml(
        this.editor,
        '<p>x</p><p contenteditable="false" id="x">foobar</p><p>y{}</p>'
      );
    ...
    "test insertHtml with range": function () {
      ...
      assert.areSame(
        undefined,
        afterInsertData.intoRange,
        "intoRange should be null if insertHtml was used."
      );
    },
  });
})();
