...
var quirksTools = (function () {
  ...
  function assertKeystroke(key, keyModifiers, handled, html, expected) {
    ...
    return function () {
      ...
      var message =
        "(" + keyNames[key] + ") Correct DOM state after the keystroke";
      ...
      assert.areSame(
        handled,
        handledNatively,
        "(" + keyNames[key] + ") Keystroke handled by the browser"
      );
  ...
  function d() {
    return assertKeystroke.apply(
      this,
      [DEL, 0, 0].concat([].slice.call(arguments))
    );
  }
  ...
  function b() {
    return assertKeystroke.apply(
      this,
      [BACKSPACE, 0, 0].concat([].slice.call(arguments))
    );
  }
