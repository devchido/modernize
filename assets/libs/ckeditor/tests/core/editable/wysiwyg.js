...
(function () {
  ...
  bender.test({
    ...
    setUp: function () {
      ...
      if (!editor.editable()) editor.editable(doc.getById("editable"));
      editor.setData("");
    },
    ...
    testData: function () {
      ...
      assert.areSame(
        "<p>foo</p>",
        tools.compatHtml(editor.editable().getHtml()),
        "set data"
      );
      assert.areSame(
        "<p>foo</p>",
    ...
    testAttachListeners: function () {
      ...
      function recorder(evt) {
        if (!counter[evt.name]) counter[evt.name] = 1;
        else counter[evt.name]++;
      }
    ...
    testChangeAttr: function () {
      ...
      assert.areSame(
        "baz",
        edt.getAttribute("foo"),
        "check changed existing attr"
      );
