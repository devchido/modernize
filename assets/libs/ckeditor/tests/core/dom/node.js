...
(function () {
  ...
  bender.test({
    test_$: function () {
      ...
      assert.isTrue(
        t instanceof CKEDITOR.dom.text,
        "Should be an instanceof dom.text"
      );
      assert.isTrue(
        c instanceof CKEDITOR.dom.comment,
        "Should be an instanceof dom.comment"
      );
      assert.isTrue(
        e instanceof CKEDITOR.dom.element,
        "Should be an instanceof dom.element"
      );
    },
    ...
    test_getPrevious2: function () {
      ...
      assert.areSame(
        $("getNSN5"),
        node.getPrevious(function (node) {
    ...
    "test isReadOnly - editable is not isReadOnly": function () {
      ...
      assert.isFalse(
        target.isReadOnly(),
        "Element specify itself as editable."
      );
    },
    ...
    'test isReadOnly - contenteditable="false" is isReadOnly': function () {
      ...
      assert.isTrue(
        target.getFirst().isReadOnly(),
        "Element specify itself as non-editable."
      );
    },

    'test isReadOnly - contenteditable="false" child is isReadOnly':
      function () {
        var target = $("editable");
        target.setHtml('<div contenteditable="false"><p>foo</p></div>');

        assert.isTrue(
          target.getChild([0, 0]).isReadOnly(),
          "Element inheirit non-editable property from parent."
        );
      },
    ...
    "test isReadOnly - nested editable is not isReadOnly": function () {
      var target = $("editable");
      target.setHtml(
        '<div contenteditable="false"><p contenteditable="true"><span>foo</span></p></div>'
      );

      assert.isFalse(
        target.getChild([0, 0, 0]).isReadOnly(),
        "Element inheirit editable property from parent."
      );
    },

    'test isReadOnly - contenteditable="false" data-cke-editable="1" is not isReadOnly':
      function () {
        var target = $("editable");
        target.setHtml(
          '<input type="text" contenteditable="false" data-cke-editable="1" />'
        );

        assert.isFalse(
          target.getFirst().isReadOnly(),
          'Element marked as "cke-editable" is not ready-only.'
        );
      },
    ...
    "test isReadOnly - div with data-cke-editable is not isReadOnly (attrs check)":
      function () {
        var el = newElement("div");
    ...
    'test isReadOnly - div contenteditable="false" child is isReadOnly (attrs check)':
      function () {
        var el = newElement("div");
    ...
    'test isReadOnly - div contenteditable="true" child is not isReadOnly (attrs check)':
      function () {
        var el = newElement("div");
    ...
    test_getNextSourceNode: function () {
      ...
      assert.areSame(
        $("getNSN2"),
        node.getNextSourceNode(true, CKEDITOR.NODE_ELEMENT)
      );
      assert.areSame(
        "comment1",
        node.getNextSourceNode(true, CKEDITOR.NODE_COMMENT).$.nodeValue
      );
      assert.isNull(
        node.getNextSourceNode(true, CKEDITOR.NODE_COMMENT, $("getNSN3"))
      );
    },
    ...
    test_getPreviousSourceNode: function () {
      ...
      assert.areSame(
        $("getNSN5"),
        node.getPreviousSourceNode(true, CKEDITOR.NODE_ELEMENT)
      );
      assert.areSame(
        "comment2",
        node.getPreviousSourceNode(true, CKEDITOR.NODE_COMMENT).$.nodeValue
      );
      assert.isNull(
        node.getPreviousSourceNode(true, CKEDITOR.NODE_COMMENT, $("getNSN4"))
      );
    },
    ...
    test_getCommonAncestor: function () {
      assert.areSame(
        ...
        $("getNSN1").getCommonAncestor($("getAddress2"))
      );
    },
    ...
    test_move: function () {
      ...
      assert.areSame(
        "move1",
        parent.$.childNodes[parent.$.childNodes.length - 1].id
      );
    ...
    test_remove: function () {
      $("remove1").remove(true);
      assert.areSame(
        '<div id="remove"><i></i>text<!--comment--></div>',
        getOuterHtml($("remove"))
      );
      $("remove").remove();
    ...
    test_replace: function () {
      ...
      assert.areSame(
        '<div id="replace">12<p id="replace1"></p>3</div>',
        getOuterHtml($("replace"))
      );
    },
