...
bender.test({
  ...
  assertElementInView: function (el, offset) {
    var view = win.getViewPaneSize(),
      rect = el.getClientRect();
    assert.isTrue(
      rect.top > -1 && rect.top < view.height,
      "check element inside of viewport"
    );

    // Check within offset.
    if (offset < 0)
      assert.isTrue(
        rect.bottom - view.height > offset,
        "check element at the bottom of page"
      );
    else if (offset > 0)
