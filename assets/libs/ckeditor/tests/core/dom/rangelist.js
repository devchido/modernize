...
(function () {
  ...
  bender.test({
    test__constructor: function () {
      ...
      assert.areSame(
        rlist,
        new CKEDITOR.dom.rangeList(rlist),
        "rangeList as an argument"
      );
    ...
    test_createIterator: function () {
      ...
      while ((range = iterator.getNextRange())) ranges2.push(range);

      assert.isTrue(CKEDITOR.tools.arrayCompare(ranges, ranges2));

      iterator = rlist.createIterator();
      ranges2 = [];
      while ((range = iterator.getNextRange(true))) ranges2.push(range);
