...
function checkRangeIteration(source, opt, results, output, msg) {
  ...
  var blockList = [],
    block;
  while ((block = iter.getNextParagraph())) blockList.push(block.getName());
  ...
  if (output)
    assert.areSame(
      tools.compatHtml(output, 1, 1),
      tools.compatHtml(sandbox.getHtml(), 1, 1),
      msg
    );
}
...
function iterateScopedRange(html, opt) {
  ...
  var results = [],
    block;
  while ((block = iter.getNextParagraph())) results.push(block.getName());
...
function checkActiveFilter(source, opt, results, msg) {
  ...
  while ((block = iter.getNextParagraph()))
    blockList.push(
      block.getName() + (iter.activeFilter ? "-" + iter.activeFilter.id : "")
    );
...
bender.test({
  ...
  "test iterator works well with collapsed range position": function () {
    var msg =
      "Iteration should return the paragraph in which the range anchored";
  ...
  "test iterating over multiple paragraphs": function () {
    ...
    checkRangeIteration(
      ...
      ["p", "p"],
      null,
      "Iteration will yield  two paragraphs."
    );
  },
  ...
  "test a single range collapsed inside of an empty paragraph": function () {
    ...
    checkRangeIteration(
      ...
      ["p"],
      output,
      "Iteration will yield one single paragraph"
    );
  },
  ...
  "test iterating over end of line": function () {
    ...
    checkRangeIteration(
      ...
      ["h1", "p"],
      null,
      "Iteration will yield heading and paragraph."
    );
  },
  ...
  // https://dev.ckeditor.com/ticket/12178
  "test iterating over start of line 2 - no bogus br - rangeIterator":
    function () {
      var source = "<h1>pa[ra1</h1><p><b>]para2</b></p>";
  ...
  "test iterating over pseudo block": function () {
    ...
    checkRangeIteration(
      ...
      ["p", "p"],
      output,
      "Iteration should create real paragraph for pseudo block."
    );
  },
  ...
  "test iterating over table cells": function () {
    ...
    checkRangeIteration(
      ...
      ["th", "p", "td"],
      output1,
      "Iteration should report paragraph or table cells"
    );
    checkRangeIteration(
      source,
      { enforceRealBlocks: 1 },
      ["p", "p", "p"],
      output2,
      "Iteration should establish paragraph if it's not available inside table cell"
    );
  },
  ...
  "test iterating over table cells (with bookmarks among cells)": function () {
    var source =
        '<table><tbody><tr>[<td id="cell1">cell1</td>][<td id="cell2">cell2</td>]</tr></tbody></table>',
    ...
    assert.areSame(
      tools.compatHtml(output),
      tools.compatHtml(sandbox.getHtml())
    );
  },

  // See above an explanation of this test.
  "test iterating over table cells (with bookmarks among cells) - do not merge subsequent ranges":
    function () {
      var source =
          '<table><tbody><tr>[<td id="cell1">cell1</td>][<td id="cell2">cell2</td>]</tr></tbody></table>',
        output = source.replace(/\[|\]|\^/g, "");
  ...
  "test iterating over entire table": function () {
    var source =
        "[<table><tbody><tr><th>cell1</th><td>cell2</td></tr></tbody></table>]",
      output1 = source.replace(/\[|\]|\^/g, ""),
      output2 =
        "<table><tbody><tr><th><p>cell1</p></th><td><p>cell2</p></td></tr></tbody></table>";
    ...
    checkRangeIteration(
      ...
      ["th", "td"],
      output1,
      "Iteration should report table cells"
    );
    checkRangeIteration(
      source,
      { enforceRealBlocks: 1 },
      ["p", "p"],
      output2,
      "Iteration should establish paragraphs inside table cells"
    );
  },
  ...
  // https://dev.ckeditor.com/ticket/6728, https://dev.ckeditor.com/ticket/4450
  "test iterating over entire table - use rangeIterator and enforceRealBlocks":
    function () {
      var source =
          "[<table><tbody><tr><th>cell1</th><td>cell2</td></tr></tbody></table>]",
        output =
      ...
      assert.areSame(
        "p,p",
        iterateWithRangeIterator(ranges, true, {
          enforceRealBlocks: true,
        }).join(",")
      );
  ...
  "test iterating over list items": function () {
    ...
    checkRangeIteration(
      ...
      ["li", "p", "li", "p", "li"],
      output1,
      "Iteration should report paragraph or list item"
    );
    checkRangeIteration(
      source,
      { enforceRealBlocks: 1 },
      ["p", "p", "p", "p", "p"],
      output2,
      "Iteration should establish paragraph if not exists inside list item"
    );
  },
  ...
  "test iterator returns blocks with only non-editable content": function () {
    var source =
        '<p>[a</p><h1><i contenteditable="false">x</i></h1><h2><i contenteditable="false">x</i><b contenteditable="false">x</b></h2><p>b]</p>',
      output = source.replace(/\[|\]/g, "");
    ...
    checkRangeIteration(
      ...
      ["p", "h1", "h2", "p"],
      output,
      "all blocks are returned in enforceRealBlocks"
    );
  },

  "test iterator returns block with only non-editable content (collapsed selection)":
    function () {
      ...
      checkRangeIteration(
        ...
        ["h1"],
        output,
        "block is returned in enforceRealBlocks"
      );
    },
  ...
  "test iterator returns non-editable blocks": function () {
    ...
    checkRangeIteration(
      ...
      ["p", "div", "p"],
      output,
      "non-editable block is returned"
    );
    checkRangeIteration(
      source,
      { enforceRealBlocks: 1 },
  ...
  "test iterator returns non-editable blocks - single block selected":
    function () {
  ...
  "test iterator returns content between non-editable blocks": function () {
    var source =
        '[<div contenteditable="false">b</div><p>x</p><h1 contenteditable="false">c</h1>]',
      output = source.replace(/\[|\]/g, "");
    ...
    checkRangeIteration(
      ...
      ["div", "p", "h1"],
      output,
      "all blocks were returned"
    );
  },

  "test iterator does not return blocks inside non-editable block":
    function () {
      var source =
          '[<div contenteditable="false"><p>a</p><ul><li>b</li></ul>doNotTouchMe</div>]',
        output = source.replace(/\[|\]/g, "");
      ...
      checkRangeIteration(
        ...
        ["div"],
        output,
        "only non-editable block is returned"
      );
      checkRangeIteration(
        source,
        { enforceRealBlocks: 1 },
  ...
  "test iterator returns blocks inside nested editables": function () {
    ...
    checkRangeIteration(
      ...
      ["div", "h1", "p"],
      output,
      "non-editable element and block in nested editable are returned"
    );
    checkRangeIteration(
      source,
      { enforceRealBlocks: 1 },
  ...
  "test iterator does not return contents of blockless nested editables":
    function () {
      var source =
          '[<div contenteditable="false"><h1 contenteditable="true">a</h1></div>]',
        output = source.replace(/\[|\]/g, "");
  ...
  "test iterator does not stop on inline non-editable elements": function () {
    var source =
        '[<h1>foo</h1><p>x<i contenteditable="false">x</i>x</p><h2>bar</h2>]',
      output = source.replace(/\[|\]/g, "");
    ...
    checkRangeIteration(
      ...
      ["h1", "p", "h2"],
      output,
      "all blocks returned in enforceRealBlocks"
    );
  },

  "test iterator does not stop on inline non-editable elements - only non-editable inline element in paragraph":
    function () {
      var source =
          '[<h1>foo</h1><p><i contenteditable="false">x</i></p><h2>bar</h2>]',
        output = source.replace(/\[|\]/g, "");
  ...
  "test iterator does not leak from nested editable": function () {
    var source =
        '[<div contenteditable="false"><div contenteditable="true"><h1>a</h1></div><p>b</b></div>]',
      output = source.replace(/\[|\]/g, "");
    ...
    checkRangeIteration(
      ...
      ["div", "h1"],
      output,
      "non-editable p was not returned"
    );
    checkRangeIteration(
      source,
      { enforceRealBlocks: 1 },
  ...
  "test iterator returns contents of multiple nested editables": function () {
    ...
    checkRangeIteration(
      ...
      ["div", "h1", "h2"],
      output,
      "all nested editables were found"
    );
    checkRangeIteration(
      source,
      { enforceRealBlocks: 1 },
  ...
  "test nested iterators inherit settings from parent - enforceRealBlocks":
    function () {
      var source =
          '[<div contenteditable="false">' +
  ...
  "test iterating over 2nd level of nested editables": function () {
    ...
    checkRangeIteration(
      ...
      ["div", "h1", "div", "h2", "h3"],
      output,
      "2nd nested editable found once"
    );
    checkRangeIteration(
      source,
      { enforceRealBlocks: 1 },
  ...
  "test activeFilter property when iterating over nested editables":
    function () {
      ...
      var source =
        '[<div contenteditable="false">' +
        '<div contenteditable="true" data-cke-filter="' +
        f1 +
        '"><h1>x</h1></div>' +
      ...
      checkActiveFilter(
        source,
        null,
        ["div", "h1-" + f1, "h2", "h3-" + f2, "h4-" + f2, "p"],
        "active filter is correctly set and unset"
      );
    },

  "test activeFilter property when iterating over 2nd level of nested editables":
    function () {
      var f1 = new CKEDITOR.filter("p b").id,
        f2 = new CKEDITOR.filter("p i").id,
      ...
      checkActiveFilter(
        source,
        null,
        ["div", "p-" + f1, "div-" + f1, "h1-" + f2, "h2-" + f3, "p-" + f1],
        "active filter is correctly set and unset"
      );
    },

  "test iterator does not enter nested editables which ACF does not allow for its block tag":
    function () {
      ...
      checkRangeIteration(
        ...
        ["div", "p"],
        output,
        "block created only in one of editables"
      );
      checkRangeIteration(
        source,
        { enforceRealBlocks: 1 },
  ...
  "test iterator does not leak from range.root": function () {
    ...
    while ((block = iterator.getNextParagraph())) blocks.push(block.getName());

    assert.areSame(
      "div,h1",
      blocks.join(","),
      "nothing else except div+h1 was found"
    );
    assert.areSame(
      '<div class="root">' + output + "</div>",
      bender.tools.compatHtml(sandbox.getHtml(), 1),
