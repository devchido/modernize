...
bender.test(
  appendDomObjectTests(
    function (id) {
      if (id === "domObjectTest1") return new CKEDITOR.dom.document(document);
    ...
    },
    {
      ...
      test_getElementsByTag: function () {
        var nodeList = new CKEDITOR.dom.document(document).getElementsByTag(
            "span"
          ),
          results = [];
      ...
      "test _getHtml5ShivFrag": function () {
        ...
        assert.areSame(
          $frag,
          CKEDITOR.document._getHtml5ShivFrag(),
          "Document fragment is cached"
        );
      },
    }
  )
