...
(function () {
  ...
  bender.test({
    ...
    assertHtmlFragment: function (
      editor,
      innerHtmlWithSelection,
      expectedHtml
    ) {
      var range, clone;
    ...
    test_cloneContents_W3C_1: function () {
      ...
      assert.areSame(
        bodyHtml.replace(/\s+_cke_expando=["\d]+/g, ""),
        document
          .getElementById("playground")
          .innerHTML.replace(/\s+_cke_expando=["\d]+/g, ""),
        "The HTML must remain untouched"
      );

      // The range must also remain unchanged.
      assert.areSame(
        document.getElementById("_Para").firstChild,
        range.startContainer.$,
        "range.startContainer"
      );
      assert.areSame(1, range.startOffset, "range.startOffset");
      assert.areSame(
        document.getElementById("_Para"),
        range.endContainer.$,
        "range.endContainer"
      );
    ...
    test_cloneContents_W3C_2: function () {
      ...
      // The range must also remain unchanged.
      assert.areSame(
        document.getElementById("_B").firstChild,
        range.startContainer.$,
        "range.startContainer"
      );
      assert.areSame(1, range.startOffset, "range.startOffset");
      assert.areSame(
        document.getElementById("_B").nextSibling,
        range.endContainer.$,
        "range.endContainer"
      );
      assert.areSame(2, range.endOffset, "range.endOffset");
    ...
    test_cloneContents_W3C_3: function () {
      ...
      // The range must also remain unchanged.
      assert.areSame(
        document.getElementById("_B").previousSibling,
        range.startContainer.$,
        "range.startContainer"
      );
      assert.areSame(1, range.startOffset, "range.startOffset");
    ...
    test_cloneContents_W3C_4: function () {
      ...
      assert.areSame(
        '<h1 id="_h1">ckw3crange test</h1><p id="_para">this is <b id="_b">some</b> text.</p><p>a</p>',
        getInnerHtml(tmpDiv.$),
        "Cloned HTML"
      );
      ...
      // The range must also remain unchanged.
      assert.areSame(
        document.getElementById("_H1").firstChild,
        range.startContainer.$,
        "range.startContainer"
      );
      assert.areSame(1, range.startOffset, "range.startOffset");
      assert.areSame(
        document.getElementById("playground").lastChild.firstChild,
        range.endContainer.$,
        "range.endContainer"
      );
      assert.areSame(1, range.endOffset, "range.endOffset");
    ...
    test_cloneContents_Other: function () {
      ...
      assert.areSame(
        '<h1 id="_h1">fckw3crange test</h1><p id="_para">this is <b id="_b">some</b> text.</p><p>another paragraph.</p>',
        getInnerHtml(tmpDiv.$),
        "Cloned HTML"
      );
      ...
      // The range must also remain unchanged.
      assert.areSame(
        document.getElementById("_H1"),
        range.startContainer.$,
        "range.startContainer"
      );
      assert.areSame(0, range.startOffset, "range.startOffset");
      assert.areSame(
        document.getElementById("playground").lastChild,
        range.endContainer.$,
        "range.endContainer"
      );
      assert.areSame(1, range.endOffset, "range.endOffset");
    ...
    test_cloneContents_Other_2: function () {
      ...
      assert.areSame(
        '<h1 id="_h1">fckw3crange test</h1><p id="_para">this is <b id="_b">some</b> text.</p>',
        getInnerHtml(tmpDiv.$),
        "Cloned HTML"
      );
    ...
    "test cloneContents does not split text nodes": function () {
      ...
      assert.areSame(
        "foo",
        root.getFirst().getText(),
        "startContainer was not split"
      );
      assert.areSame(
        "bar",
        root.getLast().getFirst().getText(),
        "endContainer was not split"
      );
      assert.areSame(
        startContainer.$,
        range.startContainer.$,
    ...
    "test cloneContents - empty text node is returned if range is at a text boundary":
      function () {
        var root = doc.createElement("div"),
          range = new CKEDITOR.dom.range(doc);
    ...
    "test cloneContents - element selection preceded by a text node":
      function () {
        var root = doc.createElement("div"),
          range = new CKEDITOR.dom.range(doc);
    ...
    "test cloneContents - startOffset == 0, startContainer is element":
      function () {
        var root = doc.createElement("div"),
          range = new CKEDITOR.dom.range(doc);
    ...
    "test cloneContents - startOffset == childCount, startContainer is element":
      function () {
        var root = doc.createElement("div"),
          range = new CKEDITOR.dom.range(doc);
    ...
    "test cloneContents - endOffset == 0, endContainer is element":
      function () {
        var root = doc.createElement("div"),
          range = new CKEDITOR.dom.range(doc);
    ...
    "test cloneContents - endOffset == childCount, endContainer is element":
      function () {
        var root = doc.createElement("div"),
          range = new CKEDITOR.dom.range(doc);
    ...
    "test cloneContents - offsets = ( 0, childCount ), containers are elements":
      function () {
        var root = doc.createElement("div"),
          range = new CKEDITOR.dom.range(doc);
    ...
    "test cloneContents - finding levelClone in the right branch": function () {
      ...
      assert.isInnerHtmlMatching(
        "<b>oo<br />bar</b><i><u>x</u><s>bo</s></i>",
        clone.getHtml()
      );
    },
    ...
    "test cloneContents - element ID handling": function () {
      ...
      assert.isInnerHtmlMatching(
        '<p id="x">f<b id="y">o</b>o</p>',
        cloneWithId.getHtml(),
        "Clone with IDs."
      );
      assert.isInnerHtmlMatching(
        "<p>f<b>o</b>o</p>",
        cloneWithoutId.getHtml(),
        "Clone without IDs."
      );
    ...
    "test cloneContents - inner selection1": function () {
      this.assertHtmlFragment(
        this.editors.classic,
    ...
    "test cloneContents - no gap between": function () {
      ...
      this.assertHtmlFragment(
        ...
        "<strong>bar baz</strong>"
      );
    },
    "test cloneContents - nested elements1": function () {
      this.assertHtmlFragment(
        this.editors.classic,
        "<p>foo <strong>[bar <em>baz</em>]</strong> foo</p>",
        "bar <em>baz</em>"
      );
    },
    "test cloneContents - nested elements2": function () {
      this.assertHtmlFragment(
        this.editors.classic,
        "<p>fo[o <strong>bar <em>b]az</em></strong> foo</p>",
        "o <strong>bar <em>b</em></strong>"
      );
    },
    "test cloneContents - multiple nested elements": function () {
      this.assertHtmlFragment(
        this.editors.classic,
        "<p>fo[o <strong>bar <em>baz</em></strong></p><table><tbody><tr><td>hello</td></tr><tr><td>world</td></tr></tbody></table><p>fo]o</p>",
