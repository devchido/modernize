...
var doc = CKEDITOR.document,
  ...
  removeFillingCharSequenceString =
    CKEDITOR.dom.selection._removeFillingCharSequenceString,
...
function createPlayground(html) {
  ...
  if (fillingCharDummy) {
    ...
    if (
      (fillingCharFollowing = fillingChar.getNext()) &&
      fillingCharFollowing.type == CKEDITOR.NODE_TEXT &&
      fillingCharFollowing.getText()
    ) {
      fillingChar.setText(
        fillingChar.getText() + fillingCharFollowing.getText()
      );
...
function addBookmarkTCs(readyTCs, tcs, tcsGroupName) {
  ...
  for (tcName in tcs) {
    tc = tcs[tcName];
    if (!CKEDITOR.tools.isArray(tc)) addBookmarkTCs(readyTCs, tc, tcName);
    else {
      readyTCs[
        "test " +
          (tcsGroupName ? tcsGroupName + " - " : "") +
          tcName +
          " - createBookmark"
      ] = createBookmarkTC(tc);
    }
...
function createBookmarkTC(tc) {
  return function () {
    ...
    compareRangeWithPattern(
      playground,
      range,
      tc[2],
      "initial range after creating a bookmark"
    );
...
function addBookmark2TCs(readyTCs, tcs, tcsGroupName) {
  ...
  for (tcName in tcs) {
    tc = tcs[tcName];
    if (!CKEDITOR.tools.isArray(tc)) addBookmark2TCs(readyTCs, tc, tcName);
    else {
      readyTCs[
        "test " +
          (tcsGroupName ? tcsGroupName + " - " : "") +
...
function compareRangeWithPattern(playground, range, pattern, msg) {
  assert.areSame(
    findNode(playground, pattern.sc),
    range.startContainer,
    msg + " - startContainer"
  );
  ...
  if (pattern.ec) {
    assert.areSame(
      findNode(playground, pattern.ec),
      range.endContainer,
      msg + " - endContainer"
    );
...
function findNode(container, query) {
  ...
  while ((node = walker.next())) {
    if (
      fcseqQuery &&
      node.getText() == CKEDITOR.dom.selection.FILLING_CHAR_SEQUENCE
    ) {
      return node;
    } else if (
      textQuery &&
      node.type == CKEDITOR.NODE_TEXT &&
      removeFillingCharSequenceString(node.getText()) == textQuery
    ) {
      return node;
    } else if (
      !textQuery &&
...
var tcs = {
  "test util createPlayground": function () {
    ...
    assert.areSame(
      "ab<i>cde</i>",
      bender.tools.compatHtml(playground.getHtml())
    );
...
addBookmarkTCs(tcs, {
  ...
  "element selection": {
    a: [
      "<i>a</i>",
      { sc: "root", so: 0, ec: "root", eo: 1 },
      { sc: "root", so: 1, ec: "root", eo: 2 },
    ],

    b: [
      "x<i>b</i>y",
      { sc: "root", so: 1, ec: "root", eo: 2 },
      { sc: "root", so: 2, ec: "root", eo: 3 },
    ],
    "b 2": [
      "x.y<i>b</i>z",
      { sc: "#y", so: 1, ec: "#z", eo: 0 },
      { sc: "root", so: 3, ec: "root", eo: 4 },
      { sc: "root", so: 2, ec: "root", eo: 3 },
    ],
...
addBookmark2TCs(tcs, {
  ...
  "collapsed in empty text nodes": {
    ...
    "(foo)ab(bar) - range in foo": [
      "(foo)ab(bar)",
      { sc: "(foo)", so: 0 },
      { sc: "root", so: 0 },
    ],
    "(foo)ab(bar) - range in bar": [
      "(foo)ab(bar)",
      { sc: "(bar)", so: 0 },
      { sc: "#ab", so: 2 },
    ],
    "<i>a</i>(foo)(bar)<u>b</u> - range in bar": [
      "<i>a</i>(foo)(bar)<u>b</u>",
      { sc: "(bar)", so: 0 },
      { sc: "root", so: 1 },
    ],
    "(foo)<i>a</i> - range in foo": [
      "(foo)<i>a</i>",
      { sc: "(foo)", so: 0 },
      { sc: "root", so: 0 },
    ],
  ...
  "collapsed in element": {
    ...
    "abcdef 1": [
      "<b>(foo)<i>def</i></b>",
      { sc: "b", so: 1, ec: "#def", eo: 1 },
      { sc: "b", so: 0, ec: "#def", eo: 1 },
    ],
    // <b>(foo)(bar)[<i>d]ef</i></b>
    "abcdef 2": [
      "<b>(foo)(bar)<i>def</i></b>",
      { sc: "b", so: 2, ec: "#def", eo: 1 },
      { sc: "b", so: 0, ec: "#def", eo: 1 },
    ],
  ...
  "text selection": {
    ...
    "#10301 1": [
      "<p>a.b<i>c</i>d.e</p>",
      { sc: "#e", so: 0, ec: "#e", eo: 1 },
      { sc: "#de", so: 1, ec: "#de", eo: 2 },
    ],
    "#10301 2": [
      "<p>a.b<i>c</i>d.e</p>",
      { sc: "p", so: 4, ec: "p", eo: 5 },
      { sc: "#de", so: 1, ec: "p", eo: 3 },
    ],
  },
  ...
  "filling character sequence": {
    "tc 1": [
      "<p>abc</p><p><b>%def</b></p>",
      { sc: "#abc", so: 1, ec: "#def", eo: FCSLength + 2 },
      { sc: "#abc", so: 1, ec: "#def", eo: 2 },
    ],
    "tc 2": [
      "%abc<b>def</b>",
      { sc: "#abc", so: FCSLength + 1, ec: "#def", eo: 1 },
      { sc: "#abc", so: 1, ec: "#def", eo: 1 },
    ],
    "tc 2b": [
      "%<b>def</b>",
      { sc: "root", so: 1, ec: "#def", eo: 1 },
      { sc: "root", so: 0, ec: "#def", eo: 1 },
    ],
    "tc 2c": ["%<b>def</b>", { sc: "%", so: FCSLength }, { sc: "root", so: 0 }],
    "tc 2d": ["%<b>def</b>", { sc: "%", so: 0 }, { sc: "root", so: 0 }],
    "tc 3": [
      "<b>a</b>%<b>c</b>d<i>e</i>",
      { sc: "root", so: 2, ec: "#e", eo: 1 },
      { sc: "root", so: 1, ec: "#e", eo: 1 },
    ],
    "tc 4": [
      "<b>%<i>abc</i></b>",
      { sc: "b", so: 1, ec: "b", eo: 2 },
    ...
    "tc 5h": [
      "<b>%</b>",
      { sc: "b", so: 1, ec: "root", eo: 1 },
      { sc: "b", so: 0, ec: "root", eo: 1 },
    ],
    "tc 6": [
      "<b>%(foo)<i>def</i></b>",
      { sc: "b", so: 2, ec: "#def", eo: 1 },
    ...
    "tc 6c": [
      "<b>%(foo)(bar)<i>def</i></b>",
      { sc: "(bar)", so: 0, ec: "#def", eo: 1 },
      { sc: "b", so: 0, ec: "#def", eo: 1 },
    ],
    // between foo and bar.
    "tc 6d": [
      "<b>abc%(foo)(bar)cba<i>def</i></b>",
      { sc: "b", so: 3, ec: "#def", eo: 1 },
      { sc: "#abccba", so: 3, ec: "#def", eo: 1 },
    ],
  },
});
