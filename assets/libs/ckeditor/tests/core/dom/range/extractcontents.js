...
(function () {
  ...
  var tests = {
    ...
    test_extractContents_W3C_2: function () {
      ...
      assert.areSame(
        'this is <b id="_b">s</b>ext.',
        getInnerHtml("_Para"),
        "HTML after extraction"
      );
    ...
    test_extractContents_W3C_4: function () {
      ...
      assert.areSame(
        '<h1 id="_h1">f</h1><p>nother paragraph.</p>',
    ...
    test_extractContents_Other_5: function () {
      ...
      range.setStartAfter(
        new CKEDITOR.dom.element(
          document.getElementById("playground").getElementsByTagName("i")[0]
        )
      );
      range.setEndAfter(
        new CKEDITOR.dom.element(
          document.getElementById("playground").getElementsByTagName("b")[0]
        )
      );
    ...
    "test extractContents - empty containers at different level": function () {
      ...
      assert.isInnerHtmlMatching(
        "<div><h1></h1></div><p>foo</p><div></div>",
        clone.getHtml()
      );
      assert.isInnerHtmlMatching(
        "x<div><h1></h1></div>[]<div>x</div>y",
        bender.tools.range.getWithHtml(root, range)
      );
    },
    ...
    "test extractContents - empty containers with mergeThen": function () {
      ...
      assert.isInnerHtmlMatching(
        "<b>*</b><p>foo</p><b>*</b>",
        clone.getHtml().replace(/<b><\/b>/g, "<b>*</b>")
      );
      assert.isInnerHtmlMatching(
        "x<b>[]a</b>y",
        bender.tools.range
          .getWithHtml(root, range)
          .replace(/<b><\/b>/g, "<b>*</b>")
      );
    },
    ...
    "test extractContents - ID attribute cloning of partially and fully selected elements":
      function () {
        var range = new CKEDITOR.dom.range(doc);
        ...
        assert.isInnerHtmlMatching(
          '<h1>Test</h1><p>This is <b id="_B">some</b> t</p>',
          tmpDiv.getHtml(),
          "Extracted HTML"
        );
        assert.isInnerHtmlMatching(
          '<h1 id="_H1">FCKW3CRange</h1><p id="_Para">ext.</p><p>Another paragraph.</p>',
          playground.getHtml(),
          "HTML after extraction"
        );
    ...
    "test extract nested tags": function () {
      ...
      assert.isInnerHtmlMatching(
        '111<strong>222<span>333<em>444</em></span>555<sup>666</sup>777<sub id="_sub">888</sub></strong>',
        docFrag.getHtml()
      );
    },

    // #644
    "test extractContents and cloneContents provides equal results":
      function () {
        var range = new CKEDITOR.dom.range(doc),
          extractFrag,
          cloneFrag;
