...
(function () {
  ...
  function assertElements(expected, range, selectors) {
    if (!expected) {
      ...
      assert.isTrue(
        expected.equals(range._getTableElement(selectors)),
  ...
  bender.test({
    ...
    "get row (collapsed selection inside with multiple selectors passed)":
      function () {
        var range, cell, row;
    ...
    "multi-table range": function () {
      ...
      range = createRange({
        start: startCell.getChild(0),
        startOffset: 2,
        end: endCell.getChild(0),
        endOffset: 3,
      });
