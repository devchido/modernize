...
(function () {
  ...
  function assertNodesList(wanted, nodes) {
    ...
    for (var i = 0; i < nodes.length; i++)
      simplifiedNodes.push(
        nodes[i].type == CKEDITOR.NODE_TEXT
          ? nodes[i].getText()
          : "<" + nodes[i].getName() + ">"
      );
  ...
  bender.test({
    ...
    test_next_1: function () {
      ...
      var nodes = [];
      while ((node = walker.next())) nodes.push(node);

      assertNodesList(
        ["<p>", "This is ", "<b>", "a ", "<i>", "simple", " test"],
        nodes
      );
    },
    ...
    test_next_7: function () {
      ...
      var nodes = [];
      while ((node = walker.previous())) nodes.push(node);
    ...
    "test walker.invisible() - br followed by bogus br": function () {
      assert.isTrue(
        CKEDITOR.dom.walker.invisible()(doc.getById("brFollowedByBogus"))
      );
    },
    ...
    "test walker.invisible() - nbsp": function () {
      assert.isFalse(
        CKEDITOR.dom.walker.invisible()(doc.getById("nbsp").getFirst())
      );
    },

    "test walker.invisible() - whitespaces in empty inline element surrounded by text":
      function () {
        assert.isTrue(
          CKEDITOR.dom.walker.invisible()(doc.getById("wsInline").getFirst())
        );
      },

    "test walker.invisible() - whitespaces in empty inline element surrounded by whitespaces":
      function () {
        // IE8 loses empty text nodes when parsing HTML.
        if (CKEDITOR.env.ie && CKEDITOR.env.version < 9) assert.ignore();

        assert.isTrue(
          CKEDITOR.dom.walker.invisible()(doc.getById("wsInline2").getFirst())
        );
      },

    "test walker.invisible() - empty inline element": function () {
      assert.isTrue(
        CKEDITOR.dom.walker.invisible()(doc.getById("emptyInline"))
      );
    },

    "test walker.invisible() - inline element containing whitespaces, surrounded by text":
      function () {
        assert.isFalse(
          CKEDITOR.dom.walker.invisible()(doc.getById("wsInline"))
        );
      },

    "test walker.invisible() - inline element containing whitespaces, surrounded by whitespaces":
      function () {
        assert.isTrue(
          CKEDITOR.dom.walker.invisible()(doc.getById("wsInline2"))
        );
      },
    ...
    "test walker.invisible() - non empty inline element": function () {
      assert.isFalse(
        CKEDITOR.dom.walker.invisible()(
          doc.getById("nonEmptyInline").getFirst()
        )
      );
    },

    "test walker.invisible() - inline element containing a ZWS character":
      function () {
        assert.isTrue(
          CKEDITOR.dom.walker.invisible()(doc.getById("zwsInline"))
        );
      },

    "test walker.invisible() - block with a bogus br only": function () {
      assert.isFalse(
        CKEDITOR.dom.walker.invisible()(doc.getById("filledBlock"))
      );
    },
    ...
    "test walker.temp": function () {
      ...
      var node1 = new CKEDITOR.dom.text("foo"),
        node2 = CKEDITOR.dom.element.createFromHtml(
          '<div data-cke-temp="1">foo</div>'
        ),
    ...
    "test walker.ignored": function () {
      ...
      var node1 = new CKEDITOR.dom.text("foo"),
        ...
        node3 = CKEDITOR.dom.element.createFromHtml(
          '<div data-cke-temp="1">foo</div>'
        ),
        node4 = CKEDITOR.dom.element.createFromHtml(
    ...
    "test walker.empty": function () {
      ...
      var node1 = CKEDITOR.dom.element.createFromHtml("<img>"),
        node2 = CKEDITOR.dom.element.createFromHtml(
          '<div><div data-cke-temp="1">foo</div></div>'
        ),
        node3 = CKEDITOR.dom.element.createFromHtml(
          '<div data-cke-temp="1">foo</div>'
        ),
        ...
        node5 = CKEDITOR.dom.element.createFromHtml(
          '<span data-cke-bookmark="1">&nbsp;</span>'
        ),
    ...
    "test walker.editable": function () {
      ...
      var node1 = new CKEDITOR.dom.text("foo"),
        ...
        node5 = CKEDITOR.dom.element.createFromHtml("<p>foo</p>"),
        node6 = CKEDITOR.dom.element.createFromHtml(
          '<p contenteditable="false">foo</p>'
        ),
        node7 = CKEDITOR.dom.element.createFromHtml(
        ...
        node9c = CKEDITOR.dom.element
          .createFromHtml("<table><tr><td></td></tr></table>")
          .findOne("td"),
        node9d = CKEDITOR.dom.element
          .createFromHtml("<table><tr><caption></caption></tr></table>")
          .findOne("caption"),
    ...
    "test walker.bogus": function () {
      ...
      var node1 = CKEDITOR.dom.element.createFromHtml("<div>foo</div>"),
        ...
        node5 = CKEDITOR.dom.element.createFromHtml(
          "<figure><figcaption>foo</figcaption></figure>"
        ),
    ...
    "test element#isBlockBoundary": function () {
      ...
      assert.isFalse(
        doc.getById("bbf1").isBlockBoundary(),
        "floated block span"
      );
      assert.isFalse(
        doc.getById("bbf2").isBlockBoundary(),
        "absolutely positioned span"
      );
      assert.isFalse(
        doc.getById("bbf3").isBlockBoundary(),
        "inline-block span"
      );
