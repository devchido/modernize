...
(function () {
  ...
  bender.test({
    ...
    "test getHtml": function () {
      ...
      assert.areSame(
        "<b>foo</b><i>bar</i>",
        bender.tools.fixHtml(frag.getHtml(), 1, 1),
        "HTML of documentFragment"
      );
    },
    ...
    "test getHtml with html5": function () {
      // IE8 only.
      if (!CKEDITOR.env.ie || CKEDITOR.env.version > 8) assert.ignore();

      var frag = new CKEDITOR.dom.documentFragment(CKEDITOR.document);

      CKEDITOR.dom.element
        .createFromHtml("<figure>foo</figure>")
        .appendTo(frag);
    ...
    "test clone with children": function () {
      ...
      assert.areSame(
        "<b>foo</b>",
        bender.tools.fixHtml(clone.getChild(0).getOuterHtml())
      );
      assert.areSame(
        "<i>bar</i>",
        bender.tools.fixHtml(clone.getChild(1).getOuterHtml())
      );
    },
