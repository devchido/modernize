...
bender.test({
  "test destroying editor when iframe is removed": function () {
    ...
    assert.isFalse(
      destroySpy.threw(),
      "Editor.destroy() method should not throw an exception when iframe is already removed."
    );
    assert.isTrue(warnSpy.calledOnce, "CKEDITOR.warn should be called once.");
    assert.areEqual(
      "editor-destroy-iframe",
      warnSpy.firstCall.args[0],
      'CKEDITOR.warn function should be called with error code: "editor-destroy-iframe".'
    );
  },
});
