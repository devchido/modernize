...
function doTestProcessorAttributeIsPreserved(bot) {
  ...
  assert.areSame(
    '<p><span data-foo="' + encodedAttrValue + '">foo</span></p>',
    data,
    "attribute was properly encoded when getting data"
  );
  ...
  bot.setData(data, function () {
    ...
    assert.areSame(
      attrValue,
      span.data("foo"),
      "attribute was properly preserved when loading data"
    );
  });
}

bender.test({
  "test inline editor with htmlwriter preserves attribute set in htmlFilter":
    function () {
      doTestProcessorAttributeIsPreserved(bender.editorBots.htmlWriter);
    },

  "test inline editor with basicwriter preserves attribute set in htmlFilter":
    function () {
