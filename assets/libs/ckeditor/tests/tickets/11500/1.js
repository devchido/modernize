...
(function () {
  ...
  bender.test({
    "test selection and focus are not lost in editor A when setting data in editor B":
      function () {
    ...
    "test selection and focus are not lost in editor A when setting data in editor B which had locked selection":
      function () {
        var bots = this.editorBots;
        ...
        bots.inline2.setData("<p>x1</p>", function () {
          ...
          assert.areSame(
            "bar",
            sel.getSelectedText(),
            "selection after 1st setData"
          );
          assert.isTrue(
            bots.inline1.editor.editable().hasFocus,
            "focus after 1st setData"
          );
          ...
          bots.inline2.setData("<p>x2</p>", function () {
            ...
            assert.areSame(
              "bar",
              sel.getSelectedText(),
              "selection after 2nd setData"
            );
            assert.isTrue(
              bots.inline1.editor.editable().hasFocus,
              "focus after 2nd setData"
            );
          });
        });
      },
