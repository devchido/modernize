...
bender.test({
  assertEditor: function (nameOrObject, name) {
    ...
    assert.areSame(
      $("#" + name).get(0),
      nameOrObject.element.$,
      "Instance element should match."
    );
  },
  ...
  assertEditorMode: function (editorName, mode) {
    assert.areSame(
      mode,
      CKEDITOR.instances[editorName].elementMode,
      editorName + " should be created by replace."
    );
  },
  ...
  assertEditorInstances: function (expected) {
    arrayAssert.itemsAreEqual(
      expected,
      CKEDITOR.tools.object.keys(CKEDITOR.instances),
      "Editors instances list should match with expected."
    );
  },
  ...
  destroyAll: function () {
    ...
    for (var i in instances) instances[i].destroy();
  },
  ...
  "test empty collection returns this": function () {
    assert.areSame(
      0,
      $("doesntExist").ckeditor().length,
      "Adapter should return 'this' when element does not exist."
    );
  },
  ...
  "test wrong type of element": function () {
    ...
    assert.isTrue(
      failed,
      "Error object must be thrown because this element is invalid."
    );
  },
    ...
    $("#editor7")
      ...
      .editor.on("instanceReady", function () {
        resume(function () {
          assert.areSame(
            "#9AB8F3",
            $("#editor7").ckeditor().editor.config.uiColor,
  ...
  "test config + callback": function () {
    $("#editor8").ckeditor({ uiColor: "#9AB8F3" }, function () {
      resume(function () {
        assert.areSame(
          "#9AB8F3",
          $("#editor8").ckeditor().editor.config.uiColor,
          "Configuration should be set."
        );
      });
    });
    ...
    $("#editor9").ckeditor(
      function () {
        resume(function () {
          assert.areSame(
            "#9AB8F3",
            $("#editor9").ckeditor().editor.config.uiColor,
            "Configuration should be set."
          );
        });
      },
  ...
  "test ckeditor().editor": function () {
    ...
    assert.isUndefined(
      $("#editor12").editor,
      "$( selector ).editor should be undefined"
    );
      ...
      .promise.done(function () {
        resume(function () {
          arrayAssert.itemsAreSame(
            ["editor15a", "editor15b"],
            ready.sort(),
            "Editor15a and Editor15b should be ready."
          );
        });
      });
