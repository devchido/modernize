...
bender.test({
      ...
      .on("instanceReady.ckeditor", function (event, editor) {
        resume(function () {
          assert.areSame(
            "instanceReady",
            event.type,
            "Event type should be 'instanceReady'."
          );
          assert.areSame(
            "ckeditor",
            event.namespace,
            "Event namespace should be 'ckeditor'."
          );
          assert.areSame(
            CKEDITOR.instances.instanceReadyEditor.name,
            editor.name,
            "Given editor should match."
          );
        });
      });
      ...
      $("#beingCreated").ckeditor(function () {
        resume(function () {
          assert.areSame(
            1,
            callCount,
            "InstanceReady callback for the editor being created cannot be called more than once."
          );
        });
      });
    });
  ...
  "test event getData": function () {
    var oldData, type, namespace, editorName;
    ...
    $("#editor")
      ...
      .done(function () {
        resume(function () {
          assert.areSame(
            BAR,
            bender.tools.compatHtml($("#editor").val()),
            "Data should be modified."
          );
          assert.areSame(
            FOO,
            bender.tools.compatHtml(oldData),
            "Event was carrying data"
          );
          ...
          assert.areSame(
            "editor",
            editorName,
            "Event was fired on correct editor"
          );
        });
      });
  ...
  "test event destroy": function () {
    ...
    $("#destroyEditor")
      .ckeditor()
      .on("destroy.ckeditor", function (event) {
        resume(function () {
          assert.areSame(
            "destroyEditor",
            instantiatedEditor,
