...
bender.test({
  ...
  checkWithoutEditor: function (elements) {
    ...
    assert.isString(
      jqueryObj.jquery,
      "jQuery object should be returned by val() when collection is not containing editors."
    );
  ...
  setUp: function () {
    ...
    for (var i in instances) CKEDITOR.instances[i].destroy();
  ...
  "test value with val()": function () {
      ...
      this.setData(FOO, function () {
        ...
        resume(function () {
          assert.areSame(
            FOO,
            value,
            "Returned value should be the same as set."
          );
    ...
    $("#editorGetValue")
      .ckeditor()
      .on("getData.ckeditor", function (event, editor, data) {
        ++eventCounter;
  ...
  "test val() of multiple editors (https://dev.ckeditor.com/ticket/7876)":
    function () {
      $(".editorMultipleGet").ckeditor();

      $.when(
        $("#editorMultipleGetA").val(FOO),
        $("#editorMultipleGetB").val(BAR)
      ).then(function () {
        resume(function () {
          assert.areSame(
            FOO,
            bender.tools.compatHtml($(".editorMultipleGet").val()),
            "The value returned by val() called on multiple objects should return first result."
          );
        });
      });
  ...
  "test use val() as called with undefined (https://dev.ckeditor.com/ticket/9019)":
    function () {
      $("#editorUndefined").ckeditor();

      assert.isFunction(
        $("#editorUndefined").val(undefined).done,
        "val( undefined ) should be treated as setter and return promise."
      );
    },
});
