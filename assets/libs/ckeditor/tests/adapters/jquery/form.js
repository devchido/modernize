...
function assertTextareaValue(id, value, msg) {
  assert.areSame(
    value,
    bender.tools.compatHtml(CKEDITOR.document.getById(id).getValue()),
    msg
  );
}
...
bender.test({
            ...
            beforeSubmit: function () {
              resume(function () {
                assertTextareaValue(
                  "editor-inside",
                  FOO,
                  "updateElement method should be called on submit and value should change."
                );
              });
              return false;
            },
    ...
    $("#form-save").ajaxForm(function () {
      resume(function () {
        assertTextareaValue(
          "editor-save",
          FOO,
          "ajaxForm method should be called when command 'save' is executed (if jQuery and jQuery.form are included)."
        );
      });
    });
  ...
  "test overwrite save button with adapter": function () {
    $("#editor-overwrite-save")
      .ckeditor()
      .editor.on("save", function () {
        resume(function () {
          assert.isTrue(
            true,
            "This method should be called instead of ajaxForm."
          );
        });
        return false;
      });
    ...
    $("#form-overwrite-save").ajaxForm(function () {
      resume(function () {
        assert.fail(
          "Save button should be overwritten and this method shouldn't be called"
        );
      });
    });
