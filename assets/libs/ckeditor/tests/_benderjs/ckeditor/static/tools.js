...
(function (bender) {
  ...
  function browserHtmlFix(html) {
    if (
      ...
      html = html.replace(/href="(.*?)#(.*?)"/gi, function (m, base, anchor) {
  ...
  bender.tools = {
    ...
    ignoreUnsupportedEnvironment: function (pluginName, editor) {
      ...
      function ignoreUnsupportedEnvironment() {
        var plugin = editor
          ? editor.plugins[pluginName]
          : CKEDITOR.plugins.registered[pluginName];
    ...
    getInnerHtml: function (elementOrId, stripLineBreaks) {
      ...
      if (typeof elementOrId == "string") {
        ...
        html =
          elementOrId.innerHTML || // retrieve from innerHTML
    ...
    fixHtml: function (html, stripLineBreaks, toLowerCase) {
      ...
      html = html.replace(
        ...
        function (match, start, body, end) {
          ...
          body = body.replace(
            /\s([^\s=]+)=((?:"[^"]*")|(?:'[^']*')|(?:[^\s]+))/g,
            function (match, attName, attValue) {
              if (attName == "style") {
    ...
    compatHtml: function (
      ...
      if (fixStyles) {
        html = html.replace(/ style="([^"]+)"/g, function (match, style) {
          style = CKEDITOR.tools.writeCssText(
            CKEDITOR.tools.parseCssText(style, true),
            true
          );
          // Encode e.g. "" in urls().
    ...
    getAttribute: function (element, attrName) {
      ...
      if (attrName == "style") {
        ...
        return retval
          .replace(/(?:^|;)\s*([A-Z-_]+)(:\s*)/gi, function (match, property) {
    ...
    getValueAsHtml: function (id) {
      return bender.tools.fixHtml(
        CKEDITOR.document.getById(id).getValue(),
        false
      );
    },
    ...
    testInputOut: function (playground, fn, trimSelection) {
      ...
      var source = bender.tools.getValueAsHtml(playground).split("=>"),
        input = source[0],
        output = /\(no change\)/.test(source[1])
          ? trimSelection
            ? input.replace(/\^|\[|\]/g, "")
            : input
          : source[1];
    ...
    setHtmlWithSelection: function (editorOrElement, html, root) {
      ...
      root = isEditor
        ...
        : root instanceof CKEDITOR.dom.document
          ? root.getBody()
          : root || CKEDITOR.document.getBody();
    ...
    getHtmlWithSelection: function (editorOrElement, root) {
      ...
      var html = browserHtmlFix(
        isEditor ? editorOrElement.getData() : element.getHtml()
      );
      html = html.replace(
        /<span\b[^>]*?id="?cke_bm_\d+(\w)"?\b[^>]*?>.*?<\/span>/gi,
        replaceWithBookmark
      );
        ...
        if (node.type == CKEDITOR.NODE_COMMENT) {
          ...
          if (marker) {
            ...
            range[
              marker == "["
                ...
                : marker == "]"
                  ? "setEndAt"
                  : "moveToPosition"
            ](node, CKEDITOR.POSITION_BEFORE_START);
            ...
            for (var i = 0, bm; i < toRemove.length; i++) {
              ...
              if (
                bm.getParent().equals(range.startContainer) &&
              ...
              if (
                bm.getParent().equals(range.endContainer) &&
    ...
    mockNativeDataTransfer: function () {
      return {
        ...
        getData: function (type) {
          ...
          if (
            typeof this._data[type] === "undefined" ||
    ...
    mockPasteEvent: function () {
      ...
      return {
        $: {
          ctrlKey: true,
          clipboardData:
            CKEDITOR.env.ie && CKEDITOR.env.version < 16
              ? undefined
              : dataTransfer,
        },
        preventDefault: function () {
      ...
      for (
        ...
        for (var testName in inputTests) {
          ...
          if (outputTests[specificTestName]) {
            throw new Error(
              'Test named "' + specificTestName + '" already exists'
            );
          }

          // We are creating new editor instance to isolate each test case (#1696).
          if (isolateTests && editorCounter++) {
            outputTests[specificTestName] = (function (
              testName,
              editorName,
              editorNum
            ) {
              var options = CKEDITOR.tools.object.merge(
                bender.editors[editorName],
                {
  ...
  bender.tools.range = {
    ...
    setWithHtml: (function () {
      ...
      function evaluator(node) {
        if (node.type == CKEDITOR.NODE_TEXT) {
          ...
          if (
            (text = node.getText().replace("{}", replaceTextMarker)) &&
            markerFound == "{}"
          ) {
          ...
          if (
            (text = text.replace("{", replaceTextMarker)) &&
            markerFound == "{"
          ) {
            range = new CKEDITOR.dom.range(root);
            range.setStart(node, markerIndex);
    ...
    getWithHtml: (function () {
      ...
      return function (element, range) {
        ...
        return browserHtmlFix(clone.getHtml()).replace(
          selectionMarkerComments,
          "$1"
        );
      };
    })(),
    ...
    getWithHtml: function (editor) {
      ...
      if (ranges.length > 1) {
        throw new Error(
          "There are " +
            ranges.length +
            " ranges in editor's selection, while only one was expected."
        );
      }
  ...
  bender.tools.html = {
    ...
    prepareInnerHtmlForComparison: function (innerHtml, options) {
      ...
      var sortAttributes =
          "sortAttributes" in options ? options.sortAttributes : true,
      ...
      innerHtml = bender.tools.compatHtml(
        innerHtml,
        options.noInterWS,
        sortAttributes,
        fixZWS,
