...
(function (window, bender) {
  ...
  bender.assert.isMatching = function (expected, actual, message, reversed) {
    ...
    if (
      ...
      !!actual.match(expected) !== desiredMatchResult
    ) {
      throw new YTest.ComparisonFailure(
        YTest.Assert._formatMessage(
          message,
  ...
  bender.assert.isNotMatching = function (expected, actual, message) {
    this.isMatching(
      expected,
      actual,
      message || "Value can not match the pattern.",
      true
    );
  };
   ...
   * @param {String} [message]
   */
  bender.assert.isInnerHtmlMatching = function (
    expected,
    actual,
    options,
    message
    ...
    function expectedToString(expected) {
      ...
      if (typeof expected === "object") {
        for (i = 0; i < expected.length; i++) {
          strings.push(
            bender.tools.html.prepareInnerHtmlPattern(expected[i]).toString()
          );
        }
        return "\n" + strings.join("\n");
      } else {
  ...
  bender.assert.isNumberInRange = function (expected, min, max, message) {
    ...
    if (min >= max) {
      throw new YUITest.AssertionError(
        "Min value is greater or equal than max."
      );
    }
  ...
  bender.assert.sameData = function (expected, actual, message) {
    assert.areSame(
      expected,
      bender.tools.compatHtml(actual, false, true, false, true, true),
      message
    );
  };
  ...
  YTest.Runner._ignoreTest = function (node) {
    ...
    function updateResult(testNode, testName) {
      testNode.results[testName] = {
        ...
        type: "test",
        name:
          testName.indexOf("ignore:") === 0 ? testName.substring(7) : testName,
      };
    ...
    try {
      ...
      if (thrown instanceof YUITest.Ignore) {
        ...
        if (!shouldError) {
          ...
          typeof shouldError == "function" &&
          !(thrown instanceof shouldError)
        ) {
          error = new YUITest.UnexpectedError(thrown);
          failed = true;
        } else if (
          (typeof shouldError == "object" &&
            shouldError !== null &&
  ...
  bender.configureEditor = function (config) {
    ...
    removePlugins =
      config["remove-plugins"] || (config.remove && config.remove.plugins);
    ...
    if (config.adapters) {
      for (i = 0; i < config.adapters.length; i++) {
        config.adapters[i] =
          CKEDITOR.basePath + "adapters/" + config.adapters[i] + ".js";
      }
    ...
    function start() {
      ...
      if (bender.regressions) {
        for (var name in bender.regressions) {
          bender.regressions[name] = bender.regressions[name].replace(
            /env/g,
            "CKEDITOR.env"
          );
        }
  ...
  bender.getAbsolutePath = function (path) {
    ...
    if (path.indexOf("://") == -1 && path.indexOf("/") !== 0) {
      ...
      suffix =
        suffixIndex == -1 ? "" : path.substring(suffixIndex, path.length);
