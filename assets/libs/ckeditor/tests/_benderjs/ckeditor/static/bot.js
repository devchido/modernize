...
(function (bender) {
  ...
  bender.editorBot.create = function (profile, callback) {
    ...
    element =
      CKEDITOR.document.getById(name) ||
      CKEDITOR.document
        .getBody()
        .append(
    ...
    if (creator == "replace" && !element.is("textarea")) {
      throw new Error(
        "[EditorBot:" +
          name +
          "] Are you sure you want to replace element with a classic editor?"
      );
    }

    if (typeof profile.startupData == "string") {
      element[element.is("textarea") ? "setValue" : "setHtml"](
        profile.startupData
      );
    }
    ...
    CKEDITOR.once(
      ...
      function (event) {
        ...
        if (
          ...
          writer.attribute = CKEDITOR.tools.override(
            writer.attribute,
            function (org) {
    ...
    config.extraPlugins =
      (creator == "replace" || creator == "append"
        ? "wysiwygarea"
        : "floatingspace") +
      "," +
      (config.extraPlugins || "");
  ...
  bender.editorBot.createAsync = function (profile) {
    return new CKEDITOR.tools.promise(function (resolve, reject) {
      ...
      profile.ignoreEditorWaits =
        profile.ignoreEditorWaits === undefined
          ? true
          : profile.ignoreEditorWaits;
  ...
  function menuOrPanel(isPanel) {
    return function (name, callback) {
      ...
      bender.tools.fireElementEventHandler(
        btnEl,
        CKEDITOR.env.ie ? "onmouseup" : "onclick",
        {
  ...
  bender.editorBot.prototype = {
    ...
    contextmenu: function (callback) {
      ...
      if (
        !editor.readOnly &&
        editor.getSelection().getType() === CKEDITOR.SELECTION_NONE
      ) {
    ...
    execCommand: function () {
      ...
      if (
        this.editor.focusManager.hasFocus &&
