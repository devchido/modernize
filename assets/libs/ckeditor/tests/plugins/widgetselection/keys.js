...
(function () {
  ...
  function testKeyCombination(eventHost, eventData, callCount) {
    ...
    setTimeout(function () {
      resume(function () {
        ...
        assert.areSame(
          callCount,
          addFillersStub.callCount,
          "addFillers call count"
        );
      });
    }, 50);
  ...
  bender.test({
    ...
    "test ctrl + alt + a key combination": function () {
      ...
      testKeyCombination(
        editor.editable(),
        { keyCode: 65, ctrlKey: true, altKey: true },
        0
      );
    },

    // (#1719)
    "test ctrl + a in the same document but outside of the editable":
      function () {
        var eventData = {
            keyCode: 65,
            ctrlKey: true,
        ...
        this.editorBots.divarea.setHtmlWithSelection(
          "<ul><li>Sample text</li></ul><p>Paragraph</p><ul><li>Sample ^text</li></ul>"
        );
