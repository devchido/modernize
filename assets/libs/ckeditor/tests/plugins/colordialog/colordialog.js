...
(function () {
  ...
  bender.test({
    "test colordialog add hash to color's values with 6 hexadecimal digits":
      function () {
        assertColor(this.editor, "123456", "#123456");
      },

    "test colordialog add hash to color's values with 3 hexadecimal digits":
      function () {
        assertColor(this.editor, "FDE", "#FDE");
      },

    "test colordialog does not add hash to color value with 1 digit (incorrect css color value)":
      function () {
    ...
    // (#2639)
    "test colordialog setting text color by clicking on UI buttons":
      function () {
        openDialogManually(
          this.editor,
          "#ff0000",
          '<h1>[<span style="color:#ff0000">Foo</span>]</h1>',
          "TextColor"
        );
      },

    // (#2639)
    "test colordialog setting background color by clicking on UI buttons":
      function () {
        openDialogManually(
          this.editor,
          "#0000ff",
          '<h1>[<span style="background-color:#0000ff">Foo</span>]</h1>',
          "BGColor"
        );
      },
    ...
    "test text color of mixed selection": function () {
      openDialogManually(
        this.editor,
        "",
        '<h1>[<span style="color:#0000ff">Foo</span>bar]</h1>',
        "TextColor"
      );
    },
    ...
    "test background color of mixed selection": function () {
      openDialogManually(
        this.editor,
        "",
        '<h1>[<span style="background-color:#0000ff">Foo</span>bar]</h1>',
        "BGColor"
      );
    },
  });
