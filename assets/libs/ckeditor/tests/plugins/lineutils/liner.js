  ...
  bender.test({
    ...
    "test showLine": function () {
      ...
      assert.areSame(
        liner.visible[uid],
        line,
        "Line properly added to visible."
      );
    ...
    "test hideLine": function () {
      ...
      assert.isUndefined(
        liner.visible[uid],
        "Line properly removed from visible."
      );
    ...
    "test hideVisible": function () {
      ...
      assert.isTrue(
        CKEDITOR.tools.isEmpty(liner.visible),
        "Line removed from visible."
      );
      assert.isFalse(line2.isVisible(), "Line hidden in DOM.");
      assert.areSame(
        2,
        CKEDITOR.tools.object.keys(liner.hidden).length,
        "Line moved to hidden."
      );
    },

    // https://dev.ckeditor.com/ticket/12812
    "test the constructor does not access window's parent frame in case of inline editor":
      function () {
        bender.editorBot.create(
          ...
          function (bot) {
            ...
            assert.isFalse(
              spy.called,
              "the editor.window.getFrame() was not called"
            );
    ...
    "test relative getClientRect": function () {
      ...
      assert.areSame(
        bodyComputedPosition.x,
        linerClientRect.relativeX,
        "Rect relativeX shift is stored."
      );
      assert.areSame(
        bodyComputedPosition.y,
        linerClientRect.relativeY,
        "Rect relativeY shift is stored."
      );

      assert.areSame(
        defaultClientRect.left,
        linerClientRect.left + bodyComputedPosition.x,
        "Rect left is shifted."
      );
      assert.areSame(
        defaultClientRect.right,
        linerClientRect.right + bodyComputedPosition.x,
        "Rect right is shifted."
      );
      assert.areSame(
        defaultClientRect.top,
        linerClientRect.top + bodyComputedPosition.y,
        "Rect top is shifted."
      );
      assert.areSame(
        defaultClientRect.bottom,
        linerClientRect.bottom + bodyComputedPosition.y,
        "Rect bottom is shifted."
      );
      assert.areSame(
        defaultClientRect.width,
        linerClientRect.width,
        "Rect width remains."
      );
      assert.areSame(
        defaultClientRect.height,
        linerClientRect.height,
        "Rect height remains."
      );
    },
  });
