...
(function () {
  ...
  function assertLookupElements(elements, msg, sort) {
    arrayAssert.itemsAreSame(
      elements,
      sort ? lookupEls1.sort() : lookupEls1,
      "Lookup #1: " + msg
    );
    arrayAssert.itemsAreSame(
      elements,
      sort ? lookupEls2.sort() : lookupEls2,
      "Lookup #2: " + msg
    );
  }
  ...
  bender.test({
    "test lookup functions": traverseTest({
      ...
      assert: function () {
        assertLookupElements(
          ["span", "p", "div"],
          "Lookup visited only one branch of DOM."
        );
      },
    }),
    ...
    "test lookup with limits (nested editable)": traverseTest({
      ...
      assert: function () {
        assertLookupElements(
          ["span", "p"],
          "Lookup stopped once reached the limit."
        );
      },
    }),
    ...
    "test unique relations": traverseTest({
      ...
      assert: function (editor) {
        assertRelations(
          editor,
          finder,
          '<p>a</p>|<div>|b|<p>|y|<span id="x">|x</span>|c</p>|d</div>|<p>e</p>'
        );
      },
    }),
    ...
    "test unique relations - normalization": traverseTest({
      ...
      assert: function (editor) {
        assertRelations(
          editor,
          finder,
          '<p>a</p>|<div>|<p>|<span id="x">|x</span>|</p>|</div>|<p>b</p>'
        );
      },
    }),
    ...
    "test unique relations - only child": traverseTest({
      ...
      assert: function (editor) {
        assertRelations(
          editor,
          finder,
          '|<div contenteditable="true">|<p id="x">|x</p>|</div>|'
        );
      },
    }),
    ...
    "test lookup greedy find": greedyTest({
      ...
      assert: function () {
        assertLookupElements(
          ["div", "p", "span"],
          "Lookup never checked non-editable internals.",
          true
        );
      },
    }),
    ...
    "test lookup greedy find: siblings": greedyTest({
      ...
      assert: function () {
        assertLookupElements(
          ["blockquote", "div"],
          "Lookup visited both elements in greedy search.",
          true
        );
      },
    }),
    ...
    "test lookup greedy find: invisible sibling (normalization)": greedyTest({
      ...
      assert: function () {
        assertLookupElements(
          ["blockquote"],
          "Invisible elements are not considered."
        );
      },
    }),
  });
