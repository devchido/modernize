    ...
    assertRelations: function (editor, finder, expected) {
      ...
      for (var r in relations) {
        ...
        if (current.type & CKEDITOR.LINEUTILS_BEFORE)
          ranges.push(
            finder.getRange({ uid: r, type: CKEDITOR.LINEUTILS_BEFORE })
          );

        if (current.type & CKEDITOR.LINEUTILS_AFTER)
          ranges.push(
            finder.getRange({ uid: r, type: CKEDITOR.LINEUTILS_AFTER })
          );

        if (current.type & CKEDITOR.LINEUTILS_INSIDE)
          ranges.push(
            finder.getRange({ uid: r, type: CKEDITOR.LINEUTILS_INSIDE })
          );
      ...
      assert.areSame(
        expected,
        editor.getData().replace(/<!--R-->/gi, "|"),
        "Relations discovered, collected and normalized correctly."
      );
    },
  };
})();
