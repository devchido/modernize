...
(function () {
  ...
  function assertWidgetData(data, caption) {
    return function (editor, msg) {
      ...
      if (caption)
        assert.areSame(
          caption,
          widget.parts.caption.getText(),
          "parts.caption.getText " + msg
        );
      else assert.isFalse(!!widget.parts.caption, "there's no caption");
    };
  }
  ...
  function newDialogData(data) {
    ...
    for (var key in data) newData.push(["info", key, data[key]]);
