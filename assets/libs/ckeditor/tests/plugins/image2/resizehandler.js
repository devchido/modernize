    ...
    return function (editor, bot) {
      bot.setData('<img src="%BASE_PATH%/_assets/logo.png">', function () {
        ...
        waitForImage(image, function () {
          editor.widgets.instances[editor.widgets._.nextId - 1].resizer.fire(
            "mousedown",
            {
              $: {
                screenX: 163,
          ...
          var doc = CKEDITOR.document,
            data = options[editor.name]
              ? options[editor.name].data
              : options.data,
            expected = options[editor.name]
              ? options[editor.name].expected
              : options.expected,
            actual;
