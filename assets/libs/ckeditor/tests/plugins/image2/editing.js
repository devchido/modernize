...
(function () {
  ...
  var getWidgetById = widgetTestsTools.getWidgetById,
    ...
    widgetsHtml =
      '<img src="_assets/foo.png" alt="xalt" width="100" id="x" />' +
      "<p>X</p>" +
  ...
  bender.test({
    ...
    "test image dialog has correct aspect ratio after src change": function () {
      ...
      editor.once("dialogShow", function (evt) {
        var dialog = evt.data,
          widthInput = CKEDITOR.document.findOne(
            "#" + dialog.getContentElement("info", "width")._.inputId
          );
        ...
        function assertImage() {
          ...
          assert.areEqual(
            50,
            dialog.getContentElement("info", "width").getValue(),
            "invalid width"
          );
          assert.areEqual(
            120,
    ...
    "test create inline widget with a global command": function () {
      var editorBot = this.editorBot,
        onResume = function (dialog) {
          ...
          assert.areSame(
            '<p>x<img alt="" src="_assets/foo.png" />x</p>',
     ...
     */
    "test dimensions not set automatically when disabled in option":
      function () {
            ...
            bot.dialog("image", function (dialog) {
              var i = 0,
                resetBtn = bot.editor.document
                  .getById(dialog.getContentElement("info", "lock").domId)
                  .find(".cke_btn_reset")
                  .getItem(0);
