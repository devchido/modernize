...
(function () {
  ...
  bender.test({
    "test disable Image plugin when Image2 is loaded": function () {
      ...
      bot.dialog("image", function (dialog) {
        try {
          assert.isObject(
            dialog.widget,
            "Dialog displayed in the context of the widget."
          );
        } catch (e) {
          throw e;
        } finally {
    ...
    "test disable ImageButton when Image2 is loaded": function () {
      ...
      assert.isUndefined(
        editor.ui.get("ImageButton"),
        "ImageButton button is not present when Image2 is loaded."
      );
      assert.isUndefined(
        editor.commands.imagebutton,
        "ImageButton command is not defined when Image2 is loaded."
      );
    },

    "test overwrite onShow and onOk in link dialog definition (widget focused)":
      function () {
        var bot = this.editorBot;
          ...
          bot.dialog("link", function (dialog) {
            try {
              assert.areSame(
                0,
                onShowCount,
                "Default onShow should not be called if Image2 widget is focused"
              );
              ...
              assert.areSame(
                0,
                onOkCount,
                "Default onOk should not be called if Image2 widget is focused"
              );
            } catch (e) {
              throw e;
            } finally {
    ...
    "test overwrite onShow and onOk in link dialog definition (no widget focused)":
      function () {
        var bot = this.editorBot;
            ...
            resume(function () {
              ...
              try {
                assert.areSame(
                  1,
                  onShowCount,
                  "Default onShow should be executed"
                );
