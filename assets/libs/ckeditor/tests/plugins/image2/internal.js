...
(function () {
  ...
  function assertWrapperAlignFloat(expected, msg) {
    return function (widget) {
      assert.areSame(
        expected,
        CKEDITOR.tools.trim(widget.wrapper.getStyle("float")),
        msg
      );
    };
  }
  ...
  function assertWrapperAlignClasses(expected, checkElement) {
    return function (widget) {
      var config = widget.editor.config,
        falseClasses = CKEDITOR.tools.convertArrayToObject(
          config.image2_alignClasses
        );

      assert.areSame(
        "",
        widget.wrapper.getStyle("float"),
        "Widget.wrapper is not floated"
      );
      assert.areSame(
        "",
        widget.element.getStyle("float"),
  ...
  var fixHtml = image2TestsTools.fixHtml,
    htmls = {
      ...
      imageAlignedClassLeft: '<img class="l" src="_assets/foo.png" id="x" />',
      ...
      imageCentered:
        ...
        "</p>",
      imageCenteredClass:
        '<p class="c">' + '<img src="_assets/foo.png" id="x" />' + "</p>",

      imageAlignedRight:
        '<img src="_assets/foo.png" id="x" style="float:right" />',
      imageAlignedClassRight: '<img class="r" src="_assets/foo.png" id="x" />',
  ...
  bender.test({
    "test non-captioned: center unaligned image": function () {
      assertWidget({
        ...
        html: htmls.image,
        data: { align: "center" },
        assertCreated: assertWrapperAlignFloat(
          "",
          "Widget's wrapper has no float."
        ),
        assertNewData: assertWrapperAlignFloat(
          "",
          "Widget's wrapper has no float."
        ),
    ...
    "test transition: remove align and caption of centered figure":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
          html: htmls.captionedCentered,
    ...
    "test block wrapping: wrap widget in a block when it becomes inline (caption lost)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
          html: htmls.captionedAlignLeft,
    ...
    "test block wrapping: wrap widget in a block when it becomes inline (align lost)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
          html: htmls.imageCentered,
    ...
    "test block wrapping: wrap widget in a block when it becomes inline (caption and align lost)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
          html: htmls.captionedAlignLeft,
    ...
    "test block wrapping: de-wrap widget from block when it becomes a block (gets captioned)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
          html: htmls.image,
    ...
    "test block wrapping: de-wrap widget from block when it becomes a block (gets captioned, siblings before)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
    ...
    "test block wrapping: de-wrap widget from block when it becomes a block (gets captioned, siblings after)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
    ...
    "test block wrapping: de-wrap widget from block when it becomes a block (gets captioned, two siblings)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
    ...
    "test block wrapping: de-wrap widget from block when it becomes a block (gets centered)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
    ...
    "test block wrapping: de-wrap widget from block when it becomes a block (gets centered, siblings before)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
          html: "<p>xx" + htmls.image + "</p>",
    ...
    "test block wrapping: de-wrap widget from block when it becomes a block (gets centered, siblings after)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
          html: "<p>" + htmls.image + "xx</p>",
    ...
    "test block wrapping: de-wrap widget from block when it becomes a block (gets centered, two siblings)":
      function () {
        assertWidget({
          bot: this.editorBots.alignInline,
          html: "<p>x" + htmls.image + "x</p>",
    ...
    "test non-captioned (alignClasses): remove align of centered image":
      function () {
        assertWidget({
          bot: this.editorBots.alignClasses,
          html: htmls.imageCenteredClass,
    ...
    "test non-captioned (alignClasses): remove align of aligned image":
      function () {
        assertWidget({
          bot: this.editorBots.alignClasses,
    ...
    "test captioned (alignClasses): remove align of centered figure":
      function () {
        assertWidget({
          bot: this.editorBots.alignClasses,
          html: htmls.captionedCenteredClass,
    ...
    "test captioned (alignClasses): remove align of aligned figure":
      function () {
        assertWidget({
          bot: this.editorBots.alignClasses,
