...
var image2TestsTools = (function () {
  ...
  function createStateTransitionTests(tcs, states, shiftState) {
    for (var s in states) {
      ...
      for (var t in states) {
        ...
        tcs["test shift state: [" + s + " ⇒ " + t + "]"] = (function (
          oldState,
          newState
        ) {
          return function () {
  ...
  function createContextualStateTransitionTests(
    tcs,
    states,
    contexts,
    shiftState
      ...
      tcs[i] = (function (testCase) {
        return function () {
          ...
          bender.tools.testInputOut(
            context,
            function (contextBefore, contextAfter) {
              assertStateChange({
                bot: bot,
                oldState: oldState,
                newState: newState,
  ...
  function fillContext(context, state) {
    return new CKEDITOR.template(fixHtml(context)).output({
      state: state._.html,
    });
  }
