...
(function () {
  ...
  var getWidgetById = widgetTestsTools.getWidgetById,
    ...
    captionedHtml =
      '<p><figure class="image" id="x"><img src="_assets/bar.png" alt="foo" /><figcaption>bar</figcaption></figure></p>';
    ...
    function runCase() {
      ...
      for (var i in testCase.data)
        assert.areSame(
          testCase.data[i],
          widget.data[i],
          "[" + j + "] " + "Widget.data." + i + " properly set."
        );

      assert.areSame(
        testCase.inline,
        widget.inline,
        "[" + j + "] " + "Widget must have a proper inline property."
      );
      assert.areSame(
        widget,
        editor.widgets.focused,
        "[" + j + "] " + "Widget remains focused."
      );
