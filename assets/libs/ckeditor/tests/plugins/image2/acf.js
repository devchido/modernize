  ...
  var getWidgetById = widgetTestsTools.getWidgetById,
    ...
    defs = {
      ...
      no_dimensions: {
        ...
        config: {
          allowedContent:
            "img[src,alt]{float};figure(image){float};figcaption;div p{text-align}",
        },
        fields: {
          src: true,
          alt: true,
      ...
      no_align: {
        ...
        config: {
          allowedContent:
            "img[src,alt,width,height];figure(image);figcaption;div",
        },
        fields: {
          src: true,
          alt: true,
      ...
      all_allowed: {
        ...
        config: {
          allowedContent:
            "img[src,alt,width,height]{float};figure(image){float};figcaption;div p{text-align}",
        },
        fields: {
          src: true,
          alt: true,
      ...
      resume(function () {
        try {
          for (var f in fields)
            assert.areSame(
              fields[f],
              dialog.getContentElement("info", f).isVisible(),
              'Visibility of "' + f + '" must be correct.'
            );
        } catch (e) {
          throw e;
        } finally {
  ...
  function test(name, html, data, expected) {
    ...
    bot.setData(html, function () {
      ...
      assert.areSame(
        fixHtml(expected),
        fixHtml(bot.getData()),
        "Widget data considers ACF rules."
      );

      assert.areEqual(
        !!(fields.width && fields.height),
        !!widget.resizer,
        "Resizer displayed according to ACF rules."
      );
  ...
  bender.test({
    ...
    "test allow caption link target": function () {
      var bot = this.editorBots.link_target_allowed,
        content =
          '<figure><img alt="" src="_assets/bar.png" /><figcaption>' +
    ...
    "test justify plugin integration when (alignment disallowed)": function () {
      ...
      bot.setData(
        'x<img alt="b" height="2" id="x" src="_assets/foo.png" width="1" />',
        function () {
    ...
    "test justify plugin integration (alignment allowed)": function () {
      ...
      bot.setData(
        'x<img alt="b" height="2" id="x" style="float:left" src="_assets/foo.png" width="1" />',
        function () {
          getWidgetById(bot.editor, "x").focus();
