...
(function () {
  ...
  function getParentsList(el) {
    ...
    for (var i = 0; i < parents.length; i++) arr.push(parents[i].getName());
  ...
  function assertStructure(widget, structure) {
    assert.isMatching(
      structure,
      getParentsList(widget.parts.image),
      "Link is a part of the block widget"
    );
  }
  ...
  bender.test({
    ...
    linkWithDialog: function (
      ...
      bot.dialog("link", function (dialog) {
        ...
        assert.areSame(
          "a",
          widget.parts.link.getName(),
          "Widget.parts.link is registered"
        );
    ...
    unlink: function (name, beforeStructure, afterStructure, commandStates) {
      ...
      bender.tools.testInputOut(name, function (source, expected) {
        bot.setData(source, function () {
          ...
          assert.isUndefined(
            widget.parts.link,
            "Widget.parts.link is not registered"
          );
          assert.isTrue(
            !!bot.editor.getSelection().isFake,
            "Fake selection once unlinked"
          );
          assert.areSame(
            bot.editor.widgets.focused,
            widget,
            "Widget remains focused once unlinked"
          );
          assertStructure(widget, afterStructure);
          assertCommandStates(bot.editor, commandStates);
          assert.areSame(
          ...
          try {
            for (var t in fields) {
              ...
              for (var f in tab)
                assert.isMatching(
                  tab[f],
                  dialog.getValueOf(t, f),
                  "Field " + t + "." + f + ": value must match."
                );
            }
          } catch (e) {
            throw e;
    ...
    "test unlink inline widget from a link which does not belong to the widget (command)":
      function () {
        var bot = this.editorBot,
          editor = bot.editor,
          html =
        ...
        bot.setData(html, function () {
          ...
          assert.isFalse(
            !!widget.parts.link,
            "widget.parts#link is not defined"
          );
    ...
    "test unlink linked widget (widget.setData): inline, centered":
      function () {
        this.setWidgetData(
          "unlink-inline-centered",
          {
    ...
    "test unlink linked widget (widget.setData): captioned, aligned":
      function () {
        this.setWidgetData(
          "unlink-captioned-aligned",
          {
    ...
    "test unlink linked widget (widget.setData): captioned, centered":
      function () {
        this.setWidgetData(
          "unlink-captioned-centered",
          {
    ...
    "test open dialog on doubleclick": function () {
      ...
      bot.setData(
        '<p><a href="http://x"><img alt="x" id="x" src="_assets/foo.png" /></a></p>',
        function () {
          var widget = getById(bot.editor, "x"),
            dialogs = [];
          ...
          try {
            ...
            assert.areSame(
              "image2",
              dialogs.join(","),
              "Link dialog does not open on widget doubleclick."
            );
          } catch (e) {
            throw e;
          } finally {
    ...
    "test open dialog on doubleclick - in link environment": function () {
      ...
      bot.setData(
        '<p><a href="http://x">x<img alt="x" id="x" src="_assets/foo.png" />x</a></p>',
        function () {
          var widget = getById(bot.editor, "x"),
            dialogs = [];
    ...
    "test image in link environment": function () {
      ...
      bot.setData(
        '<p><a href="http://ping">x<img alt="x" id="x" src="_assets/foo.png" />x</a></p>',
        function () {
          var widget = getById(bot.editor, "x");
          ...
          bot.dialog("link", function (dialog) {
            try {
              assert.areSame(
                "ping",
                dialog.getValueOf("info", "url"),
                "The outer link is being edited"
              );
              ...
              assert.areSame(
                fixHtml(
                  '<p><a href="http://pong">x<img alt="x" id="x" src="_assets/foo.png" />x</a></p>'
                ),
                fixHtml(editor.getData()),
                "The outer link was updated"
              );
            } catch (e) {
              throw e;
            } finally {
    ...
    "test load editor anchors into link dialog when linking for the first time":
      function () {
        var bot = this.editorBot,
          editor = bot.editor,
          html =
          ...
          bot.dialog("link", function (dialog) {
            try {
              ...
              assert.areSame(
                2,
                dialog
                  .getContentElement("info", "anchorName")
                  .getInputElement()
                  .getChildCount(),
                "Anchors loaded into dialog"
              );
            ...
            assert.areSame(
              fixHtml(expected),
              fixHtml(editor.getData()),
              "The image links to an anchor"
            );

            widget = getById(bot.editor, "x");
            CKEDITOR.dom.element
              .createFromHtml('<a id="b" name="b"></a>', editor.document)
              .insertAfter(widget.wrapper);
              ...
              try {
                ...
                assert.areSame(
                  ...
                  dialog
                    ...
                    .getInputElement()
                    .getChildCount(),
                  "Updated anchors loaded into dialog"
                );
              } catch (e) {
                throw e;
              } finally {
    ...
    "test align classes transfered from nested image to widget wrapper":
      function () {
      ...
      bot.setData(
        ...
        function () {
          ...
          objectAssert.areEqual(
            ...
            widget.data.link,
            "Custom attributes parser output."
          );
        }
      );
    },
