...
(function () {
  ...
  bender.test({
    ...
    "test list block elements not draggable": function () {
      ...
      CKEDITOR.document
        .findOne("#cke_" + stylesCombo.id)
        .findOne("a")
        .$[CKEDITOR.env.ie ? "onmouseup" : "click"]();
    ...
    assertListBlockAdd: function (expected, input) {
      ...
      CKEDITOR.ui.listBlock.prototype.add.call(
        mock,
        input.value,
        input.html,
        input.title
      ...
      assert.areEqual(
        1,
        mock._.pendingList.length,
        "Invalid count of strings moved to pending list"
      );
      ...
      var anchor = elem.getFirst().getFirst(),
        ...
        decoratedExpectedHref =
          "javascript:void('" + this._escapeSingleQuote(expected.value) + "')", // jshint ignore:line
        decoratedExpectedOnclick =
          "CKEDITOR.tools.callFunction(clickFnId,'" +
          this._escapeSingleQuote(expected.value) +
          "'); return false;";

      // All IEs have changed onclick attr.
      if (CKEDITOR.env.ie) decoratedExpectedOnclick = "return false;";

      assert.areEqual(
        expected.title,
        anchor.getAttribute("title"),
        "Invalid value in to a[title] attribute"
      );
      // Checking expected.value, which occurs in 2 attributes.
      assert.areEqual(
        decoratedExpectedHref,
        anchor.getAttribute("href"),
        "Invalid value in to a[href] attribute"
      );
      assert.areEqual(
        decoratedExpectedOnclick,
        anchor.getAttribute("onclick"),
        "Invalid value in a[onclick] attribute"
      );

      assert.areSame(
        fixHtml(expected.html),
        fixHtml(anchor.getHtml()),
        "Invalid inner HTML in anchor"
      );
    },
