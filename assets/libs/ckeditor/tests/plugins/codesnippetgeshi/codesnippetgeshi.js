...
(function () {
  ...
  function fixHtml(html) {
    return bender.tools.compatHtml(
      bender.tools.fixHtml(html, true, true, true),
      true,
      true,
      true
    );
      ...
      try {
        this.editorBot.setData(html, function () {
          var widget = objToArray(this.editor.widgets.instances)[0],
            expectedHtml = fixHtml(
              '<code class="language-php geshi">foo<strong>bar</strong></code>'
            );

          assert.areSame(
            1,
            ajaxCalled,
            "Invalid count of calls to CKEDITOR.ajax#post"
          );
          assert.isTrue(
            ajaxUrl.indexOf("myCustomGeshi/colorize.php") > -1,
            "queried URL does not match to expected pattern"
          );
          assert.areSame(
            "php",
            ajaxData.lang,
            "Invalid data.lang value in json"
          );
          assert.areSame(
            "foo",
            ajaxData.html,
            "Invalid data.html value in json"
          );
          assert.areSame(
            "application/json",
            ajaxContentType,
            "Invalid content type given to CKEDITOR.ajax#post"
          );

          // Checking html produced in pre element.
          assert.areEqual(
            "code",
            widget.element.getFirst().getName(),
            "Invalid element placed directly inside pre"
          );
          assert.areEqual(
            expectedHtml,
            fixHtml(widget.element.getHtml()),
            "Invalid innerHtml"
          );
        ...
        if (e instanceof YUITest.Wait === false) restoreOriginalCallbacks();
