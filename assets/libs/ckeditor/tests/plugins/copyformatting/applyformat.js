...
(function () {
  ...
  bender.test({
    "test applyFormat with collapsed selection": function () {
      testApplyingFormat(this.editor, "<p>Apply format h{}ere</p>", "here", [
        styles[0],
      ]);
    },

    "test applyFormat with uncollapsed selection": function () {
      testApplyingFormat(this.editor, "<p>Apply format {her}e</p>", "her", [
        styles[1],
      ]);
    },
    ...
    "test applyFormat on plain text with list styles": function () {
      ...
      expectedStyles[1].element = expectedStyles[1]._.definition.element =
        "span";

      testApplyingFormat(
        this.editor,
        "<p>Apply format h{}ere</p>",
        "here",
        listStyles,
        [],
        expectedStyles
    ...
    "test applyFormat on plain text with list styles and specific listitems attributes":
      function () {
        var expectedStyles = clone(listStyles).slice(0, 2);
    ...
    "test applyFormat on list context with list styles": function () {
      ...
      testApplyingFormat(
        this.editor,
        "<ul><li>Apply format h{}ere</li></ul>",
        "here",
        listStyles,
        [],
        expectedStyles
      ...
      assert.isTrue(
        this.editor.editable().findOne("li").getStyle("text-decoration") ===
          "underline"
      );
    },
    ...
    "test applyFormat on mixed context with list styles": function () {
      ...
      bender.tools.selection.setWithHtml(
        editor,
        "<p>Apply for{mat</p><ul><li>Maybe h}ere</li></ul>"
      );
      ...
      // is apparently not working with it due to `li` being a block.
      assert.isTrue(
        editable.findOne("li").getStyle("text-decoration") === "underline"
      );

      // Now check if all new styles were applied to the paragraph.
      applied = 0;
      elementPath = new CKEDITOR.dom.elementPath(
        editable.findOne("p").findOne("b"),
        editable
      );
    ...
    "test applyFormat on table context with table styles": function () {
      ...
      testApplyingFormat(
        this.editor,
        "<table><tr><td>Apply format h{}ere</td></tr>",
        "here",
        tableStyles,
        [],
        expectedStyles
      ...
      // is apparently not working with it due to `li` being a block.
      assert.isTrue(
        this.editor.editable().findOne("td").getStyle("text-decoration") ===
          "underline"
      );
    },

    "test applyFormat on table context with table styles (within thead)":
      function () {
        var expectedStyles = clone(tableStyles);
        expectedStyles.splice(1, 1);

        expectedStyles[2].element = expectedStyles[2]._.definition.element =
          "thead";

        testApplyingFormat(
          this.editor,
          "<table><thead><tr><th>Apply format h{}ere</th></tr></thead>",
          "here",
          tableStyles,
          [],
          expectedStyles
        ...
        // is apparently not working with it due to `li` being a block.
        assert.isTrue(
          this.editor.editable().findOne("th").getStyle("text-decoration") ===
            "underline"
        );
      },
    ...
    "test applyFormat on mixed context with table styles": function () {
      ...
      bender.tools.selection.setWithHtml(
        editor,
        "<p>Apply for{mat</p><table><tr><td>Maybe h}ere</td></tr></table>"
      );
      ...
      // is apparently not working with it due to `li` being a block.
      assert.isTrue(
        editable.findOne("td").getStyle("text-decoration") === "underline"
      );
    ...
    "test applyFormat on skipped elements": function () {
      ...
      for (i = 0; i < input.length; i++) {
        ...
        assert.areSame(
          1,
          editor.editable().find(elements[i]).count(),
          elements[i] + " element count"
        );
      }
    },

    "test removing formatting from list while applying plain-text styles":
      function () {
        var editor = this.editor,
          content =
            '<ol start="5">' +
    ...
    "test removing formatting from table while applying plain-text styles":
      function () {
        var editor = this.editor,
          content =
            '<table border="1">' +
    ...
    "test filter styles": function () {
      var filterStyles = [
        ...
        filteredStyles =
          CKEDITOR.plugins.copyformatting._filterStyles(filterStyles);
    ...
    "test determining context": function () {
      ...
      bender.tools.selection.setWithHtml(
        editor,
        "<p>Paragra{}ph</p><ul><li>And a list</li></ul>"
      );
      assert.areSame(
        textConstant,
        determineContext(),
        "Caret in text before list item"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<p>Paragraph</p><ul><li>And a l{}ist</li></ul>"
      );
      assert.areSame(
        listConstant,
        determineContext(),
        "Caret in first list item"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<p>Paragrap{h</p><ul><li>And a l}ist</li></ul>"
      );
      assert.areSame(
        listConstant,
        determineContext(),
        "Selection started in text, ended inside of a list item"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<ul><li>L{ist</li></ul><p>And a par}agraph</p>"
      );
      assert.areSame(
        listConstant,
        determineContext(),
        "Selection started in list item, ended inside of a text"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<ul><li>Fiz{z</li><li>Boo}m</li></ul>"
      );
      assert.areSame(
        listConstant,
        determineContext(),
        "Selection within two list items"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<p>Parag{}raph</p><table><tr><td>Cell 1</td></tr></table>"
      );
      assert.areSame(
        textConstant,
        determineContext(),
        "Caret in text before table"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<p>Paragraph</p><table><tr><td>Ce{}ll 1</td></tr></table>"
      );
      assert.areSame(
        tableConstant,
        determineContext(),
        "Caret in first table cell"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<p>Parag{raph</p><table><tr><td>Ce}ll 1</td></tr></table>"
      );
      assert.areSame(
        tableConstant,
        determineContext(),
        "Selection started in text, ended inside of a table cell"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<table><tr><td>Ce{ll 1</td></tr></table><p>Parag}raph</p>"
      );
      assert.areSame(
        tableConstant,
        determineContext(),
        "Selection started in table cell, ended inside of a text"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<table><tr><td>Ce{ll 1</td><td>Cel}l 2</td></tr></table>"
      );
      assert.areSame(
        tableConstant,
        determineContext(),
        "Selection within two cells"
      );

      bender.tools.selection.setWithHtml(
        editor,
        "<table><tr><td>Ce{ll 1</td></tr><tr><td>Ce}ll 2</td></tr></table>"
      );
      assert.areSame(
        tableConstant,
        determineContext(),
        "Selection within two rows"
      );
    },
