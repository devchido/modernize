...
function testGettingWordOffset(editor, htmlWithSelection, expected) {
  ...
  word = CKEDITOR.plugins.copyformatting._getSelectedWordOffset(
...
function testApplyingFormat(
  ...
  expectedStyles = CKEDITOR.tools.isArray(expectedStyles)
    ? expectedStyles
    : newStyles;
  ...
  assert.areSame(
    oldStyles.length,
    removed,
    "Old styles were removed correctly."
  );
  ...
  assert.areSame(
    ...
    applied,
    "New styles were applied correctly."
  );
  ...
  if (editor.editable().findOne(newStyles[0].element)) {
    assert.areSame(
      expectedContent,
      editor.editable().findOne(newStyles[0].element).getHtml()
    );
  }
}
...
function assertScreenReaderNotification(editor, msg) {
  var container = CKEDITOR.document
    .getBody()
    .find(".cke_copyformatting_notification div[aria-live]");

  assert.areSame(
    1,
    container.count(),
    "There are only one container for displaying notifications"
  );
  assert.areSame(
    editor.lang.copyformatting.notification[msg],
    container.getItem(0).getText(),
...
function assertCopyFormattingState(editor, expectedStyles, additionalData) {
  ...
  if (!additionalData || additionalData.sticky) {
    assert.isTrue(
      areaWithCursor.hasClass("cke_copyformatting_active"),
  ...
  assert.areSame(
    expectedStyles.length,
    copyFormatting.styles.length - 1,
    "There are correct amount of styles"
  );

  for (var i = 0; i < expectedStyles.length; i++) {
    assert.isInstanceOf(
      CKEDITOR.style,
      copyFormatting.styles[i],
      "Style #" + i + " is an instanceof CKEDITOR.style"
    );
    objectAssert.areDeepEqual(
      expectedStyles[i],
      copyFormatting.styles[i]._.definition,
      "Style # " + i + " has correct definition"
    );
  }
}

function assertApplyFormattingState(
  editor,
  expectedStyles,
  styledElement,
  additionalData
 ...
 */
function testCopyFormattingFlow(
  editor,
  htmlWithSelection,
  expectedStyles,
  removedStyles,
  ...
  copyFormatting.on(
    "extractFormatting",
    countExtractFormatting,
    null,
    null,
    1000
  );
  editor.execCommand("copyFormatting", additionalData);
  copyFormatting.removeListener(
    "extractFormatting",
    countExtractFormatting,
    null,
    null,
  ...
  copyFormatting.once("applyFormatting", function (evt) {
    assert.isArray(
      evt.data.styles,
      "New styles are passed to the applyFormatting event."
    );
  });
  ...
  copyFormatting.once(
    ...
    function () {
      ...
      assert.areSame(
        ...
        removed,
        "All preexisting styles are removed correctly"
      );
    },
    null,
    null,
  ...
  assert.isTrue(
    removedStyles.length <= events.extractFormatting,
