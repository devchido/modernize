  ...
  var stylesToRemove = [
    ...
    tests = {
      "test applying style on collapsed selection": function (editor) {
        testCopyFormattingFlow(
          editor,
          "<p><s>Copy t{}hat format</s> to <b>this element</b></p>",
          [
            {
              element: "s",
              attributes: {},
      ...
      "test applying style from ambiguous initial selection": function (
        editor
      ) {
        testCopyFormattingFlow(
          editor,
          '<p><s>Copy {t</s><span class="font-weight: bold">h}at</span> format</s> to <b>this element</b></p>',
          [
            {
              element: "s",
              attributes: {},
      ...
      "test applying styles from one cell to another": function (editor) {
        var inputContent = CKEDITOR.document
            .findOne("#t-16675 .input")
            .getHtml(),
          rng,
          cell,
          cellTextNode;
      ...
      "test changing list type": function (editor) {
        var inputContent = CKEDITOR.document
            .findOne("#list_type .input")
            .getHtml(),
          expectedContent = CKEDITOR.document
            .findOne("#list_type .expected")
            .getHtml();
      ...
      "test aplying styles to mixed list context": function (editor) {
        var inputContent = CKEDITOR.document
            .findOne("#list_remove_source .input")
            .getHtml(),
          expectedContent = CKEDITOR.document
            .findOne("#list_remove_source .expected")
            .getHtml();
      ...
      "test preserving inline styles of nested lists": function (editor) {
        var inputContent = CKEDITOR.document
            .findOne("#nested_lists_styles .input")
            .getHtml(),
          expectedContent = CKEDITOR.document
            .findOne("#nested_lists_styles .expected")
            .getHtml();
      ...
      "test removing inline styles while changing list type": function (
        editor
      ) {
        var inputContent = CKEDITOR.document
            .findOne("#list_type_styles .input")
            .getHtml(),
          expectedContent = CKEDITOR.document
            .findOne("#list_type_styles .expected")
            .getHtml();
      ...
      "test changing list type (nested lists)": function (editor) {
        var inputContent = CKEDITOR.document
            .findOne("#list_type_nested .input")
            .getHtml(),
          expectedContent = CKEDITOR.document
            .findOne("#list_type_nested .expected")
            .getHtml();
      ...
      "test removing formatting on collapsed selection": function (editor) {
        testCopyFormattingFlow(
          editor,
          "<p>Copy t{}hat format to <b>this element</b></p>",
          [],
          stylesToRemove,
          {
            elementName: "b",
      ...
      "test cancelling Copy Formatting command from keystroke (Escape)":
        function (editor) {
          var cmd = editor.getCommand("copyFormatting");
          bender.tools.selection.setWithHtml(
            editor,
      ...
      "test cancelling Copy Formatting command by cancelling applyFormatting event":
        function (editor) {
          var cmd = editor.getCommand("copyFormatting"),
      ...
      "test preserving styles at Copy Formatting destination": function (
        editor
      ) {
        var cmd = editor.getCommand("copyFormatting"),
          copyFormatting = editor.copyFormatting,
        ...
        assert.isTrue(
          CKEDITOR.plugins.copyformatting._extractStylesFromRange(editor, range)
            .length >= 2,
      ...
      "test that _getCursorContainer returns correct element": function (
        editor
      ) {
        ...
        assert.areSame(
          correctContainer,
          CKEDITOR.plugins.copyformatting._getCursorContainer(editor)
        );
      },
      ...
      'test disabling "disabled" cursor': function (editor) {
        ...
        assert.isFalse(
          CKEDITOR.document
            .getDocumentElement()
            .hasClass("cke_copyformatting_disabled")
        );
      ...
      "test _addScreenReaderContainer": function () {
        ...
        assert.areSame(
          ret,
          secondRet,
          "Returned the same value for the second call"
        );
        assert.areSame(
          1,
          doc.find(wrapperSelection).count(),
          "Multiple calls doesnt put multiple containers"
        );
      },
    };

  tests = bender.tools.createTestsForEditors(
    CKEDITOR.tools.object.keys(bender.editors),
    tests
  );
