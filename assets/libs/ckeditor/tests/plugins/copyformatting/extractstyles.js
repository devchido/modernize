...
(function () {
  ...
  bender.test({
    ...
    "test extract style from mapped semantic element with attributes":
      function () {
        testConvertingStyles(
    ...
    "test extract style from span with [style] attribute": function () {
      testConvertingStyles('<span style="' + styleAttr + '">Test</span>', {
        element: "span",
        attributes: {},
        styles: CKEDITOR.tools.parseCssText(
          CKEDITOR.tools.normalizeCssText(styleAttr, true)
        ),
        type: CKEDITOR.STYLE_INLINE,
      });
    },
    ...
    "test extract style from span with [id] attribute": function () {
      testConvertingStyles(
        '<span id="test-id" style="' + styleAttr + '">Test</span>',
        {
          element: "span",
          attributes: {},
          styles: CKEDITOR.tools.parseCssText(
    ...
    "test excluding attributes": function () {
      testConvertingStyles(
        '<span href="http://ckeditor.com" data-cke-saved-href="http://ckeditor.com" ' +
          'style="' +
          styleAttr +
          '" dir="ltr">Test</span>',
        {
    ...
    "test extract style from skipped elements": function () {
      var elements = [
          ...
          CKEDITOR.dom.element.createFromHtml(
            '<a href="http://cksource.com">Test</a>'
          ),
      ...
      for (i = 0; i < elements.length; i++) {
        style = CKEDITOR.plugins.copyformatting._extractStylesFromElement(
          this.editor,
          elements[i]
        );
    ...
    "test extract styles from nested elements": function () {
      var element = CKEDITOR.dom.element.createFromHtml(
          '<strong class="important" ' +
            'title="Neil Armstrong"><span style="' +
            styleAttr +
            '"><s>Neil Armstrong</s></span></strong>'
        ),
        styles = CKEDITOR.plugins.copyformatting._extractStylesFromElement(
          this.editor,
          element.findOne("s")
        );
    ...
    "test extract styles from nested lists": function () {
      var element = CKEDITOR.dom.element.createFromHtml(
          "<ol><li><ul><li>Test</li></ul></li></ol>"
        ),
        styles = CKEDITOR.plugins.copyformatting._extractStylesFromElement(
          this.editor,
          element.findOne("ul li")
        );
