...
bender.test({
  "test find and replace": function () {
    ...
    bot.dialog("find", function (dialog) {
      ...
      assert.areSame(
        '<p>foo</p><p><span title="highlight">bar</span></p>',
        bot.getData(true)
      );
  ...
  "test find highlight for read-only text": function () {
    ...
    bot.dialog("find", function (dialog) {
      ...
      assert.areSame(
        '<p>foo</p><p contenteditable="false"><span title="highlight">bar</span></p>',
        bot.getData(true)
      );
      dialog.getButton("cancel").click();
    });
  ...
  "test find in read-only mode": function () {
    ...
    bot.setData("<p>example text</p>", function () {
      ...
      bot.dialog("find", function (dialog) {
        ...
        assert.areSame(
          '<p><span title="highlight">example</span> text</p>',
          bot.getData(true)
        );
        dialog.getButton("cancel").click();
      });
  ...
  "test find and replace with pattern change - replace text after selection":
    function () {
      var bot = this.editorBot;

      bot.setHtmlWithSelection(
        "<p>example text <strong>example</strong> text</p>"
      );
      bot.dialog("replace", function (dialog) {
        ...
        assert.areSame(
          '<p>example t<span title="highlight">ext </span><strong><span title="highlight">example</span></strong> text</p>',
          bot.getData(true),
          "Text after previous selection was correctly highlighted."
        );

        dialog.setValueOf("replace", "txtReplace", "example2");
        dialog.getContentElement("replace", "btnFindReplace").click();
        assert.areSame(
          '<p>example t<span title="highlight">example2</span> text</p>',
          bot.getData(true),
  ...
  // https://dev.ckeditor.com/ticket/11697
  "test find and replace with pattern change - replace text before selection":
    function () {
      var bot = this.editorBot;
      ...
      bot.dialog("replace", function (dialog) {
        ...
        dialog.setValueOf(
          "replace",
          "txtFindReplace",
          "Apollo 11 was the space"
        );
        dialog.getContentElement("replace", "btnFindReplace").click();

        assert.areSame(
          '<p><span title="highlight">apollo 11 was the space</span>flight that...</p>',
          bot.getData(true),
          "Text before previous selection was correctly highlighted."
        );

        dialog.setValueOf("replace", "txtReplace", "A ");
        dialog.getContentElement("replace", "btnFindReplace").click();
        assert.areSame(
          '<p><span title="highlight">a </span>flight that...</p>',
          bot.getData(true),
          "Highlighted text was correctly replaced."
        );
  ...
  "test find and replace with options change": function () {
    ...
    bot.dialog("replace", function (dialog) {
      ...
      assert.areSame(
        "<p>example text</p>",
        bot.getData(true),
        "Text was not replace with case sensitive find."
      );
