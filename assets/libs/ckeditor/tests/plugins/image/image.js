    ...
    bot.dialog("image", function (dialog) {
      ...
      for (key in inpValMap) {
        if (inpValMap.hasOwnProperty(key)) {
          ...
          if (keySplit.length == 1) inputName = keySplit[0];
          else {
          ...
          var realValue =
            typeof expectedValue == "number"
              ? parseInt(field.getValue(), 10)
              : field.getValue();
          var errorMessage = "Wrong value for input " + inputName + ".";

          if (isNaN(expectedValue)) assert.isNaN(realValue, errorMessage);
          else assert.areSame(expectedValue, realValue, errorMessage);
        }
      }
  ...
  bender.test({
    "test read image (inline styles)": function () {
      var htmlWithSelection =
        '[<img src="' +
        SRC +
        '" style="border:solid 2px;height:86px;margin:10px 5px;float:right;width:414px;">]';
    ...
    "test read image (attributes)": function () {
      var htmlWithSelection =
        '[<img src="' +
        SRC +
        '" border="2" height="86" width="414" vspace="10" hspace="5" align="right">]';
    ...
    "test read image (align)": function () {
      var htmlWithSelection =
        '[<img src="' + SRC + '" align="texttop" style="float:inherit">]';
    ...
    "test read image (sync styles from advanced tab)": function () {
      ...
      testReadImage(
        ...
        function (dialog) {
          var styleTextField = dialog.getContentElement(
            "advanced",
            "txtdlgGenStyle"
          );
          styleTextField.setValue(
            "height:300px;width:200px;border: 1px solid;margin:10px 5px;vertical-align:top;float:left"
          );
        }
      );
    },
    ...
    "test update image (inline styles)": function () {
      ...
      var htmlWithSelection =
        '[<img src="' +
        SRC +
        '" style="height:300px;width:200px;border: 1px solid;float:left"/>]';
    ...
    "test update image (attributes)": function () {
      ...
      var htmlWithSelection =
        '[<img src="' +
        SRC +
        '" height="300" width="200" border="1" align="right" vspace="10" hspace="5"/>]';
     ...
     */
    "test dimensions not set automatically when disbled in option":
      function () {
        bender.editorBot.create(
          {
        ...
        function (bot) {
          bot.dialog("image", function (dialog) {
            var stub = sinon.stub(dialog, "getValueOf", function (field, prop) {
              return prop === "txtWidth"
                ? getFixedImageSize("width")
                : getFixedImageSize("height");
            });

            dialog.originalElement.once(
              "load",
              function () {
                setTimeout(function () {
                  resume(function () {
     ...
     */
    "test dimension should be empty after resetting size and loading image":
      function () {
        bender.editorBot.create(
          {
          ...
          function (bot) {
            bot.dialog("image", function (dialog) {
              var i = 0,
                resetBtn = bot.editor.document
                  .getById(dialog.getContentElement("info", "ratioLock").domId)
                  .find(".cke_btn_reset")
                  .getItem(0);
              ...
              function onResume() {
                ...
                assert.areSame(
                  imgs[i].width,
                  dialog.getContentElement("info", "txtWidth").getValue()
                );
                assert.areSame(
                  imgs[i].height,
                  dialog.getContentElement("info", "txtHeight").getValue()
    ...
    "test replace selected content": function () {
      ...
      var expectedOutput =
        '<p>my <img alt="" src="' +
        SRC +
        '" style="height:10px;width:10px;" /> content</p>';
    ...
    "test replace selected content in link": function () {
      var htmlWithSelection =
        '<p>x<a href="#">y[<span contenteditable="false">foo</span>]y</a>x</p>';
      var expectedOutput =
        '<p>x<a href="#">y<img alt="" src="' +
        SRC +
        '" style="height:10px;width:10px;" />y</a>x</p>';
    ...
    "test replace link with text": function () {
      ...
      if (CKEDITOR.env.ie && CKEDITOR.env.version < 9)
        expectedOutput =
          '<p>x<img alt="" src="' +
          SRC +
          '" style="height:10px;width:10px;" />x</p>';
      else
        expectedOutput =
          '<p>x<a href="#"><img alt="" src="' +
          SRC +
          '" style="height:10px;width:10px;" /></a>x</p>';
