...
(function () {
  ...
  bender.test({
    "test config.normalizeBackground": function () {
      ...
      assert.areSame(
        input,
        this.editors.normalizeBackground.dataProcessor.toHtml(input)
      );
    },
    ...
    "test config.colorButton_colors labels": function () {
      ...
      colorOptions = bgColorBtn._.panel
        .getBlock(bgColorBtn._.id)
        .element.find("a.cke_colorbox");

      CKEDITOR.tools.array.map(colorOptions.toArray(), function (el, index) {
        assert.areSame(
          expectedLabels[index],
          colorOptions.getItem(index).getAttribute("title"),
          "Title for color at index " + index
        );
      });
    },
    ...
    "test config.colorButton_colors applies color": function () {
      ...
      colorSquare = bgColorBtn._.panel
        .getBlock(bgColorBtn._.id)
        .element.findOne("a.cke_colorbox[data-value=FF9900]");

      colorSquare.$.onclick();

      assert.beautified.html(
        bender.tools.selection.getWithHtml(editor),
        '<p><span style="background-color:#ff9900;">[foo]</span></p>',
        {
