...
(function () {
  ...
  bender.test({
    "test panel create": function () {
      ...
      assert.areSame(
        panel.parts.panel.getParent(),
        CKEDITOR.document.getBody(),
        "Panel in top-most document"
      );
      assert.isTrue(
        CKEDITOR.tools.isEmpty(panel.activeShowListeners),
        "Has no listeners until shown"
      );

      assert.areSame(
        100,
        panel.width,
        "Definition passed to the panel (width)"
      );
      assert.areSame(
        200,
        panel.height,
        "Definition passed to the panel (height)"
      );

      assert.isFalse(
        panel.parts.panel.isVisible(),
        "Panel is not visible until attached"
      );
      assert.isFalse(
        panel.rect.visible,
        "Panel is not visible until attached (rect)"
      );
      ...
      assert.areSame(
        360,
        anotherPanel.width,
        "Definition is passed per-panel (width)"
      );
      assert.areSame(
        "auto",
        anotherPanel.height,
        "Definition is passed per-panel (height)"
      );
    ...
    "test panel listeners": function () {
      ...
      assert.areSame(
        1,
        CKEDITOR.tools.object.keys(panel.showListeners).length,
        "Before #show: Registered one listener"
      );
      assert.areSame(
        0,
        CKEDITOR.tools.object.keys(panel.activeShowListeners).length,
        "Before #show: No active listeners"
      );
      assert.isFalse(
        listenerRemoved,
        "Before #show: Listener not removed yet."
      );
      panel.show();
      ...
      assert.areSame(
        1,
        CKEDITOR.tools.object.keys(panel.showListeners).length,
        "After #show: Registered one listener"
      );
      assert.areSame(
        1,
        CKEDITOR.tools.object.keys(panel.activeShowListeners).length,
        "After #show: Listener activated on show"
      );
      ...
      assert.areSame(
        1,
        CKEDITOR.tools.object.keys(panel.showListeners).length,
        "After another #show: Registered one listener"
      );
      assert.areSame(
        1,
        CKEDITOR.tools.object.keys(panel.activeShowListeners).length,
        "After another #show: Listener activated on show"
      );
      assert.isFalse(
        listenerRemoved,
        "After another #show: Listener not removed yet."
      );
      ...
      assert.areSame(
        1,
        CKEDITOR.tools.object.keys(panel.showListeners).length,
        "After #hide: Registered one listener"
      );
      assert.areSame(
        0,
        CKEDITOR.tools.object.keys(panel.activeShowListeners).length,
        "After #hide: Deactivated listener"
      );
    ...
    "test panel attach focus": function () {
      ...
      panel.attach(strong);
      assert.isTrue(
        doc.getActive().equals(panelElement),
        "Panel element is focused"
      );

      // Giving focus element explicitly.
      panel.attach(strong, { focusElement: closeElement });
      assert.isTrue(
        doc.getActive().equals(closeElement),
        "Close element is focused"
      );
      ...
      assert.isTrue(
        doc.getActive().equals(doc.getBody()),
        "Focus remains on body element"
      );
    },
    ...
    "test panel show option": function () {
      ...
      assert.isFalse(
        showListener.called,
        "Event show should not be fired when show param is false."
      );
      resetState();
      ...
      assert.isTrue(
        showListener.called,
        "Event show should be fired when show param is true."
      );
      resetState();
      ...
      assert.isTrue(
        showListener.called,
        "Event show should be fired when show param is undefined."
      );
      resetState();
      ...
      assert.isTrue(
        showListener.called,
        "Event show should be fired when show param is null."
      );
      resetState();
    },
    ...
    "test panel destroy": function () {
      ...
      while ((p = panels.pop())) {
        assert.areSame(
          0,
          CKEDITOR.tools.object.keys(p.activeShowListeners).length,
          p.title + ": no active listeners once panel is destroyed"
        );
        assert.isNull(
          p.parts.panel.getParent(),
          p.title + ": panel out of DOM"
        );
      }
    },
  });
