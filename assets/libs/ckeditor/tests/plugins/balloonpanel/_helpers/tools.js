    ...
    if (actual < min || actual > max) {
      throw new YTest.ComparisonFailure(
        YTest.Assert._formatMessage(
          message || "Number is not within a valid range."
        ),
        "[" + min + ", " + max + "]",
        actual
      );
  ...
  assertMoveTo: function (options) {
    ...
    if (maxX || maxY) {
      ...
      this.assertInRange(
        ...
        maxX,
        actualX,
        "balloon move() x argument is not in range"
      );
      this.assertInRange(
        options.expectedY,
        maxY,
        actualY,
        "balloon move() y argument is not in range"
      );
    } else {
      assert.areEqual(
        options.expectedX,
        actualX,
        "invalid balloon move() x value"
      );
      assert.areEqual(
        options.expectedY,
        actualY,
        "invalid balloon move() y value"
      );
    }
  },
  ...
  attachBalloon: function (balloon, elementId) {
    var elem =
      typeof elementId == "string"
        ? balloon.editor.editable().findOne("#" + elementId)
        : elementId;
    balloon.attach(elem);
