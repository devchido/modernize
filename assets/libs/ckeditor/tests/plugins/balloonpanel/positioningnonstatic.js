...
*/
  ...
  var tests = {
    setUp: function () {
      // Stub view port size.
      getViewPaneSizeStub = sinon.stub(
        CKEDITOR.dom.window.prototype,
        "getViewPaneSize"
      );
    ...
    "test positioning in nonstatic body": function (editor) {
      ...
      var bodyMarginLeft = parseInt(
          CKEDITOR.document.getBody().getComputedStyle("margin-left"),
          10
        ),
        bodyMarginTop = parseInt(
          CKEDITOR.document.getBody().getComputedStyle("margin-top"),
          10
        ),
        DELTA = 1,
        pos;
      ...
      assert.isNumberInRange(
        pos.triangleTip.x + bodyMarginLeft,
        pos.elementRect.left - DELTA,
        pos.elementRect.right + DELTA,
        "Triangle tip x position outsid of element"
      );
      assert.isNumberInRange(
        pos.triangleTip.y + bodyMarginTop,
        pos.elementRect.top - DELTA,
        pos.elementRect.bottom + DELTA,
        "Triangle tip y position outside of element"
      );
      pos.panel.destroy();
    },
