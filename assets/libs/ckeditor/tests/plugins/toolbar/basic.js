...
function testInline(editor, collapser) {
  ...
  assert.isFalse(
    toolboxInner.hasClass("cke_toolbox_main"),
    "There should be no .cke_toolbox_main inside toolbox space"
  );
}
...
bender.test({
  "test toolbar": function () {
    ...
    assert.isNull(
      this.editor.ui.space("toolbar_collapser"),
      "No collapser by default"
    );
  },
  ...
  "test toolbar collapse/expand fire resize event": function () {
    bender.editorBot.create(
      ...
      function (bot) {
        ...
        assert.areEqual(
          200,
          resizeData[0].outerWidth,
          "Width should be set properly."
        );
        assert.areEqual(
          400,
          resizeData[0].outerHeight,
          "Height should be set properly."
        );

        editor.execCommand("toolbarCollapse");
        assert.isTrue(
          resizeData[1].outerHeight < resizeData[0].outerHeight,
          "Height after collapse should be less."
        );

        editor.execCommand("toolbarCollapse");
        assert.areSame(
          resizeData[0].outerHeight,
          resizeData[2].outerHeight,
          "Height should properly restore to same value."
        );
      }
    );
  },
