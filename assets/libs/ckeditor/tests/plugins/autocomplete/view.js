...
(function () {
  ...
  bender.test({
    ...
    "test select item": function () {
      ...
      assert.isTrue(
        view.getItemById(item2.id).hasClass("cke_autocomplete_selected")
      );
    ...
    "test attach": function () {
      ...
      view.element.fire(
        "click",
        new CKEDITOR.dom.event({ target: view.getItemById(item2.id).$ })
      );
  ...
  function assertViewElement(editor, element) {
    var zIndex = editor.config.baseFloatZIndex - 3,
      expectedHtml =
        '<ul class="cke_autocomplete_panel" style="z-index: ' +
        zIndex +
        ';"></ul>';

    assert.areEqual(
      expectedHtml,
      bender.tools.compatHtml(element.$.outerHTML, false, true)
    );
  }
  ...
  function getCaretRect(editor, caretPosition, offset) {
    var view = new CKEDITOR.plugins.autocomplete.view(editor),
      getClientRectsStub = sinon
        .stub(CKEDITOR.dom.range.prototype, "getClientRects")
    ...
    if (editor.editable().isInline()) {
      ...
      offsetStub = sinon
        .stub(CKEDITOR.editable.prototype, "getParent")
        .returns({
