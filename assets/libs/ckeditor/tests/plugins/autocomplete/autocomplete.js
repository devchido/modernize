...
(function () {
  ...
  bender.test({
    ...
    "test esc key closes view": function () {
      ...
      editor
        .editable()
        .fire("keydown", new CKEDITOR.dom.event({ keyCode: 27 })); // ESC
    ...
    "test autocomplete starts with the first item selected": function () {
      ...
      assertViewOpened(ac, true);
      assert.isTrue(
        ac.view.getItemById(1).hasClass("cke_autocomplete_selected")
      );
    ...
    "test mouseover changes selected item": function () {
      ...
      ac.view.element.fire(
        "mouseover",
        new CKEDITOR.dom.event({ target: target.$ })
      );
    ...
    "test arrow down selects next item": function () {
      ...
      assertViewOpened(ac, true);
      assert.isTrue(
        ac.view.getItemById(2).hasClass("cke_autocomplete_selected")
      );
      ...
      assertViewOpened(ac, true);
      assert.isTrue(
        ac.view.getItemById(3).hasClass("cke_autocomplete_selected")
      );
    ...
    "test custom config.autocomplete_commitKeystrokes (array format)":
      function () {
    ...
    "test custom config.autocomplete_commitKeystrokes (primitive number)":
      function () {
    ...
    "test view position on scroll": function () {
      ...
      var getClientRectStub = sinon
          .stub(CKEDITOR.dom.element.prototype, "getClientRect")
          .returns({ top: 0, bottom: 100 }),
        viewport = CKEDITOR.document.getById(editor.id + "_contents");

      sinon
        .stub(ac.view, "getViewPosition")
        .returns({ top: 150, bottom: 160, left: 50 });
    ...
    "test custom view template": function () {
      ...
      assert.beautified.html(
        '<ul><li class="cke_autocomplete_selected" data-id="1"><strong>anna</strong></li></ul>',
    ...
    "test view position starts from the beginning of the match": function () {
      var editor = this.editors.standard,
        ...
        getDocumentPositionStub = sinon
          .stub(CKEDITOR.dom.element.prototype, "getDocumentPosition")
          .returns({ x: 0, y: 0 }),
        editorViewportRectStub = sinon
          .stub(CKEDITOR.dom.element.prototype, "getClientRect")
        ...
        ac = new CKEDITOR.plugins.autocomplete(editor, {
          textTestCallback: function () {
            ...
            sinon
              .stub(range, "getClientRects")
              .returns([invalidRect, invalidRect, lastRangeRect]);
    ...
    "test limit view items": function () {
      var editor = this.editors.standard,
        itemsCount = 1,
        ac = new CKEDITOR.plugins.autocomplete(
          editor,
          CKEDITOR.tools.object.merge(configDefinition, {
