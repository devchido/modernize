...
bender.test({
  assertBackspace: function (name, key) {
    ...
    bender.tools.testInputOut(name, function (source, expected) {
      ...
      bender.Y.Event.simulate(bot.editor.editable().$, "keydown", {
        keyCode: key,
      });
  ...
  "test XML Entities": function () {
    ...
    assert.areEqual(
      "&apos;&quot;&lt;&gt;&amp;",
      this.editor.dataProcessor.toDataFormat(xmlEntities)
    );
  },

  "test Other Special Characters": function () {
    var specials =
      " ¡¢£¤¥¦§¨©ª«¬®¯°±²³´µ¶·¸¹º»¼½¾¿×÷ƒ•…′″‾⁄℘ℑℜ™ℵ←↑→↓↔↵⇐⇑⇒⇓⇔∀∂∃∅∇∈∉∋∏∑−∗√∝∞∠∧∨" + // jshint ignore:line
      "∩∪∫∴∼≅≈≠≡≤≥⊂⊃⊄⊆⊇⊕⊗⊥⋅⌈⌉⌊⌋⟨⟩◊♠♣♥♦ˆ˜   ‌‍‎‏–—‘’‚“”„†‡‰‹›"; // jshint ignore:line
    // Other characters are encoded as numeric entities.
    assert.isFalse(
      /&\w+?;/.test(this.editor.dataProcessor.toDataFormat(specials))
    );
  },
  ...
  "test HTML encoded in textarea": function () {
    var inputHtml =
        '<p><textarea> <b style="font-color: red"> aa </b> cc </textarea></p>',
      expectedHtml =
        "<p><textarea> &lt;b style=&quot;font-color: red&quot;&gt; aa &lt;/b&gt; cc </textarea></p>",
      editor = this.editor,
      bot = this.editorBot;
