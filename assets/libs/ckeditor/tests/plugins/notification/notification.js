...
function assertNotifications(editor, expectedNotifications) {
  ...
  assert.areSame(
    ...
    "Expected and actual notifications count are different"
  );
...
function assertNotification(expectedNotification, actualNotification) {
  ...
  assert.areSame(
    expectedNotification.duration,
    actualNotification.duration,
    "Duration should be the same."
  );
  if (actualNotification.type == "progresss") {
    assert.areSame(
      expectedNotification.progress,
...
function assertNotificationElement(expectedNotification, element) {
  ...
  assert.areSame(
    expectedNotification.message,
    messageElement.getHtml(),
    "Element message should be the same."
  );
  ...
  switch (expectedNotification.type) {
    ...
    case "success":
      assert.isTrue(
        element.hasClass("cke_notification_" + expectedNotification.type),
        "Element should have proper class."
      );
      assert.isNull(progressElement);
      break;
    case "progress":
      assert.isTrue(
        element.hasClass("cke_notification_info"),
        "Element should have proper class."
      );
      assert.isObject(progressElement, "There should be a progress element.");
      if (expectedNotification.progress) {
        assert.areSame(
          Math.round(expectedNotification.progress * 100) + "%",
          progressElement.getStyle("width")
        );
      } else {
...
bender.test({
  ...
  "test showNotification error": function () {
    ...
    assertNotifications(editor, [
      { message: "Foo", type: "error", duration: 3000 },
    ]);
  },
  ...
  "test showNotification progress": function () {
    ...
    assertNotifications(editor, [
      { message: "Foo", type: "progress", progress: 0.4 },
    ]);
  },
  ...
  "test close after timeout - info": function () {
    ...
    assertNotifications(editor, [
      { message: "Foo", type: "info", duration: 100 },
    ]);
  ...
  "test close after timeout - default value": function () {
    ...
    bender.editorBot.create(
      ...
      function (bot) {
        ...
        assertNotifications(editor, [
          { message: "Foo", type: "info", alert: true },
        ]);
  ...
  "test remove close timeout after update": function () {
    ...
    assertNotifications(editor, [
      { message: "Foo", type: "warning", alert: false },
    ]);
  },
  ...
  "test close using ESC": function () {
    ...
    editor.fire("key", {
      keyCode: 27,
      domEvent: { getKey: sinon.stub().returns(27) },
    }); /* ESC */
  ...
  "test do not close on other than ESC key": function () {
    ...
    editor.fire("key", {
      keyCode: 65,
      domEvent: { getKey: sinon.stub().returns(65) },
    }); /* A */
  ...
  "test update shown message": function () {
    ...
    assertNotifications(editor, [
      { message: "Bar", type: "info", alert: false },
    ]);
  },
  ...
  "test update shown message - important": function () {
    ...
    assertNotifications(editor, [
      { message: "Bar", type: "info", alert: true },
    ]);
  },
  ...
  "test update progress": function () {
    ...
    assertNotifications(editor, [
      { message: "Foo", type: "progress", progress: 0, alert: false },
    ]);

    notification.update({ type: "progress", progress: 0.5 });

    assertNotifications(editor, [
      { message: "Foo", type: "progress", progress: 0.5, alert: false },
    ]);

    notification.update({ type: "progress", progress: 1 });

    assertNotifications(editor, [
      { message: "Foo", type: "progress", progress: 1, alert: false },
    ]);

    notification.update({ type: "success" });

    assertNotifications(editor, [
      { message: "Foo", type: "success", alert: false },
    ]);
  },
  ...
  "test notification change type if progress is set": function () {
    ...
    assertNotifications(editor, [
      { message: "Foo", type: "warning", alert: true },
    ]);
