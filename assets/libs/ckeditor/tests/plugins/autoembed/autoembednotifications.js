...
bender.test({
  "test notifications showed when embedding is finished correctly":
    function () {
      ...
      jsonpCallback = function (urlTemplate, urlParams, callback) {
        resume(function () {
          ...
          assert.isTrue(
            ...
            "Notification should be showed once."
          );
          assert.areEqual(
            showNotificationSpy.firstCall.returnValue.type,
            "info",
            'Notification should be "info" type.'
          );
        });
      };
  ...
  "test notifications showed when embedding is finished with error":
    function () {
      var bot = this.editorBot,
        editor = bot.editor,
      ...
      jsonpCallback = function (
        ...
        resume(function () {
          ...
          showNotificationSpy.restore();
          assert.isTrue(
            showNotificationSpy.calledTwice,
            "Notification was showed twice."
          );
          assert.areEqual(
            showNotificationSpy.firstCall.returnValue.type,
            "info",
            'First notification should have "info" type.'
          );
          assert.areEqual(
            showNotificationSpy.secondCall.returnValue.type,
            "info",
            'Second notification should have "info" type.'
          );
        });
      };
