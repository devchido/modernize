...
bender.test({
  "test getWidgetDefinition with default config": function () {
    assert.areSame(
      this.editor.widgets.registered.embed,
      CKEDITOR.plugins.autoEmbed.getWidgetDefinition(this.editor, "url")
    );
  },

  "test getWidgetDefinition with default config and no embed widget":
    function () {
      var EMBED_SEMANTIC = {},
        editorMock = {
          widgets: {
      ...
      assert.areSame(
        EMBED_SEMANTIC,
        CKEDITOR.plugins.autoEmbed.getWidgetDefinition(editorMock, "url")
      );
    },
  ...
  "test getWidgetDefinition with string config - first existing used":
    function () {
  ...
  "test getWidgetDefinition with string config - null if none found":
    function () {
  ...
  "test getWidgetDefinition is used by the plugin": function () {
    var editor = this.editor,
      mock = sinon
        .stub(CKEDITOR.plugins.autoEmbed, "getWidgetDefinition")
        .returns(editor.widgets.registered.embedSemantic);
    ...
    wait(function () {
      ...
      assert.areSame(
        "<p>foo</p><oembed>http://foo.bar/100</oembed><p>bar</p>",
        editor.getData(),
        "embedsemantic was used"
      );
      assert.isTrue(mock.calledOnce, "getWidgetDefinition was called once");
      assert.areSame(
        "http://foo.bar/100",
        mock.args[0][1],
        "getWidgetDefinition was called with the URL being embedded"
      );
    }, 400);
  },
});
