...
bender.test({
  ...
  "test working example": function () {
    ...
    this.editor.once(
      ...
      function (evt) {
        assert.isMatching(
          /^<a data-cke-autoembed="\d+" href="https:\/\/foo.bar\/g\/200\/300">https:\/\/foo.bar\/g\/200\/300<\/a>$/i,
          evt.data.dataValue
        );
      },
      null,
    ...
    bot.setData("<p>This is an embed</p>", function () {
      ...
      assert.areSame(
        '<p>This is an<a href="https://foo.bar/g/200/300">https://foo.bar/g/200/300</a> embed</p>',
        bot.getData()
      );

      wait(function () {
        assert.areSame(
          '<p>This is an</p><div data-oembed-url="https://foo.bar/g/200/300"><img src="https://foo.bar/g/200/300" /></div><p>embed</p>',
          bot.getData()
        );
      }, 400);
    });
  ...
  "test when user splits the link before the request is finished": function () {
    ...
    bot.setData("", function () {
      ...
      // Note: afterPaste is fired asynchronously, but we can test editor data immediately.
      assert.areSame(
        '<p><a href="https://foo.bar/g/200/304">https://foo.bar/g/200/304</a></p>',
        bot.getData(1)
      );
  ...
  "test link with encodable characters": function () {
    ...
    for (var i = links.length; i--; ) {
      assertPasteEvent(this.editor, { dataValue: links[i] }, function (data) {
        ...
        assert.isMatching(
          autoEmbedRegExp,
          bender.tools.html.prepareInnerHtmlForComparison(data.dataValue)
        );
      });
    }
  },
  ...
  "test link with attributes is auto embedded": function () {
    var pastedText =
        '<a id="kitty" name="colonelMeow" href="https://foo.bar/bom">https://foo.bar/bom</a>',
      expected =
  ...
  "test anchor is not auto embedded": function () {
    ...
    assertPasteEvent(
      ...
      { dataValue: pastedText, type: "html" }
    );
  },

  // Because it means that user copied a linked text, not a link.
  "test link with text different than its href is not auto embedded":
    function () {
  ...
  "test 2 step undo": function () {
    var bot = this.editorBot,
      ...
      finalData =
        '<p>foo</p><div data-oembed-url="' +
        pastedText +
        '"><img src="' +
        pastedText +
        '" /></div><p>bar</p>',
      linkData =
        '<p>foo<a href="' + pastedText + '">' + pastedText + "</a>bar</p>",
  ...
  "test internal paste is not auto embedded - link": function () {
    var editor = this.editor,
      pastedText =
        '<a href="https://foo.bar/g/185/310">https://foo.bar/g/185/310</a>';
      ...
      resume(function () {
        ...
        wait(function () {
          assert.areEqual(
            ...
            "Link should not be re–embedded."
          );
        }, 400);
      });
    };
  ...
  "test when user press undo before embedding process finishes": function () {
    var bot = this.editorBot,
      editor = bot.editor,
      pastedText = "https://foo.bar/g/200/382",
      finalizeCreationSpy = sinon.spy(
        CKEDITOR.plugins.widget.repository.prototype,
        "finalizeCreation"
      );
    ...
    bot.setData("", function () {
      ...
      wait(function () {
        ...
        assert.areSame(
          finalizeCreationSpy.called,
          false,
          "finalize creation was not called"
        );
      }, 400);
    });
  },

  "check if notifications are showed after unsuccessful embedding":
    function () {
      var bot = this.editorBot,
        editor = bot.editor,
        firstRequest = true,
        notificationShowSpy = sinon.spy(
          CKEDITOR.plugins.notification.prototype,
          "show"
        );
      ...
      jsonpCallback = function (
        ...
        if (firstRequest) {
          ...
          resume(function () {
            ...
            assert.isTrue(
              notificationShowSpy.calledThrice,
              "Notification should be showed three times."
            );
          });
        }
      };
  ...
  "test selection after auto embedding - empty editor": function () {
    ...
    bot.setData("", function () {
      ...
      wait(function () {
        ...
        assert.isTrue(
          range.startContainer.equals(p),
          "selection inside correct p element"
        );
      }, 400);
    });
  },
  ...
  "test selection after auto embedding - inside content": function () {
    ...
    bot.setData("<p>foo</p><p>bar</p>", function () {
      ...
      wait(function () {
        ...
        assert.isTrue(
          !!new CKEDITOR.dom.elementPath(container).contains(p),
          "selection inside correct p element"
        );
        assert.isInnerHtmlMatching(
          "bar@",
          (container.type == CKEDITOR.NODE_TEXT
            ? container.getParent()
            : container
          ).getHtml(),
          "selection inside correct p element"
  ...
  // https://dev.ckeditor.com/ticket/13429.
  "test selection after auto embedding - content and selection change before insert":
    function () {
      var bot = this.editorBot,
        editor = bot.editor,
      ...
      bot.setData("", function () {
        ...
        wait(function () {
          ...
          assert.isMatching(
            ...
            "selection anchored at the right position"
          );
        }, 400);
      });
    },
