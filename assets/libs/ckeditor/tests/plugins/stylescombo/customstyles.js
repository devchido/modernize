...
(function () {
  ...
  function parseListblock(listBlock) {
    ...
    for (var i = 0; i < els.count(); ++i) {
      ...
      if (el.getFirst().type == CKEDITOR.NODE_ELEMENT) el = el.getFirst();
  ...
  bender.test({
    "test display order of a custom styleSet": function () {
      ...
      assert.areSame(
        "Object Styles, St1, St5, St6, Block Styles, St4, Inline Styles, St2, St3",
        labels.join(", ")
      );
      ...
      assert.areSame(
        "St1, St6, St2",
        buildPreviewCalled.join(", "),
        "custom buildPreview were called on every style"
      );
    },
  });
})();
