  ...
  bender.test({
    "test display order of a custom styleSet": function () {
      ...
      assert.areEqual(
        testStyles.length,
        keys.length,
        "A number of styles matches."
      );
      arrayAssert.itemsAreSame(
        blockNames.concat(inlineNames),
        keys,
        "Styles are in ascending order, grouped."
      );
    },
    ...
    "test external styles are registerd in ACF": function () {
      bender.editorBot.create(
        ...
        function (bot) {
          assert.areSame(
            '<h2 style="font-style:italic">A</h2><p>X<big>Y</big>Z</p>',
            bot.editor.dataProcessor.toHtml(
              '<h2 style="font-style:italic">A</h2><p>X<big>Y</big>Z</p>'
            )
          );
        }
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          bot.combo("Styles", function (combo) {
            ...
            assert.isNotNull(
              list.findOne("#" + squareOptionId),
              "Square Bulleted List should be avialable."
            );
            assert.areNotSame(
              "none",
              list
                .findOne("#" + squareOptionId)
                .getStyle("display")
                .toLowerCase(),
