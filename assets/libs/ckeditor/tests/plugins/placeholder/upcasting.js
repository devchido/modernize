  ...
  bender.test({
    "test placeholder in title": function () {
      ...
      this.editorBots.fullPage.setData(sourceHtml, function () {
        ...
        assert.isTrue(
          data.indexOf("<title>foo[[bar]]bom</title>") > -1,
    ...
    "test placeholder in textarea": function () {
      var sourceHtml =
        '<p><textarea cols="10" rows="10">foo[[bar]]bom</textarea></p>';
    ...
    "test placeholder in custom element": function () {
      ...
      this.editorBots.fullPage.setData(sourceHtml, function () {
        ...
        assert.isMatching(
          ...
          data,
          "Custom element survived"
        );
        assert.areSame(
          1,
          CKEDITOR.tools.object.keys(this.editors.fullPage.widgets.instances)
            .length,
