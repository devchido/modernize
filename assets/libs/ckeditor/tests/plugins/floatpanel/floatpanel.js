...
(function () {
  ...
  bender.test({
    ...
    "test panel reposition on window resize": function () {
      ...
      assert.isTrue(
        showBlockSpy.calledTwice,
        "panel.showBlock should be called twice"
      );
      assert.isTrue(
        repositionSpy.calledOnce,
        "panel.reposition should be called once"
      );

      // Check if showBlock is called second time with same parameters.
      args = showBlockSpy.firstCall.args;
      assert.isTrue(
        showBlockSpy.secondCall.calledWith(args[0], args[1], args[2]),
        "Second showBlock() call should use same arguments as first call."
      );
    },
      ...
      editor.once("panelHide", function () {
        editor.once("panelHide", function () {
          resume(function () {
            assert.isTrue(
              true,
              "Second panel was closed after editor's been resized."
            );
          });
        });
        editor.once("panelShow", function () {
