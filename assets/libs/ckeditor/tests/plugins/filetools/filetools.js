...
(function () {
  ...
  bender.test({
    ...
    "test getUploadUrl - throw error if no matching config": function () {
      ...
      assert.isNull(
        uploadUrl,
        "null returned when none of upload URLs is defined"
      );
    },

    "test isTypeSupported 1": function () {
      assert.isTrue(
        isTypeSupported({ type: "image/jpeg" }, /image\/(png|jpeg|gif)/)
      );
    },

    "test isTypeSupported 2": function () {
      assert.isFalse(
        isTypeSupported({ type: "image/jpeg" }, /image\/(png|gif)/)
      );
    },
    ...
    "test UploadRepository.create allows changing fileLoader type":
      function () {
        function CustomType() {}
    ...
    "test UploadRepository instanceCreated event": function () {
      ...
      assert.areSame(
        loader,
        listener.firstCall.args[0].data,
        "Should be called with loader."
      );
    },
    ...
    "test fileUploadResponse event": function () {
      ...
      assert.areEqual(
        message,
        log.firstCall.args[1].responseText,
        "responseText should match"
      );
    },
        ...
        fileLoader: {
          ...
          xhr: {
            ...
            send: function () {
              resume(function () {
                assert.isTrue(
                  appendSpy.lastCall.calledWithExactly(
                    "ckCsrfToken",
    ...
    "test ensure onAbort is called (https://dev.ckeditor.com/ticket/13812)":
      function () {
        var file;
