...
(function () {
  ...
  function addTestWidget(editor) {
    ...
    plugin.addImageWidget(
      editor,
      "testWidget",
      plugin.addFeature(editor, "caption", {})
    );
  }
  ...
  function assertVisibility(caption, isVisible, msg, callback) {
    assert["is" + (isVisible ? "False" : "True")](
      !!caption.data("cke-caption-hidden"),
      msg
    );
  ...
  function createMultipleToggleTest(options) {
    return function (editor, bot) {
      ...
      bot.setData(getFixture(options.fixture), function () {
        var widgets = bender.tools
          .objToArray(editor.widgets.instances)
          .slice(0, options.widgetsCount);
  ...
  function assertPlaceholder(widget, isVisible) {
    ...
    assert["is" + (isVisible ? "True" : "False")](
      !!placeholder,
    ...
    if (isVisible) {
      assert.areSame(
        widget.editor.lang.imagebase.captionPlaceholder,
        placeholder,
        "Placeholder value"
      );
    }
  ...
  var tests = {
    ...
    "test upcasting widget without figcaption element": function (editor, bot) {
      ...
      widgetTestsTools.assertWidget({
        ...
        assertCreated: function (widget) {
          ...
          assert.isTrue(
            figcaptions.getItem(0).equals(widget.parts.caption),
            "Widget caption part element"
          );
          assert.isTrue(
            CKEDITOR.plugins.widget.isDomNestedEditable(widget.parts.caption),
    ...
    "test toggling caption (one widget with non-empty caption)":
      createToggleTest({
        fixture: "toggleOne",
        initial: true,
        focus: true,
    ...
    "test toggling caption (one widget with empty caption that gets filled)":
      createToggleTest({
        fixture: "toggleOneEmpty",
        initial: false,
        focus: true,
    ...
    "test toggling caption (one widget with non-empty caption that gets emptied)":
      createToggleTest({
        fixture: "toggleOne",
        initial: true,
        focus: true,
    ...
    "test toggling caption (one widget with non-empty caption that gets emptied; focus in caption)":
      createToggleTest({
        fixture: "toggleOne",
        initial: true,
        focus: true,
    ...
    "test toggling captions (two widgets with empty captions)":
      createMultipleToggleTest({
    ...
    "test toggling captions (two widgets with non-empty captions)":
      createMultipleToggleTest({
        fixture: "toggleTwo",
        widgetsCount: 2,
        initial: [true, true],
    ...
    "test placeholder visibility (widget with non-empty caption)":
      createToggleTest({
        fixture: "toggleOne",
        initial: true,
        focus: true,
    ...
    "test blurring editor hides visible, empty caption (widget focused)":
      createToggleTest({
        fixture: "toggleOneEmpty",
        initial: false,
        focus: true,
    ...
    "test blurring editor hides visible, empty caption (caption focused)":
      createToggleTest({
        fixture: "toggleOneEmpty",
        initial: false,
        focus: true,
    ...
    // (#1776)
    "test empty caption placeholder is hidden when blurred": function (
      editor,
      bot
    ) {
      addTestWidget(editor);
      ...
      bot.setData(getFixture("blurEmpty"), function () {
        ...
        blurEditor({
          assert: function () {
            assert.areEqual(
              "true",
              emptyCaptionWidget.parts.caption.data("cke-caption-hidden")
            );
          },
    ...
    bender.editorBot.create(
      ...
      function (bot) {
        ...
        bot.setData(getFixture("toggleOneEmpty"), function () {
          assert.isFalse(
            !!widgetTestsTools.getWidgetByDOMOffset(editor, 0).parts.caption,
            "Caption presence"
          );
        });
      }
