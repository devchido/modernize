...
(function () {
  ...
  function assertAttributes(editor, linkElement, linkData) {
    ...
    for (attribute in attributes.set) {
      assert.areSame(
        ...
        linkElement.getAttribute(attribute),
    ...
    CKEDITOR.tools.array.forEach(attributes.removed, function (attribute) {
      assert.isFalse(
        linkElement.hasAttribute(attribute),
        "Link does not have " + attribute + " attribute"
      );
    });
  }
  ...
  function assertLinkWidgetStatus(options) {
    var editor = options.editor,
      ...
      widget =
        options.widget || widgetTestsTools.getWidgetByDOMOffset(editor, 0);

    widget.focus();

    assert.areSame(
      options.linkCount,
      widget.element.find("a").count(),
      "There is only one link element inside widget"
    );
    assert.areSame(options.linkCmdState, linkCmd.state, "Link command state");
    assert.areSame(
      options.unlinkCmdState,
      unlinkCmd.state,
      "Unlink command state"
    );
  ...
  function testLinkCommand(options) {
    var bot = options.bot,
      ...
      unlinkCmd = editor.getCommand("unlink"),
      linkCmdState =
        typeof options.linkCmdState === "number"
          ? options.linkCmdState
          : CKEDITOR.TRISTATE_OFF,
      unlinkCmdState =
        typeof options.unlinkCmdState === "number"
          ? options.unlinkCmdState
  ...
  function testLinkOptionInContextMenu(editor, bot, data, testCase, cb) {
    ...
    bot.setData(data, function () {
      var widget = editor.widgets.getByElement(
        editor.editable().findOne("figure")
      );
  ...
  var tests = {
    ...
    "test adding image widget with link feature": function (editor) {
      ...
      objectAssert.areDeepEqual(
        expectedParts,
        editor.widgets.registered.testWidget.parts
      );
    },
    ...
    "test add link with attribute to existing image widget": function (
      editor,
      bot
    ) {
      addTestWidget(editor);
    ...
    "test edit link attribute in existing image widget": function (
      editor,
      bot
    ) {
      addTestWidget(editor);
    ...
    "test remove link attribute in existing image widget": function (
      editor,
      bot
    ) {
      addTestWidget(editor);
    ...
    "test dialog ok listener is deleted when closing the dialog": function (
      editor,
      bot
    ) {
      addTestWidget(editor);
        ...
        dialogCallback: function (dialog) {
          ...
          editor.once("dialogShow", function () {
            resume(function () {
              ...
              assert.areSame(
                1,
                editor.editable().find("p > a").count(),
                "Link is correctly added"
              );
            });
          });
    ...
    // #tp3298
    "test calling setData after unlink keeps correct data.link": function (
      editor,
      bot
    ) {
      addTestWidget(editor);
    ...
    "test link option in context menu": function (editor, bot) {
      testLinkOptionInContextMenu(
        editor,
        bot,
        '<figure><a href="http://foo"><img src="%BASE_PATH%_assets/logo.png"></a></figure>',
        ...
        function (itemsExist) {
          assert.areSame(
            1,
            itemsExist,
            "there is one link item in context menu"
          );
        }
      );
    },
      ...
      testLinkOptionInContextMenu(
        ...
        function (itemsExist) {
          assert.areSame(
            1,
            itemsExist,
            "there is one unlink item in context menu"
          );
        }
      );
    },
    "test no link option in context menu": function (editor, bot) {
      testLinkOptionInContextMenu(
        editor,
        bot,
        '<figure><img src="%BASE_PATH%_assets/logo.png"></figure>',
        function (editor, index) {
          return (
            editor.contextMenu.items[index].command == "unlink" ||
            editor.contextMenu.items[index].command == "link"
          );
        },
        function (itemsExist) {
          assert.areSame(
