    ...
    tests = {
      init: function () {
        ...
        plugin.addImageWidget(
          editor,
          imageWidgetDef.name,
          plugin.addFeature(editor, "upload", imageWidgetDef)
        );

        plugin.addImageWidget(
          editor,
          textWidgetDef.name,
          plugin.addFeature(editor, "upload", textWidgetDef)
        );

        plugin.addImageWidget(
          editor,
          asyncSuccessWidgetDef.name,
      ...
      "test loader can be customized": function () {
        ...
        assertPasteFiles(editor, {
          ...
          callback: function () {
            ...
            editor.widgets.registered.testImageWidget.loaderType =
              originalLoader;

            assert.areSame(
              1,
              CustomDummyType.callCount,
              "CustomDummyType constructor calls"
            );
          },
        });
      },
      ...
      "test events": function () {
        ...
        assertPasteFiles(editor, {
          ...
          callback: function () {
            assert.areSame(
              1,
              stubs.uploadStarted.callCount,
              "uploadStarted event count"
            );
            sinon.assert.calledWithExactly(
              stubs.uploadStarted,
              sinon.match.has("data", editor.uploadRepository.loaders[0])
            );

            assert.areSame(
              1,
              stubs.uploadDone.callCount,
              "uploadDone event count"
            );

            assert.areSame(
              0,
              stubs.uploadFailed.callCount,
              "uploadFailed event count"
            );
          },
        });
      },
      ...
      "test upload error": function () {
        ...
        assertPasteFiles(editor, {
          ...
          callback: function (widgets) {
            ...
            sinon.assert.calledWithExactly(
              stubs.uploadStarted,
              sinon.match.has("data", loaderInstance)
            );
            ...
            assert.areSame(
              loaderInstance,
              stubs.uploadFailed.args[0][0].data.loader,
              "Event data.loader"
            );
      ...
      "test upload error event is cancelable": function () {
        ...
        this.listeners.push(
          editor.widgets.on("instanceCreated", function (evt) {
            that.listeners.push(
              evt.data.on("uploadFailed", function (evt) {
                evt.cancel();
              })
            );
          })
      ...
      "test default progress reporter": function () {
        var imageBase = CKEDITOR.plugins.imagebase,
          ProgressBarSpy = this.sandbox.spy(
            this.editor.widgets.registered.testImageWidget,
            "progressReporterType"
          ),
          bindLoaderSpy = this.sandbox.spy(
            imageBase.progressBar.prototype,
            "bindLoader"
          ),
          editor = this.editor;
        ...
        assertPasteFiles(this.editor, {
          ...
          callback: function () {
            assert.areSame(
              1,
              ProgressBarSpy.callCount,
              "ProgressBar constructor call count"
            );

            sinon.assert.calledWithExactly(
              bindLoaderSpy,
              editor.uploadRepository.loaders[0]
            );
          },
        });
      },
      ...
      "test progress reporter customization": function () {
        ...
        assertPasteFiles(this.editor, {
          ...
          callback: function () {
            ...
            assert.areSame(
              1,
              CustomProgress.callCount,
              "CustomProgress constructor call count"
            );
          },
        });
      },
      ...
      "test preventing default progress reporter with event": function () {
          ...
          def = {
            ...
            init: function () {
              disposableListeners.push(
                this.on("uploadStarted", function (evt) {
                  evt.cancel();
                })
              );
            },
        ...
        imageBase.addImageWidget(
          editor,
          def.name,
          imageBase.addFeature(editor, "upload", def)
        );

        editor.widgets.registered.progressPrevent.progressReporterType =
          ProgressBarSpy;
      ...
      "test preventing default progress reporter with widgetDefinition.progressReporterType":
        function () {
      ...
      "test dropping file into a readonly does not create a widget":
        function () {
          var editor = this.editor;
      ...
      "test widget gets class during the upload": function () {
        ...
        assertPasteFiles(editor, {
          ...
          callback: function (widgets) {
            ...
            assert.isTrue(
              widget.wrapper.hasClass("cke_widget_wrapper_uploading"),
              "Class is present during the upload"
            );
      ...
      "test data.uploadId behavior": function () {
        ...
        assertPasteFiles(editor, {
          ...
          callback: function (widgets) {
            ...
            assert.areSame(
              0,
              widget.data.uploadId,
              "Value immediately after creating a widget"
            );
            widget.once("uploadDone", function () {
              resume(function () {
                assert.isUndefined(
                  widget.data.uploadId,
                  "Value after upload is complete"
                );
              });
            });
      ...
      "test copy and paste file during upload": function () {
        ...
        this.listeners.push(
          this.editor.widgets.on("instanceCreated", function (evt) {
            var widget = evt.data;
              ...
              callback: function (widgets) {
                setTimeout(function () {
                  resume(function () {
                    ...
                    assert.areSame(
                      2,
                      doneEventSpy.callCount,
                      "uploadDone events count"
                    );
      ...
      "test copy and paste in progress widget while original was loaded":
        function () {
      ...
      "test copy in-progress widget into a different editor": function () {
        ...
        bender.editorBot.create(
          ...
          function (bot) {
            ...
            bot.editor.widgets.add(
              "testAsyncSuccess",
              sourceEditor.widgets.registered.testAsyncSuccess
            );
              ...
              callback: function (initialWidgets) {
                ...
                assertPasteFiles(destinationEditor, {
                  ...
                  callback: function (widgets) {
                    assert.areSame(1, widgets.length, "Widgets count");
                    assert.areNotSame(
                      sourceEditor.uploadRepository.loaders[0],
                      destinationEditor.uploadRepository.loaders[0],
