...
bender.test({
  ...
  "test button label": function () {
    ...
    assert.areEqual(
      "aria label",
      label.getText(),
      "innerText of label doesn't match"
    );
  },
  ...
  "test button label with arrow": function () {
    ...
    var buttonEl = this.getButtonDomElement(button),
      label = CKEDITOR.document.getById(
        buttonEl.getAttribute("aria-labelledby")
      );
  ...
  assertAttribtues: function (expectedAttributes, button) {
    ...
    for (var attrName in expectedAttributes) {
      assert.isTrue(
        buttonElement.hasAttribute(attrName),
        "Button HTML element does not contain " + attrName + " attribute."
      );
      ...
      if (expectedValue instanceof RegExp)
        assert.isTrue(
          expectedValue.test(attributeValue),
          "Attribute " +
            attrName +
            " did not matched expected " +
            expectedValue +
            " regex"
        );
      else
