...
(function () {
      ...
      "test dialog required content": function (editor, bot) {
        bot.setData(
          ...
          function () {
            ...
            rng.setStart(
              editor.editable().findOne("td"),
              CKEDITOR.POSITION_AFTER_START
            );
            rng.select();

            bot.contextmenu(function (menu) {
              var tableCellSubmenu = CKEDITOR.tools.array.filter(
                  menu.items,
                  function (item) {
              ...
              assert[editor.name === "editor" ? "isFalse" : "isTrue"](
                "tablecell_properties" in items
              );
              menu.hide();
              ...
              if (editor.name === "editor") {
                assert.areSame(
                  ...
                  editor.getCommand("cellProperties").state
                );
              } else {
                bot.dialog("cellProperties", function (dialog) {
                  for (var key in filterMap) {
                    ...
                    assert[shouldBeAllowed ? "isUndefined" : "isTrue"](
                      isNotAllowed(key),
                      "Dialog " + key
                    );
                  }
  ...
  for (var key in filterMap) {
    if (extraAllowedContentList.length) {
      extraAllowedContentList.push(
        extraAllowedContentList[extraAllowedContentList.length - 1].concat([
          key,
        ])
      );
    }
    ...
    function (array, index) {
      ...
      CKEDITOR.tools.array.forEach(array, function (key) {
        ...
        if (!extraAllowedContent[elementName][propertyName]) {
          extraAllowedContent[elementName][propertyName] = [
            allowedStyleOrAttribute,
          ];
        } else {
          extraAllowedContent[elementName][propertyName].push(
            allowedStyleOrAttribute
          );
        }
      });
