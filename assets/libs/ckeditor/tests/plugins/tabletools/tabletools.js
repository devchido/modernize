...
(function () {
  ...
  bender.test({
    ...
    "test tabletools.insertColumn() return value": function () {
      ...
      ret = CKEDITOR.plugins.tabletools.insertColumn([
        table.find("td").getItem(1),
        table.find("td").getItem(3),
      ]);

      assert.isArray(ret, "Return type");
      assert.areSame(2, ret.length, "Returned items");
      assert.areSame(
        table.find("td").getItem(2),
        ret[0],
        "Returned element  0"
      );
      assert.areSame(
        table.find("td").getItem(5),
        ret[1],
        "Returned element  1"
      );
    },
    ...
    "test split cells (7)": function () {
      ...
      bender.tools.testInputOut("split-cells-7", function (source, expected) {
        ...
        range.moveToPosition(
          bot.editor.document.getById("cursor"),
        ...
        function (source, expected) {
          if (CKEDITOR.env.ie && CKEDITOR.env.version === 8) {
            ...
            expected = expected.replace(
              "no-repeat center #00cc99",
              "#00cc99 no-repeat center 50%"
            );
          }
          bot.setHtmlWithSelection(source);
          assert.beautified.html(expected, bot.getData(true));
