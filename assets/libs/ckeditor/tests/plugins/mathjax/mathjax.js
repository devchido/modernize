...
(function () {
  ...
  function assertIFrame(iFrame) {
    ...
    assert.isTrue(
      parseInt(iFrame.getStyle("width"), 10) > 0,
      "Width of iFrame should be grater that 0."
    );
    assert.isTrue(
      parseInt(iFrame.getStyle("height"), 10) > 0,
  ...
  bender.test({
    checkMathJax: function (config) {
      ...
      if (config.when) config.when(iFrame, editor);
      ...
      CKEDITOR.once("mathJaxUpdateDone", function () {
        resume(function () {
          ...
          if (config.then) config.then(iFrame, editor);
        });
      });
      ...
      editor = new CKEDITOR.replace("editor_mathjax", {
        ...
        on: {
          instanceReady: function () {
            editor.setData(
              '<p>A<span class="math-tex">\\(1 + 1 = 2\\)</span>B</p>'
            );
          },
        },
      });
    ...
    "test copy style when element is created": function () {
      ...
      editor.setData(
        '<p style="font-size:10px">A<span class="math-tex">\\(1 + 1 = 2\\)</span>B</p>'
      );
    ...
    "test copy style when outer text format changed": function () {
      this.checkMathJax({
        when: function (iFrame, editor) {
          editor
            .getSelection()
            .selectElement(editor.document.getElementsByTag("p").getItem(0));
        ...
        then: function () {
          that.checkMathJax({
            ...
            then: function (iFrame, editor) {
              assert.areSame(
                0,
                editor.document.getElementsByTag("strong").count()
              );
            },
          });
        },
        ...
        function (bot) {
          ...
          editor.openDialog("mathjax", function (dialog) {
            ...
            dialog.on("show", function () {
              ...
              var previewDomId = dialog.getContentElement(
                  "info",
                  "preview"
                ).domId,
                iFrame = CKEDITOR.document
                  .getById(previewDomId)
