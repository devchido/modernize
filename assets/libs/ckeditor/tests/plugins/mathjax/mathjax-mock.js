    ...
    tcs = {
          ...
          dialog.on("show", function () {
            ...
            setTimeout(function () {
              resume(function () {
                assert.areSame(
                  " X \\(1 + 1 = 2\\) Y ",
                  dialog.getValueOf("info", "equation"),
      ...
      "test integration with preview plugin": function () {
        ...
        editor.once("contentPreview", function (evt) {
          ...
          resume(function () {
            assert.isTrue(
              !!evt.data.dataValue.match(
                new RegExp(
                  '<script src="' +
                    bender.tools.escapeRegExp("http://mathJaxLib-mock") +
                    '([^"]+)?">'
                )
              )
            );
      ...
      "test conflict with iframe plugin": function () {
        ...
        editor.on("afterPaste", function () {
          resume(function () {
            assert.areSame(
              2,
              editor.document.getElementsByTag("iframe").count(),
              "There should be two iFrames."
            );
          });
        });
        ...
        bender.tools.emulatePaste(
          editor,
          editor.document.getElementsByTag("p").getItem(0).$.innerHTML
        );
        ...
        bot.setData('<p><span class="math-tex">\\(\\)</span></p>', function () {
          ...
          bot.setData(data, function () {
            assert.areSame(
              '<p><span class="math-tex">\\(&amp;\\)</span></p>',
              editor.getData(),
              "& should not change after loading data."
            );
            ...
            assert.areSame(
              "\\(&\\)",
              widget.data.math,
              "data.math was loaded correctly"
            );
          });
        });
      },
