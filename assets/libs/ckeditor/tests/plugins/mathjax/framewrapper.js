  ...
  bender.test({
    checkIFrame: function (config) {
      ...
      CKEDITOR.on("mathJaxLoaded", function () {
        if (config.loadedSync) config.loadedSync(frameWrapper, doc);
      ...
      CKEDITOR.on("mathJaxUpdateDone", function () {
        if (config.done) config.done(frameWrapper, doc);
        ...
        if (counter > config.expectedUpdateCount)
          resume(function () {
            assert.fail(
              "Too many MathJaxUpdateDone (expected: " +
                config.expectedUpdateCount +
                ", actual: " +
                counter +
                ")."
            );
          ...
          resume(function () {
            assert.areSame(
              ...
              bender.tools.compatHtml(
                doc.getById("preview").getElementsByTag("script").$[0].innerHTML
              ),
            ...
            assert.areSame(
              config.expectedValue,
              bender.tools.compatHtml(
                doc.getById("buffer").getElementsByTag("script").$[0].innerHTML
              ),
              "MathJax should create script element containing equation."
            );

            assert.isTrue(
              parseInt(iFrame.getStyle("width"), 0) > 0,
              "Width of iFrame should be grater that 0."
            );
            assert.isTrue(
              parseInt(iFrame.getStyle("height"), 0) > 0,
              "Height of iFrame should be grater that 0."
            );
      ...
      if (CKEDITOR.plugins.mathjax) startTest();
      else setTimeout(startTest, 100);
    ...
    "test init": function () {
      this.checkIFrame({
        ...
        loaded: function (frameWrapper, doc) {
          resume(function () {
            assert.isTrue(
              doc.getHead().getChildCount() > 6,
              "MathJax should create additional style tags in head (number depends on browser)."
            );

            assert.isFunction(
              frameWrapper.setValue,
              "Constructor should create wrapper"
            );
          });
        },
    ...
    "test copy style": function () {
      this.checkIFrame({
        ...
        setValue: "\\(1 + 1 = 2\\)",
          ...
          assert.areSame(
            "15px",
            doc.getById("preview").getComputedStyle("font-size")
          );
          assert.areSame(
            "normal",
            doc.getById("preview").getComputedStyle("font-style")
          );
        },
        expectedUpdateCount: 1,
        expectedValue: "1 + 1 = 2",
