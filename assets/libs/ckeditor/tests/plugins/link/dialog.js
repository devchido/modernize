...
(function () {
  ...
  function assertAnchorDiscovery(bot, expIds, expNames) {
    bot.dialog("link", function (dialog) {
      ...
      var idOptions = dialog
          .getContentElement("info", "anchorId")
      ...
      for (var i = idOptions.length; i--; ) ids.push(idOptions[i].value);

      for (i = nameOptions.length; i--; ) names.push(nameOptions[i].value);

      assert.areSame(
        expIds.sort().join(","),
        ids.sort().join(","),
        "Anchor IDs discovered properly"
      );
      assert.areSame(
        expNames.sort().join(","),
        names.sort().join(","),
        "Anchor names discovered properly"
      );
    });
  }
  ...
  bender.test({
    ...
    "test discovery of anchors (framed)": function () {
      assertAnchorDiscovery(
        this.editorBots.framed,
        ["aa", "ba", "ca", ""],
        ["ab", "bb", "cb", ""]
      );
    },
