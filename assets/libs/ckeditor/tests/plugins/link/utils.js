...
(function () {
  ...
  var parseLinkiAttributes, getLinkAttributes;
  ...
  function assertLink(
    ...
    html,
    expectedData,
    expectedAttributesSet,
    expectedAttributesRemoved
  ) {
    ...
    bot.setData(html, function () {
      ...
      assert.areSame(
        ...
        JSON.stringify(data),
        "Link attributes parsed correctly"
      );
      assert.areSame(
        JSON.stringify(expectedAttributesSet),
        JSON.stringify(attributes.set),
        "Attributes to be set"
      );
      assert.areSame(
        expectedAttributesRemoved.sort().join(","),
        attributes.removed.sort().join(","),
  ...
  bender.test({
    ...
    "test link attributes - complex": function () {
      var html =
        '<a accesskey="b" charset="i" class="h" dir="rtl" href="http://x" id="a" lang="d" name="c" ' +
      ...
      assertLink(
        ...
        ["data-cke-pa-onclick", "onclick"]
      );
    },

    "test link attributes - complex (partial)": function () {
      var html =
        '<a accesskey="b" charset="i" class="h" dir="rtl" href="http://x" id="a" name="c" rel="j" style="margin-right: 0px;" target="a" title="f" type="g" download="">foo</a>';
      ...
      assertLink(
        ...
        ["data-cke-pa-onclick", "onclick", "lang", "tabindex"]
      );
    },
    ...
    "test link attributes - target popup": function () {
      var html =
        "<a href=\"http://foo\" onclick=\"window.open(this.href, 'pop', 'resizable=yes,status=yes,location=yes,toolbar=yes,menubar=yes," +
      ...
      assertLink(
        ...
        },
        {
          ...
          "data-cke-pa-onclick":
            "window.open(this.href, 'pop', 'resizable=yes,status=yes,location=yes,toolbar=yes,menubar=yes,fullscreen=yes," +
            "scrollbars=yes,dependent=yes,width=10,left=20,height=30,top=40'); return false;",
          href: "http://foo",
        },
        ["data-cke-saved-name", "onclick", "target", "download"]
      );
    },
    ...
    "test link attributes - target _top": function () {
      ...
      assertLink(
        ...
        ["data-cke-pa-onclick", "data-cke-saved-name", "onclick", "download"]
      );
    },
  });
})();
