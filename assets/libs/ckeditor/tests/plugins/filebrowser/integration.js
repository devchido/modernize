...
(function () {
  ...
  bender.test({
    "test browse button visibility if no filebrowser plugin is loaded":
      function () {
        CKEDITOR.on("dialogDefinition", function (evt) {
          ...
          def.onShow = function () {
            ...
            resume(function () {
              try {
                assert.isTrue(
                  dialog.getContentElement("info", "browse").hidden,
                  "Browse button in the dialog should remain hidden."
                );
              } catch (e) {
                throw e;
              } finally {
