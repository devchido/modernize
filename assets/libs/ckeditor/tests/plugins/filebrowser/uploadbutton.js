...
(function () {
  ...
  bender.test({
          ...
          function () {
            resume(function () {
              ...
              objectAssert.areEqual(
                { foo: "bar", hello: "world" },
                this.requests[0].requestHeaders
              );
              dialog.hide();
              inputStub.restore();
            });
    ...
    // (#1835)
    "test for XHR request to CKFinder (no responseType parameter)":
      function () {
        if (!CKEDITOR.fileTools.isFileUploadSupported) {
          assert.ignore();
        }
            ...
            function () {
              resume(function () {
                assert.isMatching(
                  /responseType=json/g,
                  this.requests[0].url,
                  "responseType parameter was added"
                );
                dialog.hide();
                inputStub.restore();
              });
    ...
    // (#1835)
    "test for XHR request to CKFinder (responseType parameter present)":
      function () {
        if (!CKEDITOR.fileTools.isFileUploadSupported) {
          assert.ignore();
        }
          ...
          editor.once(
            ...
            function () {
              resume(function () {
                assert.areSame(
                  1,
                  this.requests[0].url.match(/responseType=json/g).length,
    ...
    // (#1835)
    "test for XHR request not to CKFinder (responseType parameter)":
      function () {
        if (!CKEDITOR.fileTools.isFileUploadSupported) {
          assert.ignore();
        }
        ...
        bot.dialog("testDialog", function (dialog) {
          ...
          editor.once(
            ...
            function () {
              resume(function () {
                assert.isNotMatching(
                  /responseType=json/g,
                  this.requests[0].url,
                  "responseType parameter is not added"
                );
                dialog.hide();
                inputStub.restore();
              });
        ...
        editor.once("fileUploadRequest", function (evt) {
          ...
          loader.once("abort", function () {
            resume(function () {
              ...
              assert.isTrue(
                sendButton.isEnabled(),
                "Button should be enabled after abort"
              );
