...
(function () {
    ...
    "test param allowFullScreen present in code with true value": function () {
      ...
      bot.setData("", function () {
        bot.dialog("flash", function (dialog) {
          ...
          var allowFullScreenBtn = dialog.getContentElement(
            "properties",
            "allowFullScreen"
          );
          assert.areEqual(
            true,
            allowFullScreenBtn.getValue(),
          ...
          assert.areEqual(
            "true",
            fullScreenParam.attributes.value,
          ...
          assert.areSame(
            "true",
            embed.attributes.allowfullscreen,
            "allowfullscreen set for embed"
          );

          selectFirstFlashElementInEditor(bot);

          bot.dialog("flash", function (dialog) {
            allowFullScreenBtn = dialog.getContentElement(
              "properties",
              "allowFullScreen"
            );
            ...
            assert.areEqual(
              true,
              allowFullScreenBtn.getValue(),
              "Default value should be still set to true"
            );
          });
        });
      });
        ...
        bot.dialog("flash", function (dialog) {
          ...
          bot.dialog("flash", function (dialog) {
            ...
            assert.areEqual(
              false,
              allowFullScreenBtn.getValue(),
              "Default value should be still set to false"
            );
          });
        });
      });
  ...
  function getFullScreenParam(bot) {
    return getObjectChildElement(bot, function (element) {
      return (
        element.name == "param" && element.attributes.name == "allowFullScreen"
      );
    });
  }
