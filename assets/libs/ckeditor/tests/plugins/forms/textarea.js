...
bender.test({
  test_createFillFields: function () {
    ...
    editorBot.dialog("textarea", function (dialog) {
      ...
      assert.areSame(
        '<p><textarea cols="80" name="test_textarea" required="required" rows="5">some testing value.</textarea></p>',
        editorBot.getData(true)
      );
    });
  },
  ...
  test_emptyFields: function () {
    ...
    bot.setHtmlWithSelection(
      '[<textarea cols="80" name="test_textarea" required="required" rows="5">some testing value.</textarea>]'
    );

    bot.dialog("textarea", function (dialog) {
      assert.areSame(
        "test_textarea",
        dialog.getValueOf("info", "_cke_saved_name")
      );
      ...
      assert.areSame(
        "<textarea>some testing value.</textarea>",
