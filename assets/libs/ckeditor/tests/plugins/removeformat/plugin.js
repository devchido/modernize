...
bender.test({
  "test remove format always fire editor#selectionChange": function () {
    ...
    ed.once("selectionChange", function () {
      assert.isTrue(
        true,
        '"selectionChange" event always fired after remove format.'
      );
    });
  ...
  "test remove format inside nested editable": function () {
    ...
    bot.setHtmlWithSelection(
      '<p>foo</p><div contenteditable="false"><p contenteditable="true">[<b>foo</b> bar]</p></div>'
    );

    bot.editor.execCommand("removeFormat");
    assert.areEqual(
      '<p>foo</p><div contenteditable="false"><p contenteditable="true">foo bar</p></div>',
      bot.getData()
    );
    ...
    assert.areSame(
      ...
      "Selection should not leak from nested editable"
    );
  },
  ...
  "test remove format outside the contenteditable=false": function () {
    ...
    bot.setHtmlWithSelection(
      '<p><strong>[<span contenteditable="false">foo</span>]</strong></p>'
    );
    bot.editor.execCommand("removeFormat");
    assert.areEqual(
  ...
  "test remove format outside of multiple contenteditable=false": function () {
    ...
    var bot = this.editorBot;
    bot.setHtmlWithSelection(
      '<p><strong>[<span contenteditable="false">foo</span>x<span contenteditable="false">foo</span>]</strong></p>'
    );
    bot.editor.execCommand("removeFormat");
    assert.areEqual(
      '<p><span contenteditable="false">foo</span>x<span contenteditable="false">foo</span></p>',
      bot.getData()
    );
  },
  ...
  "test remove format for cite element": function () {
    ...
    assert.areEqual(
      "<p>foo bar baz</p>",
      this.editor.getData(),
      "Cite element should be removed."
    );
  },
  ...
  "test editor#addRemoveFormatFilter": function () {
    bender.editorBot.create(
      ...
      function (bot) {
        bot.setHtmlWithSelection(
          '<p>[<span style="color:red">foo</span> <b>bar</b>]</p>'
        );
