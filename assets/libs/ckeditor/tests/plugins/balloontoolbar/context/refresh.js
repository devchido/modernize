  ...
  bender.test({
    ...
    "test refresh returning custom element": function () {
      this.editorBot.setHtmlWithSelection(
        "<p>foo<strong>ba^r</strong>baz<em>em</em></p>"
      );
    ...
    "test refresh subsequent negative matches will not override focus element":
      function () {
        this.editorBot.setHtmlWithSelection(
          "<p>foo<strong>ba^r</strong>baz<em>em</em></p>"
        );
        ...
        this._getContextStub(
          sinon.stub().returns(false),
          false,
          CKEDITOR.plugins.balloontoolbar.PRIORITY.HIGH
        );
    ...
    "test _matchRefresh return type": function () {
      var contextFalse = this._getContextStub(sinon.stub().returns(false)),
        ...
        contextElem = this._getContextStub(
          sinon.stub().returns(new CKEDITOR.dom.element("p"))
        );

      assert.isNull(contextFalse._matchRefresh(null, null));
      assert.isInstanceOf(
        CKEDITOR.dom.element,
        contextTrue._matchRefresh(null, null),
        "contextTrue return type"
      );
      assert.isInstanceOf(
        CKEDITOR.dom.element,
        contextElem._matchRefresh(null, null),
        "contextTrue return type"
      );
    },
