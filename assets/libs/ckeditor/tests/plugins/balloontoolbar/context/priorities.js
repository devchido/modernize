  ...
  bender.test({
    ...
    "test options.refresh has the highest priority": function () {
      this.editorBot.setData(
        CKEDITOR.document.getById("widgetWithEditable").getHtml(),
        function () {
          ...
          contextTools._assertToolbarVisible(
            true,
            contexts.refresh,
            "contexts.refresh visibility"
          );
          contextTools._assertToolbarVisible(
            false,
            contexts.widgets,
            "contexts.widgets visibility"
          );
          contextTools._assertToolbarVisible(
            false,
            contexts.cssSelector,
            "contexts.cssSelector visibility"
          );
        }
      );
    },

    "test options.refresh has the highest priority with widget selected":
      function () {
        this.editorBot.setData(
          CKEDITOR.document.getById("widgetWithEditable").getHtml(),
          function () {
    ...
    "test options.cssSelector the top-most patch match wins": function () {
      ...
      this.editorBot.setHtmlWithSelection(
        CKEDITOR.document.getById("nestedElements").getHtml()
      );

      var contexts = this._createContexts(
        ["cssSelector", "emContext", "citeContext"],
        true,
        additionalContextDefinitions
      );

      contextTools._assertToolbarVisible(
        true,
        contexts.citeContext,
        "contexts.citeContext visibility"
      );
      ...
      contextTools._assertToolbarVisible(
        false,
        contexts.emContext,
        "contexts.emContext visibility"
      );
    },

    "test options.cssSelector with a HGIH priority takes the precedence":
      function () {
        var additionalContextDefinitions = {
          cssSelector: {
            buttons: "Bold,Italic",
        ...
        var contexts = this._createContexts(
          ["cssSelector", "citeContext", "refresh"],
          true,
          additionalContextDefinitions
        );
    ...
    "test options.cssSelector with a LOW priority are less favorable":
      function () {
        var additionalContextDefinitions = {
          refresh: {
            buttons: "Bold,Italic",
    ...
    _createContexts: function (whitelist, autoRefresh, additionalMappings) {
      ...
      whitelist = CKEDITOR.tools.isArray(whitelist)
        ? whitelist
        : CKEDITOR.tools.object.keys(optionsMapping);

      // Eventually one might provide even more mappings.
      if (additionalMappings) {
        optionsMapping = CKEDITOR.tools.extend(
          optionsMapping,
          additionalMappings,
          true
        );
