...
(function () {
  ...
  function indexOfElement(dict, needle) {
    return CKEDITOR.tools.array.filter(
      CKEDITOR.tools.object.keys(dict),
      function (key) {
  ...
  bender.test({
    "test if balloon toolbar elements are registered as focusable":
      function () {
        ...
        assert.isNotUndefined(
          -1,
          indexOfElement(focusables, boldButton),
          "Button is contained in focusable list"
        );

        toolbar.deleteItem("Bold");
        assert.isUndefined(
          indexOfElement(focusables, boldButton),
          "Button was removed from focusable list"
        );
    ...
    "test focusing toolbar doesn't blur editor": function () {
      ...
      boldButton = context.toolbar._view.parts.content.findOne(
        "#" + context.toolbar._items.Bold._.id
      );
      ...
      boldButton.once("focus", function () {
        ...
        setTimeout(function () {
          resume(function () {
            assert.isFalse(
              blurSpy.called,
              "Editor should remain focused, when balloontoolbar is focused."
            );
            context.destroy();
          });
        }, 210);
