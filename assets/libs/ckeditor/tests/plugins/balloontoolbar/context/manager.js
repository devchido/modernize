  ...
  bender.test({
    ...
    "test manager uses custom selection if provided": function () {
      this.editorBot.setHtmlWithSelection(
        "<p>foo <strong>b^ar</strong> <em>baz</em></p>"
      );

      var selectionClone = new CKEDITOR.dom.selection(
      ...
      assert.areSame(
        "body,p,em",
        this._elementPathSerialize(refreshStub.args[0][1]),
        "Path provided to the options.refresh"
      );
      assert.areSame(
        selectionClone,
        refreshStub.args[0][2],
        "Selection provided to the options.refresh"
      );
    },
    ...
    "test manager uses shrinked selection": function () {
      ...
      assert.areSame(
        "body,p,strong",
        this._elementPathSerialize(refreshStub.args[0][1]),
        "Path provided to options.refresh"
      );
    },
    ...
    _elementPathSerialize: function (elementPath) {
      return CKEDITOR.tools.array
        .map(elementPath.elements, function (elem) {
          return elem.getName();
        })
        .reverse()
        .join(",");
