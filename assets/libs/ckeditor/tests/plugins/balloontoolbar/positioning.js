  ...
  var tests = {
    ...
    "test panel - out of view - bottom center": function (editor) {
      ...
      var markerElement = editor.editable().findOne("#marker"),
        ...
        viewPaneSpy = sinon
          .stub(CKEDITOR.dom.window.prototype, "getViewPaneSize")
          .returns({ width: 1000, height: 1000 }),
        scrollTop,
        balloonToolbarRect,
        rectTop;
      ...
      rectTop =
        CKEDITOR.env.ie && !CKEDITOR.env.edge
          ? Math.round(balloonToolbarRect.top)
          : balloonToolbarRect.top;
      ...
      var expectedLeft = makeExpectedLeft(
        frame.left + elementFrame.left + elementFrame.width / 2 - 50
      );
      assert.areEqual(
        expectedLeft,
        balloonToolbarRect.left.toFixed(2),
        "left align"
      );
      // We have to add 1px because of border.
      assert.areEqual(
        (frame.top + frame.height - scrollTop).toFixed(2),
        (
          rectTop +
          balloonToolbar.height +
          balloonToolbar.triangleHeight +
          1
        ).toFixed(2),
        "top align"
      );
    ...
    "test panel - out of view - hcenter top": function (editor) {
      if (
        editor.name == "divarea" ||
        (bender.config.isTravis && bender.tools.env.isBuild)
      ) {
        // divarea tests are failing, it's an upstream issue from balloonpanel (#1064).
      ...
      assert.areEqual(
        (frame.top - scrollTop).toFixed(2),
        (rectTop - balloonToolbar.triangleHeight).toFixed(2),
        "top align"
      );
      balloonToolbar.destroy();
      balloonToolbar = null;
    ...
    "test panel adds cke_balloontoolbar class": function (editor) {
      ...
      assert.isTrue(
        balloonToolbar.parts.panel.hasClass("cke_balloontoolbar"),
        "Panel has a cke_balloontoolbar class"
      );
      assert.isTrue(
        balloonToolbar.parts.panel.hasClass("cke_balloon"),
        "Class cke_balloon class was not removed"
      );
      balloonToolbar.destroy();
      balloonToolbar = null;
    },
    ...
    "test panel prefers bottom positioning": function (editor) {
      ...
      var res = balloonToolbar._getAlignments(
        editor.editable().getFirst().getClientRect(),
        10,
        10
      );
      ...
      arrayAssert.itemsAreEqual(
        ...
        CKEDITOR.tools.object.keys(res)
      );
    },
    ...
    "test panel refresh position": function (editor, bot) {
      bot.setData(
        '<img src="' + bender.basePath + '/_assets/lena.jpg">',
        function () {
          balloonToolbar = new CKEDITOR.ui.balloonToolbarView(editor, {
            width: 100,
            height: 200,
