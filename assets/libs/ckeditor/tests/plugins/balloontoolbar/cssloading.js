  ...
  bender.test({
    _should: {
      ignore: {
        ...
        "test loading CSS with path":
          bender.tools.env.isBuild ||
    ...
    "test loading CSS with path": function () {
      assert.areSame(
        "kama,%BASE_PATH%../../apps/ckeditor/skins/kama/",
        CKEDITOR.skinName,
        "Config skinName should be with path"
      );
      // (#2796)
      ...
      sinon.assert.calledWith(
        spy,
        balloonTestsTools.getDocumentOrigin() +
          balloonTestsTools.removeDotsFromUrl(
            "%BASE_PATH%../../apps/ckeditor/plugins/balloontoolbar/skins/kama/balloontoolbar.css"
          )
      );
    },
  });
