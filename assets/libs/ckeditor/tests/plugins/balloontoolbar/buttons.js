...
(function () {
  ...
  var tests = {
    ...
    "test button group wrapping": function () {
      ...
      assert.isNotNull(
        panel._view.parts.content.findOne(".cke_toolgroup"),
        "Button should be wrapped in group"
      );
    },
    ...
    "test group wrapping omitting rich combo": function () {
      ...
      assert.isNull(
        panel._view.parts.content.findOne(".cke_toolgroup"),
        "Checkbox should not be wrapped in group"
      );
    },
    ...
    "test group wrapping omiting rich combo at the beginning": function () {
      ...
      assert.areEqual(
        1,
        panel._view.parts.content.find(".cke_toolgroup").count(),
        "There should be only one toolgroup"
      );
      assert.isNull(
        panel._view.parts.content
          .findOne(".cke_toolgroup")
          .findOne(".richCombo"),
        "Rich combo should not be inside toolgroup"
      );
    },
    ...
    "test mixed group": function () {
      ...
      assert.areEqual(
        2,
        panel._view.parts.content.find(".cke_toolgroup").count(),
        "There should be two toolgroups"
      );
    },
  };
