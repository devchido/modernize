...
(function () {
  ...
  var tests = {
    ...
    "test balloonToolbarView.render": function () {
      ...
      var expectedOutput = CKEDITOR.env.ie
        ? '<span class="cke_toolgroup" unselectable="on">aabb</span>'
        : '<span class="cke_toolgroup">aabb</span>';
    ...
    "test Balloon Toolbar doesnt steal the focus": function () {
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.areSame(
            this.editor.window.getFrame(),
            CKEDITOR.document.getActive(),
            "Editor frame is focused"
          );
    ...
    "test Balloon Toolbar show and hide methods": function () {
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.isTrue(
            view.rect.visible,
            "Toolbar should be shown after show method"
          );
          assert.areEqual(
            4,
            view._listeners.length,
            "Listeners should be attached after show method"
          );

          view.hide();
          assert.isFalse(
            view.rect.visible,
            "Toolbar should not be shown after hide method"
          );
          assert.areEqual(
            0,
            view._listeners.length,
            "Listeners should not be attached after hide method"
          );
