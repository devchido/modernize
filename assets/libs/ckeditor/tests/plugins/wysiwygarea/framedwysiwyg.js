...
bender.test({
    ...
    editor.setMode("source", function () {
      editor.setMode("wysiwyg", function () {
        resume(function () {
          ...
          assert.isTrue(
            editable.is("body"),
            'editable should be the "body" element'
          );
          assert.areSame(
            editor.document.$,
  ...
  test_setData: function () {
    ...
    editor.setData(data, function () {
      resume(function () {
        assert.isMatching(
          /^<p>foo(<br \/>)?<\/p>$/,
          tools.compatHtml(editable.getHtml()),
          "set data"
        );
  ...
  "test editable listeners after setData": function () {
    ...
    editor.setData("abc", function () {
      resume(function () {
        ...
        assert.isTrue(
          doc.$ !== editor.editable().getDocument(),
  ...
  test_detach: function () {
    ...
    assert.areSame(
      0,
      this.editor.ui.space("contents").$.getElementsByTagName("iframe").length
    );
  },
});
