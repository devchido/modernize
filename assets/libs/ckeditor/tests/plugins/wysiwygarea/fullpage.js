...
(function () {
  ...
  bender.test({
      ...
      bender.tools.testInputOut("fullpage1", function (source, expected) {
        bot.setData(source, function () {
          assert.areSame(
            ...
            bot.getData(true).replace(removeStyle, "")
          ); // remove styles from data
        });
      });
    },
    ...
    "test load full-page data (with doctype)": function () {
      ...
      bender.tools.testInputOut("fullpage2", function (source, expected) {
        bot.setData(source, function () {
          assert.areSame(
            bender.tools.compatHtml(expected),
            bot.getData(true, true).replace(removeStyle, "")
          ); // remove styles from data
        });
      });
    },

    "test base tag is placed before every element it affects in the head":
      function () {
    ...
    "test base tag is correctly added when head has an attribute": function () {
      ...
      bot.setData(
        '<head foo="xxx"><title>x</title><body><p>foo</p></body>',
        function () {
          var base = getBaseElement(bot.editor);
          ...
          if (!CKEDITOR.env.ie) {
            ...
            assert.isTrue(
              base.getParent().hasAttribute("foo"),
              "attribute was not lost"
            );
          }
        }
      );
