...
bender.test({
  ...
  assertToHtml: function (html, bbcode) {
    ...
    assert.areSame(
      ...
      CKEDITOR.tools.convertRgbToHex(
        bender.tools.fixHtml(processor.toHtml(evtData.dataValue))
      ),
      "bbcode->html"
    );
  },
  ...
  "test HTML to bbcode": function () {
    ...
    this.assertToBBCode(
      ":)",
      '<img src="' +
        this.editor.config.smiley_path +
        'regular_smile.png" alt="smiley"/>'
    );
    this.assertToBBCode(
      "[list]\n[*]foo\n[*]bar\n[/list]\n",
  ...
  "test HTML to bbcode justify": function () {
    ...
    this.assertToBBCode(
      "[right][img]http://a.cksource.com/c/1/inc/img/demo-little-red.jpg[/img][/right]",
  ...
  "test bbcode to HTML": function () {
    ...
    this.assertToHtml(
      "<blockquote><cite>citation</cite><div>quoted text</div></blockquote>",
      '[quote="citation"]quoted text[/quote]\n'
    );
    ...
    this.assertToHtml(
      '<img alt="smiley" src="' +
        this.editor.config.smiley_path +
        'regular_smile.png" title="smiley" />',
      ":)"
    );
    this.assertToHtml(
      "<ul><li>foo</li><li>bar</li></ul>",
  ...
  "test escape HTML entities in bbcode": function () {
    var html = '<a href="foo&amp;bar">&amp;foo&lt;bar&gt;</a>',
      bbcode = "[url=foo&bar]&foo<bar>[/url]";
