...
(function () {
  ...
  var tcs = {
    ...
    setUp: function () {
      ...
      if (!this.tools) this.tools = undoEventDispatchTestsTools(this);
    ...
    "test snapshot on click": function () {
      ...
      this.editorBot.setData("<p>foo bar</p>", function () {
        ...
        assert.areEqual(
          expectedSnapshots,
          undoManager.snapshots.length,
          "Snapshots count increased"
        );

        var interestingSnapshot = undoManager.snapshots[expectedSnapshots - 1];
        assert.areEqual(
          3,
          interestingSnapshot.bookmarks[0].startOffset,
        ...
        if (!this.isIe8) {
          assert.areEqual(
            3,
            interestingSnapshot.bookmarks[0].endOffset,
            "Invalid bookmark end offset"
          );
        }
      });
    },
      ...
      this.editor.setData("<p>foo</p>", {
        ...
        callback: function () {
          resume(function () {
            ...
            assert.areEqual(
              1,
              that.editor.undoManager.snapshots.length,
              "Invalid snapshots count"
            );
    ...
    "test no snapshot after drop": function () {
      ...
      this.editorBot.setData("<p>foo bar</p>", function () {
        ...
        assert.areEqual(
          0,
          typeCalledTimes,
          "undoManager.type should not be called"
        );
        ...
        assert.areEqual(
          1,
          typeCalledTimes,
          "undoManager.type should be called"
        );
