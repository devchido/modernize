...
var fillingCharSequence = CKEDITOR.dom.selection.FILLING_CHAR_SEQUENCE,
  ...
  createFillingCharSequenceNode =
    CKEDITOR.dom.selection._createFillingCharSequenceNode;
...
bender.test({
  doUndoCommand: function (input, cmd) {
    ...
    this.wait(function () {
      ...
      output = bender.tools.compatHtml(
        bot.editor.dataProcessor.toDataFormat(output)
      );
  ...
  "test undoManager.updateSelection": function () {
    ...
    bender.editorBot.create(editorBotConfig, function (bot) {
      ...
      assert.areEqual(
        ...
        "Snapshots count should not change"
      );
      assert.areEqual(
        1,
        undoManager.snapshots[0].bookmarks[0].startOffset,
        "Invalid bookmark startOffset"
      );
      !isIe8 &&
        assert.areEqual(
          1,
          undoManager.snapshots[0].bookmarks[0].endOffset,
          "Invalid bookmark endOffset"
        );
      ...
      assert.areEqual(
        2,
        undoManager.snapshots[0].bookmarks[0].startOffset,
        "Bookmark startOffset not updated"
      );
      !isIe8 &&
        assert.areEqual(
          2,
          undoManager.snapshots[0].bookmarks[0].endOffset,
          "Bookmark endOffset not updated"
        );
      assert.areSame(
        undoManager.currentImage,
        img2,
        "undoManager.currentImage not updated"
      );
  ...
  "check initial command states": function () {
    ...
    bot.editor.setMode("source", function () {
      resume(function () {
        assert.isFalse(
          isActive(undo),
          "Undo command disabled when switching to source."
        );
        assert.isFalse(
          isActive(redo),
          "Redo command disabled when switching to source."
        );
        ...
        bot.editor.setMode("wysiwyg", function () {
          resume(function () {
            assert.isFalse(
              isActive(undo),
              "Undo command disabled when switching to wysiwyg."
            );
            assert.isFalse(
              isActive(redo),
              "Redo command disabled when switching to wysiwyg."
            );
          });
        });
  ...
  "check command states on readOnly": function () {
    ...
    bot.editor.once("readOnly", function () {
      resume(function () {
        assert.isFalse(
          isActive(undo),
          "Undo command disabled when in read-only mode."
        );
        assert.isFalse(
          isActive(redo),
          "Redo command disabled when in read-only mode."
        );
        ...
        bot.editor.once("readOnly", function () {
          resume(function () {
            assert.isFalse(
              isActive(undo),
              "Undo command disabled when leaving read-only mode."
            );
            assert.isFalse(
              isActive(redo),
              "Undo command disabled when leaving read-only mode."
  ...
  "test filling char is not recorded": function () {
    ...
    if (CKEDITOR.env.webkit)
      assert.isTrue(
        initialHtml.length > editable.getHtml().length,
        "Filling char has been removed"
      );
  ...
  "test selection is restored despite filling char": function () {
    ...
    range = editor.createRange();
    range.setStart(
      editor.document.getById("p1").getFirst(),
    ...
    assert.areSame(
      2,
      editor.undoManager.snapshots.length,
      "Number of snapshots recorded."
    );
    assert.isInnerHtmlMatching(
      '<p id="p1">abc<em>def</em>@</p>',
      editor.undoManager.snapshots[0].contents,
      "Snapshot does not contain FCSeq."
    );
    ...
    assert.isInnerHtmlMatching(
      '<p id="p1">abc<em>def</em>@</p>',
      editable.getHtml(),
      "Snapshot restored without FCSeq."
    );
    ...
    assert.isTrue(
      range.startContainer.equals(editor.document.getById("p1").getFirst()),
      "Range starts in the right text node."
    );
    assert.isTrue(
      range.endContainer.equals(
        editor.document.getById("p1").findOne("em").getFirst()
      ),
      "Range ends in the right text node."
    );
    ...
    assert.areSame(
      1,
      range.startOffset,
      "Start offset does not include FCSeq."
    );
  ...
  "test lock with dontUpdate option": function () {
    ...
    assert.areSame(
      1,
      editor.undoManager.snapshots.length,
      "one snapshot after lock+change+save"
    );
    ...
    assert.areSame(
      2,
      editor.undoManager.snapshots.length,
      "two snapshots after unlock+save"
    );
  ...
  "test lock with dontUpdate cannot be overriden by normal lock": function () {
    ...
    assert.areSame(
      1,
      editor.undoManager.snapshots.length,
      "one snapshot after unlocking 2nd lock"
    );
    ...
    assert.areSame(
      1,
      editor.undoManager.snapshots.length,
      "one snapshot after unlocking undo manager completely"
    );
  ...
  "test lock with forceUpdate option": function () {
    ...
    assert.areSame(
      1,
      editor.undoManager.snapshots.length,
      "only one snapshot after unlock and save"
    );
    ...
    assert.areSame(
      "<p>foo3</p>",
      editor.getData(),
      "snapshot contains latest content"
    );
  },
  ...
  "test automatic DOM changes handling": function () {
    ...
    var firstElement = sel.getStartElement(),
      currentPath = new CKEDITOR.dom.elementPath(
        firstElement,
        editor.editable()
      );
    ...
    assert.isMatching(
      /<p>abc(<br>)?<\/p>/i,
      root.getHtml(),
      "Auto paragraphing executed correctly"
    );
    assert.isFalse(
      isActive(undo),
      "Auto paragraphing hasn't created undo snapshot"
    );
    ...
    wait(function () {
      assert.areEqual(
        "<p>ab</p><p>c</p>",
        bot.getData(),
        "Enter command executed correctly"
      );
      ...
      assert.areEqual(
        "<p>ab</p><p>c</p>",
        bot.getData(),
        "Enter command redone correctly"
      );
    }, 0);
  },
  ...
  'test undo with "control" type selection in IE': function () {
    ...
    bot.setHtmlWithSelection(
      '<div>[<p dir="rtl">foo</p>]<p dir="rtl" id="target">bar</p></div>'
    );
    bot.editor.resetUndo();
  ...
  "test CTRL+Z/Y/SHIFT+Z default actions are blocked in wysiwyg mode":
    function () {
      var bot = this.editorBot,
        prevented = 0,
  ...
  "test CTRL+Z/Y/SHIFT+Z default actions are not blocked in source mode":
    function () {
      var bot = this.editorBot;
        ...
        bot.editor.setMode("wysiwyg", function () {
          resume(function () {
            assert.areSame(
              0,
              prevented,
              "None of the keystrokes was prevented"
            );
          });
        });
