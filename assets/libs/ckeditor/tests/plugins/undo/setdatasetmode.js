...
(function () {
  ...
  function onInstanceReady() {
    this.dataProcessor.dataFilter.addRules({
      attributeNames: [[/^data-foo$/, "data-foo-internal"]],
    });
    this.dataProcessor.htmlFilter.addRules({
      attributeNames: [[/^data-foo-internal$/, "data-foo"]],
    });
  }
  ...
  var tests = {
    ...
    "test switch mode without changes": function (editor) {
      if (editor.elementMode == CKEDITOR.ELEMENT_MODE_INLINE) assert.ignore();
    ...
    "test switch mode with content that is differently represented in data and inner HTML":
      function (editor) {
        if (editor.elementMode == CKEDITOR.ELEMENT_MODE_INLINE) assert.ignore();
