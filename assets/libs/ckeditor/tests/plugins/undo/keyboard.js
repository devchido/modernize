  ...
  function simulateHoldKey(
    ...
    while (times--) {
      ...
      if (!UndoManager.ieFunctionalKeysBug(keyCode)) {
        keyTools.singleKeyEvent(0, {
          type: CKEDITOR.env.ie ? "keypress" : "input",
        });
      }
    }
  ...
  var keyCodesEnum, // keyCodesEnum will be inited in first setUp call.
    ...
    tcs = {
      ...
      _moveTextNodeRange: function (newStartOffset, newEndOffset) {
        ...
        rng.setEnd(
          textNode,
          newEndOffset === undefined ? newStartOffset : newEndOffset
        );
      ...
      "test character typing counter": function () {
        ...
        assert.areSame(
          0,
          undoManager.strokesRecorded[keyGroups.PRINTABLE],
          "Initial undoManager.strokesRecorded[ keyGroups.PRINTABLE ] val is invalid"
        );
        // Count of pressed functional keys, should not change during whole TC.
        assert.areSame(
          0,
          undoManager.strokesRecorded[keyGroups.FUNCTIONAL],
          "Initial undoManager.strokesRecorded[ keyGroups.FUNCTIONAL ] val is invalid"
        );
        ...
        assert.areSame(
          1,
          undoManager.strokesRecorded[keyGroups.PRINTABLE],
          "undoManager.strokesRecorded[ keyGroups.PRINTABLE ] was not correctly"
        );
        assert.areSame(
          0,
          undoManager.strokesRecorded[keyGroups.FUNCTIONAL],
          "Count of functional keystrokes should remain unchanged"
        );
      ...
      "test multiple key being pressed": function () {
        ...
        assert.areEqual(
          2,
          undoManager.strokesRecorded[keyGroups.PRINTABLE],
          "Invalid undoManager.strokesRecorded[ keyGroups.PRINTABLE ]"
        );
      },
      ...
      "test undoManager.resetType()": function () {
        ...
        arrayAssert.itemsAreSame(
          [0, 0],
          undoManager.strokesRecorded,
          "strokesRecorded were not zeroed"
        );
        assert.areEqual(
          -1,
          undoManager.previousKeyGroup,
          "undoManager.previousKeyGroup"
        );
      },

      "test undoManager change event firing": tcWithExpectedChanges(
        30,
        function () {
      ...
      "test undoManager change event for functional keys":
        tcWithExpectedChanges(2, function () {
          var textNode = this.editor.editable().getFirst().getFirst(),
      ...
      "test buggy Android keyboard model": tcWithExpectedChanges(
        1,
        function () {
      ...
      "test function keys typing counter": tcWithExpectedChanges(
        3,
        function () {
          assert.areSame(
            0,
            this.undoManager.strokesRecorded[keyGroups.FUNCTIONAL],
            "Initial undoManager.strokesRecorded[ keyGroups.FUNCTIONAL ] value"
          );
          ...
          assert.areSame(
            3,
            this.undoManager.strokesRecorded[keyGroups.FUNCTIONAL],
            "undoManager.strokesRecorded[ keyGroups.FUNCTIONAL ] should be increased"
          );
        }
      ),
      ...
      "test undoManager.strokesRecorded reseting": function () {
        ...
        assert.areSame(
          0,
          undoManager.strokesRecorded[keyGroups.PRINTABLE],
          "undoManager.strokesRecorded for characters not zeroed"
        );
        ...
        assert.areSame(
          0,
          undoManager.strokesRecorded[keyGroups.FUNCTIONAL],
          "undoManager.strokesRecorded for functional keys not zeroed"
        );
      },

      "test strokesRecorded reset after exceeding limit": tcWithExpectedChanges(
        keystrokesPerSnapshotLimit,
        function () {
          ...
          this.keyTools.keyEventMultiple(
            keystrokesPerSnapshotLimit,
            keyCodesEnum.KEY_D,
            null,
            false,
          ...
          assert.areSame(
            0,
            undoManager.strokesRecorded[keyGroups.PRINTABLE],
            "undoManager.strokesRecorded[ keyGroups.PRINTABLE ] is not zeroed"
          );
          assert.areSame(
            1,
            snapshotEventsCount,
            "Wrong editor#saveSnapshot events count"
          );
        }
      ),
      ...
      "test undoManager.typing property": function () {
        ...
        for (i = 0; i < iterationsCount; i++) {
          ...
          assert.isTrue(
            undoManager.typing,
            "Invalid undoManager.typing at character key " + i + ". iteration"
          );
        }
        ...
        for (i = 0; i < iterationsCount; i++) {
          ...
          assert.isTrue(
            undoManager.typing,
            "Invalid undoManager.typing at functional keys " + i + ". iteration"
          );
        }
      },
      ...
      "test undoManager selection overwriting": function () {
        ...
        assert.areEqual(
          2,
          undoManager.snapshots.length,
          "Invalid snapshots count"
        );
      ...
      "test undoManager extra snapshot on navigation key after recordable keystroke":
        function () {
          this.editorBot.setData("<p>foo bar</p>", function () {
            ...
            assert.areEqual(
              false,
              undoManager.typing,
              "undoManager.typing wasn't changed"
            );
            arrayAssert.itemsAreSame(
              [0, 0],
              undoManager.strokesRecorded,
              "Invalid undoManager.strokesRecorded"
            );
          });
        },
      ...
      "test backspace snapshot content": function () {
        ...
        assert.areEqual(
          2,
          undoManager.snapshots.length,
          "Invalid initial snapshots count"
        );
        ...
        this.keyTools.keyEvent(keyCodesEnum.KEY_D, null, false, function () {
          ...
          // to be aware.
          if (that.isIe8) textNode.setText("foo D");
          else textNode.setText("foo Dbar");
          that._moveTextNodeRange(5);
        });
      ...
      "test affecting commands state": function () {
        ...
        assert.areEqual(
          ...
          undoCommand.state,
          "Invalid undo command state"
        );
        assert.areEqual(
          CKEDITOR.TRISTATE_DISABLED,
          redoCommand.state,
          "Invalid redo command state"
        );
        ...
        assert.areEqual(
          ...
          undoCommand.state,
          "Invalid undo command state after typing character"
        );
        assert.areEqual(
          CKEDITOR.TRISTATE_DISABLED,
          redoCommand.state,
          "Invalid redo command state after typing character"
        );
      },

      "test snapshot created on more than 25 backspaces": function () {
        this.editorBot.setData(
          "<p>12345678901234567890123451234567890123456789012345</p>",
          function () {
            var undoManager = this.editor.undoManager,
              textNode = this.editor.editable().getFirst().getFirst(),
            ...
            bender.assert.isInnerHtmlMatching(
              expected,
              undoManager.snapshots[1].contents
            );
          }
        );
      },

      "test snapshot created on more than 25 changes by holding different keys - one after another":
        function () {
          ...
          this.editorBot.setData("<p>Hi</p>", function () {
            ...
            bender.assert.isInnerHtmlMatching(
              "<p>HIffffffffffffffffffffppppp@</p>",
              undoManager.snapshots[2].contents
            );
          });
        },
      ...
      "test snapshot created on more than 25 changes by holding different keys - both at the same time":
        function () {
          this.editorBot.setData("<p>Hi</p>", function () {
            var undoManager = this.editor.undoManager,
              textNode = this.editor.editable().getFirst().getFirst(),
            ...
            bender.assert.isInnerHtmlMatching(
              "<p>Hiffffffffffffffffffffddddd@</p>",
              undoManager.snapshots[2].contents
            );
          });
        },
      ...
      "test undo command disable undo and enable redo ui button": function () {
        ...
        this.editorBot.setData(initData, function () {
          ...
          assert.areEqual(
            initData,
            this.editor.getData(),
            "Data should be same as initial one, because we just undo."
          );

          simulateCtrlZ(this.keyTools);
          assert.areEqual(
            CKEDITOR.TRISTATE_DISABLED,
            this.editor.getCommand("undo").state,
            "Undo should be disabled because we undo only change."
          );
          assert.areEqual(
            CKEDITOR.TRISTATE_OFF,
            this.editor.getCommand("redo").state,
            "Redo be enabled because we just uno."
          );

          function simulateCtrlZ(keyTools) {
            keyTools.singleKeyEvent(17 /*CTRL*/, {
              type: "keydown",
              ctrlKey: true,
            });
            keyTools.singleKeyEvent(90 /*Z*/, {
              type: "keydown",
              ctrlKey: true,
            });
      ...
      "test undoManager.isNavigationKey": function () {
        ...
        for (var i = 0; i < naviKeys.length; i++) {
          curKey = naviKeys[i];
          assert.isTrue(
            isNavigationKey(keyCodesEnum[curKey]),
            "Invalid result for " + curKey
          );
        }

        assert.isFalse(
          isNavigationKey(keyCodesEnum.BACKSPACE),
          "Invalid result for Backspace"
        );
        assert.isFalse(
          isNavigationKey(keyCodesEnum.KEY_D),
          "Invalid result for D key"
        );
      },
    };
