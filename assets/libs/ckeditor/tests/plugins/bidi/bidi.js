...
bender.test({
  "test apply RTL": function () {
    ...
    assert.areSame(
      '<p dir="rtl">foo<br />bar</p><p dir="rtl">baz</p>',
  ...
  "test apply RTL (list)": function () {
    ...
    bot.execCommand("bidirtl");
    assert.areSame(
      '<ul dir="rtl"><li>item1</li><li>item2</li></ul>',
      bot.getData(false, true)
    );
  },
  ...
  "test apply direction mirror contents style": function () {
    ...
    bender.tools.testInputOut("apply_dir_mirror", function (source, expected) {
      ...
      assert.areSame(
        "rtl",
        evtDir,
        "check editor#dirChanged event fired on bidi command call"
      );
      assert.areSame(
        bender.tools.compatHtml(expected),
        bot.getData(true),
    ...
    bender.tools.testInputOut(
      ...
      function (source, expected) {
        ...
        assert.areSame(
          "ltr",
          evtDir,
          "check editor#dirChanged event fired on dom element attr change."
        );
        assert.areSame(
          bender.tools.compatHtml(expected),
          bot.getData(true),
