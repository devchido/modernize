...
(function () {
  ...
  function assertAjaxPost(
    ...
    try {
      ...
      assert.areSame(
        expectedContentType ? 1 : 0,
        setRequestHeaderCalled,
        "XMLHttpRequest.setRequestHeader called"
      );
    } catch (e) {
      throw e;
    } finally {
  ...
  bender.test({
    ...
    test_loadXml_sync: function () {
      ...
      assert.isNotNull(
        data.selectSingleNode("//list/item"),
        "The loaded data doesn't match (null)"
      );
      assert.isNotUndefined(
        data.selectSingleNode("//list/item"),
        "The loaded data doesn't match (undefined)"
      );
    },
    ...
    "test CKEDITOR.ajax.post: ContentType application/x-www-form-urlencoded":
      function () {
        var url = "404.post",
          data = "key=value",
