  ...
  bender.test({
    "test creatediv in a selection containing non-editable divs": function () {
      ...
      bot.dialog("creatediv", function (dialog) {
        ...
        assert.areSame(
          expectedHtml,
          fixHtml(bot.htmlWithSelection()),
          "Non-editable divs remain untouched."
        );
      });
    },

    "test removediv from a selection containing non-editable divs":
      function () {
        var bot = this.editorBot,
          editor = this.editor;
        ...
        assert.areSame(
          expectedHtml,
          fixHtml(bot.htmlWithSelection()),
          "Only the outer, editable div is removed."
        );
      },
      ...
      bot.setData(html, function () {
        ...
        bot.dialog("creatediv", function (dialog) {
          ...
          assert.areSame(
            expectedHtml,
            fixHtml(bot.getData()),
            "Non-editable div is wrapped with editable div."
          );
    ...
    "test removediv from a non-editable div": function () {
      ...
      bot.setData(html, function () {
        ...
        assert.areSame(
          html,
          fixHtml(bot.getData()),
          "No div is removed, non-editable div remains untouched."
        );
        assert.isTrue(!!editor.getSelection().isFake);
      });
    },

    "test removediv from an editable div containing a non-editable div":
      function () {
        var editor = this.editor,
          bot = this.editorBot;
        ...
        assert.areSame(
          expectedHtml,
          fixHtml(bot.htmlWithSelection()),
          "Outer, editable div is removed, non-editable div remains untouched."
        );
      },
    ...
    "test removediv from a non-editable div withing editable div": function () {
      ...
      var html =
          ...
          "</div>",
        expectedHtml =
          '<div contenteditable="false" id="f">' + "<div>foo</div>" + "</div>";
