  ...
  bender.test({
    ...
    "test elements path on selection change": function () {
      ...
      bot.setHtmlWithSelection(
        "<div><p><span><b><i><u>^</u></i></b></span></p></div>"
      );
    ...
    "test elements path with contenteditable": function () {
      this.editorBot.setHtmlWithSelection(
        bender.tools.getValueAsHtml("mixedContentEditableTree")
      );
    ...
    "test widget mockup element path": function () {
      this.editorBot.setHtmlWithSelection(
        bender.tools.getValueAsHtml("testWidgetSample")
      );
    ...
    "test elements path items not draggable": function () {
      ...
      CKEDITOR.tools.array.forEach(elements, function (element) {
        assert.areEqual(
          ...
          "Element draggable attribute value."
        );
        assert.areEqual(
          "return false;",
          element.getAttribute("ondragstart"),
          "Element ondragstart attribute value."
        );
      });
    },
  });
