...
(function () {
  ...
  bender.test({
    ...
    assertHtmlEqual: function (expected, actual) {
      ...
      assert.areEqual(
        this.fixHtml(expected),
        this.fixHtml(actual).replace(/<br\s*\/\>/g, "")
      );
    },
    ...
    "test clicking typical strong element": function () {
      ...
      this.assertHtmlEqual(
        ...
        bender.tools.getHtmlWithRanges(
          ...
          new CKEDITOR.dom.rangeList([curRange])
        )
      );
    },
    ...
    "test clicking contenteditable strong element": function () {
      var bot = this.editorBot,
        ...
        expectedHtml =
          '<div contenteditable="false"><figure><figcaption contenteditable="true" id="sel_caption">[roll out of saturn v on launch pad]</figcaption></figure></div>';

      bot.setHtmlWithSelection(
        bender.tools.getValueAsHtml("testClickingContentEditableSource")
      );
