...
bender.test({
  ...
  "test with no config": function () {
    ...
    assert.areSame(
      0,
      editor.editable().find('a[data-widget="uploadfile"]').count()
    );
  },
  ...
  "test with uploadfile plugin": function () {
    ...
    assert.sameData(
      '<p><a href="%BASE_PATH%_assets/sample.txt" target="_blank">name.txt</a></p>',
      editor.getData()
    );
    assert.areSame(
      0,
  ...
  "test pasting files in the editor with uploadfile and uploadimage plugins":
    function () {
  ...
  "test pasting images in the editor with uploadfile and uploadimage plugins":
    function () {
      var editor = this.editors.uploadfileAndUploadimage;
  ...
  "test aborting the upload after editor was removed wont break it":
    function () {
