...
(function () {
  ...
  function testDelKey(editor, keyName, range, shouldBeBlocked, msg) {
    ...
    var fullMsg =
        keyName +
        " " +
        (shouldBeBlocked ? "was blocked" : "was not blocked") +
        " at " +
        msg,
      keyCodes = {
        del: 46,
  ...
  function appendEditable(widget, editableName, editableElementName, options) {
    var el = new CKEDITOR.dom.element.createFromHtml(
      "<" +
        editableElementName +
        ' class="' +
        editableName +
    ...
    widget.initEditable(
      editableName,
      CKEDITOR.tools.extend({ selector: "." + editableName }, options)
    );
  ...
  function assertEnterModes(enterMode, shiftEnterMode, editable, msg) {
    ...
    assert.areSame(
      String(enterMode),
      editable.getAttribute("data-cke-enter-mode"),
      msg + "data-cke-enter-mode attr"
    );
    assert.areSame(enterMode, editable.enterMode, msg + "enterMode");
    assert.areSame(
      shiftEnterMode,
      editable.shiftEnterMode,
      msg + "shiftEnterMode"
    );
  }
  ...
  bender.test({
    "test basics": function () {
      var editor = this.editor,
        data =
          '<p>Foo</p><div data-widget="testbasic1" id="w1"><p>A</p><div id="foo"><p>B</p></div></div>';
      ...
      this.editorBot.setData(data, function () {
        ...
        assert.isInstanceOf(
          ...
          editable,
          "editable is an instance of CKEDITOR.plugins.widget.nestedEditable class"
        );
        assert.isInstanceOf(
          CKEDITOR.dom.element,
          editable,
          "editable is an instance of CKEDITOR.dom.element class"
        );
        assert.areSame(
          "true",
        ...
        assert.areSame(
          "foo",
          editable.getAttribute("data-cke-widget-editable"),
          "has data-cke-widget-editable attribute"
        );
        assert.areSame(
          String(CKEDITOR.ENTER_P),
          editable.getAttribute("data-cke-enter-mode"),
          "has data-cke-enter-mode attribute"
        );
        assert.isTrue(
          editable.hasClass("cke_widget_editable"),
          "has cke_widget_editable class"
        );
    ...
    "test #initEditable and #destroyEditable": function () {
      ...
      this.editorBot.setData(
        '<div data-widget="testmethods1" id="w1"><p>A</p><p id="foo">B</p><div id="bar">C</div></div>',
        function () {
          var widget = getWidgetById(editor, "w1");

          assert.isTrue(
            widget.initEditable("bar", {
              selector: "#bar",
              allowedContent: "p br",
            }),
            "return value"
          ...
          assert.areSame(
            editor.document.getById("bar"),
            eBar,
            "editable was added"
          );
          assert.areSame(
            "true",
            eBar.getAttribute("contenteditable"),
            "has contenteditable attr"
          );
          assert.areSame(
            "bar",
            eBar.getAttribute("data-cke-widget-editable"),
            "has data-cke-widget-editable attribute"
          );
          assert.areSame(
            String(CKEDITOR.ENTER_P),
            eBar.getAttribute("data-cke-enter-mode"),
            "has data-cke-enter-mode attribute"
          );
          assert.areSame(
            CKEDITOR.ENTER_P,
          ...
          assert.isNumber(
            parseInt(eBar.getAttribute("data-cke-filter"), 10),
            "has data-cke-filter attribute"
          );
          assert.isInstanceOf(
            CKEDITOR.filter,
            eBar.filter,
            "editable.filter is instance of CKEDITOR.filter"
          );
          assert.areSame(
            eBar.filter,
            CKEDITOR.filter.instances[eBar.getAttribute("data-cke-filter")],
            "data-cke-filter points to an existing filter instance"
          );
          assert.isTrue(
            eBar.hasClass("cke_widget_editable"),
            "has cke_widget_editable class"
          );
          ...
          assert.areNotSame(
            "true",
            eBar.getAttribute("contenteditable"),
            "does not have contenteditable attr"
          );
          assert.isFalse(
            eBar.hasAttribute("data-cke-widget-editable"),
            "does not have data-cke-widget-editable attribute"
          );
          assert.isFalse(
            eBar.hasAttribute("data-cke-enter-mode"),
            "does not have data-cke-enter-mode attribute"
          );
          assert.isFalse(
            eBar.hasClass("cke_widget_editable"),
            "does not have cke_widget_editable class"
          );

          // Focus manager also uses removeListener, so events are doubled.
          assert.areSame(
            "blur,blur,focus,focus",
            removedListeners.sort().join(","),
          ...
          assert.areSame(
            "true",
            eFoo.getAttribute("contenteditable"),
            "has contenteditable attr - foo"
          );
        }
      );
    },
    ...
    "test initEditable ignores not allowed editable elements": function () {
      ...
      this.editorBot.setData(
        '<div data-widget="testmethods2" id="w1"><span id="bar">B</span></div>',
        function () {
          var widget = getWidgetById(editor, "w1");

          assert.isFalse(
            widget.initEditable("bar", { selector: "#bar" }),
            "return value"
          );
          assert.isFalse(
            !!widget.editables.bar,
            "editable was not initialized"
          );
        }
      );
    },
    ...
    "test #destroyEditable destroys nested widgets": function () {
      ...
      this.editorBot.setData(
        '<div data-widget="testmethods3" id="w1"><p id="foo"><span data-widget="testmethods3" id="w2">x</span></p></div>',
        function () {
    ...
    "test #destroyEditable in offline mode does not destroy nested widgets":
      function () {
        var editor = this.editor;
        ...
        this.editorBot.setData(
          '<div data-widget="testmethods4" id="w1"><p id="foo"><span data-widget="testmethods4" id="w2">x</span></p></div>',
          function () {
            var w1 = getWidgetById(editor, "w1");
            ...
            assert.isNotNull(
              getWidgetById(editor, "w2", true),
              "nested widget was not destroyed"
            );
          }
        );
      },
    ...
    "test #destroyEditable destroys unused editable filters": function () {
      ...
      var widget1Html =
          '<div data-widget="testmethod5" id="w1"><p>A</p><p class="foo">B</p></div>',
        widget2Html =
          '<div data-widget="testmethod5" id="w2"><p>A</p><p class="foo">B</p></div>';
      ...
      this.editorBot.setData(widget1Html + widget2Html, function () {
        ...
        widget1.initEditable("foo", {
          selector: ".foo",
          allowedContent: "p br",
        });
        widget2.initEditable("foo", {
          selector: ".foo",
          allowedContent: "p br",
        });
    ...
    "test nestedEditable enter modes are limited by ACF": function () {
      ...
      this.editorBot.setData(
        '<div data-widget="testentermode1" id="w1"></div>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            editable;
          ...
          editable = appendEditable(widget, "e2", "div", {
            allowedContent: "br",
          });
          assertEnterModes(
            CKEDITOR.ENTER_BR,
          ...
          editable = appendEditable(widget, "e3", "div", {
          ...
          editable = appendEditable(widget, "e4", "div", {
            allowedContent: "p",
          });
    ...
    "test nestedEditable enter modes are limited by blockless editable":
      function () {
        var editor = this.editor;
        ...
        this.editorBot.setData(
          '<div data-widget="testentermode2" id="w1"></div>',
          function () {
            var widget = getWidgetById(editor, "w1"),
              editable;
    ...
    "test nestedEditable auto paragraphing (limited by widgetDef.allowedContent)":
      function () {
        var editor = this.editor;
        ...
        this.editorBot.setData(
          '<p>x</p><div data-widget="autoparagraphtest" id="w1"><div id="foo">foo</div></div>',
          function () {
            var widget = getWidgetById(editor, "w1"),
              editable = widget.editables.foo,
              range;
            ...
            assert.areEqual(
              CKEDITOR.ENTER_BR,
              editable.enterMode,
              "Enter mode should be CKEDTIOR.ENTER_BR."
            );
            assert.areEqual(
              "foo",
              editable.getData(),
    ...
    "test nestedEditable auto paragraphing (limited by config.enterMode)":
      function () {
        bender.editorBot.create(
          {
    ...
    "test nestedEditable auto paragraphing (limited by config.autoParagraph)":
      function () {
        bender.editorBot.create(
          {
            ...
            bot.setData(
              ...
              function () {
                ...
                assert.areEqual(
                  CKEDITOR.ENTER_P,
                  editable.enterMode,
                  "Enter mode should be CKEDTIOR.ENTER_P."
                );
                assert.areEqual(
                  "foo",
                  editable.getData(),
    ...
    "test nestedEditable.setData - data processor integration": function () {
      var editor = this.editor,
        data =
          '<p>Foo</p><div data-widget="testsetdata1" id="w1"><p>A</p><p id="foo">B</p></div>';
      ...
      editor.dataProcessor.dataFilter.addRules({
        elements: {
          i: function (el) {
            if (el.attributes.testsetdata1) el.attributes.testsetdata1 = "2";
          },
        },
      });
      ...
      this.editorBot.setData(data, function () {
        ...
        assert.areSame(
          '<i testsetdata1="2">B</i>',
          fixHtml(editable.getHtml()),
    ...
    "test nestedEditable.setData - ACF integration": function () {
      var editor = this.editor,
        data =
          '<p>Foo</p><div data-widget="testsetdata2" id="w1"><p>A</p><p id="foo">B</p></div>';
      ...
      this.editorBot.setData(data, function () {
        ...
        editable.setData(
          '<i class="red blue">B</i><b class="testsetdata2">B</b>'
        );
    ...
    "test nestedEditable.setData - destroyAll(false,editable) is not called on first nestedEditable.setData":
      function () {
        var editor = this.editor;
        ...
        this.editorBot.setData(
          ...
          function () {
            ...
            ed.setHtml(
              '<span data-widget="testsetdata3" id="w2">x</span><span data-widget="testsetdata3" id="w3">x</span>'
            );

            assert.areEqual(
              1,
              keysLength(editor.widgets.instances),
              "1 widget was initialized"
            );

            var original = editor.widgets.destroyAll,
              destroyAllCalls = 0,
              revert = bender.tools.replaceMethod(
                editor.widgets,
                "destroyAll",
            ...
            assert.areSame(
              0,
              destroyAllCalls,
              "destroyAll is not called on initial nestedEditable.setData"
            );
            assert.areEqual(
              3,
              keysLength(editor.widgets.instances),
              "3 widgets were initialized"
            );

            w1.editables.foo.setData(
              '<span data-widget="testsetdata3" id="w2">x</span>'
            );

            assert.areSame(
              1,
              destroyAllCalls,
              "destroyAll is called on 2nd+ nestedEditable.setData"
            );
            assert.areEqual(
              2,
              keysLength(editor.widgets.instances),
    ...
    "test nestedEditable.getData - data processor integration": function () {
      var editor = this.editor,
        data =
          '<p>Foo</p><div data-widget="testgetdata1" id="w1"><p>A</p><p id="foo">B</p></div>';
      ...
      editor.dataProcessor.htmlFilter.addRules({
        elements: {
          i: function (el) {
            if (el.attributes.testgetdata1) el.attributes.testgetdata1 = "2";
          },
        },
      });
      ...
      this.editorBot.setData(data, function () {
        ...
        editable.setData('B<img alt="foo" src="../../_assets/img.gif">B');
        assert.isMatching(
          'B<img alt="foo" src="../../_assets/img.gif" />B',
          editable.getData(),
          "data processor's preprocessor rule was applied"
        );
      });
    ...
    "test nestedEditable.getData - ACF integration": function () {
      var editor = this.editor,
        data =
          '<p>Foo</p><div data-widget="testgetdata2" id="w1"><p>A</p><p id="foo">B</p></div>';
    ...
    "test nested editable content is processed and filtered when setting/getting editor's data":
      function () {
        var editor = this.editor,
          data =
        ...
        this.editorBot.setData(data, function () {
          ...
          assert.isMatching(
            ...
            fixHtml(editable.getHtml()),
            "nested editable data was processed"
          );
    ...
    "test nested editable content is processed with its filter when getting data":
      function () {
        var editor = this.editor,
          data =
            '<p>Foo</p><div data-widget="testprocessing2" id="w1"><p id="foo">B</p></div>';
        ...
        this.editorBot.setData(data, function () {
          ...
          assert.areSame(
            ...
            editor.getData(),
            "nested editable's data was filtered on output"
          );
          ...
          assert.areSame(
            ...
            editor.getData(),
            "nested editable's data was processed as an editable content on output"
          );
        });
      },

    // #568
    "test nested editable editableDef.disallowedContent filter works with editableDef.allowedContent":
      function () {
        var editor = this.editor,
          data =
            '<p>Foo</p><div data-widget="testprocessing3" id="w1"><p id="foo">B</p></div>';
        ...
        this.editorBot.setData(data, function () {
          var editable = getWidgetById(editor, "w1").editables
            .disallowedWithAllowedContent;
    ...
    // #568
    "test nested editable editableDef.disallowedContent filter works based on editor.filter":
      function () {
        var editor = this.editor,
          data =
            '<p>Foo</p><div data-widget="testprocessing4" id="w1"><p id="foo">B</p></div>',
    ...
    "test nested editables processed with their enter modes": function () {
      ...
      this.editorBot.setData(
        '<p>Foo</p><div data-widget="testprocessing3" id="w1"><div class="foo">B</div></div>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            editable = widget.editables.foo;
          ...
          assert.areSame(
            "B",
            editable.getHtml(),
            "content of nested editable was not auto paragraphed on setData"
          );
          assert.areSame(
            "B",
            editable.getData(),
            "content of nested editable was not auto paragraphed on getData"
          );

          this.editorBot.setData(
            '<p>Foo</p><div data-widget="testprocessing3" id="w1"><div class="bar">C</div></div>',
            function () {
              var widget = getWidgetById(editor, "w1"),
              ...
              assert.areSame(
                "<div>c</div>",
                editable.getHtml().toLowerCase(),
                "content of nested editable was auto paragraphed with div on setData"
              );
              ...
              assert.areSame(
                "<div>D</div>",
                editable.getData(),
                "content of nested editable was auto paragraphed with div on getData"
              );
            }
          );
        }
    ...
    "test downcasting nested editable to attribute": function () {
      var editor = this.editor,
        data =
          '<p>Foo</p><div data-widget="testdowncasting1" id="w1"><p id="foo">B</p></div>';
      ...
      this.editorBot.setData(data, function () {
        ...
        assert.areSame(
          '<p>Foo</p><div data-editable="' +
            CKEDITOR.tools.htmlEncode("<i>A</i><i>B</i>") +
            '" data-widget="testdowncasting1" id="w1">X</div>',
          editor.getData(),
          "nested editable's data was correctly copied to attribute"
        );
      });
    },
    ...
    "test downcasting widget with two nested editables": function () {
      var editor = this.editor,
        data =
          '<p>Foo</p><div data-widget="testdowncasting2" id="w1"><div id="foo">X</div><div id="bar">X</div></div>';
    ...
    "test downcasting widget with nestedEditable#getData executed in downcast function":
      function () {
        var editor = this.editor,
          data =
            "<p>Foo</p>" +
        ...
        this.editorBot.setData(data, function () {
          assert.areSame(data, editor.getData());
          assert.areSame(
            "w1-in,w1-out,w2-in,w2-out,w3-in,w3-out",
            downcasted.join(",")
          );
        });
      },
          ...
          function assertEditables(
            ...
            if (widgetHoldingFocusedEditable)
              assert.areSame(
                ...
                "widget holds focused editable" + msg
              );
            else
              assert.isFalse(
                !!editor.widgets.widgetHoldingFocusedEditable,
                "none widget holds focused editable" + msg
              );
            ...
            while ((widget = allWidgets.shift())) {
              if (widgetHoldingFocusedEditable === widget) {
                assert.areSame(
                  ...
                  "widget has focused editable" + msg
                );
                assert.isInstanceOf(
                  CKEDITOR.plugins.widget.nestedEditable,
                  widget.focusedEditable,
                  "widget.focusedEditable is instance of widget.nestedEditable" +
                    msg
                );
              } else {
                assert.isFalse(
                  !!widget.focusedEditable,
                  "widget does not have focued editable" + msg
                );
              }
            }
            ...
            while ((editable = allEditables.shift())) {
              if (focusedEditable === editable) {
                assert.isTrue(
                  editable.hasClass("cke_widget_editable_focused"),
                  "#" + editable.$.id + " has focused class" + msg
                );
                assert.areSame(
                  editable.enterMode,
                  editor.activeEnterMode,
                  "#" + editable.$.id + "'s enter mode propagated to editor"
                );
                assert.areSame(
                  editable.shiftEnterMode,
                  editor.activeShiftEnterMode,
                  "#" +
                    editable.$.id +
                    "'s shift enter mode propagated to editor"
                );
              } else {
                assert.isFalse(
                  editable.hasClass("cke_widget_editable_focused"),
                  "#" + editable.$.id + " does not have focused class" + msg
                );
              }
            }
            ...
            if (CKEDITOR.env.gecko && focusedEditable)
              assert.isTrue(
                !!focusedEditable.getBogus(),
                "bogus was appended to editable" + msg
              );
          }
        }
      );
    ...
    "test editables' enter modes propagated to editor": function () {
      ...
      this.editorBot.setData(
        '<p id="x">X</p><div data-widget="testsel1" id="w1"><div id="foo">B</div><p>C</p><div id="bar">D</div></div>',
        function () {
          var widget1 = getWidgetById(editor, "w1"),
            eFoo1 = widget1.editables.foo,
            eBar1 = widget1.editables.bar;
    ...
    "test blurring nested editable when editor is blurred": function () {
      ...
      this.editorBot.setData(
        '<p id="x">X</p><div data-widget="testblur1" id="w1"><p id="foo">B</p></div>',
        function () {
          var widget = getWidgetById(editor, "w1"),
    ...
    "test focusing editor when focusing nested editable": function () {
      ...
      this.editorBot.setData(
        '<p id="x">X</p><div data-widget="testfocus1" id="w1"><p id="foo">B</p></div>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            eFoo = widget.editables.foo;
    ...
    "test destroying widget with nested editables": function () {
      ...
      wait(function () {
        this.editorBot.setData(
          '<p id="x">X</p><div data-widget="testdestroy1" id="w1"><p id="foo">B</p></div>',
          function () {
            var widget = getWidgetById(editor, "w1");
            ...
            assert.isFalse(
              !!editor.widgets.widgetHoldingFocusedEditable,
              "widgets repo does not point to nested editable"
            );
          }
        );
      }, 210);
    ...
    "test focused editable is resetted on setData": function () {
      ...
      editorBot.setData(
        '<p id="x">X</p><div data-widget="testreset1" id="w1"><p id="foo">B</p></div>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            eFoo = widget.editables.foo;
          ...
          editorBot.setData("", function () {
            assert.isFalse(
              !!editor.widgets.widgetHoldingFocusedEditable,
              "none widget holds focused editable after setData"
            );
    ...
    "test subsequent nested editable focus causes selectionChange":
      function () {
        var editor = this.editor,
          editorBot = this.editorBot;
        ...
        editorBot.setData(
          ...
          function () {
            ...
            wait(function () {
              ...
              assert.isTrue(
                !!selectionChanged,
                "selectionChange fired second time on next focus"
              );
            }, 210);
          }
        );
    ...
    "test CTRL+A in nested editable": function () {
      ...
      editorBot.setData(
        '<p id="x">X</p><div data-widget="testselectall1" id="w1"><p>Y</p><p id="foo">BA<i>R</i></p><p>Y</p></div>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            eFoo = widget.editables.foo;
          ...
          editor
            ...
            .fire(
              "keydown",
              new CKEDITOR.dom.event({ keyCode: 65, ctrlKey: true })
            ); // CTRL+A
          ...
          assert.areSame(
            eFoo,
            sel.getStartElement(),
            "selection is anchored in nested editable"
          );
          assert.areSame(
            "BAR",
            sel.getSelectedText(),
            "entire nested editable's content is selected"
          );
    ...
    "double click in nested editable is not forwarded to widget": function () {
      ...
      this.editorBot.setData(
        '<div id="w1" data-widget="testdbclick1"><p class="foo">X<i>Y</i></p></div>',
        function () {
          var widget1 = getWidgetById(editor, "w1"),
            fired = 0;
          ...
          assert.areSame(
            0,
            fired,
            "widget#doubleclick was not fired when nested editable was clicked"
          );

          editor.fire("doubleclick", {
            element: widget1.editables.foo.findOne("i"),
          });
          assert.areSame(
            0,
            fired,
            "widget#doubleclick was not fired when element child of nested editable was clicked"
          );
        }
      );
    },
    ...
    "test moving focus from nested editable to widget": function () {
      ...
      this.editorBot.setData(
        '<p>foo</p><div id="w1" data-widget="testfocustowidget1"><p class="foo">X<i>Y</i></p></div>',
        function () {
          var widget1 = getWidgetById(editor, "w1"),
    ...
    "test pasting widget which was copied (d&d) when its nested editable was focused":
      function () {
        ...
        bot.setData(
          '<p>foo</p><div id="w1" data-widget="testpasting1"><p class="foo">X</p></div>',
          function () {
            var widget1 = getWidgetById(editor, "w1"),
              eFoo = widget1.editables.foo,
              range = editor.createRange();
            ...
            bot.setData("<p>foo</p>", function () {
              editor.once("afterPaste", function () {
                resume(function () {
                  ...
                  assert.isFalse(
                    eFoo.hasClass("cke_widget_editable_focused"),
                    "editable's class was cleaned up"
                  );
                });
              });
    ...
    // (https://dev.ckeditor.com/ticket/13186)
    "test pasting into widget nested editable when range in paste data (drop)":
      function () {
        var editor = this.editor,
          bot = this.editorBot;
        ...
        bot.setData(
          '<p>foo</p><div id="w1" data-widget="widget1"><p id="widget1-nested">xx</p></div>',
          function () {
            var widget = getWidgetById(editor, "w1"),
            ...
            editor.once("afterPaste", function () {
              resume(function () {
                assert.isInnerHtmlMatching(
                  'xy<i class="a c" style="color:green">z</i>yx@',
                  nested.getHtml(),
                  {
                    fixStyles: true,
                  },
            ...
            editor.fire("paste", {
              type: "auto",
              dataValue:
                'y<i class="a b c d" style="margin-left:20px; color:green">z</i>y',
              method: "drop",
              range: range,
            });
    ...
    // #1469
    "test pasting widget into widget nested editable with selectionChange callback":
      function () {
        var editor = this.editor,
          bot = this.editorBot;
        ...
        bot.setData(
          '<div id="w1" data-widget="widget-pastenested"><p>Widget</p><div class="widget-nested">xx</div></div>',
          function () {
            var widget = getWidgetById(editor, "w1"),
              nested = widget.editables.nested,
              range = editor.createRange();
            ...
            bender.tools.emulatePaste(
              editor,
              '<div data-cke-widget-wrapper="1"><div data-cke-widget-upcasted="1" data-widget="pastenested"><div data-cke-widget-editable="nested">Test</div></div></div>'
            );
    ...
    "test widgets' commands are enabled in nested editable": function () {
      ...
      bot.setData(
        '<p>foo</p><div id="w1" data-widget="testcommand1"><div class="foo">X</div></div>',
        function () {
          var widget1 = getWidgetById(editor, "w1"),
            eFoo = widget1.editables.foo,
            range = editor.createRange();
          ...
          assert.areSame(
            ...
            editor.commands.testcommand1.state,
            "command is enabled in main editable"
          );
          ...
          assert.areSame(
            CKEDITOR.TRISTATE_OFF,
            editor.commands.testcommand1.state,
            "command is enabled in nested editable"
          );
        }
      );
    },

    "test selection in nested editable is preserved after opening and closing dialog":
      function () {
        var editor = this.editor,
          bot = this.editorBot;
        ...
        bot.setData(
          '<p>foo</p><div id="w1" data-widget="testselection1"><p class="foo">X</p></div><p>foo</p>',
          function () {
            var widget1 = getWidgetById(editor, "w1"),
              eFoo = widget1.editables.foo,
              range = editor.createRange(),
              insertedElement = new CKEDITOR.dom.element(
                "span",
                editor.document
              ),
              dialogSelStartEl;
              ...
              dialog.once("hide", function () {
                ...
                setTimeout(function () {
                  resume(function () {
                    assert.isTrue(
                      dialogSelStartEl &&
                        (eFoo.contains(dialogSelStartEl) ||
                          eFoo.equals(dialogSelStartEl)),
                      "selection retrieved on dialog show is placed in nested editable"
                    );

                    assert.isTrue(
                      eFoo.contains(insertedElement),
                      "element has been inserted into nested editable"
                    );
                    ...
                    assert.isTrue(
                      selStartEl &&
                        (eFoo.contains(selStartEl) || eFoo.equals(selStartEl)),
    ...
    "test selection in nested editable is preserved after opening and closing dialog - inline editor":
      function () {
        ...
        bender.editorBot.create(
          ...
          function (bot) {
            ...
            bot.setData(
              '<p>foo</p><div id="testselection2-w1" data-widget="testselection2"><p class="foo">X</p></div><p>foo</p>',
              function () {
                  ...
                  dialog.once("hide", function () {
                    ...
                    setTimeout(function () {
                      resume(function () {
                        ...
                        var selStartEl = editor
                          .getSelection()
                          .getStartElement();

                        assert.isTrue(
                          selStartEl &&
                            (eFoo.contains(selStartEl) ||
                              eFoo.equals(selStartEl)),
                          "selection retrieved after dialog is hidden is placed in nested editable"
                        );
                      });
                    }, 10);
    ...
    "test nested editables' ACF integration with styles system": function () {
      ...
      bot.setData(
        '<p>foo</p><div data-widget="testacfstyles1"><p class="foo">X</p><p class="bar">X</p></div><p>foo</p>',
        function () {
          editor.focus();
          ...
          assert.areSame(
            '<p><i>foo</i></p><div data-widget="testacfstyles1"><p class="foo">X</p><p class="bar"><i>X</i></p></div><p><i>foo</i></p>',
            editor.getData()
          );
        }
      );
    ...
    "test nested editable with preexisting numeric id": function () {
      ...
      bot.setData(
        '<p>foo</p><div data-widget="testpreexistingnumericid"><p id="123">X</p></div><p>foo</p>',
        function () {
