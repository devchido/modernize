...
(function () {
  ...
  function clickElement(editor, element, callback, noMouseup) {
    editor.document.fire(
      "mousedown",
      new CKEDITOR.dom.event({ target: element.$ })
    );
  ...
  function listen(editor, widgets) {
    var that = {
      ...
      removeListeners: function () {
        ...
        while ((listener = that.listeners.pop())) listener.removeListener();
      },
    };
  ...
  function assertHasBeenFocused(editor, widget) {
    ...
    assert.isTrue(
      widget.wrapper.hasClass("cke_widget_focused"),
      "wrapper has cke_widget_focused class"
    );
  ...
  function assertHasBeenSelected(editor, widget) {
    ...
    assert.isTrue(
      widget.wrapper.hasClass("cke_widget_selected"),
      "wrapper has cke_widget_selected class"
    );
  }
  ...
  function assertHasBeenBlurred(editor, widget, noFocusedCheck) {
    ...
    assert.isFalse(
      widget.wrapper.hasClass("cke_widget_focused"),
      "wrapper does not have cke_widget_focused class"
    );
  ...
  function assertHasBeenDeselected(editor, widget) {
    ...
    assert.isFalse(
      widget.wrapper.hasClass("cke_widget_selected"),
      "wrapper does not have cke_widget_selected class"
    );
  }
  ...
  function assertFakeSelection(selection, element) {
    ...
    assert.areSame(
      ...
      "selected element when focusing widget"
    );
  }
  ...
  function assertWidgetsEvents(
    ...
    assert.areSame(
      expectedFocused.length,
      result.widgetsFocused.length,
      "widget.focus length"
    );
    assert.areSame(
      expectedSelected.length,
      result.widgetsSelected.length,
      "widget.select length"
    );
    assert.areSame(
      expectedBlurred.length,
      result.widgetsBlurred.length,
      "widget.blur length"
    );
    assert.areSame(
      expectedDeselected.length,
      result.widgetsDeselected.length,
      "widget.deselect length"
    );

    arrayAssert.itemsAreSame(
      expectedFocused.sort(compareWidgets),
      result.widgetsFocused.slice(0).sort(compareWidgets),
      "widget.focus"
    );
    arrayAssert.itemsAreSame(
      expectedSelected.sort(compareWidgets),
      result.widgetsSelected.slice(0).sort(compareWidgets),
      "widget.select"
    );
    arrayAssert.itemsAreSame(
      expectedBlurred.sort(compareWidgets),
      result.widgetsBlurred.slice(0).sort(compareWidgets),
      "widget.blur"
    );
    arrayAssert.itemsAreSame(
      expectedDeselected.sort(compareWidgets),
      result.widgetsDeselected.slice(0).sort(compareWidgets),
      "widget.deselect"
    );
  }
  ...
  function assertRepoEvents(result, expectedFocused, expectedBlurred) {
    ...
    assert.areSame(
      expectedFocused.length,
      result.repoWidgetsFocused.length,
      "repo.widgetFocused length"
    );
    arrayAssert.itemsAreSame(
      expectedFocused,
      result.repoWidgetsFocused,
      "repo.widgetFocused"
    );

    assert.areSame(
      expectedBlurred.length,
      result.repoWidgetsBlurred.length,
      "repo.widgetBlurred length"
    );
    arrayAssert.itemsAreSame(
      expectedBlurred,
      result.repoWidgetsBlurred,
      "repo.widgetBlurred"
    );
  }
  ...
  bender.test({
    "test focusing widget": function () {
      ...
      bot.setData(
        '<p id="a">foo</p><div data-widget="testfocus1" id="x">bar</div><div data-widget="testfocus1">bom</div>',
        function () {
          ...
          assert.areSame(
            ...
            "selectionChange has been fired once when focusing widget"
          );
          assertFakeSelection(result.selection, widget.wrapper);
          assert.areSame(
            widget,
            result.widgetFocusedOnSelectionChange,
            "widget was already focused on selectionChange"
          );
          ...
          bot.setData("<p>foo</p>", function () {
            ...
            assert.isNull(
              ...
              "widgets.focused after set data"
            );
            assert.areSame(
              0,
              editor.widgets.selected.length,
              "widgets.selected after set data"
            );
            assert.isFalse(
              !!result.widgetFocusedOnSelectionChange,
              "widget was already blurred on selectionChange"
            );
          });
        }
      );
    ...
    "test blurring widget": function () {
      ...
      this.editorBot.setData(
        '<p id="a">foo</p><div data-widget="testfocus2" id="x">bar</div><div data-widget="testfocus2">bom</div>',
        function () {
          var widget = getWidgetById(editor, "x");
          ...
          assert.areSame(
            1,
            result.selectionChange,
            "selectionChange has been fired once when blurring widget"
          );
    ...
    "test focusing 2 widgets in a row": function () {
      ...
      this.editorBot.setData(
        '<div data-widget="testfocus3" id="x">bar</div><div data-widget="testfocus3" id="y">bom</div>',
        function () {
          ...
          assert.areSame(
            1,
            result.selectionChange,
            "selectionChange has been fired once"
          );
    ...
    "test widgets in selection": function () {
      ...
      this.editorBot.setData(
        multipleWidgetsTpl.replace(/\{num\}/g, "4"),
        function () {
    ...
    "test focusing by click": function () {
      ...
      this.editorBot.setData(
        '<p id="a">foo</p><div data-widget="testfocus3" id="x">bar</div><div data-widget="testfocus3" id="y">bom</div>',
        function () {
          var widgetX = getWidgetById(editor, "x"),
            widgetY = getWidgetById(editor, "y");
    ...
    "test focus editor when focusing widget by click": function () {
      ...
      this.editorBot.setData(
        '<p>foo</p><div data-widget="testfocus4" id="x">bar</div>',
        function () {
          ...
          wait(function () {
            ...
            clickWidget(editor, widget, function () {
              ...
              assert.isTrue(
                result.selectionChange >= 1,
                "selectionChange has been fired at least once"
              );
              assertFakeSelection(result.selection, widget.wrapper);
              assert.areSame(
                widget,
              ...
              wait(function () {
                ...
                assert.areSame(
                  widget,
                  editor.widgets.focused,
                  "widget is focused after editor gets focus"
                );
              }, 50);
            });
          }, 200);
    ...
    "test focus editor when focusing widget by method": function () {
      ...
      this.editorBot.setData(
        '<p>foo</p><div data-widget="testfocus5" id="x">bar</div>',
        function () {
          // We need to wait for focusManager.
          wait(function () {
            var widget = getWidgetById(editor, "x");
    ...
    "test blur widget when editor blurs": function () {
      ...
      this.editorBot.setData(
        '<p>foo</p><div data-widget="testfocus6" id="x">bar</div>',
        function () {
          var widget = getWidgetById(editor, "x");
          ...
          wait(function () {
            ...
            assert.areSame(
              0,
              result.selectionChange,
              "selectionChange has not been fired"
            );
    ...
    "test clicking nested editable does not focus widget": function () {
      ...
      this.editorBot.setData(
        '<p id="a">foo</p><div data-widget="testfocus7" id="x">bar<div id="foo">bar</div></div>',
        function () {
          var widget = getWidgetById(editor, "x");
          editor.getSelection().selectElement(editor.document.getById("a"));
    ...
    "test mousedown on drag handler does not focus inline widget": function () {
      ...
      this.editorBot.setData(
        '<p id="a">foo</p><p><span data-widget="testfocus8" id="x">bar</span></p>',
        function () {
          var widget = getWidgetById(editor, "x");
          editor.getSelection().selectElement(editor.document.getById("a"));
          ...
          clickElement(
            editor,
            widget.dragHandlerContainer.findOne("img"),
            function () {
              result.removeListeners();
