      ...
      bender.editorBot.create(
        {
          name: "basic1",
          startupData:
            '<p><b data-widget="test1">foo</b><i data-widget="test2" foo="1">bar</i>' +
      ...
      bender.editorBot.create(
        {
          name: "basic3",
          startupData:
            '<p><b data-widget="test1a">fooa</b><i data-widget="test2a">bara</i>' +
        ...
        function (bot) {
          ...
          assert.areSame(
            '<p><b>fooa</b><i data-widget="test2a">bara</i><u>foob</u><s data-widget="test2b">barb</s></p>',
            editor.getData()
          );

          assert.areSame(
            CKEDITOR.TRISTATE_DISABLED,
            editor.getCommand("test1a").state,
            "test1a is disabled"
          );
          assert.areSame(
            CKEDITOR.TRISTATE_OFF,
            editor.getCommand("test2a").state,
            "test2a is enabled"
          );

          assert.areSame(
            CKEDITOR.TRISTATE_DISABLED,
            editor.getCommand("test1b").state,
            "test1b is disabled"
          );
          assert.areSame(
            CKEDITOR.TRISTATE_OFF,
            editor.getCommand("test2b").state,
            "test2b is enabled"
          );
        }
      );
    ...
    "test widgets pasting": function () {
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          bot.setData(
            '<p><s data-widget="pasting1"><u class="pasting1">foo</u>bar</s>bom</p>',
            function () {
              ...
              editor.once("afterPaste", function () {
                resume(function () {
                  ...
                  assert.areSame(
                    ...
                    "one widget is initialized after paste"
                  );
