...
(function () {
  ...
  function generateWidgetsData(howMany) {
    ...
    for (var i = 0; i < howMany; ++i)
      html.push(
        widgetData.replace(/w([pn])-x/g, "w$1-" + i).replace(/p-x/g, "p-" + i)
      );
  ...
  bender.test({
    ...
    "test init nested widgets when pasting widget with nested widgets":
      function () {
        var editor = this.editors.editor;

        this.editorBots.editor.setData(
          '<p id="pasteTarget">foo</p>' + generateWidgetsData(2),
          function () {
            var widget = getWidgetById(editor, "wp-0"),
      ...
      this.editorBots.editor.setData(generateWidgetsData(2), function () {
        ...
        editor.once("afterPaste", function () {
          resume(function () {
            ...
            assert.isTrue(
              getWidgetById(editor, "wp-1").editables.ned.contains(
                getWidgetById(editor, "wp-0").wrapper
              ),
    ...
    "test widgets reinitialized after undo/redo": function () {
      ...
      this.editorBots.editor.setData(generateWidgetsData(1), function () {
        ...
        assert.areSame(
          CKEDITOR.TRISTATE_OFF,
          undo.state,
          "undoable after editable.setHtml"
        );
        assert.areSame(
          0,
          objToArray(editor.widgets.instances).length,
        ...
        assert.areSame(
          ...
          undo.state,
          "undo disabled after undoing"
        );
        assert.areSame(
          CKEDITOR.TRISTATE_OFF,
          redo.state,
          "redoable after undoing"
        );
        assert.areSame(
          3,
          objToArray(editor.widgets.instances).length,
          "3 widgets after undo"
        );
        ...
        assert.areSame(
          CKEDITOR.TRISTATE_OFF,
          undo.state,
          "undoable after redoing"
        );
        assert.areSame(
          CKEDITOR.TRISTATE_DISABLED,
          redo.state,
          "redo disabled after redoing"
        );
        assert.areSame(
          0,
          objToArray(editor.widgets.instances).length,
          "0 widgets after redo"
        );
      });
    },
    ...
    "test drag handler is created for every widget": function () {
      ...
      this.editorBots.editor.setData(generateWidgetsData(1), function () {
        ...
        assert.areSame(
          w1.wrapper,
          w1.dragHandlerContainer.getParent(),
          "main widget's drag handler is directly in the wrapper"
        );
        assert.areSame(
          wn1.wrapper,
          wn1.dragHandlerContainer.getParent(),
          "1st nested widget's drag handler is directly in the wrapper"
        );
        assert.areSame(
          wn2.wrapper,
          wn2.dragHandlerContainer.getParent(),
          "2nd nested widget's drag handler is directly in the wrapper"
        );
      });
    },

    "test all widgets are destroyed once when setting editor data":
      function () {
        var editor = this.editors.editor,
    ...
    "test all nested widgets are destroyed when setting nested editable data":
      function () {
        var editor = this.editors.editor,
          bot = this.editorBots.editor,
          destroyed = [];
    ...
    "test pasting widget with nested editable into nested editable":
      function () {
        ...
        bot.setData(
          generateWidgetsData(1) + "<p>xxx</p>" + widget2Data,
          function () {
            var w1 = getWidgetById(editor, "wp-0"),
    ...
    "test findOneNotNested": function () {
      ...
      this.editorBots.editor.setData(editorHtml, function () {
        ...
        assert.areSame(
          null,
          col1,
          "findOneNotNested for selector .col1 returns"
        );
        ...
        assert.areSame(
          "uppercol2",
          col2.getId(),
          "findOneNotNested returned .col2 with id"
        );
      });
    },
    ...
    "test editables are not matched from among nested widgets": function () {
      ...
      editor.widgets.add("testwidget", {
        template:
          '<div class="testwidget"><div class="col1"></div><div class="col2"></div></div>',
        editables: {
          col1: { selector: ".col1" },
          col2: { selector: ".col2" },
        },
      ...
      this.editorBots.editor.setData(editorHtml, function () {
        ...
        assert.areSame(
          "uppercol1",
          widgetUpper.editables.col1.getId(),
          "upper widget has editable .col1 with id"
        );
        assert.areSame(
          "uppercol2",
          widgetUpper.editables.col2.getId(),
          "upper widget has editable .col2 with id"
        );
        assert.areSame(
          "nestedcol1",
          widgetNested.editables.col1.getId(),
          "nested widget has editable .col1 with id"
        );
        assert.areSame(
          "nestedcol2",
          widgetNested.editables.col2.getId(),
          "nested widget has editable .col2 with id"
        );
      });
    },
  });
