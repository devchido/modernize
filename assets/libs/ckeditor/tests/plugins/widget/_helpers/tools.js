...
var widgetTestsTools = (function () {
  ...
  function addTests(tcs, config) {
    var editor,
      ...
      editorConfig = {
        extraPlugins:
          config.extraPlugins + ",sourcearea,undo,clipboard,toolbar",
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.isNotUndefined(
            editor.widgets.registered[config.widgetName],
            "widget definition is registered"
          );
    ...
    function assertWidgets(msg) {
      var instances = bender.tools.objToArray(editor.widgets.instances);
      assert.areSame(
        config.initialInstancesNumber,
        instances.length,
        "instances number " + msg
      );
      ...
      for (var i = 0; i < instances.length; ++i)
        assert.isTrue(
          editable.contains(instances[i].wrapper),
          "editable contains wrapper " + msg
        );
    ...
    html = html.replace(/style="([^"]*)"/g, function (styleStr) {
      ...
      return (
        ...
        CKEDITOR.tools.htmlEncodeAttr(
          CKEDITOR.tools.writeCssText(CKEDITOR.tools.parseCssText(style))
        ) +
        '"'
      );
    });
  ...
  function getAttrData(widget) {
    return JSON.parse(
      decodeURIComponent(widget.element.data("cke-widget-data"))
    );
  }
  ...
  function getWidgetById(editor, id, byElement) {
    ...
    if (widget && byElement) return widget.element.$.id == id ? widget : null;
  ...
  function getWidgetByDOMOffset(editor, offset) {
    ...
    if (wrapper) ret = editor.widgets.getByElement(wrapper);
  ...
  function assertDowncast(
    ...
    editorBot.setData(data, function () {
      var instancesArray = bender.tools.objToArray(
        editorBot.editor.widgets.instances
      );

      assert.areEqual(
        expectedInstancesCount,
        instancesArray.length,
        "Invalid count of created widget instances."
      );
      assert.areEqual(
        fixHtml(data),
        fixHtml(editorBot.getData()),
        "Editor html after performing downcast is not matching."
      );
      ...
      if (expectedWidgetName) {
        for (var i = instancesArray.length - 1; i >= 0; i--)
          assert.areEqual(
            expectedWidgetName,
            instancesArray[i].name,
            "Widget at index " + i + " has widget invalid definition name."
          );
      }
    });
  }
  ...
  function assertWidgetDialog(
    editorBot,
    commandName,
    editorHtml,
    focusedWidgetIndex,
    expectedValues,
    htmlWithSelectionSource,
    onResume
  ) {
      ...
      resume(function () {
        ...
        try {
          ...
          for (var i in expectedValues)
            assert.areSame(
              expectedValues[i],
              dialog.getValueOf(dialogCheckedTab, i),
              'Dialog field "' +
                i +
                '" in tab "' +
                dialogCheckedTab +
    ...
    editorBot.setData(editorHtml, function () {
      ...
      if (typeof focusedWidgetIndex == "number") {
        ...
        assert.isNotNull(
          widget,
          "Could not find widget with index " + focusedWidgetIndex
        );
        ...
        assert.isNotNull(
          widget,
          "Could not find widget with id " + focusedWidgetIndex
        );
      }
  ...
  function assertWidget(config) {
    ...
    config.bot.setData(config.html, function () {
      var widget = null,
        pickWidget = function () {
          ...
          if (config.widgetId)
            ...
            ret = widgetTestsTools.getWidgetByDOMOffset(
              editor,
              config.widgetOffset
            );
      ...
      typeof config.count !== "undefined" &&
        assert.areSame(
          Number(config.count),
          instancesArray.length,
          "Invalid count of widgets found."
        );
      ...
      if (typeof config.count !== "number" || config.count > 0) {
        ...
        assert.areSame(
          config.nameCreated,
          widget.name,
          "Initial widget name must match."
        );
        ...
        config.nameNewData &&
          assert.areSame(
            config.nameNewData,
            widget.name,
            "Widget name must match after data is set."
          );
