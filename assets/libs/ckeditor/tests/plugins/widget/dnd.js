  ...
  bender.test({
    ...
    "test handler - block widget": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testwidget" id="w1">foo</p>',
        function () {
          ...
          assert.isTrue(
            handlerContainer.hasClass("cke_widget_drag_handler_container")
          );
    ...
    "test handler - inline widget": function () {
      ...
      this.editorBot.setData(
        '<p><span data-widget="testwidget" id="w1">bar</span></p>',
        function () {
          var widget = getWidgetById(editor, "w1");
    ...
    "test handler - positioning": function () {
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.areNotEqual(
            "",
            handlerContainer.getStyle("top"),
            "Handler should have top style."
          );
          assert.areNotEqual(
            "",
            handlerContainer.getStyle("left"),
            "Handler should have left style."
          );
        }
      );
    },
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.areEqual(
            "",
            handlerContainer.getStyle("top"),
            "Handler should not have top style precalculated."
          );
          assert.areEqual(
            "",
            handlerContainer.getStyle("left"),
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.areEqual(
            1,
            snapshotLockCalled,
            "lockSnapshot should be fired exactly once"
          );
          assert.areEqual(
            1,
            snapshotUnlockCalled,
            "unlockSnapshot should be fired exactly once"
          );
          assert.areEqual(
            1,
            setStylesCalled,
            "setStyles should be called exactly once"
          );
          ...
          assert.areEqual(
            1,
            snapshotLockCalled,
            "lockSnapshot should not be fired"
          );
          assert.areEqual(
            1,
            snapshotUnlockCalled,
            "unlockSnapshot should not be fired"
          );
    ...
    "test widget.updateDragHandlerPosition - editor dirty state": function () {
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.isFalse(
            editor.checkDirty(),
            "Editor should not be marked as dirty"
          );
        }
      );
    },
    ...
    "test handler - is repositioned on #data and mouseenter after widget reinitialization":
      function () {
        var editor = this.editor,
          updated = 0;
          ...
          function () {
            ...
            wait(function () {
              ...
              widget.dragHandlerContainer.setStyles = function (styles) {
                ...
                if (styles.top && styles.left) updated++;
              };
              ...
              assert.areEqual(
                1,
                updated,
                "position has been updated on widget#data"
              );
              ...
              assert.areEqual(
                2,
                updated,
                "position has been updated on wrapper#mouseenter"
              );
            }, 100);
          }
        );
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.areSame(
            0,
            widgetWasDestroyed,
            "Original widget should not be destroyed"
          );
        }
      );
    },
    ...
    "test drop - cross-editor drop": function () {
      ...
      this.editorBot.setData(
        '<p class="x">foo</p><p><b>x<span data-widget="testwidget" id="w1">foo</span>x</b></p>',
        function () {
          ...
          assert.areSame(
            true,
            dropCalled,
            "the drop event should have been called"
          );
          assert.areSame(
            false,
            dropNotCancelled,
            "the drop event should have been cancelled"
          );
        }
      );
    },
    ...
    "test drop - successful drag and drop": function () {
      ...
      this.editorBot.setData(
        '<p class="x">foo</p><p>x<span data-widget="testwidget" id="w1">foo</span>x</p>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            evt = { data: bender.tools.mockDropEvent() },
            range = editor.createRange(),
          ...
          bender.tools.resumeAfter(editor, "afterPaste", function () {
            ...
            assert.isTrue(
              widgetWasDestroyedCounter.calledOnce,
              "original widget was destroyed"
            );
            assert.areSame(
              '<p class="x">f<span data-widget="testwidget" id="w1">foo</span>oo</p><p>xx</p>',
              editor.getData()
            );
            assert.isTrue(
              widgetCreatedCounter.calledOnce,
    ...
    "test drop widget with formating": function () {
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          bender.tools.resumeAfter(editor, "afterPaste", function () {
            assert.areSame(
              '<p class="x">f<b><span data-widget="testwidget" id="w1">foo</span></b>oo</p><p><b>xx</b></p>',
              editor.getData()
            );
          });
    ...
    "test drag and drop - block widget": function () {
      ...
      var revert = bender.tools.replaceMethod(
        CKEDITOR.plugins.lineutils.finder.prototype,
        "getRange",
        function () {
          var range = editor.createRange();

          range.moveToPosition(
            editor.document.getById("a"),
      ...
      this.editorBot.setData(
        '<p id="a">foo</p><div data-widget="testwidget" id="w1">bar</div>',
        function () {
          var widget = getWidgetById(editor, "w1"),
          ...
          try {
            ...
            assert.isNull(
              ...
              "widget not focused before mousedown"
            );
            ...
            assert.areSame(
              widget,
              editor.widgets.focused,
              "widget focused after mouseup"
            );

            bender.tools.resumeAfter(editor, "afterPaste", function () {
              assert.isTrue(pasteCounter.calledOnce, "paste called once");
              assert.isTrue(
                dragstartCounter.calledOnce,
                "dragstart called once"
              );
              assert.isTrue(dragendCounter.calledOnce, "dragend called once");
              assert.isTrue(dropCounter.calledOnce, "drop called once");
              assert.areSame(
                '<div data-widget="testwidget" id="w1">bar</div><p id="a">foo</p>',
                editor.getData(),
                "Widget moved on drop."
              );
    ...
    "test drag and drop - block widget into widget in nested editable":
      function () {
        var editor = this.editor,
          html =
            '<div data-widget="testwidget4" id="w4">' +
            '<div class="n1">' +
        ...
        this.editorBot.setData(html, function () {
          ...
          assertRelations(
            editor,
            finder,
            '|<div data-widget="testwidget4" id="w4"><div class="n1"><div data-widget="testwidget4" id="w4a"><div class="n1"><p>x</p></div></div></div></div>|'
          );
        });
      },

    "test drag and drop - block widget into nested editable (ACF)":
      function () {
        var editor = this.editor,
          html =
            '<div data-widget="testwidget2">' +
            '<div class="n1">' +
            "<p>x</p>" +
            "</div>" +
            '<div class="n2">' +
        ...
        this.editorBot.setData(html, function () {
          ...
          assertRelations(
            editor,
            finder,
            '|<div data-widget="testwidget2"><div class="n1">|<p>x</p>|</div><div class="n2"><p>y</p></div></div>|'
          );
        });
      },

    "test drag and drop - block widget into nested editable (ACF) - no filter":
      function () {
        var editor = this.editor,
          html =
            '<div data-widget="testwidget4">' +
            '<div class="n1">' +
            "<p>x</p>" +
            "</div>" +
            "</div>" +
        ...
        this.editorBot.setData(html, function () {
          ...
          assertRelations(
            editor,
            finder,
            '|<div data-widget="testwidget4"><div class="n1">|<p>x</p>|</div></div>|'
          );
        });
      },

    "test drag and drop - block widget into nested editable (ACF) - self-drop":
      function () {
        var editor = this.editor,
          html =
            '<div data-widget="testwidget4" id="w4">' +
        ...
        this.editorBot.setData(html, function () {
          ...
          assertRelations(
            editor,
            finder,
            '|<div data-widget="testwidget4" id="w4"><div class="n1"><p>x</p></div></div>|'
          );
        });
      },
      ...
      function testButton(button, isOn, callback) {
        bot.setData(
          ...
          function () {
            ...
            setTimeout(function () {
              resume(function () {
                assert[isOn ? "isTrue" : "isFalse"](
                  editor.editable().hasClass("cke_widget_dragging")
                );
