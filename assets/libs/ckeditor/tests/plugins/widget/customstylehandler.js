...
(function () {
  ...
  function widgetPath(startId, editor) {
    return new CKEDITOR.dom.elementPath(
      getWidgetById(editor, startId).wrapper,
      editor.editable()
    );
  }
  ...
  bender.test({
    ...
    "test custom style methods return false when editor not specified":
      function () {
        var editor = this.editors.editor,
          style = st({ type: "widget", widget: "someOtherWidget" });

        this.editorBots.editor.setData(
          '<p data-widget="testWidget" id="w1">x</p>',
          function () {
            var wrapper = getWidgetById(editor, "w1").wrapper;

            assert.isFalse(
              style.checkApplicable(
                style.checkApplicable(widgetPath("w1", editor))
              ),
              "checkApplicable"
            );
            assert.isFalse(
              style.checkApplicable(
                style.checkApplicable(widgetPath("w1", editor), editor.filter)
              ),
              "checkApplicable with filter as 2nd argument"
            );
            assert.isFalse(
              style.checkActive(widgetPath("w1", editor)),
              "checkActive"
            );
            assert.isFalse(
              style.checkElementMatch(wrapper),
              "checkElementMatch"
            );
            assert.isFalse(
              style.checkElementRemovable(wrapper),
    ...
    "test checkApplicable on matching widget": function () {
      ...
      this.editorBots.editor.setData(
        '<p data-widget="testWidget" id="w1">x</p><p>foo<b data-widget="testWidget" id="w2">x</b></p>',
        function () {
          assert.isTrue(
            style.checkApplicable(widgetPath("w1", editor), editor),
            "block widget"
          );
          assert.isTrue(
            style.checkApplicable(widgetPath("w2", editor), editor),
            "inline widget"
          );
        }
      );
    ...
    "test checkApplicable on non-widget elements": function () {
      ...
      this.editorBots.editor.setData(
        '<p id="p1">x</p><div data-widget="testWidget" id="w1"><p class="editable"><b>x</b></p></div>',
        function () {
          ...
          assert.isFalse(
            style.checkApplicable(path, editor),
            "some other element"
          );
          ...
          path = new CKEDITOR.dom.elementPath(
            widget.editables.foo.findOne("b"),
            editor.editable()
          );
          assert.isFalse(
            style.checkApplicable(path, editor),
            "element inside nested editable"
          );
        }
      );
    },
    ...
    "test checkElementMatch returns result of widget.checkStyleActive":
      function () {
        var editor = this.editors.editor,
          style = st({ type: "widget", widget: "testWidget" });
    ...
    "test checkElementMatch and checkActive pass style instance to widget.checkStyleActive":
      function () {
        var editor = this.editors.editor,
          style = st({ type: "widget", widget: "testWidget" });
    ...
    "test checkElementMatch returns false on non-widget element": function () {
      ...
      this.editorBots.editor.setData('<p id="p1">x</p>', function () {
        assert.isFalse(
          style.checkElementMatch(editor.document.getById("p1"), 0, editor)
        );
    ...
    "test apply/remove calls applyStyle/removeStyle on focused widget":
      function () {
        var editor = this.editors.editor,
          style = st({ type: "widget", widget: "testWidget" });

        this.editorBots.editor.setData(
          '<p data-widget="testWidget" id="w1">x</p><p data-widget="testWidget" id="w2">x</p>',
          function () {
    ...
    "test apply/remove passes style instance to the applyStyle/removeStyle method":
      function () {
        var editor = this.editors.editor,
          style = st({ type: "widget", widget: "testWidget" });
    ...
    "test apply/remove does not proceed if checkApplicable returns false":
      function () {
        var editor = this.editors.editor,
          style = st({ type: "widget", widget: "testWidget" });
    ...
    "test buildPreview": function () {
      ...
      assert.areSame(
        "Foo",
        style.buildPreview(),
        "buildPreview returns style's name"
      );
      assert.areSame(
        "Bar",
        style.buildPreview("Bar"),
        "buildPreview returns provided label if defined"
      );
    },

    "test toAllowedContentRules - no styleableElements, no styleToAllowedContentRules":
      function () {
        var editor = this.editors.editor,
          style = st({
            name: "Foo",
    ...
    "test removeStylesFromSameGroup": function () {
      ...
      this.editorBots.editor.setData(
        '<p data-widget="testWidget" id="w1" class="c1 c2 c3 c4">x</p>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            path;
          ...
          assert.isFalse(
            styles[0].checkActive(path, editor),
            "Only one style from group should be active."
          );
          assert.isFalse(
            styles[1].checkActive(path, editor),
            "Only one style from group should be active."
          );
          assert.isTrue(
            styles[2].checkActive(path, editor),
            "Style should be active after style.removeStylesFromSameGroup"
          );
          assert.isFalse(
            styles[3].checkActive(path, editor),
            "Only one style from group should be active."
          );
        }
      );
    ...
    "test styles groups": function () {
      ...
      this.editorBots.editor.setData(
        ...
        function () {
          ...
          assert.isFalse(
            styles.group1[0].checkActive(path, editor),
            "Styles from same group should not be applied together"
          );
          assert.isFalse(
            styles.group1[1].checkActive(path, editor),
            "Styles from same group should not be applied together"
          );
          assert.isTrue(
            styles.group1[2].checkActive(path, editor),
            "Last applied style from the group should be active."
          );

          assert.isFalse(
            styles.group2[0].checkActive(path, editor),
            "Styles from same group should not be applied together"
          );
          assert.isFalse(
            styles.group2[1].checkActive(path, editor),
            "Styles from same group should not be applied together"
          );
          assert.isTrue(
            styles.group2[2].checkActive(path, editor),
            "Last applied style from the group should be active."
          );

          assert.isTrue(
            styles.withoutGroup[0].checkActive(path, editor),
            "Styles without group should not be removed."
          );
          assert.isTrue(
            styles.withoutGroup[1].checkActive(path, editor),
            "Styles without group should not be removed."
          );
        }
      );
    ...
    "test styles multiple groups": function () {
      ...
      this.editorBots.editor.setData(
        ...
        function () {
          ...
          assert.isTrue(
            styles[0].checkActive(path, editor),
            "Style from group1 should be active."
          );
          assert.isTrue(
            styles[1].checkActive(path, editor),
            "Style from group2 should be active."
          );
          ...
          assert.isFalse(
            styles[1].checkActive(path, editor),
            "Style from group2 should not be active."
          );
          assert.isTrue(
            styles[2].checkActive(path, editor),
            "Style should be active"
          );
          ...
          assert.isFalse(
            styles[0].checkActive(path, editor),
            "Style from group1 should not be active."
          );
          assert.isFalse(
            styles[1].checkActive(path, editor),
            "Style from group2 should not be active."
          ...
          assert.isFalse(
            styles[2].checkActive(path, editor),
            "Style from group3 should not be active."
          );
          assert.isTrue(
            styles[3].checkActive(path, editor),
            "Style should be active."
