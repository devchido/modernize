...
(function () {
  ...
  function getItemElements(stylesCombo) {
    ...
    for (var value in items) elements.push(doc.getById(items[value]));
    ...
    "test basic styles": function () {
      ...
      function assertStates(
        ...
        assert.areSame(
          expectedActiveFilterChanged,
          activeFilterChanged,
          msg + "active filter changed"
        );
        assert.areSame(
          italicIsEnabled ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED,
          cItalic.state,
          msg + "italic"
        );
        assert.areSame(
        ...
        function () {
          ...
          function checkDialog(tabStates, fieldStates, callback) {
            bot.dialog("link", function (dialog) {
              ...
              assert.areSame(
                tabStates[0],
                !tabs.getItem(0).hasClass("cke_dialog_tab_disabled"),
                "Tab info"
              );
              assert.areSame(
                tabStates[1],
                !tabs.getItem(1).hasClass("cke_dialog_tab_disabled"),
                "Tab target"
              );
              assert.areSame(
                tabStates[2],
                !tabs.getItem(3).hasClass("cke_dialog_tab_disabled"),
                "Tab advanced"
              );

              dialog.selectPage("target");
              assert.areSame(
                tabStates[1] ? "target" : "info",
                dialog._.currentTabId,
                "Tab should change only if it is available."
              );

              assert.areSame(
                fieldStates[0],
                dialog.getContentElement("info", "url").isEnabled(),
                "info url"
              );
              assert.areSame(
                fieldStates[1],
                dialog.getContentElement("advanced", "advId").isEnabled(),
                "advanced Id"
              );
              assert.areSame(
                fieldStates[2],
                dialog.getContentElement("advanced", "advName").isEnabled(),
                "advanced Name"
              );
        ...
        function () {
          ...
          function checkStylesCombo(state, stylesCount, msg) {
            ...
            for (var i in elements) {
              if (elements[i].getStyle("display") != "none") count++;
            }
      ...
      editor.widgets.add("testFontCombo", {
        editables: {
          ...
          all: {
            selector: ".all",
            allowedContent:
              "span{font-family,font-size,color,background-color}[lang,dir];p{margin-left,margin-right}",
          },
        },
      });
          ...
          function checkState(elementName, expexted) {
            assert.areSame(
              expexted.font
                ? CKEDITOR.TRISTATE_OFF
                : CKEDITOR.TRISTATE_DISABLED,
              fontFamilyCombo.getState(),
              elementName + " - family " + expexted.font
            );
            ...
            assert.areSame(
              expexted.size
                ? CKEDITOR.TRISTATE_OFF
                : CKEDITOR.TRISTATE_DISABLED,
              fontSizeCombo.getState(),
              elementName + " - size " + expexted.size
            );
            ...
            assert.areSame(
              expexted.color
                ...
                : CKEDITOR.TRISTATE_DISABLED,
              textColor.getState(),
              elementName + " - text " + expexted.color
            );

            assert.areSame(
              expexted.bg ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED,
              bgColor.getState(),
              elementName + " - background " + expexted.bg
            );
            ...
            assert.areSame(
              expexted.indent
                ...
                : CKEDITOR.TRISTATE_DISABLED,
              indent.getState(),
              elementName + " - indent " + expexted.indent
            );
          }
        }
      );
