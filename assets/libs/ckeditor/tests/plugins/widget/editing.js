  ...
  bender.test({
    "test edit method": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="test1" id="x">foo</p>',
        function () {
          var widget = getWidgetById(editor, "x"),
            openedDialog,
            retVal;

          var revert = replaceMethod(
            editor,
            "openDialog",
            function (dialogName) {
    ...
    "test edit method with no default dialog name": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="test2" id="x">foo</p>',
        function () {
          var widget = getWidgetById(editor, "x"),
            openedDialog = null,
            retVal;
    ...
    "test cancelling doubleclick when widget.edit() returns true": function () {
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.isTrue(
            retVal !== false,
            "editor#doubleclick should not be canceled"
          );
        }
      );
    },
          ...
          editor.once("dialogHide", function () {
            resume(function () {
              ...
              assert.areSame(
                1,
                onData,
                "Updating two fields result in one data event"
              );
            });
          });
          ...
          editor.once("dialogHide", function () {
            resume(function () {
              assert.areNotSame(
                "foofoo",
                value1,
                "Cancelled dialog event prevented from default data loading"
              );
    ...
    "test enter triggering edit": function () {
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          editor
            .editable()
            .fire("keydown", new CKEDITOR.dom.event({ keyCode: 13 }));
        }
      );
    },
    ...
    "test creating widget using command": function () {
      ...
      this.editorBot.setData("<p>FooBar</p>", function () {
        ...
        assert.areSame(
          '<p>Foo</p><div data-widget="testcommand1" id="w1">abc</div><p>Bar</p>',
    ...
    "test creating with canceled edit": function () {
      ...
      this.editorBot.setData("<p>x</p>", function () {
        ...
        assert.areSame(
          "<p>x</p>",
          editor.getData(),
          "widget should not be inserted because edit was canceled"
        );
      });
    },

    "test creating widget using command - no data-cke-widget attribute in template":
      function () {
        var editor = this.editor,
          editFired = 0;
        ...
        this.editorBot.setData("<p>FooBar</p>", function () {
          ...
          assert.areSame(
            ...
            "Widget element has data-cke-widget attr"
          );
          assert.areSame(
            '<p>Foo</p><div id="w1">abc</div><p>Bar</p>',
            editor.getData(),
            "Data"
          );
          assert.areSame(1, editFired, "Widget.edit was called");
    ...
    "test creating widget using command - DOM fixing": function () {
      ...
      this.editorBot.setData("<p>FooBar</p>", function () {
        ...
        assert.areSame(
          '<p>Foo</p><span data-widget="testcommand3" id="w1">foo</span><p>Bar</p>',
          fixHtml(editor.getData()),
          "Data"
        );
      });
        ...
        editor.once("dialogHide", function () {
          resume(function () {
            assert.areSame(
              0,
              readyFiredBeforeHide,
              "ready was not fired before closing dialog"
            );
            assert.isFalse(
              widgetWasAttached,
              "widget was not attached to DOM before closing dialog"
            );
            assert.areSame(1, readyFired, "ready event is fired");
            assert.isTrue(
              editor.editable().contains(widget.wrapper),
              "widegt is attached to DOM"
            );
          });
        });
    ...
    "test widget is transformed during its creation - transformation is made immediately":
      function () {
        var editor = this.editor,
          widgetA,
          widgetB;
        ...
        editor.widgets.add("testcommand4a", {
          ...
          data: function () {
            ...
            assert.isFalse(
              editor.editable().contains(this.element),
              "widget has not been inserted yet"
            );
        ...
        this.editorBot.setData("<p>FooBar</p>", function () {
          ...
          assert.areSame(
            "testcommand4b",
            widgetB.name,
            "widget was transformed"
          );
          assert.areNotSame(
            widgetA.id,
            widgetB.id,
            "transformed widget has a new id"
          );
          assert.areSame("b", widgetB.element.getName());
          assert.areSame(
            '<p>Foo<b><span id="w1">foo</span></b>Bar</p>',
            editor.getData()
          );
        });
      },

    "test widget is transformed during its creation - transformation is made on dialog close":
      function () {
        var editor = this.editor,
          editFired = 0,
          widgetA,
    ...
    "test cancelling cancel widget dialog does not destroy widget (https://dev.ckeditor.com/ticket/13158).":
      function () {
        var editor = this.editor,
