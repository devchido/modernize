  ...
  bender.test({
    ...
    "test initialization - init method and data event": function () {
      ...
      editor.widgets.on("instanceCreated", function (evt) {
        if (evt.data.name != "testinit1") return;
      ...
      this.editorBot.setData(
        '<p>X<b data-widget="testinit1">bar</b>X</p>',
        function () {
          assert.areSame(
            "instanceCreated,init:1,initEnd,data:2,ready",
            order.join(",")
          );

          widget.setData("foo", 4);
          assert.areSame(
            "instanceCreated,init:1,initEnd,data:2,ready,data:4",
            order.join(",")
          );
        }
      );
    ...
    "test initialization - element data": function () {
      ...
      editor.widgets.on("instanceCreated", function (evt) {
        if (evt.data.name != "testinit2") return;
      ...
      this.editorBot.setData(
        '<p>X<b data-widget="testinit2" data-cke-widget-data="' +
          data2Attr({ foo: 3 }) +
          '">bar</b>X</p>',
        function () {
          assert.areSame(
            "instanceCreated,init:1,initEnd,data:3",
            order.join(",")
          );
          ...
          widget.setData("foo", 4);
          assert.areSame(
            "instanceCreated,init:1,initEnd,data:3,data:4",
            order.join(",")
          );
        }
      );
    ...
    "test initialization - defaults is a function": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testinit3" id="w1">a</p><p data-widget="testinit3" id="w2">b</p>',
        function () {
          ...
          assert.areSame(
            "0,1",
            [widget1.data.foo, widget2.data.foo].sort().join(","),
            "data.foo"
          );
    ...
    "test initialization - startup data": function () {
      ...
      editor.widgets.on("instanceCreated", function (evt) {
        if (evt.data.name != "testinit4") return;
      ...
      this.editorBot.setData("", function () {
        var el = CKEDITOR.dom.element.createFromHtml(
          '<p>X<b data-widget="testinit4" data-cke-widget-data="' +
            data2Attr({ foo: 3 }) +
            '">bar</b>X</p>',
          editor.document
        );
        ...
        var widget = editor.widgets.initOn(el.findOne("b"), null, {
          foo: 4,
          bar: 5,
        });

        assert.areSame(
          "instanceCreated,init:1,initEnd,data:4",
          order.join(","),
    ...
    "test initialization - basic properties": function () {
      ...
      assert.isUndefined(
        regWidgetDef.repository,
        "no repository set in definition"
      );
      assert.isUndefined(
        regWidgetDef.definiotion,
        "no definition set in definition"
      );
      ...
      this.editorBot.setData(
        '<p>X<b data-widget="testinit5" id="w1">bar</b>X</p>',
        function () {
    ...
    "test destroy during initialization": function () {
      ...
      this.editorBot.setData("", function () {
        editor
          ...
          .append(
            CKEDITOR.dom.element.createFromHtml(
              '<p>X<b data-widget="testinitdestroy">bar</b>X</p>',
              editor.document
            )
          );
        ...
        assert.areSame(
          "p",
          el.getParent().getName(),
          "widget element is not wrapped"
        );
        assert.areSame(
          0,
          objToArray(editor.widgets.instances).length,
          "no instances"
        );
      });
    },
    ...
    "test set data": function () {
      ...
      this.editorBot.setData(
        '<p>X<b data-widget="testdata1" id="x">bar</b>X</p>',
        function () {
          var widget = editor.widgets.getByElement(
          ...
          assert.isTrue(
            CKEDITOR.tools.objectCompare(widgetData, widget.data),
            "widgetData vs widget.data 1"
          );
    ...
    "test data loading": function () {
      ...
      this.editorBot.setData(
        '<p>X<b data-widget="testdata2" data-cke-widget-data="' +
          data2Attr({ foo: 1 }) +
          '" id="x">bar</b>X</p>',
        function () {
          var widget = editor.widgets.getByElement(
          ...
          assert.isMatching(
            ...
            fixHtml(widget.element.getOuterHtml()),
            "widget.getOutput()"
          );
    ...
    "test classes loading": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testclassesloading1" id="w1" class="foo bar">foo</p>',
        function () {
          ...
          assert.areSame(
            "bar,foo",
            getClasses(widget).join(","),
            "widget.data.classes is loaded"
          );
          assert.isTrue(widget.hasClass("foo"));
          assert.isTrue(widget.hasClass("bar"));
          assert.isTrue(
            wrapper.hasClass(prefix + "foo"),
            "Prefixed class should be applied on wrapper."
          );
          assert.isTrue(
            wrapper.hasClass(prefix + "bar"),
            "Prefixed class should be applied on wrapper."
          );
        }
      );
    ...
    "test classes loading - existing data.classes is chosen": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testclassesloading2" data-cke-widget-data="' +
          data2Attr({ classes: { foo: 1, bar: 1 } }) +
          '" id="w1">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            wrapper = widget.wrapper;
    ...
    "test addClass, removeClass": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testaddremoveclass1" id="w1">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1"),
          ...
          assert.isFalse(
            ...
            "element does not have class foo"
          );
          assert.isFalse(
            wrapper.hasClass(prefix + "foo"),
            "wrapper does not have class " + prefix + "foo"
          );
          assert.isTrue(element.hasClass("bar"), "element still has class bar");
          assert.isTrue(
            wrapper.hasClass(prefix + "bar"),
            "wrapper does not have class " + prefix + "bar"
          );
        }
      );
    },
    ...
    "test hasClass": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testhasclass1" id="w1" class="foo bar">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1");
          ...
          assert.isFalse(
            widget.hasClass("bom"),
            "widget does not have class bom"
          );
        }
      );
    },
    ...
    "test getClasses": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testgetclasses1" id="w1">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1");
          ...
          assert.areSame(
            "bar,foo",
            classes2Array(widget.getClasses()).join(",")
          );
    ...
    "test applyStyle, removeStyle - integration test": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testapplyremovestyle1" id="w1">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            wrapper = widget.wrapper,
            style1 = st({
          ...
          assert.isTrue(
            wrapper.hasClass(prefix + "foo"),
            "style 1 has been applied to the wrapper too"
          );
          ...
          assert.isTrue(
            wrapper.hasClass(prefix + "foo"),
            "style 1 is still applied to the wrapper"
          );
          assert.isTrue(widget.hasClass("bar"), "style 2 has been applied");
          assert.isTrue(
            wrapper.hasClass(prefix + "bar"),
            "style 2 has been applied to the wrapper too"
          );
          ...
          assert.isFalse(
            wrapper.hasClass(prefix + "foo"),
            "style 1 has been removed from the wrapper"
          );
          assert.isTrue(widget.hasClass("bar"), "style 2 is sitll applied");
          assert.isTrue(
            wrapper.hasClass(prefix + "bar"),
            "style 2 is still applied to the wrapper"
          );
          ...
          assert.isFalse(
            wrapper.hasClass(prefix + "bar"),
            "style 2 has been removed from the wrapper"
          );
        }
      );
    },

    "test applyStyle, removeStyle - multiple classes - integration test":
      function () {
        var editor = this.editor,
        ...
        this.editorBot.setData(
          '<p data-widget="testapplyremovestyle2" id="w1">foo</p>',
          function () {
            var widget = getWidgetById(editor, "w1"),
              wrapper = widget.wrapper;
            ...
            assert.isTrue(
              wrapper.hasClass(prefix + "foo"),
              "1st class has been applied to the wrapper"
            );
            assert.isTrue(widget.hasClass("bar"), "2nd class has been applied");
            assert.isTrue(
              wrapper.hasClass(prefix + "bar"),
              "2nd class has been applied to the wrapper"
            );
            ...
            assert.isFalse(
              widget.hasClass("foo"),
              "1st class has been removed"
            );
            assert.isFalse(
              wrapper.hasClass(prefix + "foo"),
              "1st class has been removed from the wrapper"
            );
            assert.isFalse(
              widget.hasClass("bar"),
              "2nd class has been removed"
            );
            assert.isFalse(
              wrapper.hasClass(prefix + "bar"),
              "2nd class has been removed from the wrapper"
            );

            assert.isTrue(
              widget.hasClass("bom"),
              "unrelated class remains untouched"
            );
            assert.isTrue(
              wrapper.hasClass(prefix + "bom"),
              "unrelated class remains untouched on the wrapper"
            );
          }
        );
      },
    ...
    "test applyStyle - data.classes": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testapplystyledata1" id="w1">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            dataFired = 0;
          ...
          assert.areNotSame(
            oldClasses,
            widget.data.classes,
            "reference was broken - foo bar"
          );
          assert.areSame(2, dataFired, "2nd applyStyle fired data event");
          assert.areSame(
            "bar,foo",
            getClasses(widget).join(","),
            "2nd applyStyle added bar class"
          );
          ...
          assert.areSame(
            3,
            dataFired,
            "applying already applied styles does not fire data event"
          );
        }
      );
    },
    ...
    "test removeStyle - data.classes": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testremovestyledata1" id="w1">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            dataFired = 0;
          ...
          assert.areSame(
            "bar,bom",
            getClasses(widget).join(","),
            "1st removeStyle added foo class"
          );
          ...
          assert.areSame(
            "bom",
            getClasses(widget).join(","),
            "2nd removeStyle added bar class"
          );
          ...
          assert.areSame(
            "",
            getClasses(widget).join(","),
            "3rd removeStyle added bom class"
          );
          ...
          assert.areSame(
            3,
            dataFired,
            "removing already removed styles does not fire data event"
          );
        }
      );
    },
    ...
    "test classes updated on setData": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testsetdataclasses1" id="w1">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1"),
            wrapper = widget.wrapper;
          ...
          assert.isFalse(
            wrapper.hasClass(prefix + "foo"),
            "wrapper: 3 - " + prefix + "foo"
          );
          assert.isFalse(widget.hasClass("bom"), "3 - bom");
          assert.isFalse(
            wrapper.hasClass(prefix + "bom"),
            "wrapper: 3 - " + prefix + "bom"
          );
    ...
    "test checkStyleActive": function () {
      ...
      this.editorBot.setData(
        '<p data-widget="testcheckstyleactive1" id="w1" class="foo bar">foo</p>',
        function () {
          var widget = getWidgetById(editor, "w1");
          ...
          assert.isFalse(
            widget.checkStyleActive(style3),
            "style 3 is not active"
          );
          assert.isFalse(
            widget.checkStyleActive(style4),
            "style 4 is not active"
          );

          widget.removeStyle(style1);
          assert.isFalse(
            widget.checkStyleActive(style1),
            "style 1 is not active"
          );
          assert.isFalse(
            widget.checkStyleActive(style2),
            "style 2 is not active"
          );
        }
      );
    },

    "test checkStyleActive, applyStyle and removeStyle do not fail on style not defining any classes":
      function () {
        var editor = this.editor,
        ...
        this.editorBot.setData(
          '<p data-widget="teststylenoclasses1" id="w1">foo</p>',
          function () {
            var widget = getWidgetById(editor, "w1");
    ...
    "test checkStyleActive, applyStyle and removeStyle use widget methods":
      function () {
        var editor = this.editor,
        ...
        this.editorBot.setData(
          '<p data-widget="teststylemethodsused1" id="w1">foo</p>',
          function () {
            var widget = getWidgetById(editor, "w1"),
              called = [];
    ...
    "test paste widget with special characters in data": function () {
      ...
      this.editorBot.setData(
        '<p><b class="pastingencoding" id="w">Foo</b></p>',
        function () {
    ...
    "test special characters set in data during upcast": function () {
      ...
      this.editorBot.setData(
        '<p><b class="upcastingencoding" id="w">Foo</b></p>',
        function () {
          var widget = getWidgetById(editor, "w");
    ...
    // #1094
    "test upcasts methods are invoked only for specified elements":
      function () {
        var editor = this.editor,
          spy = sinon.spy();
        ...
        this.editorBot.setData("<p><b>Foo</b><del>Bar</del></p>", function () {
          ...
          assert.areSame(
            "del",
            spy.getCall(0).args[0].name,
            "Upcast was called on del element"
          );
        });
      },
    ...
    "test scope of upcast": function () {
      ...
      this.editorBot.setData(
        '<p><b class="upcastscope">Foo</b></p>',
        function () {
          assert.areSame(
            widget,
            scope,
            "Upcast is called in the context of widget"
          );
        }
      );
    },
    ...
    "test scope of upcasts": function () {
      ...
      this.editorBot.setData(
        '<p><b class="upcastscope2">Foo</b></p>',
        function () {
          assert.areSame(
            widget,
            scope,
            "Upcasts are called in the context of widget"
          );
        }
      );
    },
