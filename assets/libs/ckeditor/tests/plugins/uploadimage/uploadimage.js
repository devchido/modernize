...
(function () {
  ...
  bender.test({
    ...
    "test classic with image1 (integration test)": function () {
      ...
      var image = editor
        .editable()
        .find('img[data-widget="uploadimage"]')
        .getItem(0);
      ...
      waitForImage(image, function () {
        ...
        assert.sameData(
          '<p><img src="' + IMG_URL + '" style="height:1px; width:1px" /></p>',
          editor.getData()
        );
        assert.areSame(
          0,
          editor.editable().find('img[data-widget="image"]').count()
        );
    ...
    "test finish upload notification marked as important and is visible (https://dev.ckeditor.com/ticket/13032).":
      function () {
        var editor = this.editors.classic;
        ...
        waitForImage(image, function () {
          ...
          assert.areSame(
            1,
            area.notifications.length,
            "Successs notification is present because it's important one."
          );
    ...
    "test inline with image2 (integration test)": function () {
      ...
      waitForImage(image, function () {
        ...
        assert.sameData(
          '<p><img alt="" height="1" src="' + IMG_URL + '" width="1" /></p>',
          editor.getData()
        );
        assert.areSame(
          1,
        ...
        assert.areSame(0, uploadCount);
        assert.areSame(
          "http://foo/upload?type=Images&responseType=json",
          lastUploadUrl
        );
      });
    },
    ...
    "test paste img as html (integration test)": function () {
      ...
      bot.setData("", function () {
        pasteFiles(
          editor,
          [],
          '<p>x<img src="' + bender.tools.pngBase64 + '">x</p>'
        );
        ...
        waitForImage(image, function () {
          ...
          assert.sameData(
            '<p>x<img src="' +
              IMG_URL +
              '" style="height:1px; width:1px" />x</p>',
            editor.getData()
          );
          assert.areSame(
            0,
    ...
    "test setting image dimensions via response (integration test) (https://dev.ckeditor.com/ticket/13794)":
      function () {
        var bot = this.editorBots.classic,
          editor = this.editors.classic;
        ...
        bot.setData("", function () {
          ...
          resumeAfter(loader, "uploaded", function () {
            assert.sameData(
              '<p><img src="' +
                IMG_URL +
                '" style="height:444px; width:555px" /></p>',
              editor.getData()
            );
            assert.areSame(
              0,
    ...
    "test not supportedTypes tiff": function () {
      ...
      bot.setData("", function () {
        resumeAfter(editor, "paste", function () {
          assert.areSame(
            0,
            editor.editable().find('img[data-widget="uploadimage"]').count()
          );
        });
    ...
    "test paste nested image": function () {
      ...
      resumeAfter(editor, "paste", function (evt) {
        var imgs = CKEDITOR.dom.element
            .createFromHtml(evt.data.dataValue)
            .find('img[data-widget="uploadimage"]'),
          img,
          i;
    ...
    "test paste image already marked": function () {
      ...
      editor.fire("paste", {
        dataValue:
          '<img src="' +
          bender.tools.pngBase64 +
          '" data-widget="uploadimage" data-cke-upload-id="0">',
      });
    ...
    "test XSS attack": function () {
      ...
      editor.fire("paste", {
        dataValue:
          '<img src="x" onerror="window.attacked();">' + bender.tools.pngBase64,
      });
    ...
    "test prevent upload fake elements (https://dev.ckeditor.com/ticket/13003)":
      function () {
        var editor = this.editors.inline,
        ...
        editor.fire("paste", {
          dataValue:
            '<img src="data:image/gif;base64,aw==" alt="nothing" data-cke-realelement="some" />',
        });
      ...
      editor.once("afterPaste", function () {
        resume(function () {
          ...
          assert.isMatching(
            /image-\d+-\d+\.gif/,
            createSpy.args[0][1],
            "file name passed to first call"
          );
          assert.isMatching(
            /image-\d+-\d+\.gif/,
            createSpy.args[1][1],
            "file name passed to second call"
          );
          assert.areNotSame(
            createSpy.args[0][1],
            createSpy.args[1][1],
            "first and second call names are different"
          );
          assert.isMatching(
            /image-\d+-\d+\.png/,
            createSpy.args[2][1],
            "png type is recognized"
          );
        });
      });
    ...
    "test no error if missing configuration": function () {
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          ...
          assert.isFalse(
            !!bot.editor.widgets.registered.uploadimage,
            "uploadimage widget"
          );
        }
      );
