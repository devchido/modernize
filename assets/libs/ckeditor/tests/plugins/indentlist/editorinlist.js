...
bender.test({
  "test outdent does not unwrap parent li with multiple lists and both lists selected":
    function () {
      ...
      assert.isTrue(
        body.contains(originalEditableParent),
        "editable's parent was not removed from the body element"
      );
      assert.areSame(
        originalEditableParent,
        originalEditable.getParent(),
        "editable's parent did not change"
      );
      ...
      assert.areSame(
        "<p>1</p><ol><li>2</li></ol>",
        editor.getData(),
        "the first of the selected lists was outdented"
      );
    },

  "test outdent does not unwrap parent li with multiple lists and one list selected":
    function () {
      var bot = this.editorBot,
        editor = bot.editor;
