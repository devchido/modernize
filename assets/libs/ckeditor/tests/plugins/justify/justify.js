  ...
  var tests = {
    "test alignment command on selected image": function () {
      ...
      bot.setHtmlWithSelection(
        '<p>[<img src="http://tests/404" style="float:left;"/>]</p>'
      );

      // Check commands state, center/justify should be disabled at this point.
      return assertCommandState(1, 2, 0, 0, bot).then(function () {
        // Remove the existing image alignment.
        bot.execCommand("justifyleft");
        assert.areSame(
          '<p><img src="http://tests/404" /></p>',
          bot.getData(true)
        );
        // Align image right.
        bot.execCommand("justifyright");
        assert.areSame(
          '<p><img src="http://tests/404" style="float:right;" /></p>',
          bot.getData(true)
        );
    ...
    "test alignment command on selected image (align attribute)": function () {
      var bot = this.editorBot;
      bot.setHtmlWithSelection(
        '<p>[<img src="http://tests/404" align="left"/>]</p>'
      );
      // Check commands state, center/justify should be disabled at this point.
      return assertCommandState(1, 2, 0, 0, bot).then(function () {
        // Remove the existing image alignment.
    ...
    "test alignment command on paragraph": function () {
      this.editorBot.setHtmlWithSelection(
        '<p>[<img src="http://tests/404"/>bar]</p>'
      );

      // Check commands state, all commands should be enabled, left should be turned on.
      return assertCommandState(1, 2, 2, 2, this.editorBot).then(
        function (bot) {
          // Align paragraph right;
          bot.execCommand("justifyright");
          assert.areSame(
            '<p style="text-align:right;"><img src="http://tests/404" />bar</p>',
            bot.getData(true)
          );
          ...
          assert.areSame(
            '<p style="text-align:center;"><img src="http://tests/404" />bar</p>',
            bot.getData(true)
          );
          ...
          assert.areSame(
            '<p style="text-align:justify;"><img src="http://tests/404" />bar</p>',
            bot.getData(true)
          );
        }
      );
    ...
    "test alignment commands with justifyClasses": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          ...
          bot.execCommand("justifyright");
          assert.areSame(
            '<p class="alignright"><img src="http://tests/404" />bar</p>',
            bot.getData(true)
          );
        ...
        .then(function (bot) {
          ...
          bot.execCommand("justifycenter");
          assert.areSame(
            '<p class="aligncenter"><img src="http://tests/404" />bar</p>',
            bot.getData(true)
          );
        ...
        .then(function (bot) {
          ...
          bot.execCommand("justifyblock");
          assert.areSame(
            '<p class="alignjustify"><img src="http://tests/404" />bar</p>',
            bot.getData(true)
          );
    ...
    "test alignment commands with justifyClasses - one disallowed":
      function () {
        return bender.editorBot
          .createAsync({
    ...
    "test alignment commands in br mode": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          ...
          bot.execCommand("justifyright");
          assert.areSame(
            '<div style="text-align:right;">foobar</div>bom',
            bot.getData(true)
          );
          return assertCommandState(2, 1, 2, 2, bot);
        })
        ...
        .then(function (bot) {
          assert.isTrue(
            bot.editor.filter.check("div{text-align}"),
            "Check whether justify allows div in br mode"
          );
        });
    },
    ...
    "test alignment commands in div mode and with justifyClasses": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          ...
          bot.execCommand("justifyright");
          assert.areSame(
            '<div class="alignright">foobar</div>',
            bot.getData(true)
          );
          return assertCommandState(2, 1, 2, 2, bot);
        })
    ...
    "test justify on selection containing non-editable inline": function () {
      var bot = this.editorBot;
      bot.setHtmlWithSelection(
        '[<p>xxx<span contenteditable="false">foo</span>zzz</p>]'
      );
      bot.execCommand("justifyright");
      assert.areSame(
        '<p style="text-align:right;">xxx<span contenteditable="false">foo</span>zzz</p>',
        bot.getData(true)
      );

      bot.setHtmlWithSelection(
        '[<p><span contenteditable="false">foo</span></p>]'
      );
      bot.execCommand("justifyright");
      assert.areSame(
        '<p style="text-align:right;"><span contenteditable="false">foo</span></p>',
        bot.getData(true)
      );
    },
    ...
    "test alignment on disabled elements paragraph": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          bot.setHtmlWithSelection(
            "<p>Foo</p><ul><li>on^e</li><li>two</li><li>three</li></ul>"
          );
          return assertCommandState(0, 0, 0, 0, bot);
        })
        .then(function (bot) {
          bot.setHtmlWithSelection(
            "<p>Fo^o</p><ul><li>one</li><li>two</li><li>three</li></ul>"
          );
          return assertCommandState(1, 2, 2, 2, bot);
        });
    },
    ...
    "test alignment on disabled elements div mode": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          bot.setHtmlWithSelection(
            "<div>Foo</div><ul><li>on^e</li><li>two</li><li>three</li></ul>"
          );
          return assertCommandState(0, 0, 0, 0, bot);
        })
        .then(function (bot) {
          bot.setHtmlWithSelection(
            "<div>F^oo</div><ul><li>one</li><li>two</li><li>three</li></ul>"
          );
          return assertCommandState(1, 2, 2, 2, bot);
        });
    },
    ...
    "test alignment on disabled elements br mode": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          bot.setHtmlWithSelection(
            "foo<ul><li>on^e</li><li>two</li><li>three</li></ul>"
          );
          return assertCommandState(0, 0, 0, 0, bot);
        })
        .then(function (bot) {
          bot.setHtmlWithSelection(
            "f^oo<ul><li>one</li><li>two</li><li>three</li></ul>"
          );
          return assertCommandState(2, 2, 2, 2, bot);
        })
        .then(function (bot) {
        ...
        .then(function (bot) {
          assert.isInnerHtmlMatching(
            '<div style="text-align:justify;">foo</div><ul><li>one</li><li>two</li><li>three</li></ul>',
            bot.getData(true)
          );
        });
    },
    ...
    "test alignment on disabled elements br mode (class)": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          assert.isInnerHtmlMatching(
            '<div class="alignJustify">foo</div><ul><li>one</li><li>two</li><li>three</li></ul>',
            bot.getData()
          );
        });
    },
    ...
    "test alignment on multi-element non-collapsed selection": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          bot.setHtmlWithSelection(
            "<p>F[oo</p><ul><li>one</li><li>two</li><li>three</li></ul><p>B]ar</p>"
          );
          return assertCommandState(1, 2, 2, 2, bot);
        })
        .then(function (bot) {
        ...
        .then(function (bot) {
          assert.areSame(
            '<p style="text-align:center;">foo</p><ul><li>one</li><li>two</li><li>three</li></ul><p style="text-align:center;">bar</p>',
            bot.getData(true)
          );
        });
    },
    ...
    // (#455)
    "test alignment on multi-element non-collapsed selection (class)":
      function () {
        return bender.editorBot
          .createAsync({
          ...
          .then(function (bot) {
            assert.areSame(
              '<p class="alignCenter">foo</p><ul><li>one</li><li>two</li><li>three</li></ul><p class="alignCenter">bar</p>',
              bot.getData()
            );
          });
      },
    ...
    "test alignment on multi-element with disallowContent": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          bot.setHtmlWithSelection(
            "<p>fo[o</p><h1>bar</h1><p>foooos</p><h1>b]az</p>"
          );
          return assertCommandState(1, 2, 2, 2, bot);
        })
        .then(function (bot) {
        ...
        .then(function (bot) {
          assert.areSame(
            '<p style="text-align:right;">foo</p><h1>bar</h1><p style="text-align:right;">foooos</p><h1>baz</h1>',
            bot.getData(true)
          );
        });
    },
    ...
    // (#455)
    "test alignment on multi-element with disallowContent (class)":
      function () {
        return bender.editorBot
          .createAsync({
          ...
          .then(function (bot) {
            assert.areSame(
              '<p class="alignRight">foo</p><h1>bar</h1><p class="alignRight">foooos</p><h1>baz</h1>',
              bot.getData()
            );
          });
      },
    ...
    "test alignment div-type editor": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          assert.areSame(
            '<p style="text-align:center;">foo</p><h1>bar</h1><p style="text-align:center;">baz</p>',
            bot.getData(true)
          );
        });
    },
    ...
    "test alignment div-type editor (class)": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          assert.areSame(
            '<p class="alignCenter">foo</p><h1>bar</h1><p class="alignCenter">baz</p>',
            bot.getData()
          );
        });
    },
    ...
    // (#455)
    "test alignment on disabled elements block type under editable":
      function () {
        return bender.editorBot
          .createAsync({
            name: "editor_div_3",
          ...
          .then(function (bot) {
            ...
            bot.setHtmlWithSelection(
              "Foo<br /><ul><li>one</li><li>two</li><li>three</li></ul>"
            );
    ...
    "test alignment on styled elements in br mode": function () {
      return bender.editorBot
        ...
        .then(function (bot) {
          assert.isInnerHtmlMatching(
            '<div style="text-align:center"><span class="marker">Foo bar baz</span></div>',
            bot.getData(),
            { fixStyles: true }
          );
        });
