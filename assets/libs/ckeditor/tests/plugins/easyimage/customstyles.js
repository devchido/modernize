...
(function () {
  ...
  var tests = {
    ...
    "test style command state on widget with predefined style": function (
      editor,
      bot
    ) {
      bot.setData(
        CKEDITOR.document.getById("predefined").getHtml(),
        function () {
          var widget = editor.widgets.getByElement(
            editor.editable().findOne("figure")
          );
    ...
    "test style command does not change element tag name": function (
      editor,
      bot
    ) {
      bot.setData(CKEDITOR.document.getById("standard").getHtml(), function () {
    ...
    "test widget fallback to full style": function (editor, bot) {
      bot.setData(CKEDITOR.document.getById("standard").getHtml(), function () {
        ...
        assert.isTrue(
          widget.hasClass("easyimage-full"),
          "Widget has correct style"
        );
      });
    },

    "test custom style widget does not get the default style": function (
      editor,
      bot
    ) {
      bot.setData(
  ...
  tests["test override default styles"] = function () {
    return bender.editorBot.create(
      ...
      function (bot) {
        ...
        bot.setData(
          CKEDITOR.document.getById("standard").getHtml(),
          function () {
            var widget = editor.widgets.getByElement(
              editor.editable().findOne("figure")
            );
            ...
            editor.once("afterCommandExec", function () {
              resume(function () {
                ...
                assert.isTrue(
                  widget.hasClass("customClass"),
                  "Widget has appropriate class"
                );
              });
            });
