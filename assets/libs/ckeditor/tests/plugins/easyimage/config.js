...
(function () {
  ...
  function createCustomClassTest(command) {
    return function () {
      ...
      bot.setData(
        CKEDITOR.document.getById("changedClass").getHtml(),
        function () {
          var editor = bot.editor,
            widgetInstance = widgetTestsTools.getWidgetById(
              editor,
              "customSideId",
              true
            );
          ...
          if (CKEDITOR.env.ie && !CKEDITOR.env.edge) {
            ...
            editor.commands["easyimage" + command].refresh(
              editor,
              editor.elementPath()
            );
          }
          ...
          assert.beautified.html(
            CKEDITOR.document
              .getById("expectedCustom" + command + "Class")
              .getHtml(),
            editor.getData()
          );
        }
  ...
  function createMenuTest(editorName, expectedItems) {
    ...
    return function () {
      ...
      bot.setData(
        CKEDITOR.document.getById("standardWidget").getHtml(),
        function () {
          ...
          bot.contextmenu(function (menu) {
            arrayAssert.containsItems(
              expectedItems,
              getItemsNames(menu.items),
              "Context menu items types"
            );
  ...
  bender.test({
    ...
    "test easyimage_class - changed": function () {
      widgetTestsTools.assertWidget({
        ...
        assertNewData: function (widget) {
          assert.isTrue(
            widget.element.hasClass("easyimage-full"),
            "Widget has a proper class by default"
          );
        },
      });
    },
    ...
    "test easyimage_defaultStyle - changed": function () {
      widgetTestsTools.assertWidget({
        ...
        assertNewData: function (widget) {
          assert.isTrue(
            widget.element.hasClass("easyimage-side"),
            "Widget element has a proper class"
          );
        },
      });
    },
    ...
    "test easyimage_defaultStyle - unset": function () {
      widgetTestsTools.assertWidget({
        ...
        assertNewData: function (widget) {
          assert.areSame(
            "cke_widget_element",
            widget.element.getAttribute("class"),
    ...
    "test balloon toolbar buttons (default settings)": createToolbarTest(
      "standard",
      ["EasyImageFull", "EasyImageSide", "EasyImageAlt"]
    ),
    "test balloon toolbar buttons (string syntax)": createToolbarTest(
      "toolbarString",
      ["Bold", "Italic"]
    ),
    "test balloon toolbar buttons (array syntax)": createToolbarTest(
      "toolbarArray",
      ["Bold", "Italic"]
    ),
    ...
    "test context menu items (string syntax)": createMenuTest("toolbarString", [
      "bold",
      "italic",
    ]),
    "test context menu items (array syntax)": createMenuTest("toolbarArray", [
      "bold",
      "italic",
    ]),
  });
