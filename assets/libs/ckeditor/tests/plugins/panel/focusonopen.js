...
(function () {
  ...
  bender.test({
    "test mark first panel block item": function () {
      ...
      assert.isTrue(
        block.onMark.calledWith(block.element.getElementsByTag("a").getItem(0))
      );
    },
    ...
    "test mark first displayed panel block item": function () {
      ...
      assert.isTrue(
        block.onMark.calledWith(block.element.getElementsByTag("a").getItem(1))
      );
    },
    ...
    "test mark first displayed item with aria-selected": function () {
      ...
      assert.isTrue(
        block.onMark.calledWith(
          block.element.getElementsByTag("a").getItem(itemIndex)
        )
      );
    },
  });
