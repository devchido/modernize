...
bender.test({
  "test if embedding shows notifications during successful request":
    function () {
      var bot = this.editorBots.inline,
        editor = bot.editor,
        successCallbackSpy = sinon.spy(),
        errorCallbackSpy = sinon.spy(),
        createTaskSpy = sinon.spy(
  ...
  "test if embedding notifications can be blocked during successful request":
    function () {
      var bot = this.editorBots.inline,
        editor = bot.editor,
        successCallbackSpy = sinon.spy(),
  ...
  "test if embedding shows notifications during unsuccessful request":
    function () {
      var bot = this.editorBots.inline,
        editor = bot.editor,
        successCallbackSpy = sinon.spy(),
        ...
        jsonpCallback = function (
          ...
          resume(function () {
            ...
            assert.isTrue(
              ...
              "Notification should be showed."
            );
            assert.areEqual(
              showNotificationSpy.firstCall.returnValue.type,
              "warning",
              'Notification should have "warning" type.'
            );
          });
        };
  ...
  "test if embedding notifications can be blocked during unsuccessful request":
    function () {
      var bot = this.editorBots.inline,
        editor = bot.editor,
        successCallbackSpy = sinon.spy(),
      ...
      bot.setData(dataWithWidget, function () {
        ...
        jsonpCallback = function (
          ...
          resume(function () {
            ...
            assert.isFalse(createTaskSpy.called, "Task should not be created.");
            assert.isFalse(
              showNotificationSpy.called,
              "Notification should not be showed."
            );
          });
        };
