...
var dataWithWidget =
    '<p>x</p><div data-oembed-url="foo" id="w1">foo</div><p>x</p>',
  dataWith2Widgets =
    '<p>x</p><div data-oembed-url="foo" id="w1">foo</div><div data-oembed-url="foo" id="w2">foo</div><p>x</p>',
...
bender.test({
  ...
  "test def._cacheResponse, def._getCachedResponse": function () {
    ...
    assert.isUndefined(
      def2._getCachedResponse("a"),
      "def2.get a - cache per definition"
    );
  },
  ...
  "test def._createTask returned value": function () {
    ...
    this.spies.push(
      sinon.stub(
        CKEDITOR.plugins.notificationAggregator.prototype,
        "createTask",
        function () {
          return ret++;
        }
  ...
  "test def._createTask creates new aggregator once all tasks are finished":
    function () {
      var origCreateTask =
          CKEDITOR.plugins.notificationAggregator.prototype.createTask,
      ...
      assert.areNotSame(
        aggregators[0],
        aggregators[1],
        "two definitions use different aggregators"
      );
      ...
      assert.areNotSame(
        aggregators[0],
        aggregators[2],
        "after all tasks are finished, new notifagg is created"
      );
    },
  ...
  "test def._responseToHtml - rich, video": function () {
    ...
    assert.areSame(
      "a",
      def._responseToHtml("http://foo", { type: "rich", html: "a" }),
      "rich"
    );
    assert.areSame(
      "b",
      def._responseToHtml("http://foo", { type: "video", html: "b" }),
      "video"
    );
  },
  ...
  "test def._responseToHtml - photo": function () {
    ...
    assert.areSame(
      '<img src="a&quot;b" alt="" style="max-width:100%;height:auto" />',
      def._responseToHtml("http://foo", { type: "photo", url: 'a"b' }),
      "no title"
    );

    assert.areSame(
      '<img src="a" alt="x&quot;y" style="max-width:100%;height:auto" />',
      def._responseToHtml("http://foo", {
        type: "photo",
        url: "a",
        title: 'x"y',
      }),
  ...
  "test def._responseToHtml - unknown": function () {
    ...
    assert.isNull(
      def._responseToHtml("http://foo", { type: "foo", html: "ignore me" })
    );
  },
  ...
  "test getErrorMessage - with custom message": function () {
    ...
    assert.areSame(
      "foo //bar.bom",
      def.getErrorMessage("foo {url}", "//bar.bom")
    );
    assert.areSame(
      "foo //bar.bom2",
      def.getErrorMessage("foo {url}", "//bar.bom2", "ignore me")
    );
  ...
  "test getErrorMessage - with translation string": function () {
    ...
    assert.areSame(
      "bar //bar.bom",
      def.getErrorMessage("getErrorMessage2", "//bar.bom")
    );
    assert.areSame(
      "bom //bar.bom",
      def.getErrorMessage("getErrorMessage", "//bar.bom", "Suff")
    );
  },
  ...
  "test def.loadContent fires sendRequest event": function () {
    ...
    bot.setData(dataWithWidget, function () {
      ...
      assert.isFalse(
        spy.called,
        "canceling event stops executing _sendRequest"
      );
    });
  },
          ...
          widget2.loadContent("//caches/responses", {
            callback: function () {
              ...
              resume(function () {
                assert.isTrue(
                  wasAsync,
                  "loadContent using cache is still async"
                );
                assert.areSame(
                  '<p>x</p><div data-oembed-url="//caches/responses" id="w1"><p>url:%2F%2Fcaches%2Fresponses</p></div>' +
                    '<div data-oembed-url="foo" id="w2">foo</div><p>x</p>',
                  data1,
                  "data after first loadContent"
                );
                ...
                assert.areSame(
                  ...
                  editor.getData(),
                  "data after second loadContent"
                );

                assert.isFalse(
                  createTaskSpy.called,
                  "when cache is hit, task is not created"
                );
                assert.isTrue(
                  handleResponseSpy.calledOnce,
                  "handleResponse is fired"
                );
              });
            },
          });
        ...
        callback: function () {
          resume(function () {
            assert.areSame(
              1,
              eventsCount,
              "handleResponse event is fired once"
            );
            assert.isTrue(
              handleResponseSpy.calledOnce,
              "_handleResponse is called once"
            );
            assert.isTrue(
              handleResponseSpy.args[0][0].task.isDone(),
              "task was done"
            );
            assert.isTrue(
              handleResponseSpy.returnValues[0],
              "_handleResponse returned true"
            );
            assert.areSame(
              '<p>x</p><div data-oembed-url="//response/handling" id="w1">' +
                "<p>url:%2F%2Fresponse%2Fhandling</p><p>customized</p></div><p>x</p>",
              editor.getData(),
              "response was customized"
            );
            assert.areSame(
              "//response/handling",
              widget.data.url,
              "widget's url has been changed"
            );
          });
        },
      });
  ...
  "test def.loadContent request error handling": function () {
    ...
    bot.setData(dataWithWidget, function () {
      ...
      that.spies.push(
        sinon.stub(widget, "_sendRequest", function (request) {
        ...
        errorCallback: function () {
          resume(function () {
            ...
            assert.areSame(
              "foo",
              widget.data.url,
              "widget's url has not been changed"
            );

            assert.isTrue(
              showNotificationSpy.calledOnce,
              "showNotification was called"
            );
            assert.areSame(
              editor.lang.embedbase.fetchingFailed.replace(
                /{url}/,
                "//error/handling"
              ),
              showNotificationSpy.args[0][0],
              "notification message"
            );

            assert.isUndefined(
              widget._getCachedResponse("//error/handling"),
      ...
      widget.loadContent(url, {
        ...
        errorCallback: function () {
          resume(function () {
            ...
            assert.isTrue(
              handleResponseSpy.calledOnce,
              "_handleResponse was called once"
            );
            assert.isFalse(
              handleResponseSpy.returnValues[0],
              "_handleResponse returned false"
            );
            ...
            assert.areSame(
              editor.lang.embedbase.unsupportedUrl.replace(/{url}/, url),
              showNotificationSpy.args[0][0],
              "notification message"
            );

            assert.isUndefined(
              widget._getCachedResponse(url),
              "response was not cached"
            );
          });
        },
      ...
      widget.loadContent("//canceling/handleresponse", {
        ...
        errorCallback: function () {
          resume(function () {
            ...
            assert.isTrue(
              showNotificationSpy.calledOnce,
              "showNotification was called once"
            );
            assert.areSame(
              "foo error //canceling/handleresponse",
              showNotificationSpy.args[0][0],
              "notification message"
            );

            assert.isUndefined(
              widget._getCachedResponse("//canceling/handleresponse"),
              "response was not cached"
            );
          });
        },
    ...
    bot.setData(dataWithWidget, function () {
      ...
      jsonpCallback = function (urlTemplate, urlParams, callback) {
        resume(function () {
          ...
          assert.isFalse(
            successCallbackSpy.called,
            "Success callback was not called."
          );
          assert.isFalse(
            errorCallbackSpy.called,
