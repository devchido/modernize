...
bender.test({
  ...
  assertCleanup: function (url) {
    ...
    assert.areSame(
      1,
      CKEDITOR.tools.object.keys(CKEDITOR._.jsonpCallbacks).length,
  ...
  "test sendRequest": function () {
    ...
    Jsonp.sendRequest(
      new CKEDITOR.template("%TEST_DIR%_assets/{foo}1.js"),
      { foo: "twitter" },
      function (response) {
        resume(function () {
  ...
  "test sendRequest and cancel": function () {
    ...
    var jsonpCallbacksNumberPre = CKEDITOR.tools.object.keys(
      CKEDITOR._.jsonpCallbacks
    ).length;

    jsonpRequest.cancel();

    var jsonpCallbacksNumberPost = CKEDITOR.tools.object.keys(
      CKEDITOR._.jsonpCallbacks
    ).length;

    assert.isFalse(successCallback.called, "success callback was not executed");
    assert.areSame(
      jsonpCallbacksNumberPre - 1,
      jsonpCallbacksNumberPost,
      "callback has been removed on cancel()"
    );
  },
});
