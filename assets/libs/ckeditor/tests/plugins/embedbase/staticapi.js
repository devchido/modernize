...
bender.test({
  ...
  "test if dialog is explicitly required": function () {
    assert.areSame(
      0,
      CKEDITOR.plugins.registered.embedbase.requires.indexOf("dialog,")
    );
  },

  "test embedBase.createWidgetBaseDefinition": function () {
    var def1 = CKEDITOR.plugins.embedBase.createWidgetBaseDefinition(
        this.editorMock
      ),
      def2 = CKEDITOR.plugins.embedBase.createWidgetBaseDefinition(
        this.editorMock
      );
