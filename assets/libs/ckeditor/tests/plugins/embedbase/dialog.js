      ...
      widget = {
        ...
        loadContent: function (url, opts) {
          ...
          wait(function () {
            ...
            if (isCreation) {
              assert.isTrue(
                finalizeSpy.calledOnce,
                "widgetsRepo.finalizeCreation was called once"
              );
              assert.areSame(1, finalizeSpy.args[0][0]);
            } else {
              assert.isFalse(
                finalizeSpy.called,
                "widgetsRepo.finalizeCreation was not called"
              );
            }
          }, 50);
        },
...
bender.test({
  ...
  "test successful committing (new widget)":
    createSuccessfulCommittingTest(true),

  "test successful committing (editing widget)":
    createSuccessfulCommittingTest(false),
