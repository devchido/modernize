...
function drop(editor, evt, config, onDrop, onFinish) {
  var isCustomDataTypesSupported =
      CKEDITOR.plugins.clipboard.isCustomDataTypesSupported,
    dropTarget = CKEDITOR.plugins.clipboard.getDropTarget(editor),
    range = new CKEDITOR.dom.range(editor.document),
    values = {
      beforePasteEventCounter: 0,
      pasteEventCounter: 0,
      dropEventCounter: 0,
    },
    expectedPasteEventCount =
      typeof config.expectedPasteEventCount !== "undefined"
        ? config.expectedPasteEventCount
        : 1,
    expectedBeforePasteEventCount =
      typeof config.expectedBeforePasteEventCount !== "undefined"
        ? config.expectedBeforePasteEventCount
        : expectedPasteEventCount;
  ...
  dropListener = function (dropEvt) {
    ...
    values.dropInstanceOfDataTransfer =
      dropEvt.data.dataTransfer instanceof
      CKEDITOR.plugins.clipboard.dataTransfer;
    ...
    if (CKEDITOR.env.ie && CKEDITOR.env.version == 8) {
      ...
      values.dropRangeStartContainerMatch =
        !!dropEvt.data.dropRange.startContainer;
      values.dropRangeStartOffsetMatch = !!dropEvt.data.dropRange.startOffset;
    } else {
      values.dropRangeStartContainerMatch =
        config.dropContainer == dropEvt.data.dropRange.startContainer;
      values.dropRangeStartOffsetMatch =
        config.dropOffset == dropEvt.data.dropRange.startOffset;
    }
    ...
    resume(function () {
      if (!config.expectedDropPrevented) {
        ...
        assert.areSame(
          1,
          values.dropEventCounter,
          "There should be always one drop."
        );

        assert.isTrue(
          values.dropInstanceOfDataTransfer,
          "On drop: dropEvt.data.dataTransfer should be instance of dataTransfer."
        );
        if (config.expectedText && isCustomDataTypesSupported) {
          assert.areSame(
            config.expectedText,
            values.dropDataText,
            "On drop: text data should match."
          );
        }
        if (config.expectedHtml) {
          // isInnerHtmlMatching remove space from the end of strings we compare, adding 'x' fix this problem.
          assert.isInnerHtmlMatching(
            "x" + config.expectedHtml + "x",
            "x" + values.dropDataHtml + "x",
            "On drop: HTML data should match."
          );
        }
        assert.isTrue(
          values.dropRangeStartContainerMatch,
          "On drop: drop range start container should match."
        );
        assert.isTrue(
          values.dropRangeStartContainerMatch,
          "On drop: drop range start offset should match."
        );

        assert.isTrue(
          values.dropNativeEventMatch,
          "On drop: native event should match."
        );
        // Check that it's the mocked drop target created by the mockDropEvent().
        assert.areSame(
          CKEDITOR.NODE_TEXT,
          values.dropTarget.type,
          "On drop: drop target node type should match."
        );
        assert.areSame(
          "targetMock",
          values.dropTarget.getText(),
          "On drop: drop target should match."
        );
      }
      ...
      assert.areSame(
        expectedBeforePasteEventCount,
        values.beforePasteEventCounter,
        "Before paste event should be called " +
          expectedBeforePasteEventCount +
          " time(s)."
      );
      assert.areSame(
        expectedPasteEventCount,
        values.pasteEventCounter,
        "Paste event should be called " + expectedPasteEventCount + " time(s)"
      );

      if (expectedPasteEventCount > 0) {
        assert.areSame(
          config.expectedTransferType,
          values.pasteTransferType,
          "On paste: transferType should match."
        );
        // Do not check Text data on IE.
        if (isCustomDataTypesSupported) {
          assert.areSame(
            config.expectedText,
            values.pasteDataText,
            "On paste: text data should match."
          );
        }
        // isInnerHtmlMatching remove space from the end of strings we compare, adding 'x' fix this problem.
        assert.isInnerHtmlMatching(
          "x" + config.expectedHtml + "x",
          "x" + values.pasteDataHtml + "x",
          "On paste: HTML data should match."
        );
        assert.areSame(
          "drop",
          values.pasteMethod,
          "On paste: method should be drop."
        );
        config.expectedDataType &&
          assert.areSame(
            config.expectedDataType,
            values.pasteDataType,
            "On paste: data type should match."
          );
        assert.isInnerHtmlMatching(
          "x" + config.expectedDataValue + "x",
          "x" + values.pasteDataValue + "x",
          "On paste: data value should match."
        );
      }
...
var testsForMultipleEditor = {
    ...
    "test drop to header": function (editor) {
      ...
      bot.setHtmlWithSelection(
        '<h1 class="h1">Header1</h1>' + "<p>Lorem ipsum [dolor] sit amet.</p>"
      );
      editor.resetUndo();
      ...
      drop(
        ...
        function () {
          assert.areSame(
            '<h1 class="h1">Header1dolor^</h1><p>Lorem ipsum sit amet.</p>',
    ...
    "test drop the same line, before": function (editor) {
      ...
      drop(
        ...
        function () {
          assert.areSame(
            '<p class="p">Lorem dolor^ipsum sit amet.</p>',
            bender.tools.getHtmlWithSelection(editor),
            "after drop"
          );
    ...
    "test drop the same line, after": function (editor) {
      ...
      drop(
        ...
        function () {
          assert.areSame(
            '<p class="p">Lorem dolor sit ipsum^amet.</p>',
            bender.tools.getHtmlWithSelection(editor),
            "after drop"
          );
    ...
    "test drop after range end": function (editor) {
      ...
      setWithHtml(
        editor,
        '<p class="p"><b>lor{em</b> ipsum} dolor sit amet.</p>'
      );
      editor.resetUndo();
      ...
      drop(
        ...
        evt,
        {
          ...
          dropOffset: CKEDITOR.env.ie && CKEDITOR.env.version == 8 ? 11 : 17,
          expectedTransferType: CKEDITOR.DATA_TRANSFER_INTERNAL,
        ...
        function () {
          assert.isInnerHtmlMatching(
            '<p class="p"><b>lor</b> dolor sit <b>em</b> ipsum^amet.@</p>',
            getWithHtml(editor),
            htmlMatchOpts,
            "after drop"
          );
          ...
          assert.isInnerHtmlMatching(
            '<p class="p"><b>lorem</b> ipsum dolor sit ^amet.@</p>',
            getWithHtml(editor),
            htmlMatchOpts,
            "after undo"
          );
    ...
    "test drop after paragraph": function (editor) {
      ...
      drop(
        ...
        function () {
          assert.areSame(
            '<p class="p">Lorem dolor sit amet.ipsum^</p>',
            bender.tools.getHtmlWithSelection(editor),
            "after drop"
          );
    ...
    "test drop on the left from paragraph": function (editor) {
      ...
      bot.setHtmlWithSelection(
        '<p class="p" style="margin-left: 20px">Lorem [ipsum] dolor sit amet.</p>'
      );
      editor.resetUndo();
      ...
      drop(
        ...
        function () {
          assert.isInnerHtmlMatching(
            '<p class="p" style="margin-left:20px">ipsum^Lorem dolor sit amet.@</p>',
            getWithHtml(editor),
            htmlMatchOpts,
            "after drop"
          );
    ...
    "test drop part of the link": function (editor) {
      ...
      bot.setHtmlWithSelection(
        '<p class="p" style="margin-left: 20px"><a href="foo">Lorem [ipsum] dolor</a> sit amet.</p>'
      );
      editor.resetUndo();
    ...
    "test drop text from external source": function (editor) {
      ...
      drop(
        ...
        function () {
          ...
          assert.areSame(
            '<p class="p">Lorem ipsum sit amet.</p>',
            editor.getData(),
            "after undo"
          );

          assert.isNull(
            CKEDITOR.plugins.clipboard.dragData,
            "dragData should be reset"
          );
        }
      );
    },
    ...
    "test drop html from external source": function (editor) {
      ...
      drop(
        ...
        evt,
        {
          ...
          expectedDataValue: !isCustomDataTypesSupported
            ? "&lt;b&gt;dolor&lt;/b&gt;"
            : "<b>dolor</b>",
        },
        null,
        function () {
          if (isCustomDataTypesSupported) {
            assert.areSame(
              '<p class="p">Lorem <b>dolor^</b>ipsum sit amet.</p>',
              bender.tools.getHtmlWithSelection(editor),
              "after drop"
            );
          } else {
    ...
    "test cross editor drop": function (editor) {
      var bot = bender.editorBots[editor.name],
        ...
        extractRangeSpy = sinon.spy(
          editorCross.editable(),
      ...
      setWithHtml(
        botCross.editor,
        '<p class="p">Lorem {ipsum <b>dolor</b> }sit amet.</p>'
      );
      ...
      drop(
        ...
        function () {
          ...
          assert.areSame(
            '<p class="p">Lorem ipsum <b>dolor</b> ^ipsum sit amet.</p>',
            bender.tools.getHtmlWithSelection(editor),
            "after drop"
          );
          assert.areSame(
            ...
            editorCross.getData(),
            "after drop - editor cross"
          );

          assert.isTrue(
            extractRangeSpy.called,
            "extractHtmlFromRange was called on the source editor's editable"
          );
      ...
      drop(
        ...
        function (evt) {
          if (
            ...
            assert.areSame(
              editor.editable().findOne(".drag1"),
              evt.data.dragRange.startContainer,
              "dropRange.startContainer"
            );
            assert.areSame(
              1,
              evt.data.dragRange.startOffset,
              "dropRange.startOffset"
            );
          }

          evt.data.dragRange.selectNodeContents(
            editor.editable().findOne(".drag2")
          );
        ...
        function () {
          assert.areSame(
            '<p>x<b class="drag1">xdrag1x</b>xx<b class="drop1">drop1</b>x<b class="drop2">drop2</b><b class="drag1">drag1^</b>x</p>',
            bender.tools.getHtmlWithSelection(editor),
            "after drop"
          );
          ...
          assert.areSame(
            '<p>x<b class="drag1">xdrag1x</b>x<b class="drag2">drag2</b>x<b class="drop1">drop1</b>x<b class="drop2">drop2</b>x</p>',
            editor.getData(),
            "after undo"
          );
        }
    ...
    "test internal drag and drop on editors margin": function (editor) {
      ...
      drop(editor, evt, {
        ...
        expectedTransferType: CKEDITOR.DATA_TRANSFER_INTERNAL,
        expectedText: CKEDITOR.env.edge
          ? "onetwothreefour"
          : "one\ntwo\nthree\nfour",
        expectedHtml:
          '<ol><li><a href="http://test.com">one</a>@</li><li>two</li><li>three</li><li>four</li></ol>',
        expectedDataType: "html",
        expectedDataValue:
          '<ol><li><a href="http://test.com">one</a>@</li><li>two</li><li>three</li><li>four</li></ol>',
      });
  ...
  testsForOneEditor = {
    ...
    "test fix split nodes": function () {
      ...
      // Fix nodes.
      CKEDITOR.plugins.clipboard.fixSplitNodesAfterDrop(
        dragRange,
        dropRange,
        1,
        1
    ...
    "test fix split nodes 2 (https://dev.ckeditor.com/ticket/13011)":
      function () {
    ...
    "test isDropRangeAffectedByDragRange 1": function () {
      ...
      assert.isTrue(
        CKEDITOR.plugins.clipboard.isDropRangeAffectedByDragRange(
          dragRange,
          dropRange
        )
      );
    ...
    "test isDropRangeAffectedByDragRange adjacent positions (https://dev.ckeditor.com/ticket/13140)":
      function () {
        var editor = this.editors.framed,
          bot = this.editorBots[editor.name],
          dragRange = editor.createRange(),
        ...
        bot.setHtmlWithSelection(
          '<div><p id="foo">foo</p><p id="bar">bar</p></div>'
        );
    ...
    "test dragEnd event": function () {
      ...
      editor.once("dragend", function (dragendEvt) {
        ...
        assert.areSame(
          "foo",
          dragendEvt.data.dataTransfer.getData("Text"),
          "cke/custom"
        );
        assert.areSame(evt.data.$, dragendEvt.data.$, "nativeEvent");
        assert.areSame(
          CKEDITOR.NODE_TEXT,
          dragendEvt.data.target.type,
          "drag target node type"
        );
        assert.areSame(
          "targetMock",
          dragendEvt.data.target.getText(),
          "drag target node"
        );
      });
    ...
    "test drop on non editable (https://dev.ckeditor.com/ticket/13015)":
      function () {
        ...
        bot.setHtmlWithSelection(
          '<div><p contenteditable="false">foo</p></div>'
        );
        editor.resetUndo();
    ...
    "test preventDefaultDropOnElement": function () {
      ...
      assert.areEqual(
        "none",
        data.$.dataTransfer.dropEffect,
        "dropEffect reset to 'none'"
      );
    },
