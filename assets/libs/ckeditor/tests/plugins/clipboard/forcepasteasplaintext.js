...
(function () {
  ...
  var fixtures = createFixtures({
    ...
    tests = {
      ...
      "test paste with plain text after pastetext click": function (
        editor,
        bot
      ) {
        var tc = bot.testCase;
        ...
        assertAfterPasteContent(
          tc,
          getExpected(editor.name, "pasteText", true)
        );
        tc.wait();
      },

      "test paste with HTML content after pastetext click": function (
        editor,
        bot
      ) {
        var tc = bot.testCase;
        ...
        assertAfterPasteContent(
          tc,
          getExpected(editor.name, "pasteHTML", true)
        );
        tc.wait();
      },

      "test paste with Word content after pastetext click": function (
        editor,
        bot
      ) {
        var tc = bot.testCase;
        ...
        assertAfterPasteContent(
          tc,
          getExpected(editor.name, "pasteWord", forcePlain)
        );
        tc.wait();
      },

      "test paste with plain text after pastefromword click": function (
        editor,
        bot
      ) {
        var tc = bot.testCase;
      ...
      "test paste with HTML content after pastefromword click": function (
        editor,
        bot
      ) {
        var tc = bot.testCase;
      ...
      "test paste with Word content after pastefromword click": function (
        editor,
        bot
      ) {
        var tc = bot.testCase;
  ...
  function getExpected(editorName, fixtureName, forcePlain) {
    ...
    if (
      !forcePlain &&
      (editorName === "force_default" ||
        (editorName === "force_ignoreword" && fixtureName === "pasteWord"))
    ) {
      expected = "data";
    }
    ...
    return (
      "<p>" +
      fixtures.get(fixtureName)[expected].replace(/\s+class="MsoNormal"/g, "") +
      "</p>"
    );
  }
})();
