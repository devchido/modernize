...
bender.test({
  ...
  assertDataTransfer: function (expected, dataTransfer) {
    assert.areSame(
      ...
      dataTransfer.getTransferType(expected.targetEditor),
      "transferType"
    );
    assert.areSame(
      expected.sourceEditor,
      dataTransfer.sourceEditor,
      "sourceEditor"
    );
    assert.areSame(
      expected.text,
  ...
  "test id": function () {
    ...
    assert.areSame(
      dataTransfer1a.id,
      dataTransfer1b.id,
      "Ids for object based on the same event should be the same."
    );
    ...
    if (CKEDITOR.plugins.clipboard.isCustomDataTypesSupported) {
      assert.areNotSame(
        dataTransfer1a.id,
        dataTransfer2.id,
        "Ids for object based on different events should be different."
      );
    }
  },
  ...
  "test set-get data, data type: text/plain, dataTransfer with event":
    function () {
      var nativeData = bender.tools.mockNativeDataTransfer(),
        dataTransfer = new CKEDITOR.plugins.clipboard.dataTransfer(nativeData);
  ...
  "test set-get data, data type: CKE/custom, dataTransfer with event":
    function () {
      var nativeData = bender.tools.mockNativeDataTransfer(),
        dataTransfer = new CKEDITOR.plugins.clipboard.dataTransfer(nativeData);
      ...
      assert.areSame(
        "bar",
        dataTransfer.getData("cke/custom"),
  ...
  "test set-get data, data type: text/html, dataTransfer with event":
    function () {
      var nativeData = bender.tools.mockNativeDataTransfer(),
        dataTransfer = new CKEDITOR.plugins.clipboard.dataTransfer(nativeData);
  ...
  "test set-get data, data type: Text, dataTransfer without event":
    function () {
  ...
  "test set-get data, data type: text/plain, dataTransfer without event":
    function () {
      var dataTransfer = new CKEDITOR.plugins.clipboard.dataTransfer();
  ...
  "test set-get data, data type: CKE/custom, dataTransfer without event":
    function () {
      var dataTransfer = new CKEDITOR.plugins.clipboard.dataTransfer();
  ...
  "test set-get data, data type: text/html, dataTransfer without event":
    function () {
      var dataTransfer = new CKEDITOR.plugins.clipboard.dataTransfer();
  ...
  "test set-get data, data type: undefined data, dataTransfer without event":
    function () {
      var dataTransfer = new CKEDITOR.plugins.clipboard.dataTransfer();
  ...
  "test getData meta filter": function () {
    ...
    nativeData.setData(
      "text/html",
      '<meta http-equiv="content-type" content="text/html; charset=utf-8">foo<b>bom</b>x\nbar'
    );
  ...
  "test initDragDataTransfer binding": function () {
    ...
    assert.areSame(
      evt1a.data.dataTransfer,
      evt1b.data.dataTransfer,
      "If we init dataTransfer object twice on the same event this should be the same object."
    );
    ...
    assert.areNotSame(
      evt1a.data.dataTransfer,
      evt2.data.dataTransfer,
      "If we init dataTransfer object twice on different events these should be different objects."
    );
  },
  ...
  "test initDragDataTransfer binding no native event": function () {
    ...
    assert.areSame(
      evt1a.data.dataTransfer,
      evt1b.data.dataTransfer,
      "Without native event events should be the same."
    );

    assert.areNotSame(
      evt1a.data.dataTransfer,
      evt2.data.dataTransfer,
      "After reset events should be different."
    );
  },
  ...
  "test initPasteDataTransfer constructor": function () {
    ...
    var dataTransfer = CKEDITOR.plugins.clipboard.initPasteDataTransfer(
      evt,
      editor
    );
  ...
  // (#962)
  "test new dataTransfer id is created for copy/cut/dragstart events":
    function () {
      if (!CKEDITOR.plugins.clipboard.isCustomCopyCutSupported) {
        assert.ignore();
      }
      ...
      assert.isTrue(
        dataTransfer1.id.length > 0,
        "dataTransfer1 id is not empty"
      );
      assert.isTrue(
        dataTransfer2.id.length > 0,
        "dataTransfer2 id is not empty"
      );
      assert.isTrue(
        dataTransfer3.id.length > 0,
      ...
      if (
        ...
        assert.areSame(
          dataTransfer1.id,
          nativeData1.getData("cke/id"),
          "cke/id type holds dataTransfer1 id"
        );
        assert.areSame(
          dataTransfer2.id,
          nativeData2.getData("cke/id"),
          "cke/id type holds dataTransfer2 id"
        );
        assert.areSame(
          dataTransfer3.id,
          nativeData3.getData("cke/id"),
          "cke/id type holds dataTransfer3 id"
        );
      } else {
        assert.areSame(
          dataTransfer1.id,
          dtFallback1._extractDataComment(
            nativeData1.getData(dtFallback1._customDataFallbackType)
          ).data["cke/id"],
        ...
        assert.areSame(
          dataTransfer2.id,
          dtFallback2._extractDataComment(
            nativeData2.getData(dtFallback2._customDataFallbackType)
          ).data["cke/id"],
        ...
        assert.areSame(
          dataTransfer3.id,
          dtFallback3._extractDataComment(
            nativeData3.getData(dtFallback3._customDataFallbackType)
          ).data["cke/id"],
  ...
  // (#962)
  "test no new dataTransfer id is created for paste/drop/dragend events":
    function () {
      if (!CKEDITOR.plugins.clipboard.isCustomCopyCutSupported) {
        assert.ignore();
      }
      ...
      if (
        ...
        assert.areSame(
          "",
          nativeData1.getData("cke/id"),
          "dataTransfer1 id is empty"
        );
        assert.areSame(
          "",
          nativeData2.getData("cke/id"),
          "dataTransfer2 id is empty"
        );
        assert.areSame(
          "",
          nativeData3.getData("cke/id"),
        ...
        assert.areSame(
          "",
          nativeData1.getData(
            dataTransfer1._.fallbackDataTransfer._customDataFallbackType
          ),
          "dataTransfer1 id is empty"
        );
        assert.areSame(
          "",
          nativeData2.getData(
            dataTransfer2._.fallbackDataTransfer._customDataFallbackType
          ),
          "dataTransfer2 id is empty"
        );
        assert.areSame(
          "",
          nativeData2.getData(
            dataTransfer3._.fallbackDataTransfer._customDataFallbackType
          ),
          "dataTransfer2 id is empty"
        );
      }
