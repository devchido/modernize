...
function assertPasteEvent(
  ...
  if (!eventData.type) eventData.type = "auto";
  ...
  if (async) wait();
  else assert.isTrue(executed, message + "paste listener was executed");

  function assertPaste(data) {
    if (typeof expected == "function") expected(data, message);
    else {
...
function assertPasteCommand(editor, expected, cmdData, pasteData) {
  simulatePasteCommand(editor, cmdData, pasteData, function (evt) {
    assert.areSame(
      expected.method || "paste",
      evt.data.method,
      "Paste event has correct method."
    );
    assert.areSame(
      expected.type,
      evt.data.type,
      "Paste event has correct data type."
    );
    assert.areSame(
      expected.content,
      editor.getData(),
      "Editor contains correct content."
    );
  });
}
...
function assertPasteNotification(editor, expected, cmdData, pasteData) {
  ...
  simulatePasteCommand(editor, cmdData, pasteData, function () {
    ...
    assert.areSame(
      expected.count,
      spy.callCount,
      "showNotification was called correct number of times."
    );
    assert.areSame(
      expected.content,
      editor.getData(),
...
function testResetScenario(editor, queue) {
  ...
  function assertPaste(evt, expected, index) {
    assert.areSame(
      evt.dataValue,
      expected.dataValue,
      "Paste #" + index + " should have correct value."
    );
    assert.areSame(
      evt.type,
      expected.type,
      "Paste #" + index + " should have correct type."
    );
    assert.isUndefined(
      editor._.nextPasteType,
      "Forced type after paste #" + index + " should be deleted."
    );
  }
...
function getDefaultNotification(editor, command, keyInfo) {
  var keystroke =
      keyInfo ||
      CKEDITOR.tools.keystrokeToString(
        editor.lang.common.keyboard,
        editor.getCommandKeystroke(editor.commands[command])
      ),
    msg = editor.lang[
      command === "paste" ? "clipboard" : command
    ].pasteNotification.replace(
      /%1/,
      '<kbd aria-label="' + keystroke.aria + '">' + keystroke.display + "</kbd>"
    );
