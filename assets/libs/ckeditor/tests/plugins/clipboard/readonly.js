...
var tests = {
  ...
  "test if cut is prevented depending on read-only mode": function (
    editor,
    bot
  ) {
    ...
    assert.areSame(
      !editor.readOnly,
      CKEDITOR.plugins.clipboard.initPasteDataTransfer.called,
      "initPasteDataTransfer call"
    );
  },
  ...
  bender.editorBot.create(
    ...
    function (bot) {
      ...
      function assertCommands(expected, msg) {
        assert.areSame(
          expected,
          editor.getCommand("paste").state,
          "paste " + msg
        );
        assert.areSame(
          expected,
          editor.getCommand("pastetext").state,
          "pastetext " + msg
        );
      }
    }
