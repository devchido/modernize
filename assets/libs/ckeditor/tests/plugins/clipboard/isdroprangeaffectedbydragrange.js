...
(function () {
  ...
  function createSnippetOne(dropRangeDesc, dragRangeDesc) {
    ...
    dragRange.setStart(
      typeof dragRangeDesc.sc === "number"
        ? element.getChild(dragRangeDesc.sc)
        : element,
      dragRangeDesc.so
    );
    dragRange.setEnd(
      typeof dragRangeDesc.ec === "number"
        ? element.getChild(dragRangeDesc.ec)
        : element,
      dragRangeDesc.eo
    );
  ...
  function setupRanges(
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (typeof dropRangeValue === "string" || dropRangeValue === null) {
        ...
        var snippet = snippetFactory(
          { sca: dropContainerAddress, so: dropContainerIndex },
          snippetArgs
        );
  ...
  function prepareTestCaseName(suiteName, snippet) {
    ...
    var testName = element.getOuterHtml
      ? element.getOuterHtml()
      : element.getText() + ": ";

    var dragStart = splice(
      dragRange.startContainer.getText(),
      dragRange.startOffset,
      "["
    );
    var dragEnd = splice(
      dragRange.endContainer.getText(),
      dragRange.endOffset,
      "]"
    );
    ...
    if (dropRange.startContainer.type === CKEDITOR.NODE_ELEMENT) {
      ...
      drop =
        "text" +
        splice(dropRange.startContainer.getText(), dropRange.startOffset, "^");
    }
  ...
  function createTestCase(suiteName, dropRangeValue, snippet) {
    ...
    testCases[testName] = function () {
      ...
      var result = clipboard.isDropRangeAffectedByDragRange(
        dragRange,
        dropRange
      );
