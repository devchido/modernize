...
(function () {
  ...
  var contents = {
    ...
    various:
      "<div><h1>Header 1</h1><h3>Header <span>3</span></h3><p>Heeey</p></div>",
    classyAndStylish:
      '<h1 id="foo" class="ugly" style="background-color: red;">I am so classy and stylish :)</h1>',
  };
  ...
  tests["editor no configuration"] = function () {
    ...
    if (CKEDITOR.env.webkit) {
      ...
      assert.areNotSame(
        this.editors.editorSemantic.pasteFilter,
        editor.pasteFilter
      );
    } else {
  ...
  tests[
    "editor has pasteFilter defined if forcePasteAsPlainText is set to true"
  ] = function () {
  ...
  tests["editor has pasteFilter defined if pasteFilter is set to plain-text"] =
    function () {
  ...
  tests["unique content filter is created for each editor instance"] =
    function () {
  ...
  tests["allow to remove and create editor.pasteFilter on the fly"] =
    function () {
      var editor = this.editors.editorModifyInstance;
      ...
      assertPasteEvent(
        editor,
        { dataValue: "<h2>Foo <strong>bar</strong></h2>" },
        { dataValue: "<p>Foo <strong>bar</strong></p>" },
        "new filter"
      );
    };

  tests["test content is filtered even if config.pasteFilter is undefined"] =
    function () {
      var editor = this.editors.editorNoConfiguration;
  ...
  tests[
    "evt.data.dontFilter does not disable the paste filter when forcePAPT is true"
  ] = function () {
    var editor = this.editors.editorForcePAPT;
  ...
  createTest(
    "test plain",
    "editorPlain",
    contents.listWithSpan,
    "<p>hefkdjfkdjllo</p><p>moto</p>"
  );

  createTest(
    "test forcePasteAsPlainText",
    "editorForcePAPT",
    contents.listWithSpan,
    "<p>hefkdjfkdjllo</p><p>moto</p>"
  );
  ...
  createTest(
    "test semantic with forcePasteAsPlainText (forcePAPT has precedence)",
    "editorSemanticPAPT",
    contents.various,
  ...
  createTest(
    "test semantic removes styles and classes but keeps attrs",
    "editorSemantic",
    contents.classyAndStylish,
    '<h1 id="foo">I am so classy and stylish :)</h1>'
  );

  createTest(
    "test semantic removes divs, spans",
    "editorSemantic",
