...
(function () {
  ...
  bender.test({
    ...
    "test plugin exposes loader": function () {
      assert.isInstanceOf(
        Function,
        this.cloudservices.cloudServicesLoader,
        "cloudServicesLoader property type"
      );
    },

    "test loader uses config url/token": function () {
      var instance = new this.cloudservices.cloudServicesLoader(
          this.editor,
          bender.tools.pngBase64
        ),
        // Stub loader.xhr methods before it's actually called.
        listener = this.editor.once(
          "fileUploadRequest",
          this.commonRequestListener,
          null,
          null,
          0
        );
          ...
          resume(function () {
            ...
            for (var i = xhrServer.requests.length - 1; i >= 0; i--) {
              ...
              if (curReq.method === "POST" && curReq.url === UPLOAD_URL) {
                objectAssert.hasKey(
                  "Authorization",
                  curReq.requestHeaders,
                  "Token is included as a header"
                );
                assert.isMatching(
                  /sample\-token\-value\d+/,
                  curReq.requestHeaders.Authorization,
      ...
      bender.editorBot.create(botDefinition, function (bot) {
        ...
        setTimeout(function () {
          resume(function () {
            ...
            assert.areSame(
              initialCount,
              getActiveTokenUrlCount(),
              "Token requests count does not increase"
            );
          });
        }, 250);
    ...
    "test no token error": function () {
      ...
      bender.editorBot.create(botDefinition, function (bot) {
        ...
        var instance = new plugin.cloudServicesLoader(
            bot.editor,
            bender.tools.pngBase64
          ),
          listener = CKEDITOR.once("log", function (evt) {
            ...
            assert.areEqual(
              ...
              evt.data.errorCode,
              "There should be TOKEN error log."
            );
          });
