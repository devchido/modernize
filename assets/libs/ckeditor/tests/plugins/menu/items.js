...
bender.test({
  ...
  _testMenuItemRender: function (expectedRole, definition, callback) {
    ...
    assert.areNotEqual(
      -1,
      outputHtml.search('role="' + expectedRole + '"'),
      '[role="' +
        expectedRole +
        '"] attribute not found in html generated by CKEDITOR.menuItem.proto.render()'
    );
  ...
  _assertAriaChecked: function (expectedValue, html) {
    var expectedSubstring =
        'aria-checked="' + (expectedValue ? "true" : "false") + '"',
      assertMessage =
        '"' + expectedSubstring + '" substring has not been found';

    assert.areNotEqual(
      -1,
      String(html).search(expectedSubstring),
      assertMessage
    );
  },
  ...
  "test default menuitem state ON": function () {
    ...
    this._testMenuItemRender("menuitem", {}, function (html) {
      assert.areEqual(
        -1,
        html.search("aria-checked"),
        "aria-checked attribute should not be present in output html"
      );
    });
  },
