  ...
  "test set page title with dialog": function () {
    ...
    bot.dialog("docProps", function (dialog) {
      ...
      tc.wait(function () {
        ...
        assert.areEqual(
          title1,
          title.data("cke-title"),
          "Title cke-data attribute has been set."
        );
        assert.areEqual(
          template.output({ title: title1 }),
          editor.getData(),
          "Page title is set in editor data."
        );
      }, 0);
    });
  },
        ...
        bot.dialog("docProps", function (dialog) {
          assert.areSame(
            ...
            dialog.getValueOf(
              "general",
              "title",
              "Page title has been loaded into dialog."
            )
          );
        });
      });
