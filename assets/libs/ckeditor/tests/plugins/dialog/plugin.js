...
(function () {
  ...
  bender.test({
      ...
      ed.openDialog("testDialog3", function (dialog) {
        setTimeout(function () {
          resume(function () {
            ...
            assert.isTrue(
              dialog
                .getContentElement("tab1", "foo")
                .getInputElement()
                .isVisible()
            );
            assert.isFalse(
              dialog
                .getContentElement("tab1", "bar")
                .getInputElement()
                .isVisible()
            );
            assert.isFalse(
              dialog
                .getContentElement("tab2", "bom")
                .getInputElement()
                .isVisible()
            );
    ...
    "test dialog's HTML field always count as allowed field unless requiredContent is specified":
      function () {
        var ed = this.editor;
        ...
        ed.openDialog("testDialog4", function (dialog) {
          setTimeout(function () {
            resume(function () {
              ...
              // Tab 2a is hidden.
              assert.isFalse(
                dialog.parts.tabs.getChild(1).isVisible(),
                "tab2a"
              );
              ...
              assert.isTrue(
                dialog
                  .getContentElement("tab2b", "field2b")
                  .getInputElement()
                  .isVisible(),
                "field2b"
              );
    ...
    "test dialog is triggered on doubleclick": function () {
      ...
      assert.areSame(
        "testdoubleclick",
        openedDialog,
        "dialog was opened on doubleclick"
      );
    ...
    "test dialog setState": function () {
      ...
      editor.addCommand(
        "testDialog5",
        new CKEDITOR.dialogCommand("testDialog5")
      );
      ...
      editor.once("dialogShow", function (evt) {
        ...
        resume(function () {
          try {
            assert.isTrue(
              dialog.getButton("ok").isEnabled(),
              "OK button is enabled."
            );
            assert.isUndefined(
              dialog.parts.spinner,
              "By default dialog has no spinner"
            );
            assert.areSame(
              ...
              resume(function () {
                try {
                  assert.areSame(
                    CKEDITOR.DIALOG_STATE_IDLE,
                    dialog.state,
                    "Default dialog state after re–open"
                  );
                  assert.isTrue(
                    dialog.getButton("ok").isEnabled(),
                    "OK button is enabled after re–open"
                  );
                  assert.isObject(
                    dialog.parts.spinner,
                    "Dialog has been given a spinner before"
                  );
                } catch (e) {
                  throw e;
                } finally {
    ...
    "test dialog opens tab defined in tabId as default": function () {
      ...
      editor.addCommand(
        "testDialog6",
        new CKEDITOR.dialogCommand("testDialog6", {
          tabId: "tab2",
        })
      );
