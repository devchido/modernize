...
(function () {
  ...
  bender.editors = {
    classic: {
      ...
      startupData:
        "<p>foo :grinning_face: bar :not_emoji: this is converted emoji :star:</p>",
    },
    classic2: {
      name: "classic2",
      config: {
      ...
      startupData:
        "<p>foo :grinning_face: bar :not_emoji: this is not converted emoji :star:</p>",
    },
  };
  ...
  bender.test({
    ...
    "test custom emoji list is load": function () {
      ...
      emojiTools.runAfterInstanceReady(editor, null, function (editor) {
        ...
        objectAssert.areDeepEqual(
          { id: ":star:", symbol: "⭐", group: "travel", keywords: ["star"] },
    ...
    "test invalid emoji list": function () {
      ...
      emojiTools.runAfterInstanceReady(editor, null, function (editor) {
        assert.isUndefined(
          editor._.emoji,
          "editor._.emoji should not be initialised when invalid emoji file is loaded."
        );
      });
    },
    ...
    "test long ajax loading": function () {
      ...
      server.respondWith("GET", /^http:\/\/random\.url(?:\/?\?.*)?$/, [
        200,
      ...
      bender.editorBot.create(
        {
          ...
          startupData:
            "<p>foo :grinning_face: bar :not_emoji: this :star: is converted emoji :bug:</p>",
        },
        function (bot) {
          emojiTools.runAfterInstanceReady(bot.editor, null, function (editor) {
            assert.isUndefined(
              editor._.emoji,
              "Emoji is loaded on this stage, what should not happen here."
            );
            server.respond();
            assert.areSame(1, editor._.emoji.list.length);
