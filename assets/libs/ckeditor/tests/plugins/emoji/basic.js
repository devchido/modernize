...
(function () {
  ...
  sinon.stub(
    ...
    "throttle",
    function (minInterval, output, contextObj) {
  ...
  var singleTests = {
    "test for custom emoji characters": function () {
      ...
      emojiTools.runAfterInstanceReady(editor, bot, function (editor, bot) {
        ...
        assert.areSame(
          ":dagg",
          autocomplete.model.query,
          "Model keeps wrong query"
        );
        assert.areSame(
          1,
          autocomplete.model.data.length,
          "Emoji result contains more than one result"
        );
  ...
  var tests = {
    ...
    "test emoji objects are added to editor": function (editor) {
      emojiTools.runAfterInstanceReady(editor, null, function (editor) {
        ...
        objectAssert.ownsKeys(
          ["list", "autocomplete"],
          editor._.emoji,
          "Emoji variable is missing some keys"
        );
      });
    },
    ...
    "test emoji are not actived when too few letters are written": function (
      editor,
      bot
    ) {
      emojiTools.runAfterInstanceReady(editor, bot, function (editor, bot) {
    ...
    // (#2195)
    "test emoji suggestion box shouldn't appear inside text": function (
      editor,
      bot
    ) {
      emojiTools.runAfterInstanceReady(editor, bot, function (editor, bot) {
    ...
    // (#2195)
    "test emoji suggestion box should appear at the beginning of new line":
      function (editor, bot) {
        emojiTools.runAfterInstanceReady(editor, bot, function (editor, bot) {
          var autocomplete = editor._.emoji.autocomplete,
            editable = editor.editable();
    ...
    // (#2527)
    "test emoji autocomplete is sorted in proper order": function (
      editor,
      bot
    ) {
      emojiTools.runAfterInstanceReady(editor, bot, function (editor, bot) {
        ...
        actual = CKEDITOR.tools.array.map(
        ...
        assert.areSame(
          "😻 :smiling_cat_face_with_heart-eyes:",
          autocomplete.view.element.getChild(0).getText(),
          'First element in view should start from "smiling".'
        );
      });
    },
  };
  ...
  CKEDITOR.tools.array.forEach(
    CKEDITOR.tools.object.keys(singleTests),
    function (key) {
