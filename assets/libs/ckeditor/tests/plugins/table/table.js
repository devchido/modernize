  ...
  bender.test({
    "test create table": function () {
      ...
      bot.dialog("tableProperties", function (dialog) {
        ...
        assert.areSame(
          isSmallViewport ? "100%" : "500px",
          dialog.getValueOf("info", "txtWidth")
        );
        ...
        wait(function () {
          ...
          var expected = bender.tools.compatHtml(
            bender.tools.getValueAsHtml("create-table")
          );
    ...
    "test add caption/summary": function () {
      ...
      bender.tools.testInputOut("add-caption", function (source, expected) {
        ...
        bot.dialog("tableProperties", function (dialog) {
          ...
          assert.areSame(
            bender.tools.compatHtml(bender.tools.fixHtml(expected)),
            bot.getData(true)
          );
        });
      });
    ...
    "test delete table directly in inline editor": function () {
      ...
      assert.isTrue(
        CKEDITOR.document.getBody().contains(editable),
        "Editable should not be removed"
      );
    ...
    "test html border attribute behaviour when CSS border is 0": function () {
      ...
      assert.isTrue(
        /border="0"/.test(bot.editor.getData()),
        "Border attribute should be zero"
      );
    },

    // (#566)
    "test html border attribute behaviour when CSS border is not  0":
      function () {
        var bot = this.editorBots.editor;
        ...
        assert.isTrue(
          /border="1"/.test(bot.editor.getData()),
          "Border attribute should be one"
        );
      },
