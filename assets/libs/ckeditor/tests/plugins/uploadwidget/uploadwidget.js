...
(function () {
  var fileTools,
    resumeAfter,
    lastUploadUrl,
    lastAdditionalRequestParameters,
    loadAndUploadCount,
    loadCount,
    uploadCount,
    htmlMatchingOpts = {
  ...
  function assertUploadingWidgets(editor, widgetName, expectedWidgetsCount) {
    var widgets = editor
        .editable()
        .find('span[data-widget="' + widgetName + '"]'),
      widget,
      i;
    ...
    assert.areSame(
      ...
      widgets.count(),
      "Expected " + widgetName + " count should be " + expectedWidgetsCount
    );
  ...
  bender.test({
    ...
    "test upload (integration test)": function () {
      ...
      bot.setData("", function () {
        ...
        assert.areSame(
          1,
          loadAndUploadCount,
          "loadAndUpload should be called once."
        );
    ...
    // (#1454)
    "test calling widget onAbort function when widget is destroyed":
      function () {
        var bot = this.editorBot,
          editor = bot.editor,
        ...
        bot.setData("", function () {
          ...
          editor
            .editable()
            .findOne('[data-cke-upload-id="' + loader.id + '"]')
            .remove();
    ...
    "test markElement": function () {
      ...
      assert.sameData(
        '<p data-cke-upload-id="1" data-widget="widgetName"></p>',
    ...
    "test _getLoader": function () {
      ...
      bot.setData(
        '<p>x<span data-cke-upload-id="' +
          loader.id +
          '" data-widget="testGetLoader">uploading...</span>x</p>',
        function () {
          var widget = editor.widgets.getByElement(
            editor.editable().findOne('span[data-widget="testGetLoader"]')
          );

          assert.areSame(
            loader,
            widget._getLoader(),
            "_getLoader return value"
          );
        }
      );
    },
    ...
    "test replaceWith 1 element": function () {
      ...
      bot.setData(
        '<p>x<span data-cke-upload-id="' +
          loader.id +
          '" data-widget="testReplaceWith1">uploading...</span>x</p>',
        function () {
          editor.widgets
            .getByElement(
              editor.editable().findOne('span[data-widget="testReplaceWith1"]')
            )
            .focus(); // focus widget
    ...
    "test replaceWith multiple elements": function () {
      ...
      addTestUploadWidget(editor, "testReplaceWith1", {
        onUploaded: function () {
          this.replaceWith(
            "y<strong>uploaded1</strong><em>upl<u>oad</u>ed2</em>y"
          );
        },
      });
      ...
      bot.setData(
        ...
        function () {
          ...
          assert.isInnerHtmlMatching(
            "<p>x[y<strong>uploaded1</strong><em>upl<u>oad</u>ed2</em>y]x@</p>",
            bender.tools.selection.getWithHtml(editor),
            htmlMatchingOpts
          );
        }
    ...
    "test replaceWith preserves selection placed before the widget":
      function () {
        var bot = this.editorBot,
          editor = bot.editor,
          uploads = editor.uploadRepository,
        ...
        bot.setData(
          '<p><strong>foo<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testReplaceWith1">uploading...</span>x</strong></p>',
          function () {
            ...
            assert.isInnerHtmlMatching(
              "<p><strong>fo^ouploadedx</strong>@</p>",
              bender.tools.selection.getWithHtml(editor),
              htmlMatchingOpts
            );
          }
    ...
    "test replaceWith preserves selection placed after the widget":
      function () {
        var bot = this.editorBot,
          editor = bot.editor,
          uploads = editor.uploadRepository,
        ...
        bot.setData(
          '<p><strong>x<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testReplaceWith1">uploading...</span>foo</strong></p>',
          function () {
            var range = editor.createRange();
            ...
            assert.isInnerHtmlMatching(
              "<p><strong>xuploadedfo^o</strong>@</p>",
              bender.tools.selection.getWithHtml(editor),
              htmlMatchingOpts
            );
          }
    ...
    "test custom event lister": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testOnError">uploading...</span>'
        );
    ...
    "test custom event lister with prevent default": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testOnAbortFalse">uploading...</span>'
        );
    ...
    "test mark specific widget before general": function () {
      ...
      resumeAfter(editor, "paste", function (evt) {
        assert.sameData(
          '<span data-cke-upload-id="0" data-widget="specificWidget1">specific</span>',
          evt.data.dataValue
        );
      });
    ...
    "test mark specific widget not handle not supported extension":
      function () {
        var editor = mockEditorForPaste();
        ...
        resumeAfter(editor, "paste", function (evt) {
          assert.sameData(
            '<span data-cke-upload-id="0" data-widget="generalWidget2">general</span>',
            evt.data.dataValue
          );
        });
    ...
    "test loadMethod load": function () {
      ...
      resumeAfter(editor, "paste", function () {
        ...
        assert.areSame(
          0,
          loadAndUploadCount,
          "LoadAndUpload should not be called once."
        );
      });
    ...
    "test supports definition changes at a runtime": function () {
      ...
      editor.widgets.registered.runtimeDefChanges.supportedTypes =
        /text\/plain/;
    ...
    "test custom def.loaderType": function () {
      ...
      resumeAfter(editor, "paste", function () {
        assert.areSame(
          ...
          "CustomLoaderType.upload call count"
        );
      });
    ...
    "test bindNotifications on paste": function () {
      ...
      resumeAfter(editor, "paste", function () {
        ...
        assert.areSame(
          bender.tools.getTestPngFile().name,
          spy.firstCall.args[1].fileName
        );
      });
    ...
    "test uploadWidgetDefinition.skipNotifications": function () {
      ...
      resumeAfter(editor, "paste", function () {
        ...
        assert.areSame(
          0,
          spy.callCount,
          "CKEDITOR.fileTools.bindNotifications call count"
        );
      });
    ...
    "test undo and redo": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testUndoRedo">uploading...</span>'
        );
    ...
    "test undo during upload": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testUndoDuring">uploading...</span>'
        );
    ...
    "test error during upload": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testErrorDuring">uploading...</span>'
        );
    ...
    "test changes and undo during upload": function () {
      ...
      bot.setData("", function () {
        ...
        assertUploadingWidgets(editor, "testUndoDuring");
        assert.areSame(
          '<p>xx</p><p id="p"><strong>foo</strong></p>',
          editor.getData()
        );
    ...
    "test changes, undo and redo during upload": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testUndoRedoDuring">uploading...</span>'
        );
        ...
        assert.isInnerHtmlMatching(
          '<p>xuploadedx@</p><p id="p"><strong>[foo@]</strong></p>',
          bender.tools.selection.getWithHtml(editor),
          htmlMatchingOpts
        );
      });
    ...
    "test changes during upload and undo after upload": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testUndoRedoAfter">uploading...</span>'
        );
    ...
    "test copy upload widget": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testCopy">uploading...</span>'
        );
    ...
    "test set Class during upload": function () {
      ...
      bot.setData("", function () {
        ...
        editor.insertHtml(
          '<span data-cke-upload-id="' +
            loader.id +
            '" data-widget="testClass">...</span>'
        );

        wrapper = editor
          .editable()
          .findOne('span[data-cke-widget-wrapper="1"]');

        loader.changeStatus("loading");

        assert.isTrue(
          wrapper.hasClass("cke_upload_loading"),
          "Has class loading."
        );
        assert.isFalse(
          wrapper.hasClass("cke_upload_uploading"),
          "Has NOT class uploading."
        );
    ...
    "test text mode": function () {
      ...
      bot.setData(
        '<p><u>x<span data-cke-upload-id="' +
          loader.id +
          '" data-widget="testTextMode">uploading...</span>x</u></p>',
        function () {
          loader.changeStatus("uploaded");
