...
(function () {
  ...
  function assertInternalStructure(widget) {
    ...
    assert.isTrue(
      widget.parts.code.hasClass("hljs"),
      "Default codeSnippet_codeClass set"
    );
  ...
  bender.test({
    ...
    "test upcast: elements inside code": function () {
      ...
      this.editorBot.setData(data, function () {
        ...
        assert.areSame(
          0,
          instances.length,
          "Code with elements inside should not be upcasted at all"
        );
    ...
    "test upcast: language recognition": function () {
      ...
      this.editorBot.setData(inputCode, function () {
        ...
        assert.areSame(
          1,
          instances.length,
          "A single widget should be created"
        );
        ...
        assert.areSame(
          "javascript",
          widget.data.lang,
          "Language detected (data#lang)"
        );
        assert.areSame(code, widget.data.code, "Code loaded (data#code)");
      });
    },

    "test upcast: language recognition (invalid language-* class)":
      function () {
        var editor = this.editor;

        this.editorBot.setData(
          '<pre><code class="language-foo">c</code></pre>',
          function () {
            var instances = objToArray(editor.widgets.instances);
            ...
            assert.isUndefined(
              widget.data.lang,
              "In case of unrecognized language, lang (data#lang) should remain null"
            );
    ...
    "test upcast: language recognition (valid language-* class but config.codeSnippet_languages)":
      function () {
        var editor = this.editor;
        ...
        this.editorBot.setData(
          ...
          function () {
            ...
            assert.isUndefined(
              widget.data.lang,
              "Language which don't match config.codeSnippet_languages should not be recognized"
            );
            assert.areSame("c", widget.data.code, "Code loaded (data#code)");
          }
        );
    ...
    "test upcast: handle codeSnippet_codeClass": function () {
      ...
      bot.setData(
        '<pre><code class="hljs wrongClass language-python">c</code></pre>',
        function () {
          var instances = objToArray(editor.widgets.instances);
          ...
          assert.areSame(
            '<pre><code class="language-python">c</code></pre>',
            bot.getData(),
            "codeSnippet_codeClass and wrong class stripped out"
          );
        }
      );
    },
