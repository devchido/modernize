...
(function () {
    ...
    config: {
      on: {
        pluginsLoaded: function () {
          ...
          setHighlighter =
            CKEDITOR.plugins.registered.codesnippet.setHighlighter;
        },
      },
    },
  };
  ...
  bender.test({
    ...
    "test highlighter: change highlighter": function () {
      ...
      assert.areEqual(
        highlighter,
        getHighlighter(editor),
        "Highlighter has not been changed"
      );
      assert.areEqual(
        langs,
        getLangs(this.editor),
        "Highlighter languages has not been changed"
      );
    },
    ...
    "test highlighter: custom highlighter": function () {
      ...
      var highlighter = new CKEDITOR.plugins.codesnippet.highlighter({
        ...
        highlighter: function (code, language, callback) {
          assert.areSame(
            "foo",
            code,
            "Contents of widget.parts#code passed correctly"
          );
          assert.areSame(
            "php",
            language,
            "Language of widget.data#lang passed correctly"
          );
      ...
      this.editorBot.setData(html, function () {
        ...
        assert.areSame(
          2,
          called,
          "Highlighter should be called twice (editor.setData(), highlight())"
        );
        assert.areEqual(
          "highlighted",
          widget.parts.code.getHtml(),
          "Widget.parts#code updated correctly"
        );
      });
    },
    ...
    "test highlighter: custom highlighter with async init": function () {
      ...
      this.editorBot.setData(html, function () {
        ...
        assert.isFalse(
          !!highlighter.ready,
          "Highlighter should not be ready yet"
        );
        assert.areSame(
          2,
          highlighter.queue.length,
          "Jobs queued: editor.setData(), highlight()"
        );
        assert.areSame(
          0,
          called,
          "Highlighter should not be called because it is not ready"
        );
        assert.areEqual(
          "foo",
          widget.parts.code.getHtml(),
          "Widget.parts#code not updated yet"
        );
        ...
        assert.areSame(
          2,
          called,
          "Highlighter should be called twice (queued: editor.setData(), highlight())"
        );
        assert.areEqual(
          "highlighted async",
          widget.parts.code.getHtml(),
