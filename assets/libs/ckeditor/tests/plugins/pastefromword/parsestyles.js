...
(function () {
  ...
  function testInlining(name) {
    bender.tools.testInputOut(name, function (input, output) {
      ...
      bender.assert.beautified.html(
        output,
        bender.tools.fixHtml(parsed.getBody().getHtml()),
        name
      );
    });
  }
  ...
  function testFiltering(name) {
    bender.tools.testInputOut(name, function (styles, expected) {
      var tested = CKEDITOR.plugins.pastefromword.styles.inliner.filter(
        JSON.parse(styles)
      );
  ...
  CKEDITOR.scriptLoader.load(
    CKEDITOR.getUrl("/plugins/pastefromword/filter/default.js"),
    function () {
      bender.test(tests);
    },
    null,
