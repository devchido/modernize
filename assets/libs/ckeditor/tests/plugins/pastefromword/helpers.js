...
(function () {
  ...
  var extractImagesTestsGenerator = function () {
    ...
    function getTest(name) {
      return function () {
        CKEDITOR.ajax.load(
          "./_fixtures/" + name + ".rtf?" + cacheBuster(),
          function (rtf) {
            CKEDITOR.ajax.load(
              "./_fixtures/" + name + ".json?" + cacheBuster(),
              function (json) {
                resume(function () {
                  var actualResult =
                      CKEDITOR.plugins.pastefromword.images.extractFromRtf(rtf),
    ...
    for (i = 0; i < testsFilesNames.length; i++) {
      obj["test extract images from rtf file (" + testsFilesNames[i] + ")"] =
        getTest(testsFilesNames[i]);
    }
    return obj;
  };
      ...
      for (i = 0; i < sourceLength; i++) {
        bender.tools.testInputOut("source" + i, function (input, output) {
          var expectedResult = JSON.parse(output),
            actualResult =
              CKEDITOR.plugins.pastefromword.images.extractTagsFromHtml(input),
            j;
          assert.areSame(expectedResult.length, actualResult.length);
