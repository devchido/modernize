  ...
  var tests = {
    ...
    "test assignListLevels": function () {
      ...
      assert.beautified.html(
        CKEDITOR.document.getById("tc1expected").getHtml(),
    ...
    "test assignListLevels does early return": function () {
      ...
      this.heuristics.assignListLevels(
        this.editor,
        this.getParserElementsFrom("tc1expected")[0]
      );
    ...
    "test guessIndentationStep": function () {
      assert.isNull(
        this.heuristics.guessIndentationStep([]),
        "Returned value for empty array"
      );
      assert.areSame(20, this.heuristics.guessIndentationStep([20, 20]), "tc1");
      assert.areSame(
        20,
        this.heuristics.guessIndentationStep([20, 40, 60]),
        "tc2"
      );
      assert.areSame(
        10,
        this.heuristics.guessIndentationStep([10, 20, 20, 10]),
        "tc3"
      );
      ...
      assert.areSame(
        10,
        this.heuristics.guessIndentationStep([20, 20, 10]),
        "tc5"
      );
    },
    ...
    "test cleanupEdgeListItem": function () {
      ...
      assert.beautified.html(
        CKEDITOR.document.getById("tc3expected").getHtml(),
        html,
        "Paragraph's html"
      );
    },
