...
function createTestCase(options) {
  return function () {
    ...
    CKEDITOR.tools.promise.all(loadQueue).then(function (values) {
      ...
      if (
        inputFixtureHtml === null &&
        (!options.includeRTF || inputFixtureRtf === null)
      ) {
        resume(function () {
          assert.ignore();
        });
      ...
      // Single null when RTF is available means that one of 2 required files is missing.
        ...
        options.includeRTF &&
        (inputFixtureHtml === null || inputFixtureRtf === null)
      ) {
        resume(function () {
          assert.isNotNull(
            inputFixtureHtml,
            '"' + inputPathHtml + '" file is missing'
          );
          assert.isNotNull(
            inputFixtureRtf,
            '"' + inputPathRtf + '" file is missing'
          );
        });
      }

      var nbspListener = editor.once(
        "paste",
        function (evt) {
      ...
      assertWordFilter(editor, options.compareRawData)(
        { "text/html": inputFixtureHtml, "text/rtf": inputFixtureRtf },
        expectedValue
      ).then(
        function (values) {
          resume(function () {
