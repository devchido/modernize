    ...
    tests = {
      ...
      "test create style stack": function () {
        ...
        element.attributes.style =
          'font-family: "Calibri"; font-size: 36pt; color: yellow; background: lime';
        ...
        assertPasteEvent(
          this.editor,
          { dataValue: "<w:WordDocument></w:WordDocument>" },
          function () {
      ...
      "test create style stack multiple children": function () {
        var edgeCase =
            '<span style="font-family:Courier;font-size:14px" ><span style="font-weight:bold">Some </span>Text</span>',
        ...
        assert.areSame(
          '<span style="font-size:14px"><span style="font-family:Courier"><span style="font-weight:bold">Some </span>Text</span></span>',
          element.getOuterHtml()
        );
      },
      ...
      "test create style stack omit block styles": function () {
        var edgeCase =
            '<p style="font-size: 16pt;font-family: Arial;margin-bottom:10pt;">Test</p>',
          fragment = CKEDITOR.htmlParser.fragment.fromHtml(edgeCase),
          element = fragment.children[0];

        this.pastefromword.styles.createStyleStack(element, filterMock);
        assert.areSame(
          '<p style="margin-bottom:13px"><span style="font-size:16pt"><span style="font-family:Arial">Test</span></span></p>',
          element.getOuterHtml()
        );
      },
      "test push styles lower": function () {
        ...
        ol.attributes.style =
          'list-style-type: lower-alpha;font-family: "Calibri"; font-size: 36pt; color: yellow';
        ol.add(li);

        this.pastefromword.styles.pushStylesLower(ol);
        assert.areSame(
          '<ol style="list-style-type:lower-alpha"><li style="font-family:&quot;Calibri&quot;; font-size:36pt; color:yellow"></li></ol>',
          ol.getOuterHtml()
        );
      },
      ...
      "test remove list symbol 1": function () {
        var html =
            '<cke:li cke-list-level="1" cke-symbol="1.">1.       This</cke:li>',
        ...
        assert.areSame(
          '<cke:li cke-list-level="1" cke-symbol="1."> This</cke:li>',
          element.getOuterHtml()
        );
      },
      "test remove list symbol 2": function () {
        var html =
            '<cke:li cke-list-level="1" cke-symbol="1."><span style="font-family:Calibri">1.       This</span></cke:li>',
          fragment = CKEDITOR.htmlParser.fragment.fromHtml(html),
          element = fragment.children[0];
        ...
        assert.areSame(
          '<cke:li cke-list-level="1" cke-symbol="1."><span style="font-family:Calibri"> This</span></cke:li>',
          element.getOuterHtml()
        );
      },
      ...
      "test sort styles": function () {
        var html =
            '<p style="font-size:48pt; background:yellow; font-family:Courier">Test</p>',
          fragment = CKEDITOR.htmlParser.fragment.fromHtml(html),
          element = fragment.children[0];
      ...
      "test stack attributes": function () {
        var html =
            '<font face="Arial" color="#faebd7" size="4">There is <em>content</em> here</font>',
          fragment = CKEDITOR.htmlParser.fragment.fromHtml(html),
          element = fragment.children[0];

        CKEDITOR.plugins.pastefromword.createAttributeStack(
          element,
          filterMock
        );

        assert.areSame(
          '<font face="Arial"><font color="#faebd7"><font size="4">There is <em>content</em> here</font></font></font>',
          element.getOuterHtml()
        );
      },
      "test supportFields comment": function () {
        /*jshint nonbsp:false */
        var html =
            "<!--[if supportFields]><span lang=DE><span style='mso-element:" +
      ...
      "test dissolving lists": function () {
        ...
        var html =
          '<p class="MsoNormal"><span style="color:red">The list below does not copy + paste correctly:<o:p></o:p></span></p>' +
        ...
        assert.beautified.html(
          '<p><span style="color:red">The list below does not copy + paste correctly:</span></p>' +
          ...
          CKEDITOR.cleanWord(html, editorStub),
          { sortAttributes: true }
        );
      },

      "test isAListContinuation": function () {
        var lastListItem = this.getParserElementsFrom("isAListContinuation1")
          .children[3];
      ...
      "test cleanup": function () {
        var listItems = this.getParserElementsFrom(
          "isAListContinuation1"
        ).children;

        this.lists.cleanup(listItems);

        assert.isUndefined(
          listItems[0].attributes["cke-list-level"],
          "First list item cke-list-level"
        );
        assert.isUndefined(
          listItems[0].attributes["cke-symbol"],
          "First list item cke-symbol"
        );
        assert.isUndefined(
          listItems[0].attributes["cke-list-id"],
          "First list item cke-list-id"
        );
        assert.isUndefined(
          listItems[0].attributes["cke-indentation"],
          "First list item cke-indentation"
        );

        assert.isNotUndefined(
          listItems[0].attributes.style,
          "First list style attribute"
        );

        assert.isUndefined(
          listItems[0].attributes["cke-dissolved"],
          "Second list item cke-dissolved"
        );

        // Check third list item.
        assert.isUndefined(
          listItems[2].attributes["cke-list-level"],
          "Third list item cke-list-level"
        );
        assert.isUndefined(
          listItems[2].attributes["cke-symbol"],
          "Third list item cke-symbol"
        );
        assert.isUndefined(
          listItems[2].attributes["cke-list-id"],
          "Third list item cke-list-id"
        );
        assert.isUndefined(
          listItems[2].attributes["cke-indentation"],
          "Third list item cke-indentation"
        );

        // Make sure we don't remove too much.
        assert.areSame(
          "aa",
          listItems[2].attributes["cke-foo-bar"],
          "cke-foo-bar remains"
        );
      },
      ...
      "test calculateValue": function () {
        ...
        assert.areSame(
          1,
          this.lists.calculateValue(removedListItem),
          "Result for a removed list item"
        );
        ...
        function assertCalculatedValue(
          ...
          assert.areSame(
            expectedValue,
            that.pastefromword.lists.calculateValue(list.children[itemIndex]),
      ...
      getParserElementsFrom: function (id) {
        return CKEDITOR.htmlParser.fragment.fromHtml(
          CKEDITOR.document.getById(id).getHtml()
        ).children[0];
      },
    };
