...
(function () {
  ...
  bender.test({
    ...
    "test matching diacritic characters": function () {
      ...
      testView(this.createMentionsInstance({ feed: [diacritics] }), [
        diacritics,
      ]);
    },
    ...
    "test not matching special characters": function () {
      ...
      for (var i = 0; i < specialCharacters.length; i++) {
        ...
        assert.isFalse(
          viewElement.hasClass("cke_autocomplete_opened"),
    ...
    "test array feed with match": function () {
      ...
      testView(
        this.createMentionsInstance({ feed: feedData }),
        expectedFeedData
      );
    },
    ...
    "test case sensitive array feed with match": function () {
      this.editorBot.setHtmlWithSelection("<p>@An^</p>");
      testView(
        this.createMentionsInstance({ feed: feedData, caseSensitive: true }),
        expectedFeedData
      );
    },
    ...
    // (#1934)
    "test case sensitive array feed without match - lowercase query":
      function () {
        this.editorBot.setHtmlWithSelection("<p>@an^</p>");
        testView(
          this.createMentionsInstance({ feed: feedData, caseSensitive: true }),
    ...
    // (#1934)
    "test case sensitive array feed without match - lowercase data":
      function () {
        this.editorBot.setHtmlWithSelection("<p>@An^</p>");
        testView(
          this.createMentionsInstance({ feed: ["anna"], caseSensitive: true }),
          []
        );
      },
    ...
    "test URL gets encoded": function () {
      ...
      sinon.assert.calledWith(
        ajaxStub,
        "/controller/method/?query=F%26%2Foo&format=json",
        sinon.match.any
      );
    },
    ...
    "test valid URL called for multi char marker": function () {
      ...
      this.editor
        .editable()
        .fire("keydown", new CKEDITOR.dom.event({ keyCode: 13 })); // ENTER
    ...
    "test attach mentions from configuration": function () {
      ...
      bender.editorBot.create(
        { name: "editor_config_attach", config: config },
        function (bot) {
    ...
    "test URL has no race condition": function () {
      ...
      this.editorBot.setHtmlWithSelection("<p>@Anna^</p>");
      mentions._autocomplete.editor
        .editable()
        .fire("keyup", new CKEDITOR.dom.event({}));
  ...
  function testView(mentions, matches, skipTrigger) {
    ...
    for (var i = items.count() - 1; i >= 0; i--) {
      assert.areEqual(
        mentions.marker + matches[i],
        itemsArray[i].getHtml(),
        "Match and item html are not equal."
      );
    }
  }
