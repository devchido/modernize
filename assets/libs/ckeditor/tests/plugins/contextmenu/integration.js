        ...
        function (bot) {
          ...
          editor.once("menuShow", function () {
            resume(function () {
              ...
              assert.isTrue(
                item.item instanceof CKEDITOR.menuItem,
                "Holds CKEDITOR.menuItem under item"
              );
              assert.isTrue(
                item.element instanceof CKEDITOR.dom.element,
                "Holds CKEDITOR.dom.element under element"
              );

              assert.isNull(
                editor.contextMenu.findItemByCommandName("non-existent-item"),
                "Null for non-existing item"
              );
            });
          });
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          bot.editor.on("menuShow", function (evt) {
            resume(function () {
              assert.isTrue(
                evt.data[0] instanceof CKEDITOR.ui.floatPanel,
                "CKEDITOR.ui.panel available"
              );
            });
          });
