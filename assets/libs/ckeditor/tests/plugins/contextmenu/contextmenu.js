  ...
  bender.test({
    "test opening context menu color": function () {
      ...
      bot1.contextmenu(function (menu1) {
        ...
        assert.areSame(
          menu1._.panel._.iframe,
          CKEDITOR.document.getActive(),
          "check DOM focus inside of panel"
        );
        assert.isTrue(
          ed1.focusManager.hasFocus,
          "editor1 focused on menu open"
        );
        ...
        bender.editorBot.create(
          ...
          function (bot2) {
            ...
            bot2.contextmenu(function (menu2) {
              // Check DOM focus and virtual editor focus.
              assert.areSame(
                menu2._.panel._.iframe,
                CKEDITOR.document.getActive(),
                "check DOM focus inside of panel"
              );
              assert.isFalse(
                ed1.focusManager.hasFocus,
                "editor1 loose focus on menu open"
              );
              assert.isTrue(
                ed2.focusManager.hasFocus,
                "editor2 focused on menu open"
              );
            });
          }
        );
    ...
    "#9706: test opening contextmenu in editable that does not autoparagraph":
      function () {
        bender.editorBot.create(
          {
            name: "editor3",
    ...
    "#11306: test fake selection in Webkit/Blink on Mac when non-editable element":
      function () {
        if (!(CKEDITOR.env.mac && CKEDITOR.env.webkit)) assert.ignore();
          ...
          function (bot) {
            bot.setData(html, function () {
              ...
              assert.areSame(
                2,
                preventDefaultCalled,
                "PreventDefault called once again"
              );
              assert.isFalse(
                !!editor.getSelection().isFake,
                "Fake selection is not set for nested editables"
              );
            });
          }
        );
    ...
    "test context menu custom panel css styles": function () {
      ...
      bender.editorBot.create(
        ...
        function (bot) {
          assert.areEqual(
            customStyle,
            bot.editor.contextMenu._.panelDefinition.css
          );
        }
      );
    },
