    ...
    "test load data": function () {
      ...
      this.editorBot.setData(
        ...
        function () {
          ...
          assert.isTrue(
            breaks.getItem(0).hasClass("cke_pagebreak"),
            "has cke_pagebreak class"
          );
        }
      );
    },
    ...
    "test paste": function () {
      ...
      function assertPagebreak(prefix) {
        ...
        assert.areSame(
          1,
          elements.count(),
          prefix + "There is one non-editable div in the contents"
        );
        ...
        if (CKEDITOR.env.webkit) {
          assert.areSame(
            "page",
            element.getStyle("break-after"),
            prefix + "Pagebreak holds page-break-after style"
          );
        } else {
          assert.areSame(
            "always",
            element.getStyle("page-break-after"),
            prefix + "Pagebreak holds page-break-after style"
          );
        }
        assert.isTrue(
          element.hasClass("cke_pagebreak"),
          prefix + "Pagebreak holds cke_pagebreak class"
        );
        assert.isNotUndefined(
          element.data("cke-pagebreak"),
          prefix + "Pagebreak is marked with an attribute"
        );
      }
