...
(function () {
  ...
  function getNewLineHtml(editor, isShiftEnterMode) {
    var modeToCheck = isShiftEnterMode
  ...
  function prepareEditorAndGetExpectedResult(editor, bot, isShiftEnterMode) {
    ...
    return TABLE_WITH_SELECTION.replace(/(\[|\])/g, "")
      .replace("YY", "&nbsp;")
      .replace("BB", newLine);
  }
  ...
  var tests = {
    ...
    "test press enter key in selected table": function (editor, bot) {
      ...
      assert.beautified.html(
        expectedResult,
        editor.getData(),
        "There wasn't table clear and/or adding new paragraph in it's content after pressing Enter key."
      );

      editor.execCommand("undo");
      assert.beautified.html(
        TABLE_WITH_SELECTION,
        fixMarkersInTable(bender.tools.getHtmlWithSelection(editor)),
        "Editor's content wasn't properly restored after single undo step."
      );
    },
    "test press shift + enter key in selected table": function (editor, bot) {
      ...
      editor
        ...
        .fire(
          "keydown",
          new CKEDITOR.dom.event({ keyCode: 13, shiftKey: true })
        );
      assert.beautified.html(
        expectedResult,
        editor.getData(),
        "There wasn't table clear and/or adding new paragraph in it's content after pressing Shift+Enter key."
      );
