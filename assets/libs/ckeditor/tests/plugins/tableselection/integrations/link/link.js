...
(function () {
  ...
  var getRangesForCells = tableSelectionHelpers.getRangesForCells,
    ...
    editedLinkedTable = CKEDITOR.document
      .getById("table-with-link-edited")
      .findOne("table"),
    multiLinkedTable = CKEDITOR.document
      .getById("table-with-links")
      .findOne("table"),
    editedMultiLinkedTable = CKEDITOR.document
      .getById("table-with-links-edited")
      .findOne("table"),
    anchoredTable = CKEDITOR.document
      .getById("table-with-anchor")
      .findOne("table"),
    multiAnchoredTable = CKEDITOR.document
      .getById("table-with-anchors")
      .findOne("table"),
    editedAnchoredTable = CKEDITOR.document
      .getById("table-with-anchors-edited")
      .findOne("table"),
    removedAnchoredTable = CKEDITOR.document
      .getById("table-with-anchors-removed")
      .findOne("table");
  ...
  var tests = {
    ...
    "test create link": function () {
      ...
      bot.setData(table.getOuterHtml(), function () {
        ...
        bot.dialog("link", function (dialog) {
          ...
          assert.isTrue(
            displayTextField.isVisible(),
            '"Display Text" field visibility'
          );
          ...
          assert.beautified.html(
            editedLinkedTable.getOuterHtml(),
            bot.getData()
          );
        });
      });
    ...
    "test create link on multiple cells": function () {
      ...
      bot.setData(table.getOuterHtml(), function () {
        ...
        bot.dialog("link", function (dialog) {
          ...
          assert.areEqual(
            fixHtml(multiLinkedTable.getOuterHtml()),
            bot.getData(true)
          );
        });
      });
    ...
    "test edit link (text selected)": function () {
      ...
      bot.setData(multiLinkedTable.getOuterHtml(), function () {
        ...
        bot.dialog("link", function (dialog) {
          ...
          assert.areSame(
            fixHtml(editedMultiLinkedTable.getOuterHtml()),
            bot.getData(true)
          );
        });
      });
    ...
    "test get link info from the cell with <br> filler": function () {
      ...
      bot.setData(
        CKEDITOR.document.getById("table-with-br").getOuterHtml(),
        function () {
    ...
    "test getSelectedLink for table selection (links)": function () {
      ...
      bot.setData(multiLinkedTable.getOuterHtml(), function () {
        ...
        for (i = 0; i < links.count(); i++) {
          assert.isTrue(
            links.getItem(i).equals(selectedLinks[i]),
    ...
    "test create anchor": function () {
      ...
      bot.setData(table.getOuterHtml(), function () {
        ...
        bot.dialog("anchor", function (dialog) {
          ...
          assert.areEqual(
            fixHtml(anchoredTable.getOuterHtml()),
            bot.getData(true)
          );
        });
      });
    ...
    "test edit anchor (text selected)": function () {
      ...
      bot.setData(multiAnchoredTable.getOuterHtml(), function () {
        ...
        bot.dialog("anchor", function (dialog) {
          ...
          assert.areSame(
            fixHtml(editedAnchoredTable.getOuterHtml()),
            bot.getData(true)
          );
        });
      });
    ...
    "test removeAnchor command": function () {
      ...
      bot.setData(multiAnchoredTable.getOuterHtml(), function () {
        ...
        assert.isTrue(
          unlink.state == CKEDITOR.TRISTATE_OFF,
          "removeAnchor is enabled"
        );

        editor.execCommand("removeAnchor");
        assert.areSame(
          fixHtml(removedAnchoredTable.getOuterHtml()),
          bot.getData(true),
          "anchors are removed"
        );
      });
