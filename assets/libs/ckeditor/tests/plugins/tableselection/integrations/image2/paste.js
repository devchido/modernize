...
(function () {
    ...
    editor.once("afterCommandExec", function () {
      resume(function () {
        ...
        for (i = 0; i < ranges.length; i++) {
          ...
          assert.isTrue(
            cell.equals(cells.getItem(selectedCells[i])),
  ...
  function pasteImage(editor, callback) {
    editor.once("afterPaste", function () {
      resume(function () {
        ...
        assert.isFalse(
          images.getItem(0).hasClass("cke_widget_new"),
          "The image widget is initialized"
        );
    ...
    bender.tools.emulatePaste(
      editor,
      '<img src="_assets/bar.png" alt="xalt" width="100" id="image" />'
    );
  ...
  var tests = {
    ...
    "test the copied image to table shoud be initialized (collapsed selection)":
      function (editor, bot) {
        bot.setHtmlWithSelection(
          '<table border="1"><tbody><tr><td>Cel^l</td></tr></tbody></table>'
        );
    ...
    "test the copied image to table shoud be initialized (multiple selection)":
      function (editor, bot) {
        bot.setHtmlWithSelection(
          '<table border="1"><tbody><tr>[<td>Cell 1</td>][<td>Cell 2</td>]</tr></tbody></table>'
        );
