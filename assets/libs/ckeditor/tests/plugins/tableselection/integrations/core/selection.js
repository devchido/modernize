...
(function () {
  ...
  var tests = {
    ...
    "test check if selection is in table": function () {
      ...
      range = createRange(
        editor,
        editor.document.getById("foo").getChild(0),
        2,
        table.findOne("td").getChild(0),
        2
      );
      range.select();
      assert.isFalse(
      ...
      range = createRange(
        editor,
        table.find("td").getItem(5).getChild(0),
        2,
        editable.find("table").getItem(1).findOne("td").getChild(0),
        2
      );
      range.select();
      assert.isFalse(
      ...
      bender.tools.setHtmlWithSelection(
        editor,
        CKEDITOR.document.getById("nestedTable").getHtml()
      );
      table = editable.findOne("table");
      ...
      assert.isTrue(
        table.findOne("table").equals(selection.getSelectedElement()),
      ...
      bender.tools.setHtmlWithSelection(
        editor,
        CKEDITOR.document.getById("oneCell").getHtml()
      );
      table = editable.findOne("table");
      ...
      bender.tools.setHtmlWithSelection(
        editor,
        CKEDITOR.document.getById("nestedOneCell").getHtml()
      );
    ...
    "test make fake table selection": function () {
      ...
      ranges = getRangesForCells(
        editor,
        editor.editable().findOne("table"),
        [1, 4]
      );
      ...
      assert.isTrue(
        _getTableElementFromRange(ranges[0]).equals(
      ...
      assert.areSame(
        1,
        realSelection.getRanges().length,
        "Real selection has only one range"
      );

      if (!isQuirkyEnv) {
        assert.isTrue(
          _getTableElementFromRange(ranges[0]).equals(
            realSelection.getSelectedElement()
          ),
    ...
    "test reset fake-selection": function () {
      ...
      assert.areSame(
        1,
        selection.getRanges().length,
        "Only first range remains selected"
      );

      if (!isQuirkyEnv) {
        assert.isTrue(
          ranges[0].getEnclosedNode().equals(selection.getSelectedElement()),
    ...
    "test fire selectionchange event": function () {
      ...
      wait(function () {
        ...
        assert.isTrue(
          _getTableElementFromRange(ranges[0]).equals(selectedElement),
    ...
    "test isInTable allowPartially test (collapsed range)": function () {
      ...
      bender.tools.setHtmlWithSelection(
        editor,
        CKEDITOR.document.getById("allowPartiallyIssue").getHtml()
      );

      assert.isFalse(
        editor.getSelection().isInTable(),
        "Selection is not in table"
      );
    ...
    "test table fake selection undo": function () {
      ...
      ranges = getRangesForCells(editor, editor.editable().findOne("table"), [
        0,
      ]);
    ...
    "test navigating left inside table fake selection": function () {
      ...
      assert.isTrue(
        _getTableElementFromRange(range).equals(ranges[0].getEnclosedNode()),
    ...
    "test navigating right inside table fake selection": function () {
      ...
      assert.isTrue(!!range.collapsed, "Range is collapsed");
      assert.isTrue(
        _getTableElementFromRange(range).equals(ranges[1].getEnclosedNode()),
    ...
    "test overwriting content in table fake selection via keypress":
      function () {
        var editor = this.editor,
          selection = editor.getSelection(),
          prevented = false,
        ...
        assert.beautified.html(
          CKEDITOR.document.getById("contentOverwritingExpected").getHtml(),
    ...
    "test not overwriting content in table fake selection via keypress when no character is produced":
      function () {
        var editor = this.editor,
          selection = editor.getSelection(),
          prevented = false,
        ...
        editor.editable().fire(
          "keypress",
          getKeyEvent({ keyCode: 113, charCode: 0 }, function () {
            prevented = true;
          })
        );
        ...
        for (i = 0; i < ranges.length; i++) {
          if (
            bender.tools.compatHtml(
              _getTableElementFromRange(ranges[i]).getHtml(),
              0,
              0,
              1
            ).length === 0
    ...
    "test not overwriting content in table fake selection via keypress when Ctrl is pressed":
      function () {
        var editor = this.editor,
          selection = editor.getSelection(),
          prevented = false,
        ...
        editor.editable().fire(
          "keypress",
          getKeyEvent(
            { keyCode: 65, charCode: 65, ctrlKey: true },
            function () {
              prevented = true;
            }
          )
    ...
    "test simulating opening context menu in the same table": function () {
      ...
      realSelection = editor.getSelection(1);
      range = getRangesForCells(editor, editor.editable().findOne("table"), [
        2,
      ])[0];
    ...
    "test simulating opening context menu in the different table": function () {
      ...
      realSelection = editor.getSelection(1);
      range = getRangesForCells(
        editor,
        editor.editable().find("table").getItem(1),
        [2]
      )[0];
      ...
      editor.editable().once("selectionchange", function () {
        resume(function () {
          ...
          assert.areSame(
            1,
            selection.getRanges().length,
            "One range are selected"
          );
          assert.isNotNull(
            selection.getNative(),
            "getNative() should not be null"
          );

          assert.isTrue(
            !!selection.getRanges()[0].collapsed,
    ...
    "test simulating opening context menu in the paragraph": function () {
      ...
      bender.tools.setHtmlWithSelection(
        editor,
        "<p>Foo</p>" + CKEDITOR.document.getById("simpleTable").getHtml()
      );
    ...
    "test simulating opening context menu in the same table (WebKit, macOS)":
      function () {
    ...
    "test simulating opening context menu in the nested table (WebKit, macOS)":
      function () {
        // Webkits on macOS contrary to other browsers will collapse the selection and anchor it in a text node.
        if (!CKEDITOR.env.webkit) {
          assert.ignore();
    ...
    "test simulating opening context menu in the different table (WebKit, macOS)":
      function () {
        // Webkits on macOS contrary to other browsers will collapse the selection and anchor it in a text node.
        if (!CKEDITOR.env.webkit) {
          assert.ignore();
