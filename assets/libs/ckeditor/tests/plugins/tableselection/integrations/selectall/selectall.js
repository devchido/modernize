...
(function () {
  ...
  var getRangesForCells = tableSelectionHelpers.getRangesForCells,
    ...
    tests = {
      ...
      "test selectAll command after table selection (paragraph + table)":
        function (editor, bot) {
            ...
            editor.once("afterCommandExec", function () {
              resume(function () {
                wait(function () {
                  ...
                  assert.areSame(
                    0,
                    editable.find("[class*=cke_table-faked-selection]").count(),
                    "Fake selected cells count"
                  );
                  assert.isFalse(
                    editable.hasClass("cke_table-faked-selection-editor"),
      ...
      "test selectAll command after table selection (only table with cells selected)":
        function (editor, bot) {
          var editable = editor.editable(),
            ranges;
              ...
              resume(function () {
                wait(function () {
                  ...
                  assert.areSame(
                    4,
                    editable
                      .find("td[class=cke_table-faked-selection]")
                      .count(),
                    "Fake selected cells count"
                  );
                  assert.isTrue(
