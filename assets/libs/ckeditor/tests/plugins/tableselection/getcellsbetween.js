  ...
  var tests = {
    ...
    "test getCellsBetween": function (editor, bot) {
      ...
      bot.setHtmlWithSelection(
        CKEDITOR.document.getById("getCellsBetween").getValue()
      );
    ...
    "test getCellsBetween (irregular cells number in rows)": function (
      editor,
      bot
    ) {
      bot.setHtmlWithSelection(
        CKEDITOR.document.getById("getCellsBetween-irregular").getValue()
      );

      var inputCells = editor.editable().find("td"),
        cells;

      cells = CKEDITOR.plugins.tableselection.getCellsBetween(
        inputCells.getItem(inputCells.count() - 1),
        inputCells.getItem(0)
      );
    ...
    "test getCellsBetween (caption)": function (editor, bot) {
      bot.setHtmlWithSelection(
        CKEDITOR.document.getById("getCellsBetween-caption").getValue()
      );
