...
(function () {
  ...
  var tests = {
    ...
    "test tableselection without enterkey plugin": function (editor, bot) {
      var expectedResult = TABLE_WITH_SELECTION.replace(/(\[|\])/g, "")
        .replace("BB", "&nbsp;")
        .replace("YY", "&nbsp;");
    ...
    "test tableselection + undo without enterkey plugin": function (
      editor,
      bot
    ) {
      ...
      assert.beautified.html(
        expectedResult,
        editor.getData(),
        "Content before undo step is incorrect"
      );
      ...
      assert.beautified.html(
        source,
        editor.getData(),
        "Content after undo step is incorrect"
      );
