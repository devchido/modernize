  ...
  var tests = {
    ...
    "test overriding cell background": function (editor) {
      bender.tools.setHtmlWithSelection(
        editor,
        CKEDITOR.document.getById("cellBackground").getValue()
      );
    ...
    "test refocusing the editor": function (editor) {
      ...
      assert.isTrue(
        ranges[0].getEnclosedNode().equals(contentCells.getItem(1)),
        "Range[ 0 ] encloses correct element"
      );
      assert.isTrue(
        ranges[1].getEnclosedNode().equals(contentCells.getItem(4)),
        "Range[ 1 ] encloses correct element"
      );
      assert.areSame(
        1,
        editor.getSelection().isFake,
        "Selection remains faked"
      );
    },
    ...
    "test simulating merge cells from context menu ": function (editor) {
      ...
      var selection = editor.getSelection(),
        expected =
          '<table><tbody><tr><td>Cell 1.1</td><td rowspan="2">Cell 1.2<br />Cell 2.2</td>' +
      ...
      editor.once("menuShow", function (evt) {
        resume(function () {
          ...
          assert.areSame(
            rangesAfterCommand[0]._getTableElement(),
            expectedSelectionHolder,
    ...
    // #493
    "test simulating mouse events while scrolling and selecting cells in nested table":
      function (editor) {
        bender.tools.setHtmlWithSelection(
          editor,
          CKEDITOR.document.getById("nestedScroll").getHtml()
        );
        var cells = editor.editable().find("td");

        mockMouseSelection(
          editor,
          [cells.getItem(1), cells.getItem(3), cells.getItem(8)],
          function () {
            assert.pass();
          }
        );
    ...
    // (#2945)
    "test simulating mouse events while the table is ignored": function (
      editor
    ) {
      bender.tools.setHtmlWithSelection(
        editor,
      ...
      mockMouseSelection(
        editor,
        [cells.getItem(1), cells.getItem(2)],
        function () {
          var selection = editor.getSelection();
    ...
    "test right-click in cell with empty paragraph": function (editor, bot) {
      ...
      bot.setData(
        CKEDITOR.document.getById("emptyParagraph").getHtml(),
        function () {
          var cells = editor.editable().find("td").toArray(),
            ranges = tableSelectionHelpers.getRangesForCells(
              editor,
              [0, 1, 2, 3]
            ),
          ...
          assert.areSame(
            ...
            "Selected ranges should be unchanged"
          );
        }
      );
    },
    ...
    "test random keys are not removing readonly selection": function (editor) {
      ...
      editable.fire(
        "keypress",
        new CKEDITOR.dom.event({ keyCode: 65, charCode: 65 })
      ); // `a`
      editable.fire(
        "keypress",
        new CKEDITOR.dom.event({ keyCode: 93, charCode: 93 })
      ); // `t`

      editor.setReadOnly(false);

      assert.areSame(
        bender.tools.compatHtml(table),
        editor.getData(),
        "Editor data"
      );
    },
    ...
    // (#2945)
    "test drag handler event is not prevented for ignored element": function (
      editor
    ) {
    ...
    "test handling copy&cut events for ignored element": function (editor) {
      ...
      function testTableCopying(eventName) {
        ...
        assert.isNull(
          editable.findOne("#cke_table_copybin"),
          eventName + " event should be ignored"
        );
      }
    },
  };
