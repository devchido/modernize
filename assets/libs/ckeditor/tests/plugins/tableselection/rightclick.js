  ...
  function test(options) {
    return function (editor, bot) {
      bot.setData(CKEDITOR.document.getById("test").getHtml(), function () {
        ...
        target = (options.test.inner ? innerCells : outerCells)[
          options.test.index
        ];
        ...
        shouldBeSame =
          options.select.inner === options.test.inner &&
          options.select.indexes.indexOf(options.test.index) !== -1;

        assert[shouldBeSame ? "areSame" : "areNotSame"](
          ranges,
