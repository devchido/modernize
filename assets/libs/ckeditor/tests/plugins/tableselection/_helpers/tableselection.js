...
(function () {
  ...
  window.tableSelectionHelpers = {
    ...
    markCells: function (ranges, addSelected) {
      ...
      for (i = 0; i < ranges.length; i++) {
        ranges[i]
          ._getTableElement()
          .addClass(addSelected ? "selected" : "cke_marked");
      }
    },
  };
    ...
    return function (editor, bot) {
      bender.tools.testInputOut(fixtureId, function (source, expected) {
        ...
        bender.tools.emulatePaste(
          editor,
          CKEDITOR.document.getById(pasteFixtureId).clone(true).getOuterHtml()
        );
  ...
  window.doCommandTest = function (bot, action, options) {
    ...
    bender.tools.testInputOut(options["case"], function (source, expected) {
      ...
      if (options.cells) {
        ranges = window.tableSelectionHelpers.getRangesForCells(
          editor,
          options.cells
        );
      ...
      if (options.customCheck) {
        ...
        assert.isTrue(
          ...
          "selection after is fake"
        );
        assert.isTrue(
          editor.getSelection().isInTable(),
          "selection after is in table"
        );

        if (
          typeof action != "string" ||
          action.toLowerCase().indexOf("merge") === -1
        ) {
          assert.areSame(
            ranges.length,
            afterRanges.length,
            "appropriate number of ranges is selected"
          );

          for (i = 0; i < ranges.length; i++) {
            assert.isTrue(
              afterRanges[i]._getTableElement().hasClass("cke_marked"),
  ...
  window.mockMouseEventForCell = function (editor, type, cell, callback) {
    var host = editor.editable().isInline()
        ? editor.editable()
        : editor.document,
      event = {
        getTarget: function () {
