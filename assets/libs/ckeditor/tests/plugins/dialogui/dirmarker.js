...
(function () {
  function testBidiField(field, origValue, expectedDir) {
    ...
    if (expectedDir) {
      ...
      assert.areSame(
        ...
        "after getValue() method is called, value contains " +
          expectedDir +
          " marker"
      );
      ...
      assert.areSame(
        ...
        "getDirectionMarker() indicates direction marker attribute is set to " +
          expectedDir
      );
    } else {
      ...
      assert.isMatching(
        /^[^\u202A\u202B]/,
        field.getValue(),
        "after getValue() method is activated, value does not contain RTL/LTR marker"
      );

      // Check getDirectionMarker() method.
      assert.isNull(
        field.getDirectionMarker(),
        "getDirectionMarker() method indicates direction marker attribute does not exist"
      );
    }
  }
  ...
  function testNoValueOrNoneBidiField(field, origValue) {
    ...
    assert.areSame(
      realValue,
      origValue,
      "input real value is equal to value entered"
    );

    // Check getValue().
    var msg;
    if (origValue === "") msg = "after getValue() is called, value is empty";
    else
  ...
  bender.test({
    setUp: function () {
      this.editor.addCommand(
        "testDialog",
        new CKEDITOR.dialogCommand("testDialog")
      );
    ...
    "test behavior of a bidi field containing only the marker": function () {
      ...
      bot.dialog("testDialog", function (dialog) {
        ...
        var realValue =
          CKEDITOR.ui.dialog.uiElement.prototype.getValue.call(field);
        assert.areSame(
          "",
          realValue,
          "input real value does not contain RTL marker"
        );
        ...
        assert.areSame(
          "",
          field.getValue(),
          "after getValue() is called, value does not contain RTL marker"
        );
    ...
    "test behavior of the setDirectionMarker() method": function () {
      ...
      bot.dialog("testDialog", function (dialog) {
        ...
        msg =
          "when direction is set to rtl, direction attributes exist and contain rtl";
        dir = "rtl";
        field.setDirectionMarker(dir);
        assert.areSame(
          dir,
          inputElement.getAttribute("data-cke-dir-marker"),
          msg
        );
        ...
        msg =
          "when direction is set to ltr, direction attributes exist and contain ltr";
        dir = "ltr";
        field.setDirectionMarker(dir);
        assert.areSame(
          dir,
