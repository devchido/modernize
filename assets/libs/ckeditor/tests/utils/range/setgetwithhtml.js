...
(function () {
  ...
  function s(
    ...
    return function () {
      ...
      if (startAddress && startOffset != null)
        startContainer = doc.getByAddress(
          playgroundAddress.concat(startAddress)
        );
      ...
      if (startContainer && endContainer) {
        assert.isTrue(
          startContainer.equals(range.startContainer),
          "startContainer"
        );
      ...
      if (typeof htmlWithRangeOrCallback == "function") {
        ...
        assert.areSame(
          html.replace(/[\{\}\[\]]/g, ""),
          bender.tools.fixHtml(playground.getHtml(), 1, 1),
          "Markers cleaned - setRange"
        );
        if (htmlWithRangeOrCallback == null) htmlWithRangeOrCallback = html;
        assert.areSame(
          htmlWithRangeOrCallback,
          bender.tools.fixHtml(getRange(playground, range), 1, 1),
          "getRange"
        );
      }
    };
  ...
  bender.test({
    ...
    "test element #11": s(
      "<ol>[<li>x</li><li>y<ul><li>z</li>]<li>a</li></ul></li></ol>",
      false,
      [0],
      0,
      [0, 1, 1],
