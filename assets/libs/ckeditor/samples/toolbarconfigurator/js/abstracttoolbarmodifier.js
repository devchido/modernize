  ...
  afterInit: function (editor) {
    editor.addMode("wysiwyg", function (callback) {
      var editingBlock = CKEDITOR.dom.element.createFromHtml(
        '<div class="cke_wysiwyg_div cke_reset" hidefocus="true"></div>'
      );
      ...
      editingBlock.detach = CKEDITOR.tools.override(
        editingBlock.detach,
        function (org) {
          return function () {
...
(function () {
   ...
   * @private
   */
  AbstractToolbarModifier.prototype._onInit = function (
    callback,
    actualConfig
  ) {
    ...
    if (!this.actualConfig.toolbarGroups && !this.actualConfig.toolbar) {
      this.actualConfig.toolbarGroups = getDefaultToolbarGroups(
        this.editorInstance
      );
    }
  ...
  AbstractToolbarModifier.prototype._toggleEditable = function () {
    if (this.isEditableVisible) this._hideEditable();
    else this._showEditable();
  };
   ...
   * @private
   */
  AbstractToolbarModifier.prototype._createEditor = function (
    doRefresh,
    configOverrides
  ) {
    var that = this;
  ...
  AbstractToolbarModifier.prototype._createToolbarBtn = function (cfg) {
    var btnText = typeof cfg.text === "string" ? cfg.text : cfg.text.inactive,
      btn = ToolbarConfigurator.FullToolbarEditor.createButton(
        btnText,
        cfg.cssClass
      );
  ...
  AbstractToolbarModifier.prototype._fixGroups = function (config) {
    ...
    for (var i = 0; i < max; i += 1) {
      ...
      if (
        CKEDITOR.tools.indexOf(currentGroup.groups, currentGroup.name) == -1
      ) {
        this.editorInstance.ui.addToolbarGroup(
          currentGroup.name,
          currentGroup.groups[currentGroup.groups.length - 1],
          currentGroup.name
        );
  ...
  AbstractToolbarModifier.extendPluginsConfig = function (config) {
    ...
    config.extraPlugins =
      (extraPlugins ? extraPlugins + "," : "") + "toolbarconfiguratorarea";
  };
})();
