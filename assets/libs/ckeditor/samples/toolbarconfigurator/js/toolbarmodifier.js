...
(function () {
    ...
    this.toolbarButtons = [
      {
        ...
        clickCallback: function (button, buttonDefinition) {
          ...
          button[button.hasClass(className) ? "removeClass" : "addClass"](
            className
          );
  ...
  ToolbarModifier.prototype = Object.create(
  ...
  ToolbarModifier.prototype.getActualConfig = function () {
    ...
    if (copy.toolbarGroups) {
      ...
      for (var i = 0; i < max; i += 1) {
        ...
        copy.toolbarGroups[i] =
          ToolbarModifier.parseGroupToConfigValue(currentGroup);
      }
    }
   ...
   * @private
   */
  ToolbarModifier.prototype._onInit = function (
    callback,
    config,
    ...
    if (typeof callback === "function") callback(this.mainContainer);
  };
  ...
  ToolbarModifier.prototype._showConfig = function () {
    ...
    if (actualConfig.toolbarGroups) {
      ...
      var groups = prepareGroups(
        actualConfig.toolbarGroups,
        this.cfg.trimEmptyGroups
      );
      ...
      for (var i = 0; i < max; i++) {
        ...
        if (trimEmptyGroups) {
          ...
          while (max2--) {
            ...
            if (
              ToolbarModifier.getTotalSubGroupButtonsNumber(
                subgroup,
                that.fullToolbarEditor
              ) === 0
            ) {
        ...
        if (!(trimEmptyGroups && group.groups.length === 0)) {
          groups.push(
            AbstractToolbarModifier.stringifyJSONintoOneLine(group, {
              addSpaces: true,
              noQuotesOnKey: true,
              singleQuotes: true,
            })
    ...
    var content = [
      ...
      cfg.toolbarGroups
        ? "\tconfig.toolbarGroups = [" + cfg.toolbarGroups + "\n\t];"
        : "",
      cfg.removeButtons ? "\n\n" : "",
      cfg.removeButtons
        ? "\tconfig.removeButtons = '" + cfg.removeButtons + "';"
        : "",
      "\n};",
      "</textarea>",
    ].join("");
  ...
  ToolbarModifier.prototype._createModifier = function () {
    ...
    this.modifyContainer.on("click", function (e) {
      var origEvent = e.data.$,
        target = new CKEDITOR.dom.element(
          origEvent.target || origEvent.srcElement
        ),
        relativeGroupOrSeparatorLi =
          ToolbarModifier.getGroupOrSeparatorLiAncestor(target);
      ...
      // link clicked
        ...
        if (origEvent.preventDefault) origEvent.preventDefault();
        else origEvent.returnValue = false;

        var result = that._handleAnchorClicked(target.$);

        if (result && result.action == "remove") return;
      }
      ...
      if (that.cachedActiveElement) that.cachedActiveElement.focus();
    });
  ...
  ToolbarModifier.prototype.showToolbarBtnsByGroupName = function (groupName) {
    ...
    for (var i = 0; i < max; i += 1) {
      ...
      else currentBtn.addClass("hidden");
    }
  };
  ...
  ToolbarModifier.prototype._setActiveElement = function (type, name) {
    ...
    if (this.currentActive) this.currentActive.elem.removeClass("active");
    ...
    var liElem = this.mainContainer.findOne(
      'ul[data-type=table-body] li[data-type="' +
        type +
        '"][data-name="' +
        name +
        '"]'
    ...
    if (type == "group") this._highlightGroup(name);

    if (type == "separator") this._dehighlightActiveToolGroup();
  };
  ...
  ToolbarModifier.prototype._highlightGroup = function (name) {
    if (!this.editorInstance.container) return;

    var foundBtnName = this.getFirstEnabledButtonInGroup(name),
      foundBtn = this.editorInstance.container.findOne(
        ".cke_button__" + foundBtnName + ", .cke_combo__" + foundBtnName
      );
    ...
    if (foundBtn) {
      var btnToolbar = ToolbarModifier.getFirstAncestor(
        foundBtn,
        function (ancestor) {
      ...
      if (btnToolbar) btnToolbar.addClass("active");
    }
  };
   ...
   * @return {String|null}
   */
  ToolbarModifier.prototype.getFirstEnabledButtonInGroup = function (
    groupName
  ) {
   ...
   * @returns {String|null}
   */
  ToolbarModifier.prototype.getFirstEnabledButtonInSubgroup = function (
    subgroupName
  ) {
    ...
    for (var i = 0; i < max; i += 1) {
      ...
      if (!this.isButtonRemoved(currBtnName)) return currBtnName;
    }
  ...
  ToolbarModifier.prototype._handleCheckboxClicked = function (checkbox) {
    ...
    if (aboutToAddToRemoved) this._addButtonToRemoved(elementName);
    else this._removeButtonFromRemoved(elementName);
  };
  ...
  ToolbarModifier.prototype._handleAnchorClicked = function (anchor) {
    var anchorDOM = new CKEDITOR.dom.element(anchor),
      ...
      nearestLi =
        direction === "up" ? relativeLi.getPrevious() : relativeLi.getNext(),
      groupName,
      subgroupName,
      newIndex;

    // nothing to do
    if (anchorDOM.hasClass("disabled")) return null;
    ...
    if (!anchorDOM.hasClass("move") || !nearestLi) return { action: null };
    ...
    var found;
    while (
      (nextLi = direction === "up" ? nextLi.getPrevious() : nextLi.getNext())
    ) {
    ...
    if (!found) {
      var selector =
        '[data-direction="' + (direction === "up" ? "down" : "up") + '"]';
  ...
  ToolbarModifier.prototype._refreshMoveBtnsAvalibility = function () {
    var that = this,
      disabledBtns = this.mainContainer.find(
        "ul[data-type=table-body] li > p > span > button.move.disabled"
      );
    ...
    disableElementsInLists(
      this.mainContainer.find("ul[data-type=table-body] > li > ul")
    );
  };
  ...
  ToolbarModifier.prototype._disableElementsInList = function (ul) {
    ...
    if (firstDisabled)
      var firstDisabledBtn = firstDisabled.findOne(
        'p button[data-direction="up"]'
      );

    if (lastDisabled)
      var lastDisabledBtn = lastDisabled.findOne(
  ...
  ToolbarModifier.prototype._addSeparator = function () {
    var separatorIndex = this._determineSeparatorToAddIndex(),
      ...
      domSeparator = CKEDITOR.dom.element.createFromHtml(
        ToolbarModifier.getToolbarSeparatorString(separator)
      );

    this.actualConfig.toolbarGroups.splice(separatorIndex, 0, separator);

    domSeparator.insertBefore(
      this.modifyContainer
        .findOne("ul[data-type=table-body]")
        .getChild(separatorIndex)
    );
  ...
  ToolbarModifier.prototype._removeSeparator = function (name) {
    var separatorIndex = CKEDITOR.tools.indexOf(
      this.actualConfig.toolbarGroups,
      function (group) {
  ...
  ToolbarModifier.prototype._determineSeparatorToAddIndex = function () {
    ...
    else groupLi = this.currentActive.elem.getAscendant("li");
   ...
   * @private
   */
  ToolbarModifier.prototype._moveElement = function (
    elementsArray,
    elementIndex,
    direction
  ) {
    ...
    if (this.emptyVisible)
      ...
      nextIndex = ToolbarModifier.getFirstElementIndexWith(
        elementsArray,
        elementIndex,
        direction,
   ...
   * @private
   */
  ToolbarModifier.prototype._moveSubgroup = function (
    direction,
    groupName,
    subgroupName
  ) {
  ...
  ToolbarModifier.prototype._calculateTotalBtns = function () {
    ...
    while (i--) {
      var currentGroup = groups[i],
        totalBtns = ToolbarModifier.getTotalGroupButtonsNumber(
          currentGroup,
  ...
  ToolbarModifier.prototype._toolbarConfigToListString = function () {
    ...
    for (var i = 0; i < max; i += 1) {
      ...
      else listString += this._getToolbarGroupString(currentGroup);
    }
  ...
  ToolbarModifier.prototype._getToolbarGroupString = function (group) {
    ...
    for (var i = 0; i < max; i += 1) {
      var currentSubgroup = subgroups[i],
        subgroupBtns =
          this.fullToolbarEditor.buttonsByGroup[currentSubgroup.name];

      groupString += this._getToolbarSubgroupString(
        currentSubgroup,
        subgroupBtns
      );
    }
  ...
  ToolbarModifier.getFirstAncestor = function (element, checker) {
    ...
    while (i--) {
      if (checker(ancestors[i])) return ancestors[i];
    }
   ...
   */
  ToolbarModifier.getFirstElementIndexWith = function (
    array,
    i,
    direction,
    conditionChecker
    ...
    function whileChecker() {
      var result;
      if (direction === "up") result = i--;
      else result = ++i < array.length;

      return result;
    }

    while (whileChecker()) {
      if (conditionChecker(array[i])) return i;
    }
  ...
  ToolbarModifier.moveTo = function (offset, array, index) {
    ...
    if (index !== -1) element = array.splice(index, 1)[0];
   ...
   * @returns {Number}
   */
  ToolbarModifier.getTotalSubGroupButtonsNumber = function (
    subgroup,
    fullToolbarEditor
  ) {
   ...
   * @returns {Number}
   */
  ToolbarModifier.getTotalGroupButtonsNumber = function (
    group,
    fullToolbarEditor
  ) {
    var total = 0,
    ...
    for (var i = 0; i < max; i += 1)
      total += ToolbarModifier.getTotalSubGroupButtonsNumber(
        subgroups[i],
        fullToolbarEditor
      );
   ...
   * @private
   */
  ToolbarModifier.prototype._getToolbarSubgroupString = function (
    subgroup,
    groupBtns
  ) {
    var subgroupString = "";
  ...
  ToolbarModifier.prototype._getConfigButtonName = function (buttonName) {
    ...
    var name;
    for (name in items) {
      if (items[name].name == buttonName) return name;
    }
  ...
  ToolbarModifier.prototype.getButtonString = function (button) {
    ...
    return [
      '<li data-tab="true" data-type="button" data-name="',
      this._getConfigButtonName(button.name),
      '">',
      '<label title="',
      button.label,
      '" >',
  ...
  ToolbarModifier.getToolbarElementPreString = function (group) {
    ...
    return [
      ...
      name == "row separator"
        ? '<button title="Remove element" data-tab="true" class="remove icon-trash"></button>'
        : "",
      name,
      "</span>",
      "</p>",
  ...
  ToolbarModifier.evaluateToolbarGroupsConfig = function (cfg) {
    cfg = (function (cfg) {
      ...
      if (
        ...
        typeof config.toolbarGroups.length === "number"
      ) {
