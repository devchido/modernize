...
(function () {
  ...
  FullToolbarEditor.prototype.init = function (callback) {
    ...
    this.editorInstance.once("configLoaded", function (e) {
      ...
      e.editor.once("loaded", function () {
        that.buttons = FullToolbarEditor.toolbarToButtons(
          that.editorInstance.toolbar
        );
        ...
        if (typeof callback === "function") callback(that.buttons);
      });
    });
  };
  ...
  FullToolbarEditor.filter = function (arr, checker) {
    ...
    for (var i = 0; i < max; i += 1) {
      if (checker(arr[i])) result.push(arr[i]);
    }
  ...
  FullToolbarEditor.map = function (enumerable, modifier) {
    ...
    if (CKEDITOR.tools.isArray(enumerable)) {
      ...
      for (var i = 0; i < max; i += 1) result.push(modifier(enumerable[i]));
    } else {
      result = {};

      for (var key in enumerable) result[key] = modifier(enumerable[key]);
    }
  ...
  FullToolbarEditor.createToolbarButton = function (button) {
    var $button = new CKEDITOR.dom.element("a"),
      icon = FullToolbarEditor.createIcon(
        button.name,
        button.icon,
        button.command
      );
    ...
    if (button instanceof CKEDITOR.ui.button) {
      ...
      button instanceof CKEDITOR.ui.richCombo
    ) {
  ...
  FullToolbarEditor.createIcon = function (name, icon, command) {
    var iconStyle = CKEDITOR.skin.getIconStyle(
      name,
    ...
    iconStyle =
      iconStyle ||
      CKEDITOR.skin.getIconStyle(command, CKEDITOR.lang.dir == "rtl");
  ...
  FullToolbarEditor.groupToButtons = function (group) {
    ...
    for (var i = 0; i < max; i += 1) {
      ...
      if (
        item instanceof CKEDITOR.ui.button ||
        (CKEDITOR.ui.richCombo && item instanceof CKEDITOR.ui.richCombo)
      ) {
