...
(function () {
  ...
  ToolbarTextModifier.prototype = Object.create(
    AbstractToolbarModifier.prototype
  );
  ...
  ToolbarTextModifier.prototype._createModifier = function (cfg) {
    ...
    function hint(cm) {
      ...
      var unused = that.getUnusedButtonsArray(
      ...
      if (prevToken.string === "{") unused = ["name"];
    ...
    function setupData(cm, character) {
      ...
      result["char"] =
        character || result.tok.string.charAt(result.tok.string.length - 1);

      // Getting string between begin of line and cursor.
      var curLineTillCur = cm.getRange(
        CodeMirror.Pos(result.cur.line, 0),
        result.cur
      );
      ...
      // TO:
      // R' , [ :smeti{
      currLineTillCurReversed = currLineTillCurReversed.replace(
        /(['|"]\w*['|"])/g,
        ""
      );
      ...
      if (result.charsBetween) {
        ...
        result.charsBetween = result.charsBetween[1]
          .split("")
          .reverse()
          .join("");
      }
    ...
    this.codeContainer.on("change", function () {
      ...
      if (value !== null) {
        that.actualConfig.toolbar = value.toolbar
          ? value.toolbar
          : that.actualConfig.toolbar;
  ...
  ToolbarTextModifier.prototype._fillHintByUnusedElements = function () {
    ...
    var unusedElements = FullToolbarEditor.map(unused, function (elem) {
      var buttonsList = FullToolbarEditor.map(
        elem.buttons,
        function (buttonName) {
    ...
    this.hintContainer.setHtml(
      header + "<dl>" + listHeader + unusedElements + "</dl>"
    );
  };
   ...
   * @returns {String}
   */
  ToolbarTextModifier.prototype.getToolbarGroupByButtonName = function (
    buttonName
  ) {
   ...
   * @returns {Array}
   */
  ToolbarTextModifier.prototype.getUnusedButtonsArray = function (
    toolbar,
    sorted,
    prefix
  ) {
    ...
    var providedElements =
        ToolbarTextModifier.mapToolbarCfgToElementsList(toolbar),
    ...
    allElements = FullToolbarEditor.filter(allElements, function (elem) {
      var isSeparator = elem === "-",
        matchPrefix =
          prefix === undefined ||
          elem.toLowerCase().indexOf(prefix.toLowerCase()) === 0;

      return !isSeparator && matchPrefix;
    });

    var elementsNotUsed = FullToolbarEditor.filter(
      allElements,
      function (elem) {
    ...
    if (sorted) elementsNotUsed.sort();
  ...
  ToolbarTextModifier.mapToolbarCfgToElementsList = function (toolbar) {
    ...
    for (var i = 0; i < max; i += 1) {
      if (!toolbar[i] || typeof toolbar[i] === "string") continue;

      elements = elements.concat(
        FullToolbarEditor.filter(toolbar[i].items, checker)
      );
    }
  ...
  ToolbarTextModifier.prototype._setupActualConfig = function (cfg) {
    ...
    if (CKEDITOR.tools.isArray(cfg.toolbar)) return;

    // if toolbar group not present, we need to pick them from full toolbar instance
    if (!cfg.toolbarGroups)
      cfg.toolbarGroups =
        this.fullToolbarEditor.getFullToolbarGroupsConfig(true);

    this._fixGroups(cfg);

    cfg.toolbar = this._mapToolbarGroupsToToolbar(
      cfg.toolbarGroups,
   ...
   * @private
   */
  ToolbarTextModifier.prototype._mapToolbarGroupsToToolbar = function (
    toolbarGroups,
    removedBtns
  ) {
    ...
    while (i--) {
      var mappedSubgroup = this._mapToolbarSubgroup(
        toolbarGroups[i],
        removedBtns
      );
      ...
      if (
        CKEDITOR.tools.isArray(mappedSubgroup) &&
        mappedSubgroup.length === 0
      ) {
        toolbarGroups.splice(i, 1);
        continue;
      }

      if (typeof mappedSubgroup == "string") toolbarGroups[i] = mappedSubgroup;
      else {
        toolbarGroups[i] = {
   ...
   * @private
   */
  ToolbarTextModifier.prototype._mapToolbarSubgroup = function (
    group,
    removedBtns
  ) {
    var totalBtns = 0;
    ...
    for (var i = 0; i < max; i += 1) {
      ...
      var buttons =
        this.fullToolbarEditor.buttonsByGroup[
          typeof currSubgroup === "string" ? currSubgroup : currSubgroup.name
        ] || [];
   ...
   * @private
   */
  ToolbarTextModifier.prototype._mapButtonsToButtonsNames = function (
    buttons,
    removedBtns
  ) {
    var i = buttons.length;
    while (i--) {
      ...
      if (typeof currBtn === "string") {
        ...
        camelCasedName = this.fullToolbarEditor.getCamelCasedButtonName(
          currBtn.name
        );
      }
    ...
    try {
      ...
      (function () {
        var CKEDITOR = Function(
          "var CKEDITOR = {}; " + val + "; return CKEDITOR;"
        )();
      ...
      while (i--) if (!parsed.toolbar[i]) parsed.toolbar.splice(i, 1);
    } catch (e) {
      parsed = null;
    }
  ...
  ToolbarTextModifier.prototype.mapToolbarToToolbarGroups = function (toolbar) {
    ...
    for (var i = 0; i < max; i++) {
      ...
      for (var j = 0; j < max2; j++) {
        ...
        var buttons = (usedGroups[groupName].buttons =
          usedGroups[groupName].buttons || {});
    ...
    removeButtons = prepareRemovedButtons(
      usedGroups,
    ...
    function prepareRemovedButtons(usedGroups, buttonNames) {
      ...
      for (var groupName in usedGroups) {
        ...
        removed = removed.concat(
          removeStuffFromArray(allButtonsInGroup, Object.keys(group.buttons))
        );
      }
