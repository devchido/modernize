...
module.exports = function (grunt) {
  grunt.registerTask(
    "plugin-install",
    "Installs external plugin to the plugins/ directory.",
    function (pluginName) {
      ...
      if (isPluginInstalled(pluginName)) {
        grunt.log.writeln(
          'The "' + pluginName + '" plugin is already installed.'
        );
        return;
      }
  ...
  grunt.registerTask(
    "plugin-uninstall",
    "Uninstalls external plugin from the plugins/ directory.",
    function (pluginName) {
      var installationDir = grunt.config.get("plugin.installationDir"),
        installationPluginDir = installationDir + pluginName;
      ...
      if (!isSymlink(installationPluginDir)) {
        grunt.fail.warn(
          'The "' +
            installationPluginDir +
            '" directory is not a symlink so plugin cannot be removed.'
        );
      }
  ...
  grunt.registerTask(
    "plugin-update",
    "Updates plugin to the commit ref/branch/tag specified in package.json.",
    function (pluginName) {
      if (!pluginName) {
        grunt.fail.fatal(
          "Name of the plugin must be specified.\n" +
      ...
      if (!pluginConfig) {
        grunt.fail.warn(
          'Plugin "' +
            pluginName +
            '" is not configured in the package.json file.'
        );
      }
      ...
      try {
        ...
        if (gitStatus()) {
          grunt.log.error(
            'Cannot update the plugin "' +
              pluginName +
              '" due to messed status ' +
        ...
        grunt.log.writeln(
          'Fetching changes from the remote "' + pluginConfig.remote + '"...'
        );
        ...
        grunt.log.error(e.message);
        grunt.fail.fatal(
          'Error when updating the plugin "' + pluginName + '".'
        );
      } finally {
        shjs.popd();
      }
  ...
  grunt.registerTask(
    "plugins-list",
    "Lists all installed external plugins.",
    function () {
      assertExternalDir();
  ...
  grunt.registerTask(
    "plugins-install",
    "Installs all external plugins specified in package.json to the plugins/ directory.",
    function () {
      var pluginsToInstall = getPluginsNamesFromConfig();

      if (!pluginsToInstall.length) {
        grunt.log.writeln(
          "There are no plugins configured in the package.json file."
        );
        return;
      }

      grunt.log.writeln(
        "Installing plugins: " + pluginsToInstall.join(", ") + "..."
      );
      grunt.task.run(
        pluginsToInstall.map(function (pluginName) {
  ...
  grunt.registerTask(
    "plugins-uninstall",
    "Uninstalls all external plugins from the plugins/ directory.",
    function () {
      assertExternalDir();
      ...
      grunt.log.writeln(
        "Uninstalling plugins: " + pluginsToUninstall.join(", ") + "..."
      );
      grunt.task.run(
        pluginsToUninstall.map(function (pluginName) {
  ...
  grunt.registerTask(
    ...
    "Updates installed external plugins to the versions specified in package.json. " +
      "When used with the :install option the task will also install the plugins specified in package.json.",
    function (install) {
      assertExternalDir();
      ...
      if (!installedPlugins.length) {
        ...
        grunt.log.writeln(
          "Updating plugins: " + installedPlugins.join(", ") + "..."
        );
      }

      grunt.task.run(
        installedPlugins.map(function (pluginName) {
