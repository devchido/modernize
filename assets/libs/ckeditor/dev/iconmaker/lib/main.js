...
function extractIcon(that, iconPath, row, column, stats) {
  ...
  tmp.file(
    { prefix: "iconmaker", postfix: "png" },
    function (error, tmpIconPath) {
      ...
      exec(command, function () {
        ...
        compareIcons(iconPath, tmpIconPath, function (areSame) {
          console.log(
            "[i] Checking for changes of (%s,%s): %s",
            row,
            column,
            iconPath
          );
          ...
          if (!areSame) updateIcon(iconPath, tmpIconPath, dirName, cold);

          deferred.resolve({
            total: stats.total + 1,
            updated: stats.updated + !areSame,
          });
        });
      });
    }
...
function updateIcon(iconPath, tmpIconPath, dirName, cold) {
  ...
  !cold &&
    fs.createReadStream(tmpIconPath).pipe(fs.createWriteStream(iconPath));
}
...
function compareIcons(icon1, icon2, callback) {
  ...
  png.decode(icon1, function (pixels1) {
    png.decode(icon2, function (pixels2) {
      if (pixels1.length != pixels2.length) return callback(false);

      for (var i = pixels1.length; i--; ) {
        if (pixels1[i] !== pixels2[i]) return callback(false);
      }
