  ...
  https.get("https://unicode.org/emoji/charts/emoji-list.html", (response) => {
    ...
    response.on("end", () => {
      ...
      for (const row of rows) {
        ...
        const [full, id, symbol, name, keywords] =
          /class=\'rchars\'>(\d+)[\s\S]*<img alt=\'([^\']+?)\'[\s\S]*class=\'name\'>([^<]+)[\s\S]*class=\'name\'>([^<]*)/.exec(
            row
          );
        ...
        ret.push({
          id:
            ":" +
            name.replace(/ /g, "_").replace(/[:,]/g, "").toLowerCase() +
            ":",
          symbol,
          group: currentGroup,
      ...
      fs.writeFile(
        path.join(__dirname, "..", "..", "plugins", "emoji", "emoji.json"),
        JSON.stringify(ret),
        (err) => {
          if (err) {
