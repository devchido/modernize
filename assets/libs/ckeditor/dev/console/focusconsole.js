...
(function () {
    ...
    panels: [
      {
        ...
        refresh: function (editor) {
          ...
          return {
            header:
              "Focus (" + (focusManager.hasFocus ? "focused" : "blurred") + ")",
            activeFM: focusManager.hasFocus
              ? active2Str(focusManager.currentActive)
              : "-",
            activeInner: editor.document
              ? active2Str(editor.document.getActive())
              : "-",
        ...
        refreshOn: function (editor, refresh) {
          ...
          editor.on("contentDom", function () {
            editor
              .editable()
              .attachListener(editor.document, "mouseup", function () {
  ...
  function element2Str(element) {
    ...
    if (str.length > 40) str = str.slice(0, 35) + "...>";
  ...
  function active2Str(active) {
    if (active instanceof CKEDITOR.dom.window)
      return (
        "window (" +
        element2Str(new CKEDITOR.dom.element(active.$.document.body)) +
        ")"
      );
    else return element2Str(active);
  }
})();
