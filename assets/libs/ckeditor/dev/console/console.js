...
var CKCONSOLE = (function () {
  var that = {
    ...
    create: function (consoleName, config) {
      var editor = config.editor,
        definition = CKEDITOR.tools.prototypedCopy(
          this.definitions[consoleName]
        );

      if (typeof editor == "string") editor = CKEDITOR.instances[editor];
  ...
  function createConsole(editor, definition, config) {
    ...
    for (var i = 0; i < definition.panels.length; ++i) {
      ...
      panel = new that.panels[panelDefinition.type](
        editor,
        editorPanel,
        panelDefinition
      );

      if (panel.refreshOn) setupPanelRefresh(editor, panel);
  ...
  function createContainer() {
    ...
    if (!container) {
      container = that.container = fromHtml(
        '<div class="ckconsole cke_reset_all"></div>'
      );
  ...
  function createEditorPanel(editor, container, definition, config) {
    ...
    el.findOne(".ckconsole_header").on("click", function () {
      if (el.hasClass("ckconsole_folded")) el.removeClass("ckconsole_folded");
      else el.addClass("ckconsole_folded");
    });

    if (config.folded) el.addClass("ckconsole_folded");
  ...
  function fromHtml(html, data) {
    if (html instanceof CKEDITOR.template) html = html.output(data);
  ...
  function panelBox(editor, editorPanel, panelDefinition) {
    var container = createPanel(editorPanel, {
      ...
      content:
        '<div class="ckconsole_content">' + panelDefinition.content + "</div>",
    });
...
CKCONSOLE.addEventPanel = function (evtName, types, mapping) {
  ...
  for (var i = 0; i < types.length; i++) {
    content +=
      "<li>" +
      types[i] +
      ': <span class="ckconsole_value" data-value="' +
      types[i] +
      '"></span></li>';
  }
  content += "</ul>";
