...
(function () {
  ...
  test("Options setup", function () {
    ...
    equal(
      options.highlightSelectedDate,
      true,
      "highlightSelectedDate default ok"
    );
    ...
    equal(
      ...
      null,
      "onSelectedDateChanged default ok"
    );
    equal(
      options.selectedDate.format("YYYY-MM-DD"),
      moment().startOf("day").format("YYYY-MM-DD"),
      "selectedDate default ok"
    );
    equal(
      options.selectedDateFormat,
      "YYYY-MM-DD",
      "selectedDateFormat default ok"
    );
    ...
    equal(
      ...
      "dddd<br/>Do, MMMM YYYY",
      "textSelected default ok"
    );
    ...
    equal(
      options.highlightSelectedDate,
      opts.highlightSelectedDate,
      "highlightSelectedDate set ok"
    );
    ...
    equal(
      typeof options.onSelectedDateChanged,
      "function",
      "onSelectedDateChanged set ok"
    );
    equal(
      options.selectedDate.format("YYYY-MM-DD"),
      moment(opts.selectedDate, opts.selectedDateFormat)
        .startOf("day")
        .format("YYYY-MM-DD"),
      "selectedDate set ok"
    );
    equal(
      options.selectedDateFormat,
      opts.selectedDateFormat,
      "selectedDateFormat set ok"
    );
    equal(
      options.selectedItemWidth,
      opts.selectedItemWidth,
      "selectedItemWidth set ok"
    );
    ...
    equal(
      options.showStartOfWeek,
      opts.showStartOfWeek,
      "showStartOfWeek set ok"
    );
    equal(options.size, "sm", "size set ok");
    equal(options.startOfWeek, opts.startOfWeek, "startOfWeek set ok");
    equal(
      options.startOfWeekFormat,
      opts.startOfWeekFormat,
      "startOfWeekFormat set ok"
    );
    ...
    equal(
      ...
      moment(opts.startDate, opts.startDateFormat)
        .startOf("day")
        .format("YYYY-MM-DD"),
      "startDate set ok"
    );
    equal(
      options.startDateFormat,
      opts.startDateFormat,
      "startDateFormat set ok"
    );
    equal(
      options.endDate.format("YYYY-MM-DD"),
      moment(opts.endDate, opts.endDateFormat)
        .startOf("day")
        .format("YYYY-MM-DD"),
      "endDate set ok"
    );
  ...
  test("Highlight selected date", function () {
    init();
    equal(
      $(".dp-selected").attr("data-moment"),
      moment().format("YYYY-MM-DD"),
      "Correct date is highlighted when set to true"
    );
  ...
  test("Off days", function () {
    function dayOfWeek(obj, options) {
      return moment(
        $(obj).attr("data-moment"),
        options.selectedDateFormat
      ).format(options.offDaysFormat);
    }
    ...
    ok(
      offDaysCorrect(getOptions(init())),
      "Correct off days shown when using default"
    );
    ok(
      offDaysCorrect(getOptions(init({ offDays: "Mon" }))),
      "Correct off days shown when using a single custom value"
    );
    ok(
      offDaysCorrect(getOptions(init({ offDays: "Mon,Wed,Fri" }))),
      "Correct off days shown when using multiple custom values"
    );
    ok(
      offDaysCorrect(getOptions(init({ offDays: "0,1", offDaysFormat: "d" }))),
      "Correct off days shown when using custom offDaysFormat"
    );
    ok(
      offDaysCorrect(getOptions(init({ offDays: undefined }))),
      "Correct off days shown when offDays set to undefined"
    );
    ok(
      offDaysShown(getOptions(init())),
      "Off days shown when set as default showOffDays"
    );
    ok(
      offDaysShown(getOptions(init({ showOffDays: true }))),
      "Off days shown when showOffDays set to true"
    );
    ok(
      !offDaysShown(getOptions(init({ showOffDays: false }))),
      "No off days shown when showOffDays set to false"
    );
  });
  ...
  test("Start of week divider", function () {
    function dayOfWeek(obj, options) {
      return moment(
        $(obj).attr("data-moment"),
        options.selectedDateFormat
      ).format(options.startOfWeekFormat);
    }
    ...
    ok(
      startOfWeekCorrect(getOptions(init())),
      "Correct start of week shown when using default"
    );
    ok(
      startOfWeekCorrect(getOptions(init({ startOfWeek: "Wed" }))),
      "Correct start of week shown when using a single custom value"
    );
    ok(
      startOfWeekCorrect(getOptions(init({ startOfWeek: "Fri,Wed" }))),
      "Correct start of week shown when using multiple custom values"
    );
    ok(
      startOfWeekCorrect(
        getOptions(init({ startOfWeek: "0", startOfWeekFormat: "d" }))
      ),
      "Correct start of week shown when using custom startOfWeekFormat"
    );
    ok(
      startOfWeekCorrect(getOptions(init({ startOfWeek: undefined }))),
      "Correct start of week shown when startOfWeek set to undefined"
    );
    ok(
      startOfWeekShown(getOptions(init())),
      "Start of week divider shown when set as default showStartOfWeek"
    );
    ok(
      startOfWeekShown(getOptions(init({ showStartOfWeek: true }))),
      "Start of week divider shown when showStartOfWeek set to true"
    );
    ok(
      !startOfWeekShown(getOptions(init({ showStartOfWeek: false }))),
      "No start of week divider shown when showStartOfWeek set to false"
    );
  });
  ...
  test("Selected date", function () {
    ...
    equal(
      $(".dp-selected").attr("data-moment"),
      moment().format(options.selectedDateFormat),
    ...
    equal(
      $(".dp-selected").attr("data-moment"),
      moment("2013-10-05", options.selectedDateFormat).format(
        options.selectedDateFormat
      ), //'2013-10-05',
    ...
    equal(
      $(".dp-selected").attr("data-moment"),
      moment("05-10-2013", options.selectedDateFormat).format(
        options.selectedDateFormat
      ), //'05-10-2013',
    ...
    equal(
      $(".dp-selected").attr("data-moment"),
      moment("2013-10-06", options.selectedDateFormat).format(
        options.selectedDateFormat
      ), //'2013-10-06',
    ...
    options = getOptions(
      init({ selectedDateFormat: "DD-MM-YYYY" }).datepaginator(
        "setSelectedDate",
        "06-10-2013"
      )
    );
    equal(
      $(".dp-selected").attr("data-moment"),
      moment("06-10-2013", options.selectedDateFormat).format(
        options.selectedDateFormat
      ), //'06-10-2013',
    ...
    options = getOptions(
      init().datepaginator("setSelectedDate", ["06-10-2013", "DD-MM-YYYY"])
    );
    equal(
      $(".dp-selected").attr("data-moment"),
      moment("06-10-2013", "DD-MM-YYYY").format(options.selectedDateFormat), //'2013-10-06',
      "Correct selectedDate when set via setSelectedDate method using custom selectedDateFormat set in method arguments"
    );

    options = getOptions(
      init({ startDate: "2013-12-01", selectedDate: "2013-11-01" })
    );
    equal(
      $(".dp-selected").attr("data-moment"),
      options.startDate.format(options.selectedDateFormat),
      "Correctly overrides selectedDate when invalid : selectedDate < startDate"
    );

    options = getOptions(
      init({ endDate: "2013-12-31", selectedDate: "2014-01-01" })
    );
    equal(
      $(".dp-selected").attr("data-moment"),
      options.endDate.format(options.selectedDateFormat),
  ...
  test("Item text", function () {
    function formatText(el, format) {
      return moment(el.attr("data-moment"))
        .format(format)
        .replace("<br/>", "")
        .replace("<br>", "");
    }
    ...
    equal(
      itemEl.text(),
      formatText(itemEl, options.text),
      "Item text correct when using default"
    );
    equal(
      selectedItemEl.text(),
      formatText(selectedItemEl, options.textSelected),
      "Selected item text correct when using default value"
    );

    // then with custom values
    options = getOptions(
      init({ text: "Do<br>ddd", textSelected: "Do, MMMM YYYY<br>dddd" })
    );
    itemEl = $(".dp-item:first");
    selectedItemEl = $(".dp-selected");
    equal(
      itemEl.text(),
      formatText(itemEl, "Do<br>ddd"),
      "Item text correct when using custom value"
    );
    equal(
      selectedItemEl.text(),
      formatText(selectedItemEl, "Do, MMMM YYYY<br>dddd"),
  ...
  test("Size", function () {
    ...
    ok(
      !$(".dp-item-sm").length || !$(".dp-item-lg").length,
      "Item set correct when using default"
    );
    ok(
      !$(".dp-nav-sm").length || !$(".dp-nav-lg").length,
      "Nav item set correct when using default"
    );
    init({ size: "normal" });
    ok(
      $(".datepaginator"),
      "Correctly set .datepaginator when size set as normal "
    );
    ok(
      !$(".dp-item-sm").length || !$(".dp-item-lg").length,
      "Item set correct when size set as normal"
    );
    ok(
      !$(".dp-nav-sm").length || !$(".dp-nav-lg").length,
      "Nav item set correct when size set as normal"
    );
    init({ size: "small" });
    ok(
      $(".datepaginator-sm"),
      "Container set to .datepaginator-sm when size set to small"
    );
    ...
    ok(
      $(".datepaginator-lg"),
      "Container set to .datepaginator-lg when size set to large"
    );
  ...
  test("Square Edges", function () {
    ...
    ok(
      !$(".dp-nav-sqauare-edges").length,
      "Square edges not used when squareEdges set to false"
    );
    init({ squareEdges: true });
    ok(
      $(".dp-nav-sqauare-edges"),
      "Square edges set correctly when squareEdges set to true"
    );
  });
  ...
  test("Navigate backwards", function () {
    ...
    ok(
      getSelectedDateAsMoment(options).isSame(
        selectedDate.clone().subtract("day", 1)
      ),
    ...
    options = getOptions(
      init({ startDate: "2013-11-01", selectedDate: "2013-11-01" })
    );
    selectedDate = getSelectedDateAsMoment(options);
    $("a .dp-nav-left").trigger("click");
    ok(
      getSelectedDateAsMoment(options).isSame(selectedDate),
  ...
  test("Navigate forwards", function () {
    ...
    // Test invalid navigation
    options = getOptions(
      init({ endDate: "2013-11-01", selectedDate: "2013-11-01" })
    );
    selectedDate = getSelectedDateAsMoment(options);
    $("a .dp-nav-right").trigger("click");
    ok(
  ...
  test("Navigate to selected date", function () {
    ...
    ok(
      getSelectedDateAsMoment(options).isSame(
        moment(el.attr("data-moment"), options.selectedDateFormat)
      ),
    ...
    el.trigger("click");
    ok(
      getSelectedDateAsMoment(options).isSame(getSelectedDateAsMoment(options)),
