  ...
  DatePaginator.prototype = {
    setSelectedDate: function (date, format) {
      this._setSelectedDate(
        moment(date, format ? format : this.options.selectedDateFormat)
      );
      this._render();
    },
    ...
    _setSelectedDate: function (selectedDate) {
      if (
        !selectedDate.isSame(this.options.selectedDate) &&
    ...
    _injectStyle: function () {
      ...
      if (
        ...
        !document.getElementById("bootstrap-datepaginator-style")
      ) {
        $(
    ...
    _buildData: function () {
      var viewWidth =
        ...
        adjustedSelectedItemWidth = Math.floor(
          this.options.selectedItemWidth +
            (viewWidth - units * adjustedItemWidth)
        ),
        today = moment().startOf("day"),
        start = this.options.selectedDate
          .clone()
          .subtract("days", unitsPerSide),
        end = this.options.selectedDate
          .clone()
          .add("days", units - unitsPerSide);
      ...
      for (var m = start; m.isBefore(end); m.add("days", 1)) {
        var valid =
          (m.isSame(this.options.startDate) ||
            m.isAfter(this.options.startDate)) &&
          (m.isSame(this.options.endDate) || m.isBefore(this.options.endDate))
            ? true
            : false;
        ...
        data.items[data.items.length] = {
          ...
          isOffDay:
            this.options.offDays
              .split(",")
              .indexOf(m.format(this.options.offDaysFormat)) !== -1
              ? true
              : false,
          isStartOfWeek:
            this.options.startOfWeek
              .split(",")
              .indexOf(m.format(this.options.startOfWeekFormat)) !== -1
              ? true
              : false,
          ...
          itemWidth: m.isSame(this.options.selectedDate)
      ...
      if (typeof options === "string") {
        ...
        if (!self) {
          $.data(
            this,
            "plugin_" + pluginName,
            new DatePaginator(this, options)
          );
        } else {
          self._init(options);
        }
