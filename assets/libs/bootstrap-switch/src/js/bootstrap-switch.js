...
function prvwidth() {
  const $handles = this.$on.add(this.$off).add(this.$label).css("width", "");
  const handleWidth =
  ...
  const handlers = {
    ...
    "mousedown.bootstrapSwitch touchstart.bootstrapSwitch": (event) => {
      ...
      dragStart =
        ...
        parseInt(this.$container.css("margin-left"), 10);
...
class BootstrapSwitch {
  constructor(element, options = {}) {
    ...
    this.$wrapper = $("<div>", {
      class: () =>
        getClasses(this.options, this.$element.attr("id"))
          .map((v) => this::prvgetClass(v))
          .concat(
            [this.options.baseClass],
            this::prvgetClasses(this.options.wrapperClass)
          )
          .join(" "),
    });
  ...
  wrapperClass(value) {
    ...
    this.$wrapper.removeClass(
      this::prvgetClasses(this.options.wrapperClass).join(" ")
    );
