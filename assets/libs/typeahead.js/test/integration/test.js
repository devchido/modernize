  ...
  describe("on up arrow", function () {
    it("should cycle through suggestions", function (done) {
      driver.run(function* () {
        ...
        expect(yield suggestions[3].getAttribute("class")).to.equal(
          "tt-suggestion tt-selectable tt-cursor"
        );

        yield input.type(wd.SPECIAL_KEYS["Up arrow"]);
        expect(yield input.getValue()).to.equal("Mississippi");
        expect(yield suggestions[2].getAttribute("class")).to.equal(
          "tt-suggestion tt-selectable tt-cursor"
        );

        yield input.type(wd.SPECIAL_KEYS["Up arrow"]);
        expect(yield input.getValue()).to.equal("Minnesota");
        expect(yield suggestions[1].getAttribute("class")).to.equal(
          "tt-suggestion tt-selectable tt-cursor"
        );

        yield input.type(wd.SPECIAL_KEYS["Up arrow"]);
        expect(yield input.getValue()).to.equal("Michigan");
        expect(yield suggestions[0].getAttribute("class")).to.equal(
          "tt-suggestion tt-selectable tt-cursor"
        );
